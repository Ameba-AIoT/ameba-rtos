...
- test_mbed_uart_dma_clock     — mbed_uart_dma_clock: dual-board, peer verifies 0x55 data stream via rx markers
...
# ---------------------------------------------------------------------------
#  8. mbed_uart_dma_clock — UART DMA 模拟时钟源 (双板)
# ---------------------------------------------------------------------------
@pytest.mark.generic
@pytest.mark.multi_board
@pytest.mark.app_name("uart_tool|mbed_uart_dma_clock")
@pytest.mark.app_defines("UART_BAUD=80000|")
@pytest.mark.parametrize("count", [2], ids=["dual"], indirect=True)
@pytest.mark.parametrize("image_dir",
                         ["../tools/uart_tool/bin/baud80000|./bin/mbed_uart_dma_clock"],
                         ids=["dma_clock"], indirect=True)
def test_mbed_uart_dma_clock(dut):
    """UART DMA clock: dual-board, verify 0x55 data stream via peer UART RX.

    Board A (DUT) runs mbed_uart_dma_clock: sends continuous 0x55 bytes
