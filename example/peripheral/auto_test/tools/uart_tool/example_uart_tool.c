...
/* RX activity detection: when forwarding passive RX data (no uarttool send in
 * flight), the monitor tracks idle gaps.  A gap longer than RX_GAP_MS
 * delineates RX activity periods, printing [RX_START] / [RX_END] markers so
 * automated scripts can reliably detect ON/OFF cycles. */
#define RX_GAP_MS	200
 ...
 *   - while passively forwarding, detect RX activity: after RX_GAP_MS of idle,
 *     emit [RX_END]; on first byte after idle emit [RX_START] so
 *     automated scripts can delimit ON/OFF cycles.
 */
static void uart_tool_monitor_task(void *param)
{
	...
	while (1) {
		...
		if (UART_Readable(UART_DEV)) {
			...
			/* RX activity start detection: emit marker only during passive
			 * forwarding (not during uarttool send+verify). */
			if ((!rx_active) && (!send_pending)) {
				RTK_LOGI(NOTAG, "[RX_START]\n");
				rx_active = 1;
			}
			...
			if (rx_active) {
				no_data_ms++;
				if ((no_data_ms > RX_GAP_MS) && (!send_pending)) {
					RTK_LOGI(NOTAG, "\n[RX_END]\n");
					rx_active = 0;
				}
			}
