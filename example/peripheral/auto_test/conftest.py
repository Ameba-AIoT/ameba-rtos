...
def _auto_build_for_item(item):
    ...
    for i in range(count):
        ...
        try:
            ...
            _run_build(soc, app, img_dir, extra)
...
def _run_build(soc, app_name, image_dir, extra_defines=None):
    """Run ameba.py build from SDK root.
    image_dir is an absolute path (parsed by _auto_build_for_item before passing in).
    """
    from ameba_pytest.utils import SDK_ROOT  # defer import
