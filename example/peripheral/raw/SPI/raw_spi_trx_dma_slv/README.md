...
1. This example is the **slave** role of the SPI DMA dual-board test.
2. It uses SPI0 as slave with GDMA-driven data transfer.
3. It must work together with `raw_spi_trx_dma_mstr` on another board.
...
2. This example is the slave-side; the master-side is `raw_spi_trx_dma_mstr`.
...
1. Slave performs Phase1 (full-duplex) and Phase2 (slave write) via GDMA.
