...
static void wakepin_dslp_handler(void)
{
	...
	/*
	 * Since the WakePin is an AON (Always-On) peripheral running on a slower clock domain,
	 * the interrupt clear signal may take some time to synchronize.
	 * A delay is added here to prevent the ISR from re-entering immediately
	 * before the status is effectively cleared in hardware.
	 */
	DelayUs(50);
