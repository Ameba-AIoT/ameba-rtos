...
- If you choose another tx signal, then this step can be ignored. As well, a tx image should be build as following:
  - By means of example located in sdk/example/peripheral/raw/IR/raw_ir_tx_nec_polling/example_raw_ir_tx_nec_polling.c, some modifications are necessary:
    - Define `IR_LOOP_TEST` to  `0`
    - Define `IR_TEST_LEARNNING` to `1`
    - Rebuild the image as described in sdk/example/peripheral/raw/IR/raw_ir_tx_nec_polling/README.md
# Expect result
