...
This example demonstrates the following capabilities of CAN:
...
**Capture protocol frames**: connect the TX/RX pins of the CAN to an external transceiver, and then connect the transceiver's H/L pins to the analyzer's H/L pins.

The pins used in this example are as follows.

| Pin | Function | Note |
| :--- | :--- | :--- |
| **PA 26** | CAN0 RX | Connect to Transceiver RX (if Ext. Loopback) |
| **PA 25** | CAN0 TX | Connect to Transceiver TX (if Ext. Loopback) |
| **STB** | Standby Control | Pull LOW for Normal Mode (if supported by PHY) |

Users can modify the `example_CAN_ext.h` file according to the actual pins used by the CAN on their development board.
