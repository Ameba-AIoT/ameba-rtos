...
* On RTL8720F
	...
	- Connect `GND` to `GND` of peer device
* On RTL8735C
	- Connect `UART_TX(_PF_11)` to `UART_RX(_PA_22)` of peer device
	- Connect `UART_RX(_PF_8)` to `UART_TX(_PA_21)` of peer device
	- Connect `GND` to `GND` of peer device
...
3. RX polls incoming data using `UART_Readable()`. During TX busy-wait, RX is starved and the FIFO fills up → hardware deasserts RTS → peer pauses.
