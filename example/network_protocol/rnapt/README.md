...
This example demonstrates how to use the R-NAPT (Router NAPT) system for multi-interface routing management with automatic network segment conflict detection. It supports multiple network interfaces (WiFi STA, WiFi AP, Ethernet, USB-ETH) and provides flexible IP configuration through a clean config structure pattern.

The example shows:
- Multi-interface registration and management using `rnapt_netif_create()` and `rnapt_netif_start()`
...
- NAPT forwarding between WAN and LAN interfaces for bidirectional communication
...
4. Interface Configuration:
   ...
   const rnapt_netif_config_t g_rnapt_sta_netif_config = {
       ...
       .priority = RNAPT_ROUTE_PRIO_WAN_STA,  /* 102 */
       .ip_info = NULL,
       .if_desc = "STA",
   ...
   const rnapt_netif_config_t g_rnapt_ap_netif_config = {
       ...
       .priority = RNAPT_ROUTE_PRIO_LAN,  /* 0 */
   ...
   **Flexible Role Configuration:**
   ...
   const rnapt_netif_config_t eth_wan_config = {
       .role = RNAPT_ROLE_WAN,
       .ip_method = RNAPT_IP_METHOD_DHCP_CLIENT,
       .priority = RNAPT_ROUTE_PRIO_WAN_ETH,
       .ip_info = NULL,
       .if_desc = "ETH"
   };

   /* Example 2: ETH as LAN port (DHCP Server) */
   const rnapt_netif_config_t eth_lan_config = {
       .role = RNAPT_ROLE_LAN,
       .ip_method = RNAPT_IP_METHOD_DHCP_SERVER,
...
```
[R-NAPT-I] [AP] Custom IP: 192.168.39.1
...
[R-NAPT-I] [AP] Starting netif...
[WLAN-A] Init WIFI AP
[R-NAPT-I] [AP] LINK UP - ACTIVE
[R-NAPT-APP-A] [USER-CB] AP Link UP
[R-NAPT-I] [AP] 'AmebaRouter' started on channel 100
```
...
[R-NAPT-I] [ETH] Created (role=LAN, ip_method=1, prio=0)
[R-NAPT-I] [ETH] Starting netif...
[ETH-I] Initializing Ethernet Driver...
[R-NAPT-I] [ETH] Initialized
```
...
[R-NAPT-A] [ETH] DOWN: IP=192.168.44.1 GW=192.168.44.1 Role=LAN Method=DHCP-Server Prio=0
[R-NAPT-A] ====================================
```
...
With the above configuration, NAPT (Network Address Port Translation) forwarding is now enabled between WAN and LAN interfaces:
...
Example scenario:
```
WiFi Client (192.168.43.100) → AP (192.168.43.1) → NAPT → STA (192.168.39.9) → Internet
Ethernet Client (192.168.44.100) → ETH (192.168.44.1) → NAPT → STA (192.168.39.9) → Internet
```

This enables NAPT forwarding between WAN and LAN interfaces, allowing LAN devices to access the internet through the WAN connection.
