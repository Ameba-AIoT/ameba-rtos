...
1. Configure test scenarios
	...
	- `USBH_UVC_APP_VFS`: Capture the frame and save it to filesystem(mount SD card). User need to configure VFS correctly refer to VFS example or document.
	- `USBH_UVC_APP_HTTPC`: Capture the frame and save it to http server.

	Configure `CONFIG_USBH_UVC_FORMAT_TYPE` in `example_usbh_uvc.c` to test different video frame formats:
...
For example, configure `USBH_UVC_APP_SIMPLE` and `USBH_UVC_FORMAT_MJPEG`:
...
For example, configure `USBH_UVC_APP_VFS` and `USBH_UVC_FORMAT_MJPEG`:
...
For example, configure `USBH_UVC_APP_HTTPC` and `USBH_UVC_FORMAT_MJPEG`:
...
4. Ameba board will recognize the USB camera. Once the video stream starts, it will capture frames and upload them to the configured HTTP server. Following log shall be printed on the LOGUART console:
	```
	[UVC-I] USBH UVC demo start
	[MAIN-I] KM4 START SCHEDULER
	...
...
5. Check the HTTP server side `Apache24/uploads` and verify that the JPEG files are received and saved correctly.
