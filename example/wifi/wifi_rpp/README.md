...
2. Use menuconfig and select `CONFIG BT`->`BT Example Demo`->`BLE WiFiMate Device` & `BLE WiFiMate Configurator`, `CONFIG WIFI`->`Enable Wifi`->`Enable RMESH` & `Enable RMESH Socket`.
