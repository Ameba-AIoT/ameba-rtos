...
typedef struct {
	...
	usbh_cdc_ecm_pipe_info_t ecm_ctrl_ep;            /* Control Endpoint Info */
	usbh_cdc_ecm_pipe_info_t ecm_tx_ep;              /* Bulk OUT Endpoint Info */
	usbh_cdc_ecm_pipe_info_t ecm_rx_ep;              /* Bulk IN Endpoint Info */

	rtos_task_t             monitor_task;            /* Task handle for the monitoring thread */
	usb_os_sema_t           monitor_start_sema;      /* Semaphore for monitor task synchronization */
	...
	volatile u8             cdc_ecm_tx_block;        /* Flag indicating TX path is blocked/busy */
	volatile u8             cdc_ecm_is_ready;        /* Flag indicating driver is fully initialized and ready */
	volatile u8             ecm_hw_connect;          /* Ethernet physical link status: 0=Disconnect, 1=Connect */
} usbh_cdc_ecm_host_t;
