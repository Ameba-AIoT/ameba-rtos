 ...
 */
typedef struct {
	u8 bDescLength;                      /**< Size of this descriptor in bytes. */
	...
	/* DeviceRemovable and PortPwrCtrlMask are variable-length bitmaps in the
	 * wire format (one bit per port, size = ceil((bNbrPorts+1)/8) bytes each).
	 * DeviceRemovable: not required for IoT host -- all ports treated as
	 * removable.  PortPwrCtrlMask: obsolete since USB 2.0, all bits are 1.
	 * Neither field is used by this driver; they are intentionally omitted. */
} __PACKED usbh_hub_desc_t;
