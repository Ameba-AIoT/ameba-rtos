...
void usbh_uvc_dec_init(usbh_uvc_dec *uvc_dec)
{
	...
	/* uvc cmd 1 */
	num_packets = uvc_dec->xfer_len / uvc_dec->ep_mps;
	if (num_packets == 1) {
	...
	hcsize = (uvc_dec->xfer_len & USB_OTG_HCTSIZ_XFRSIZ) |
	...
	/* uvc cmd 2 */
	mc = uvc_dec->xfer_len / uvc_dec->ep_mps;
	...
	hcchar |=  uvc_dec->ep_mps << USB_OTG_HCCHAR_MPSIZ_Pos;
