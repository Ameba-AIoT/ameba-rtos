...
static int usbh_vendor_process(usb_host_t *host);
...
#if CONFIG_USBH_VENDOR_BULK_TEST || CONFIG_USBH_VENDOR_INTR_TEST || CONFIG_USBH_VENDOR_ISOC_TEST
...
static int usbh_vendor_init_ep(usbh_vendor_xfer_t *xfer, usbh_ep_desc_t *ep_desc)
{
	...
	RTK_LOGD(TAG, "%s EP%02X MPS=%d bInterval=%d interval=%ld\n",
...
static void usbh_vendor_xfer_process(usb_host_t *host, usbh_vendor_xfer_t *in_xfer, usbh_vendor_xfer_t *out_xfer)
{
	...
	u32 ticks;
	int is_timeout = 0;
	int is_ptx = ((out_xfer->ep_type == USB_CH_EP_TYPE_INTR) || (out_xfer->ep_type == USB_CH_EP_TYPE_ISOC));
		...
		if (urb_state == USBH_URB_DONE) {
			...
			if (out_xfer->xfer_len > 0U) {
				...
				if (out_xfer->xfer_cnt == out_xfer->xfer_max_cnt) {
					...
					usb_os_memset((void *)out_xfer->xfer_buf, out_xfer->xfer_cnt, out_xfer->xfer_len);
					out_xfer->state = VENDOR_STATE_XFER;
				}
			}
		} else if (urb_state == USBH_URB_ERROR) {
...
static void usbh_vendor_ctrl_process(usb_host_t *host)
{
	...
	case VENDOR_STATE_XFER:
		...
		setup.b.wIndex = 0U;
		setup.b.wLength = USB_CTRL_BUF_LENGTH;
...
static int usbh_vendor_init_xfer(usb_host_t *host, usbh_vendor_xfer_t *xfer, u16 xfer_len, u16 test_cnt)
{
	int status = HAL_OK;
	u8 pipe_num;
	char *xfer_type = usbh_get_xfer_type_text(xfer);
	...
	RTK_LOGI(TAG, "Allocate pipe %d for %s EP%02X\n", pipe_num, xfer_type, xfer->ep_num);
...
static int usbh_vendor_attach(usb_host_t *host)
{
	...
	vendor->interface_id = 1U; //match the usb descriptor
#else
