...
static u8 *usbd_composite_msc_get_descriptor(usb_dev_t *dev, usb_setup_req_t *req, usb_speed_type_t speed, u16 *len);
...
static u8 usbd_composite_msc_hs_itf_desc[] = {
...
static u8 usbd_composite_msc_fs_itf_desc[] = {
	/********************  Mass Storage interface ********************/
	USB_LEN_IF_DESC,			/* bLength: Interface Descriptor size */
	USB_DESC_TYPE_INTERFACE,	/* bDescriptorType: */
	USBD_COMP_MSC_ITF,			/* bInterfaceNumber: Number of Interface */
...
/* MSC Class Driver */
usbd_class_driver_t usbd_composite_msc_driver = {
...
static SDIOHCFG_TypeDef usbd_composite_sd_config = {
...
static void usbd_composite_msc_abort(usb_dev_t *dev)
{
	...
	if ((cbw->bmCBWFlags == 0U) &&
		...
		usbd_ep_set_stall(dev, USBD_COMP_MSC_BULK_OUT_EP);
	}

	usbd_ep_set_stall(dev, USBD_COMP_MSC_BULK_IN_EP);
...
static int usbd_composite_msc_set_config(usb_dev_t *dev, u8 config)
{
	...
	/* Init BULK IN EP */
	ep_mps = (dev->dev_speed == USB_SPEED_HIGH) ? COMP_MSC_HS_MAX_PACKET_SIZE : COMP_MSC_FS_MAX_PACKET_SIZE;
	usbd_ep_init(dev, USBD_COMP_MSC_BULK_IN_EP, USB_CH_EP_TYPE_BULK, ep_mps);

	/* Init BULK OUT EP */
	ep_mps = (dev->dev_speed == USB_SPEED_HIGH) ? COMP_MSC_HS_MAX_PACKET_SIZE : COMP_MSC_FS_MAX_PACKET_SIZE;
	usbd_ep_init(dev, USBD_COMP_MSC_BULK_OUT_EP, USB_CH_EP_TYPE_BULK, ep_mps);
...
static int usbd_composite_msc_clear_config(usb_dev_t *dev, u8 config)
{
	...
	/* DeInit BULK OUT EP */
	usbd_ep_deinit(dev, USBD_COMP_MSC_BULK_OUT_EP);
...
static u8 *usbd_composite_msc_get_descriptor(usb_dev_t *dev, usb_setup_req_t *req, usb_speed_type_t speed, u16 *len)
{
	...
	case USB_DESC_TYPE_CONFIGURATION:
		if (speed == USB_SPEED_HIGH) {
			buf = usbd_composite_msc_hs_itf_desc;
			*len = sizeof(usbd_composite_msc_hs_itf_desc);
		} else {
			buf = usbd_composite_msc_fs_itf_desc;
