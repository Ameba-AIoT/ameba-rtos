...
#define USBD_UAC_AC_FU_HEAD_DESC_LEN(ch_cnt)        (USB_UAC1_AC_FUNC_UNIT_DESC_LEN +  (ch_cnt))

/* AC Format Type descriptor length */
#define USBD_UAC1_AS_FRT_TYPE_DESC_LEN              (USB_UAC1_AS_FRT_TYPE_DESC_LEN + 3 * 1)

/* AC IF header descriptor length */
#define USBD_UAC_AC_IF_HEAD_DESC_LEN(ch_cnt) \
    (USB_UAC1_AC_IF_HEADER_LEN + USB_UAC1_AC_IN_TTY_DESC_LEN + \
    USBD_UAC_AC_FU_HEAD_DESC_LEN(ch_cnt) + USB_UAC1_AC_OUT_TTY_DESC_LEN)
...
#define USBD_UAC_AS_EIF_LEN                        (USB_LEN_IF_DESC + USB_UAC1_AS_IF_DESC_LEN + USBD_UAC1_AS_FRT_TYPE_DESC_LEN \
                                                   + USB_UAC1_EP_DESC_LEN + USB_UAC1_AUDIO_DATA_EP_DESC_LEN)
...
static u8 usbd_composite_uac_rx_buf[USBD_UAC_FS_ISOC_MPS] __attribute__((aligned(CACHE_LINE_SIZE)));
...
static const u8 usbd_composite_uac_fs_itf_desc[] = {
	...
	USB_UAC1_IA_DESCRIPTOR,            /* Interface Association Descriptor type */
	USBD_COMP_UAC_AC_HEADSET,          /* First Index: Audio Control Interface Index - Headset (0) */
	0x02,                              /* Audio Control Interface, Audio Streaming Interfaces () */
	USB_UAC1_IF_CLASS_AUDIO,           /* Audio Device Class  */
	0x00,                              /* No subclass */
	0x00,                              /* Audio Protocol IP version 1.00 */
	...
	USB_UAC1_IF_IDX_AC_HEADSET,          /* Audio Control Interface Index - Headset (0) */
	0x00,                              /* bAlternateSetting */
	0x00,                              /* No associated endpoints with this interface (uses endpoint 0) */
	USB_UAC1_CLASS_CODE,               /* Audio Device Class */
	USB_UAC1_SUBCLASS_AUDIOCONTROL,    /* Audio Control Interface */
	0x00,                              /* Audio Protocol IP version 2.00 */
	0x00,                              /* Interface string descriptor index (0) */

	/* 4.7.2 Class-Specific AC Interface Descriptor */
	/* Audio headset */
	USB_UAC1_AC_IF_HEADER_LEN,                                         /* Size of the descriptor, in byte_width  */
	USB_UAC1_AUDIO_INTERFACE_DESC_TYPE,                               /* CS_INTERFACE Descriptor Type   */
	USB_UAC1_HEADER,                                  /* HEADER descriptor subtype  */
	USB_LOW_BYTE(USB_UAC1_VERSION_01_10),                                /* bcdUSB */
	USB_HIGH_BYTE(USB_UAC1_VERSION_01_10),
	USB_LOW_BYTE(USBD_UAC_AC_IF_HEAD_DESC_LEN(USBD_UAC_OUT_DEFAULT_CH_CNT)),
	USB_HIGH_BYTE(USBD_UAC_AC_IF_HEAD_DESC_LEN(USBD_UAC_OUT_DEFAULT_CH_CNT)),/* Total number of byte_width returned for the class-specific AudioControl interface descriptor. Includes
	...
	USB_UAC1_AC_IN_TTY_DESC_LEN,                             /* Size of the descriptor, in byte_width  */
	USB_UAC1_AUDIO_INTERFACE_DESC_TYPE,                   /* CS_INTERFACE Descriptor Type   */
	USB_UAC1_INPUT_TERMINAL,              /* INPUT_TERMINAL descriptor subtype   */
	...
	USBD_UAC_OUT_DEFAULT_CH_CNT,                                 /* This Terminal's output audio channel cluster has 8 logical output channels   */
	USBD_UAC_CH_CONFIG_TYPE_LOW(USBD_UAC_OUT_DEFAULT_CH_CNT),
	USBD_UAC_CH_CONFIG_TYPE_HIGH(USBD_UAC_OUT_DEFAULT_CH_CNT),
	0x00,                                                    /* iChannelNames */
	0x00,                                                    /* Index of a string descriptor, describing the Input Terminal.  */

	/* FEATURE_UNIT descriptor subtype */
	USBD_UAC_AC_FU_HEAD_DESC_LEN(USBD_UAC_OUT_DEFAULT_CH_CNT),   /* Size of the descriptor, in byte_width   */
	USB_UAC1_AUDIO_INTERFACE_DESC_TYPE,                   /* CS_INTERFACE Descriptor Type   */
	USB_UAC1_FEATURE_UNIT,                /* FEATURE_UNIT descriptor subtype   */
	...
	USB_UAC1_AC_OUT_TTY_DESC_LEN,                             /* Size of the descriptor, in byte_width   */
	USB_UAC1_AUDIO_INTERFACE_DESC_TYPE,                    /* CS_INTERFACE Descriptor Type  */
	USB_UAC1_OUTPUT_TERMINAL,              /* OUTPUT_TERMINAL descriptor subtype   */
	USBD_UAC_CTRL_ENTITYID_OUTPUTTERMINAL_HEADSET_HEADPHONES, /* Constant uniquely identifying the Terminal within the audio
                                                                 function. This value is used in all requests to address this Terminal.   */
	USBD_UAC_OT_DESC_TYPE_LOW(USBD_UAC_OUT_DEFAULT_CH_CNT),
	USBD_UAC_OT_DESC_TYPE_HIGH(USBD_UAC_OUT_DEFAULT_CH_CNT),      /* A Terminal dealing with a signal carried over an endpoint in an AudioStreaming interface. The
                                                                 AudioStreaming interface descriptor points to the associated Terminal through the bTerminalLink field.  */
	...
	USB_UAC1_IF_CLASS_AUDIO,                                  /* The interface implements the Audio Interface class   */
	USB_UAC1_SUBCLASS_AUDIOSTREAMING,                         /* The interface implements the AUDIOSTREAMING Subclass   */
	...
	USB_UAC1_AS_IF_DESC_LEN,                         /* Size of the descriptor, in byte_width   */
	USB_UAC1_AUDIO_INTERFACE_DESC_TYPE,                   /* CS_INTERFACE Descriptor Type  */
	USB_UAC1_AS_IF_DESC_SUBTYPE_AS_GENERAL,                  /* AS_GENERAL descriptor subtype   */
	...
	USBD_UAC1_AS_FRT_TYPE_DESC_LEN,                   /* Size of the descriptor, in byte_width   */
	USB_UAC1_AUDIO_INTERFACE_DESC_TYPE,                   /* CS_INTERFACE Descriptor Type   */
	USB_UAC1_AS_IF_DESC_SUBTYPE_FORMAT_TYPE,                 /* FORMAT_TYPE descriptor subtype   */
	...
	USB_UAC1_EP_DESC_LEN,                                    /* Size of the descriptor, in byte_width   */
	...
	USB_UAC1_AUDIO_DATA_EP_DESC_LEN,                               /* Size of the descriptor, in byte_width   */
	USB_UAC1_AUDIO_EP_DESCRIPTOR,                         /* Class-specific Endpoint Descriptor type */
	USB_UAC1_AS_EP_DESC_SUBTYPE_EP_GENERAL,                  /* EP_GENERAL Descriptor subtype */
	0x01,                                                    /* bmAttributes */
...
static void usbd_composite_uac_append_data(void)
{
	...
	//used for usb read
	if ((buf_ctrl->sema_active_status) && (usb_ringbuf_is_empty(buf_list) == 0)) { // not empty
...
static u16 usbd_composite_uac_get_mps(usbd_audio_cfg_t *params, u8 speed)
{
	...
	if ((mps_value == 0) || (mps_value > USBD_UAC_FS_ISOC_MPS)) {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "Invalid FS MPS %d-%d %d-%d-%d\n", mps_value, USBD_UAC_FS_ISOC_MPS, params->ch_cnt, params->byte_width, params->sampling_freq);
		return 0;
	}
...
static inline void usbd_composite_uac_format_change(const usbd_composite_uac_usr_cb_t *cb, usbd_audio_cfg_t *audio_cfg)
{
	if ((NULL == cb) || (NULL == audio_cfg) || (NULL == cb->format_changed)) {
		return;
	}

	if (audio_cfg->sampling_freq && audio_cfg->ch_cnt && audio_cfg->byte_width) {
...
static void usbd_composite_uac_ep_buf_ctrl_deinit(usbd_composite_uac_buf_ctrl_t *buf_ctrl)
{
	...
	buf_ctrl->sema_active_status = 0;

	if ((buf_ctrl->isoc_sema != NULL) && (buf_ctrl->sema_wait_status)) {
		usb_os_sema_give(buf_ctrl->isoc_sema);
	}
	while (buf_ctrl->sema_wait_status) {
...
static int usbd_composite_uac_ep_buf_ctrl_init(usbd_composite_uac_buf_ctrl_t *buf_ctrl, usbd_audio_cfg_t *params)
{
	...
	if (usbd_composite_uac_ep_enable(params)) {
		...
		if (buf_ctrl->isoc_sema == NULL) {
			if (rtos_sema_create(&(buf_ctrl->isoc_sema), 0U, 1U) != RTK_SUCCESS) {
				RTK_LOGS(TAG, RTK_LOG_ERROR, "Create isoc sema fail\n");
				return HAL_ERR_MEM;
			}
		}
		buf_ctrl->sema_active_status = 1;
		buf_ctrl->sema_wait_status  = 0;
	} else {
...
/**
  ...
  */
static u8 usbd_composite_uac_volume_linear_interpolation(const u8 x_points[], const s16 y_points[], u8 num_points, s16 target_y)
{
	u8 i;
	u8 x0;
	u8 x1;
...
{
	usbd_composite_uac_device_t *uac = &usbd_composite_uac_device;
	const usbd_composite_uac_usr_cb_t *cb = uac->cb;
			...
			if (dev->dev_state == USBD_STATE_CONFIGURED) {
				...
				if ((alt_setting != uac->alt_setting) && alt_setting) {
					...
					if (ret == HAL_OK) {
						...
						//update the issue
						if (fmt_change) {
							usbd_composite_uac_format_change(cb, audio_cfg);
						}
					}
				}
			} else {
	...
	case USB_REQ_TYPE_CLASS :
		...
		if ((req->bmRequestType & USB_REQ_DIR_MASK) == USB_D2H) {
			...
			if (entityId == USBD_UAC_CTRL_ENTITYID_OUTPUTTERMINAL_FEATUREUNIT) {
				if (controlSelector == USB_UAC1_CTRL_FU_MUTE_CONTROL_SELECTOR) {
					if (req->bRequest == USB_UAC1_GET_CUR) {
					...
					if (req->bRequest == USB_UAC1_GET_CUR) {
						usb_os_memcpy((void *)ep0_in->xfer_buf, (void *) & (uac->cur_volume), 2);
						ep0_in->xfer_len = 2U;
						usbd_ep_transmit(dev, ep0_in);
					} else if (req->bRequest == USB_UAC1_GET_MIN) {
						u16 min_volume = USBD_UAC_VOLUME_CTRL_MIN;
						usb_os_memcpy((void *)ep0_in->xfer_buf, (void *)&min_volume, 2);
						ep0_in->xfer_len = 2U;
						usbd_ep_transmit(dev, ep0_in);
					} else if (req->bRequest == USB_UAC1_GET_MAX) {
						u16 max_volume = USBD_UAC_VOLUME_CTRL_MAX;
						usb_os_memcpy((void *)ep0_in->xfer_buf, (void *)&max_volume, 2);
						ep0_in->xfer_len = 2U;
						usbd_ep_transmit(dev, ep0_in);;
					} else if (req->bRequest == USB_UAC1_GET_RES) {
						u16 res = 1U;
						usb_os_memcpy((void *)ep0_in->xfer_buf, (void *)&res, 2);
						ep0_in->xfer_len = 2U;
						usbd_ep_transmit(dev, ep0_in);
					} else {
				...
				if (controlSelector == USB_UAC1_CS_SAM_FREQ_CONTROL_TYPE) {
					if (req->bRequest == USB_UAC1_GET_CUR) {
						audio_cfg = &(uac->isoc_out.audio_config);
						usb_os_memcpy((void *)ep0_in->xfer_buf, (void *) & (audio_cfg->sampling_freq), 3);
						ep0_in->xfer_len = 3U;
						usbd_ep_transmit(dev, ep0_in);
					} else {
			...
			if (entityId == USBD_UAC_CTRL_ENTITYID_OUTPUTTERMINAL_FEATUREUNIT) { //0x05 FU
				if (controlSelector == USB_UAC1_CTRL_FU_MUTE_CONTROL_SELECTOR) { //mute
					if (req->bRequest == USB_UAC1_SET_CUR) {
						usb_os_memcpy((void *)&uac->ctrl_req, (void *)req, sizeof(usb_setup_req_t));
						ep0_out->xfer_len = req->wLength;
						usbd_ep_receive(dev, ep0_out);
...
static int usbd_composite_uac_handle_ep0_data_out(usb_dev_t *dev)
{
	...
	if (((p_ctrl_req->bmRequestType & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_CLASS) && (p_ctrl_req->bRequest == USB_UAC1_SET_CUR)) {
		if ((USB_HIGH_BYTE(p_ctrl_req->wIndex) == USBD_UAC_CTRL_ENTITYID_OUTPUTTERMINAL_FEATUREUNIT &&
			...
			if ((USB_HIGH_BYTE(p_ctrl_req->wValue) == USB_UAC1_CTRL_FU_MUTE_CONTROL_SELECTOR) && (p_ctrl_req->wLength == 0x01)) {
				if (cb->mute_changed != NULL) {
					uac->cur_mute = (ep0_out->xfer_buf[0]) ? 1 : 0;
					cb->mute_changed(uac->cur_mute);
				}
			} else if ((USB_HIGH_BYTE(p_ctrl_req->wValue) == USB_UAC1_CTRL_FU_VOLUME_CONTROL_SELECTOR) && (p_ctrl_req->wLength == 0x02)) {
				if (cb->volume_changed != NULL) {
					...
					if (num_points != sizeof(usbd_composite_uac_drv_vol) / sizeof(usbd_composite_uac_drv_vol[0])) {
						RTK_LOGS(TAG, RTK_LOG_ERROR, "Vol array length err\n");
						ret = HAL_ERR_PARA;
					} else {
		...
		if (((p_ctrl_req->bmRequestType & 0x1FU) == USB_REQ_RECIPIENT_ENDPOINT)
			&& (USB_HIGH_BYTE(p_ctrl_req->wValue) == USB_UAC1_CS_SAM_FREQ_CONTROL_TYPE) && (p_ctrl_req->wLength == 0x03)) {
			sampling_freq = (ep0_out->xfer_buf[2] << 16) | (ep0_out->xfer_buf[1] << 8) | ep0_out->xfer_buf[0];
	...
	if (buf_ctrl->xfer_continue) {
		if (ep_addr == USBD_COMP_UAC_ISOC_OUT_EP) {
			if (len == 0) { //ZLP
				...
				ret = usbd_ep_receive(cdev->dev, ep_out);
			} else {
...
static u16 usbd_composite_uac_get_descriptor(usb_dev_t *dev, usb_setup_req_t *req, u8 *buf)
{
	...
	if (speed == USB_SPEED_HIGH) {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "UAC1.0 only support FS\n");
		return 0;
	}
	...
	usbd_composite_uac_device_t *uac = &usbd_composite_uac_device;
	usbd_composite_uac_buf_ctrl_t *buf_ctrl = &(uac->isoc_out);
	buf_ctrl->isoc_timeout_max_step = 1500;
#endif

	switch (USB_HIGH_BYTE(req->wValue)) {
	case USB_DESC_TYPE_CONFIGURATION:
		len = sizeof(usbd_composite_uac_fs_itf_desc);
		usb_os_memcpy((void *)buf, (void *)usbd_composite_uac_fs_itf_desc, len);
		break;
...
static void usbd_composite_uac_status_dump_thread(void *param)
{
	...
	while (1) {
		if (uac && uac->cdev && uac->cdev->dev) {
			RTK_LOGS(TAG, RTK_LOG_INFO, "USB RX %d-%d/TO %d-%d/zp %d-%d-%d/OW %d-%d/Pkt %d/len %d-%d/%d\n",
					 ...
					 buf_ctrl_out->overwrite_cnt, buf_ctrl_out->append_overwrite_cnt,
					 usb_ringbuf_get_count(&(buf_ctrl_out->buf_list)),
...
int usbd_composite_uac_init(usbd_composite_dev_t *cdev, const usbd_composite_uac_usr_cb_t *cb)
{
	...
	uac->isoc_out.audio_config.ch_cnt = USBD_UAC_OUT_DEFAULT_CH_CNT;//init the default value
...
int usbd_composite_uac_deinit(void)
{
	...
	if (uac->isoc_in.isoc_sema != NULL) {
		usb_os_sema_delete(uac->isoc_in.isoc_sema);
		uac->isoc_in.isoc_sema = NULL;
	}
	if (uac->isoc_out.isoc_sema != NULL) {
		usb_os_sema_delete(uac->isoc_out.isoc_sema);
		uac->isoc_out.isoc_sema = NULL;
	}
...
#if USBD_COMPOSITE_UAC_DEBUG
		RTK_LOGS(TAG, RTK_LOG_INFO, "First trigger sema %d-%d \n", buf_ctrl->sema_wait_status, buf_ctrl->mps);
#endif
	}
...
u8 usbd_composite_uac_config(const usbd_audio_cfg_t *uac_cfg, u8 is_record, u32 flag)
{
	...
	if (uac_cfg && usbd_composite_uac_ep_enable((usbd_audio_cfg_t *)uac_cfg)) {
		...
		usbd_composite_uac_ep_buf_ctrl_deinit(buf_ctrl);
		usbd_composite_uac_ep_buf_ctrl_init(buf_ctrl, &(buf_ctrl->audio_config));
	}
	...
	if (time_out_ms == 0) {
		...
		do {
			if (usb_ringbuf_is_empty(&(buf_ctrl->buf_list))) {
				//wait sema
				buf_ctrl->sema_wait_status = 1;

				if (!buf_ctrl->sema_active_status) {
					buf_ctrl->sema_wait_status = 0;
					break;
				}
...
u8 usbd_composite_uac_get_read_frame_time_in_us(void)
{
	...
	return usb_ringbuf_get_count(&(buf_ctrl->buf_list)) * USBD_UAC_ONE_KHZ * USBD_UAC_POW2(USBD_UAC_FS_DEFAULT_BINTERVAL - 1);
}
