...
#include "usbd.h"
#include "usbd_composite_cdc_acm_uac_hid.h"

/* Private defines -----------------------------------------------------------*/
#define USBD_COMP_HID_UAC_TYPE                 0U
#define USBD_COMP_HID_CDC_ACM_TYPE             1U
...
static int usbd_composite_sof(usb_dev_t *dev)
{
	...
	if ((cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) && (cdev->uac != NULL) && (cdev->uac->sof)) {
		cdev->uac->sof(dev);
	} else if ((cdev->cdc_acm != NULL) && (cdev->cdc_acm->sof != NULL)) {
		cdev->cdc_acm->sof(dev);
	}
...
static int usbd_composite_set_config(usb_dev_t *dev, u8 config)
{
	...
	if ((cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) && (cdev->uac != NULL) && (cdev->uac->set_config)) {
		cdev->uac->set_config(dev, config);
	} else if ((cdev->cdc_acm != NULL) && (cdev->cdc_acm->set_config != NULL)) {
		cdev->cdc_acm->set_config(dev, config);
	} else {
		// RTK_LOGS(TAG, RTK_LOG_WARN, "Set cfg msg dropped type %d\n", cdev->usbd_comp_type);
	}
...
static int usbd_composite_clear_config(usb_dev_t *dev, u8 config)
{
	...
	if ((cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) && (cdev->uac != NULL) && (cdev->uac->clear_config)) {
		ret += cdev->uac->clear_config(dev, config);
	} else if ((cdev->cdc_acm != NULL) && (cdev->cdc_acm->clear_config)) {
		ret += cdev->cdc_acm->clear_config(dev, config);
	} else {
		// RTK_LOGS(TAG, RTK_LOG_WARN, "Clr cfg msg dropped type %d\n", cdev->usbd_comp_type);
	}
...
static int usbd_composite_setup(usb_dev_t *dev, usb_setup_req_t *req)
{
	...
	case USB_REQ_TYPE_STANDARD:
		...
		default:
			...
			if ((entityId == USBD_COMP_HID_ITF) || (entityId == USBD_COMP_HID_CONSUMER_ITF)) {
			...
			} else if (cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) {
				...
				if ((entityId == USBD_COMP_CDC_COM_ITF) || (entityId == USBD_COMP_CDC_DAT_ITF)) {
					if ((cdev->cdc_acm != NULL) && (cdev->cdc_acm->setup != NULL)) {
						ret = cdev->cdc_acm->setup(dev, req);
					}
				}
			}
			break;
...
static int usbd_composite_handle_ep_data_in(usb_dev_t *dev, u8 ep_addr, u8 status)
{
	...
	} else if ((cdev->usbd_comp_type == USBD_COMP_HID_CDC_ACM_TYPE) && (cdev->cdc_acm != NULL) && (cdev->cdc_acm->ep_data_in != NULL)) {
		if (ep_addr == USBD_COMP_CDC_BULK_IN_EP) {
			ret = cdev->cdc_acm->ep_data_in(dev, ep_addr, status);
		} else {
#if CONFIG_COMP_CDC_ACM_NOTIFY
			if (ep_addr == USBD_COMP_CDC_INTR_IN_EP) {
				ret = cdev->cdc_acm->ep_data_in(dev, ep_addr, status);
			}
#endif
		}
...
static int usbd_composite_handle_ep_data_out(usb_dev_t *dev, u8 ep_addr, u16 len)
{
	...
	} else if ((cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) && (ep_addr == USBD_COMP_UAC_ISOC_OUT_EP) && (cdev->uac != NULL) &&
			   (cdev->uac->ep_data_out != NULL)) {
		ret = cdev->uac->ep_data_out(dev, ep_addr, len);
	} else if ((cdev->usbd_comp_type == USBD_COMP_HID_CDC_ACM_TYPE) && (ep_addr == USBD_COMP_CDC_BULK_OUT_EP) && (cdev->cdc_acm != NULL) &&
			   (cdev->cdc_acm->ep_data_out != NULL)) {
		ret = cdev->cdc_acm->ep_data_out(dev, ep_addr, len);
	} else {
		// RTK_LOGS(TAG, RTK_LOG_WARN, "EP%02x out msg dropped len %d type %d\n", ep_addr, len, cdev->usbd_comp_type);
	}
...
static int usbd_composite_handle_ep0_data_out(usb_dev_t *dev)
{
	...
	if ((cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) && (cdev->uac != NULL) && (cdev->uac->ep0_data_out != NULL)) {
		ret = cdev->uac->ep0_data_out(dev);
	} else if ((cdev->usbd_comp_type == USBD_COMP_HID_CDC_ACM_TYPE) && (cdev->cdc_acm != NULL) && (cdev->cdc_acm->ep0_data_out != NULL)) {
		ret = cdev->cdc_acm->ep0_data_out(dev);
	} else {
		// RTK_LOGS(TAG, RTK_LOG_WARN, "EP0 out msg dropped type %d\n", cdev->usbd_comp_type);
	}
...
static void usbd_composite_status_changed(usb_dev_t *dev, u8 status)
{
	...
	if ((cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) && (cdev->uac != NULL) && (cdev->uac->status_changed != NULL)) {
		cdev->uac->status_changed(dev, status);
	} else if ((cdev->usbd_comp_type == USBD_COMP_HID_CDC_ACM_TYPE) && (cdev->cdc_acm != NULL) && (cdev->cdc_acm->status_changed != NULL)) {
		cdev->cdc_acm->status_changed(dev, status);
	} else {
		// RTK_LOGS(TAG,  "EP0 out msg dropped type %d\n", cdev->usbd_comp_type);
	}
...
static u8 *usbd_composite_get_descriptor(usb_dev_t *dev, usb_setup_req_t *req, usb_speed_type_t speed, u16 *len)
{
	...
	case USB_DESC_TYPE_DEVICE:
		...
		if (cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) {
			...
			buf[USBD_DEV_DESC_OFFSET_PID + 0] = USB_LOW_BYTE(USBD_COMP_OTA_PID);
			buf[USBD_DEV_DESC_OFFSET_PID + 1] = USB_HIGH_BYTE(USBD_COMP_OTA_PID);
		}
	...
	case USB_DESC_TYPE_OTHER_SPEED_CONFIGURATION:
		...
		if (cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) {
			...
			buf = cdev->cdc_acm->get_descriptor(dev, req, speed, &desc_len);
			usb_os_memcpy((void *)desc, (void *)buf, desc_len);
			desc += desc_len;
			total_len += desc_len;
		}
						...
						usbd_composite_cb_t *cb)
{
	...
	if (cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE) {
		...
		cdev->cdc_acm = NULL;
	} else { /* if(cdev->usbd_comp_type == USBD_COMP_HID_CDC_ACM_TYPE) */
		ret = usbd_composite_cdc_acm_init(cdev, CONFIG_USBD_COMPOSITE_HID_CDC_ACM_BULK_OUT_XFER_SIZE, CONFIG_USBD_COMPOSITE_HID_CDC_ACM_BULK_IN_XFER_SIZE, acm_cb);
		if (ret != HAL_OK) {
			RTK_LOGS(TAG, RTK_LOG_ERROR, "Init ACM itf fail: %d\n", ret);
			usbd_composite_hid_deinit();
			usb_os_mfree(cdev->ctrl_buf);
			return ret;
		}
		cdev->cdc_acm = &usbd_composite_cdc_acm_driver;
		cdev->uac = NULL;
	}
...
void usbd_composite_switch_uac(u8 is_uac)
{
	...
	if (is_uac) {
		cdev->usbd_comp_type = USBD_COMP_HID_UAC_TYPE;
	} else {
		cdev->usbd_comp_type = USBD_COMP_HID_CDC_ACM_TYPE;
	}
}
...
u32 usbd_composite_is_in_uac_mode(void)
{
	...
	return cdev->usbd_comp_type == USBD_COMP_HID_UAC_TYPE;
}