...
int cmd_usbh_cts_phydw(u16 argc, u8 *argv[])
{
	...
	if (argc != 2) {
		...
		RTK_LOGS(TAG, RTK_LOG_ERROR, "  usbh phydw <address, hex>\n");
		return HAL_ERR_PARA;
	}
	...
	if (ret == HAL_OK) {
		RTK_LOGS(TAG, RTK_LOG_INFO, "USB PHY read 0x%02x: 0x%02x\n", addr, val);
	} else {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "USB PHY read failed (%d)\n", ret);
	}
...
int cmd_usbh_cts_phyew(u16 argc, u8 *argv[])
{
	...
	if (argc != 3) {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "Invalid arguments, usage:\n");
		RTK_LOGS(TAG, RTK_LOG_ERROR, "  usbh phyew <address, hex> <value, hex>\n");
		return HAL_ERR_PARA;
	}
	...
	if (ret == HAL_OK) {
		RTK_LOGS(TAG, RTK_LOG_INFO, "USB PHY write 0x%02x: 0x%02x\n", addr, val);
	} else {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "USB PHY write failed (%d)\n", ret);
	}
