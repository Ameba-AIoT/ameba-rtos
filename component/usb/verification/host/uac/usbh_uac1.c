...
static int usbh_uac_cb_process(usb_host_t *host);
...
static void dump_cur_setting_cfgdesc(void)
{
	...
	for (i = 0; i < USB_MAX; i ++) {
		...
		RTK_LOGI(TAG, "interNum=%d[bAlternateSetting=%d,freId=%d]\n",
...
static int usbh_uac_parse_ac(u8 *pbuf, u32 *length)
{
	*length = ((usb_descriptor_header *) pbuf)->bLength;
	RTK_LOGI(TAG, "enter usbh_uac_parse_ac *length=%d\n", *length);
	//Handle AC command
	return  0;
}
...
static int usbh_uac_parse_as(u8 *pbuf, u32 *length)
{
	...
	*length = ((usb_descriptor_header *) desc)->bLength;
	...
	while (1) {
		...
		default:
			RTK_LOGI(TAG, "bDescriptorType: %d\n", ((usb_descriptor_header *) desc)->bDescriptorType);
			return 0;
		}
	}
}
	...
	while (1) {
		...
		if (pbuf->bInterfaceClass == USB_CLASS_AUDIO) {
			...
			case USB_SUBCLASS_AUDIOCONTROL: //audio control
				...
				if (ret) {
					RTK_LOGE(TAG, "UAC parse audio control fail\n");
					return ret;
				}
			...
			case USB_SUBCLASS_AUDIOSTREAMING: //audio streaming
				RTK_LOGI(TAG, "setting=%d,len=%d,type=%d\n",
				...
				if (pbuf->bAlternateSetting == 0) { //setting 0
					...
					if (ret) {
						RTK_LOGE(TAG, "UAC parse audio streaming fail\n");
						return ret;
					}
			...
			default:
				RTK_LOGE(TAG, "subclass(%d) is not AC or AS\n", pbuf->bInterfaceClass);
				return HAL_ERR_PARA;

			}
		} else {
			//skip no audio descriptor
			RTK_LOGI(TAG, "skip ((usb_descriptor_header *)desc)->bDescriptorType=0x%x,len=%d\n", ((usb_descriptor_header *)desc)->bDescriptorType,
...
{
	UNUSED(ctrl);
	RTK_LOGI(TAG, "[UAC] bmRequestType=0x%02X bRequest=0x%02X wValue=0x%04X wIndex=0x%04X wLength=0x%02X\n",
			 ctrl->bmRequestType,
			 ctrl->bRequest,
			 ctrl->wValue,
			 ctrl->wIndex,
...
static int usbh_uac_cb_attach(usb_host_t *host)
{
	...
	if (status) {
		RTK_LOGE(TAG, "Fail to parse uac class specific descriptor\n");
		return status;
	}
	dump_cfgdesc();
	...
	for (j = 0; j < USB_MAX; j++) {
		...
		RTK_LOGI(TAG, "Index=%d,num=%d,alt_count%d,type=%d\n",
		...
		RTK_LOGI(TAG, "EP size info %d,%d,%d\n", ep_size, cur_ep_size, max_ep_size);
...
int usbh_uac_init(usbh_uac_cb_t *cb)
{
	...
	if (cb != NULL) {
		...
		if (cb->init != NULL) {
			ret = cb->init();
			if (ret != HAL_OK) {
				RTK_LOGE(TAG, "USBH uac user init fail\n");
				return ret;
			}
		}
	}
...
int usbh_uac_isoc_transmit(u8 *buf, u32 len)
{
	...
	if ((uac->state == UAC_STATE_IDLE) || (uac->state == UAC_STATE_TRANSFER)) {
		if (ep->isoc_state == UAC_TRANSFER_STATE_IDLE) {
			...
			RTK_LOGW(TAG, "ISOC OUT busy uac->state =%d\n", ep->isoc_state);
		}
	} else {
		RTK_LOGW(TAG, "ISOC OUT not ready\n");
	}
...
int usbh_uac_isoc_receive(u8 *buf, u32 len)
{
	...
	if ((uac->state == UAC_STATE_IDLE) || (uac->state == UAC_STATE_TRANSFER)) {
		if (ep->isoc_state == UAC_TRANSFER_STATE_IDLE) {
			...
			RTK_LOGW(TAG, "ISOC IN busy uac->state =%d\n", ep->isoc_state);
			ret = HAL_BUSY;
		}
	} else {
		RTK_LOGW(TAG, "ISOC IN not ready\n");
		ret = HAL_BUSY;
	}
