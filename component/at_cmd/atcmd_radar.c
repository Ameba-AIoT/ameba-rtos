...
static void at_wlradar_help(void)
{
	RTK_LOGI(NOTAG, "\r\n");
	RTK_LOGI(NOTAG, "AT+WLRADAR=[<type>,<value>,<type>,<value>......]\r\n");
...
AT command process:
	AT+WLRADAR
	Wifi AT Command:
	Config and Enable WiFi Radar.
	[+WLRADAR]:OK
****************************************************************/
void at_wlradar(u16 argc, char **argv)
{
	int ret = 0, i = 0, j = 0;
	...
	if (argc == 1) {
		RTK_LOGW(NOTAG, "[+WLRADAR] The parameters can not be ignored\r\n");
		error_no = RTW_AT_ERR_REQUIRED_PARAM_MISS;
		goto end;
	}

	if ((argc < 3) || (argc > 11)) {
		RTK_LOGW(NOTAG, "[+WLRADAR] The parameters format ERROR\r\n");
	...
	for (i = 1; argc > i; i += 2) {
		...
		if (0 == strcmp("mode", argv[i])) {
			...
			RTK_LOGW(NOTAG, "[+WLRADAR] Invalid parameter type\r\n");
...
AT command process:
	...
	[+WLRADARDBG]:OK
****************************************************************/
void at_wlradardbg(u16 argc, char **argv)
{
	char buf[64] = {0};
	int error_no = RTW_AT_OK;
	int ret = 0;
	...
	if (argc < 2 || argv[1] == NULL) {
		RTK_LOGW(NOTAG, "[WLRADARDBG] Usage: AT+WLRADARDBG=COMMAND[,PARAMETERS]\r\n");
		error_no = RTW_AT_ERR_REQUIRED_PARAM_MISS;
		goto end;
	}

	/* AP-side params: handled directly without going through NP */
	if (!strncmp("radar_", argv[1], 6) &&
		wifi_radar_dbg(argc - 1, &argv[1]) == RTK_SUCCESS) {
		goto end;
	}
...
#endif
	if (ret != RTK_SUCCESS) {
		RTK_LOGW(NOTAG, "[WLRADARDBG] Failed while iwpriv\r\n");
		error_no = RTW_AT_ERR_UNKNOWN_ERR;
	}
...
const log_item_t at_wlradar_items[] = {
	{"+WLRADAR",    at_wlradar},
	{"+WLRADARDBG", at_wlradardbg},
