...
static void server_start(void *param)
{
	...
	************************************************************/
	if (s_mode == NODE_MODE_SSL) {
		...
		if (ret != 0) {
			RTK_LOGI(NOTAG, "[server_start] mbedtls_net_bind returned -%d\r\n", -ret);
		...
		if (s_mode == NODE_MODE_UDP) {
			...
			RTK_LOGI(NOTAG, "[server_start] Unknown connection type[%d]\r\n", s_mode);
		}

		if (s_sockfd == INVALID_SOCKET_ID) {
			RTK_LOGI(NOTAG, "[server_start] Create socket error.\r\n");
			error_no = 5;
			goto last_end;
		}
		...
		if (ret < 0) {
			RTK_LOGI(NOTAG, "[server_start] Set socket option error.\r\n");
			close(s_sockfd);
			error_no = 6;
			goto last_end;
		}
		...
		if (ret < 0) {
			RTK_LOGI(NOTAG, "[server_start] ERROR on binding\r\n");
			close(s_sockfd);
			error_no = 7;
			goto last_end;
		}
	...
	if (s_mode == NODE_MODE_SSL) {
		...
		************************************************************/
		...
		if (ssl_server_cert_pem[0] == NULL) {
			RTK_LOGW(NOTAG, "[server_start] ssl_server_cert_pem[0] malloc fail\r\n");
			error_no = 29;
			goto end;
		}
		...
		if (ret > 0) {
			if (ret > SSL_CERT_SIZE) {
				RTK_LOGW(NOTAG, "[server_start] SERVER_CERT_%d length larger than SSL_CERT_SIZE\r\n", server_pki_index);
				error_no = 30;
				goto end;
			}
			ssl_server_pkey_pem[0] = rtos_mem_zmalloc(SSL_CERT_SIZE);
			if (ssl_server_pkey_pem[0] == NULL) {
				RTK_LOGW(NOTAG, "[server_start] ssl_server_pkey_pem[0] malloc fail\r\n");
				error_no = 33;
				goto end;
			}
			...
			if (ret > 0) {
				if (ret > SSL_CERT_SIZE) {
					RTK_LOGW(NOTAG, "[server_start] SERVER_KEY_%d length larger than SSL_CERT_SIZE\r\n", server_pki_index);
					error_no = 30;
					goto end;
				}
			} else {
				RTK_LOGW(NOTAG, "[server_start] SERVER_KEY_%d read failed\r\n", server_pki_index);
				error_no = 32;
				goto end;
			}
		} else if (ret == 0) {
			RTK_LOGW(NOTAG, "[server_start] NO SERVER_CERT_%d!\r\n", server_pki_index);
			error_no = 31;
			goto end;
		} else {
			RTK_LOGW(NOTAG, "[server_start] SERVER_CERT_%d read failed\r\n", server_pki_index);
			error_no = 32;
			goto end;
		}
		...
		//if((server_auth_mode == SSL_AUTH_MODE_LOAD_CERT) ||(server_auth_mode == SSL_AUTH_MODE_BOTH) )
		{
			...
			if (ServerNodeUsed->cert_crt[0] == NULL) {
				RTK_LOGI(NOTAG, "[server_start] Memory allocation failed for ServerNodeUsed->cert_crt[0]\r\n");
				error_no = 16;
				goto end;
			}
			...
			if (ServerNodeUsed->private_key[0] == NULL) {
				RTK_LOGI(NOTAG, "[server_start] Memory allocation failed for ServerNodeUsed->private_key[0]\r\n");
				error_no = 17;
				goto end;
			}
			...
			if (ret != 0) {
				RTK_LOGI(NOTAG, "[server_start] mbedtls_x509_crt_parse returned -0x%x\r\n", -ret);
				error_no = 18;
				goto end;
			}
			rtos_mem_free(ssl_server_cert_pem[0]);
			ssl_server_cert_pem[0] = NULL;
			RTK_LOGI(NOTAG, "server_cert_0.crt parse successful!\r\n");
			...
			if (ret != 0) {
				RTK_LOGI(NOTAG, "[server_start] mbedtls_pk_parse_key returned -0x%x\r\n", -ret);
				error_no = 20;
				goto end;
			}
			rtos_mem_free(ssl_server_pkey_pem[0]);
			ssl_server_pkey_pem[0] = NULL;
			RTK_LOGI(NOTAG, "server_key_0.key parse successful!\r\n");
		}
		...
		if ((server_auth_mode == SSL_AUTH_MODE_LOAD_CA) || (server_auth_mode == SSL_AUTH_MODE_BOTH))  {
			...
			if (ssl_server_ca_pem[0] == NULL) {
				RTK_LOGW(NOTAG, "[server_start] ssl_server_ca_pem[0] malloc fail\r\n");
				error_no = 34;
				goto end;
			}
			...
			if (ret > 0) {
				if (ret > SSL_CERT_SIZE) {
					RTK_LOGW(NOTAG, "[server_start] SERVER_CA_%d length larger than SSL_CERT_SIZE\r\n", server_ca_index);
					error_no = 30;
					goto end;
				}
			} else if (ret == 0) {
				RTK_LOGW(NOTAG, "[server_start] NO SERVER_CA_%d!\r\n", server_ca_index);
				error_no = 31;
				goto end;
			} else {
				RTK_LOGW(NOTAG, "[server_start] SERVER_CA_%d read failed\r\n", server_ca_index);
				error_no = 32;
				goto end;
			}
			...
			if (ServerNodeUsed->ca_crt[0] == NULL) {
				RTK_LOGI(NOTAG, "[server_start] Memory allocation failed for ServerNodeUsed->ca_crt[0]\r\n");
				error_no = 16;
				goto end;
			}
			...
			RTK_LOGI(NOTAG, "server_ca_0.crt parse successful!\r\n");
		}
		/***********************************************************
		...
		************************************************************/
		...
		while (1) {
			...
			if ((s_newsockfd = accept(s_sockfd, (struct sockaddr *) &s_cli_addr, &s_cli_addr_len)) < 0) {
				RTK_LOGI(NOTAG, "[server_start] ERROR: on accept() ret=%d\r\n", ret);
				error_no = 22;
				goto end;
			}
			...
			************************************************************/
			...
			if ((ssl == NULL) || (conf == NULL)) {
				RTK_LOGI(NOTAG, "[server_start] malloc fail for ssl client context or config!\r\n");
				error_no = 23;
				goto end;
			}
			...
			if ((ret = mbedtls_ssl_config_defaults(conf,
				...
				RTK_LOGI(NOTAG, "[server_start] mbedtls_ssl_config_defaults returned %d\r\n", ret);
				error_no = 24;
				goto end;
			}
			...
			//if((server_auth_mode == SSL_AUTH_MODE_LOAD_CERT) ||(server_auth_mode == SSL_AUTH_MODE_BOTH) )
			{
				...
				if (ret != 0) {
					RTK_LOGI(NOTAG, "ERROR: mbedtls_ssl_conf_own_cert returned %d\r\n", ret);
					error_no = 25;
					goto end;
				}
			}
			...
			if (ret != 0) {
				RTK_LOGI(NOTAG, "ERROR: mbedtls_ssl_setup returned %d\r\n", ret);
				error_no = 26;
				goto end;
			}
			/***********************************************************
			...
			************************************************************/
			...
			if (ret != 0) {
				RTK_LOGI(NOTAG, "ERROR: mbedtls_ssl_handshake returned -0x%x\r\n", -ret);
				error_no = 27;
				goto end;
			}
			...
			************************************************************/
			...
			if (seednode == NULL) {
				RTK_LOGI(NOTAG, "[server_start] create seednode failed!\r\n");
				error_no = 28;
				goto end;
			}
			...
			if (ret < 0) {
				RTK_LOGI(NOTAG, "[AT+SKTSERVER] hang_seednode() return 0\r\n");
...
static void server_start_task(void *param)
{
	...
	if (param != NULL) {
		...
		RTK_LOGW(NOTAG, "server_start_task(): param is NULL!\r\n");
	}
...
void at_sktserver(void *arg)
{
	...
	if (arg == NULL) {
		RTK_LOGW(NOTAG, "[+SKTSERVER] Input parameter is NULL\r\n");
		error_no = 1;
		goto end;
	}
	...
	if (c_mode == NODE_MODE_SSL) {
		...
		************************************************************/
		...
		if (ClientNodeUsed->ssl == NULL) {
			RTK_LOGW(NOTAG, "[client_start] Memory allocation failed for ClientNodeUsed->ssl\r\n");
			error_no = 19;
			goto end;
		}
		ClientNodeUsed->conf = (mbedtls_ssl_config *)rtos_mem_zmalloc(sizeof(mbedtls_ssl_config));
		if (ClientNodeUsed->conf == NULL) {
			RTK_LOGW(NOTAG, "[client_start] Memory allocation failed for ClientNodeUsed->conf\r\n");
			error_no = 19;
			goto end;
		}
		...
		if ((client_auth_mode == SSL_AUTH_MODE_LOAD_CERT) || (client_auth_mode == SSL_AUTH_MODE_BOTH))  {
			...
			if (ssl_client_cert_pem[0] == NULL) {
				RTK_LOGW(NOTAG, "[client_start] ssl_client_cert_pem[0] malloc fail\r\n");
				error_no = 25;
				goto end;
			}
			...
			if (ret > 0) {
				if (ret > SSL_CERT_SIZE) {
					RTK_LOGW(NOTAG, "[client_start] CLIENT_CERT_%d length larger than SSL_CERT_SIZE\r\n", client_pki_index);
					error_no = 26;
					goto end;
				}
				ssl_client_pkey_pem[0] = rtos_mem_zmalloc(SSL_CERT_SIZE);
				if (ssl_client_pkey_pem[0] == NULL) {
					RTK_LOGW(NOTAG, "[client_start] ssl_client_pkey_pem[0] malloc fail\r\n");
					error_no = 29;
					goto end;
				}
				...
				if (ret > 0) {
					if (ret > SSL_CERT_SIZE) {
						RTK_LOGW(NOTAG, "[client_start] CLIENT_KEY_%d length larger than SSL_CERT_SIZE\r\n", client_pki_index);
						error_no = 26;
						goto end;
					}
				} else {
					RTK_LOGW(NOTAG, "[client_start] CLIENT_KEY_%d read failed\r\n", client_pki_index);
					error_no = 28;
					goto end;
				}
			} else if (ret == 0) {
				RTK_LOGW(NOTAG, "[client_start] NO CLIENT_CERT_%d!\r\n", client_pki_index);
				error_no = 27;
				goto end;
			} else {
				RTK_LOGW(NOTAG, "[client_start] CLIENT_CERT_%d read failed\r\n", client_pki_index);
				error_no = 28;
				goto end;
			}
			...
			if (ClientNodeUsed->cert_crt[0] == NULL) {
				RTK_LOGW(NOTAG, "[client_start] Memory allocation failed for ClientNodeUsed->cert_crt[0]\r\n");
				error_no = 19;
				goto end;
			}
			...
			if (ClientNodeUsed->private_key[0] == NULL) {
				RTK_LOGW(NOTAG, "[client_start] Memory allocation failed for ClientNodeUsed->private_key[0]\r\n");
				error_no = 19;
				goto end;
			}
			...
			if (ret != 0) {
				RTK_LOGW(NOTAG, "[client_start] Failed for mbedtls_x509_crt_parse returned %d\r\n", ret);
				error_no = 23;
				goto end;
			}
			rtos_mem_free(ssl_client_cert_pem[0]);
			ssl_client_cert_pem[0] = NULL;
			RTK_LOGI(NOTAG, "client_cert_0.crt parse successful!\r\n");
			...
			if (ret != 0) {
				RTK_LOGW(NOTAG, "[client_start] Failed for mbedtls_pk_parse_key returned %d\r\n", ret);
				error_no = 24;
				goto end;
			}
			rtos_mem_free(ssl_client_pkey_pem[0]);
			ssl_client_pkey_pem[0] = NULL;
			RTK_LOGI(NOTAG, "client_key_0.key parse successful!\r\n");
		...
		if ((client_auth_mode == SSL_AUTH_MODE_LOAD_CA) || (client_auth_mode == SSL_AUTH_MODE_BOTH))  {
			...
			if (ssl_client_ca_pem[0] == NULL) {
				RTK_LOGW(NOTAG, "[client_start] ssl_client_ca_pem[0] malloc fail\r\n");
				error_no = 30;
				goto end;
			}
			...
			if (ret > 0) {
				if (ret > SSL_CERT_SIZE) {
					RTK_LOGW(NOTAG, "[client_start] CLIENT_CA_%d length larger than SSL_CERT_SIZE\r\n", client_ca_index);
					error_no = 26;
					goto end;
				}
			} else if (ret == 0) {
				RTK_LOGW(NOTAG, "[client_start] NO CLIENT_CA_%d!\r\n", client_ca_index);
				error_no = 27;
				goto end;
			} else {
				RTK_LOGW(NOTAG, "[client_start] CLIENT_CA_%d read failed\r\n", client_ca_index);
				error_no = 28;
				goto end;
			}
			...
			if (ClientNodeUsed->ca_crt[0] == NULL) {
				RTK_LOGW(NOTAG, "[client_start] Memory allocation failed for ClientNodeUsed->ca_crt[0]\r\n");
				error_no = 19;
				goto end;
			}
			...
			RTK_LOGI(NOTAG, "client_ca_0.crt parse successful!\r\n");
		...
		/* UDP mode. */
			...
			if (ClientNodeUsed != NULL) {
				...
				if (ClientNodeUsed->local_port) {
					...
					if (ret < 0) {
						RTK_LOGW(NOTAG, "Error binding to client address/port\r\n");
						error_no = 12;
						close(c_sockfd);
						goto last_end;
					}
...
void at_sktclient(void *arg)
{
	...
	if (mode == NODE_MODE_UDP)   {
		if (argc != 5) {
			RTK_LOGW(NOTAG, "[+SKTCLIENT] missing local port\r\n");
			error_no = 11;
			goto end;
		}
	}
	...
	if (remote_port <= 0 || remote_port > 65535) {
		RTK_LOGW(NOTAG, "[+SKTCLIENT] Invalid port range\r\n");
		error_no = 3;
		goto end;
	}
	...
	if (clientnode == NULL) {
		RTK_LOGW(NOTAG, "[+SKTCLIENT] Create client node failed\r\n");
		error_no = 4;
		goto end;
	}
...
void at_sktread(void *arg)
{
	...
	if (arg == NULL) {
		RTK_LOGI(NOTAG, "[at_sktread] Input parameter is NULL\r\n");
		error_no = 1;
		goto end;
	}
	...
	argc = parse_param(arg, argv);
	if (argc != 3) {
		RTK_LOGI(NOTAG, "[at_sktread] Invalid number of parameters\r\n");
		error_no = 1;
		goto end;
	}
...
void at_sktcliconf(void *arg)
{
	...
	if (arg == NULL) {
		RTK_LOGI(NOTAG, "[at_sktcliconf] Input parameter is NULL\r\n");
		error_no = 1;
		goto end;
	}
	argc = parse_param(arg, argv);
	if ((argc < 2) || (argc > 4)) {
		RTK_LOGW(NOTAG, "[at_sktcliconf] Invalid number of parameters\r\n");
		error_no = 1;
		goto end;
	}

	if (strlen(argv[1]) == 0) {
		RTK_LOGW(NOTAG, "[at_sktcliconf] Missing auth_mode\r\n");
		error_no = 2;
		goto end;
	}

	client_auth_mode = atoi(argv[1]);
	if ((client_auth_mode < SSL_AUTH_MODE_NO) || (client_auth_mode > SSL_AUTH_MODE_BOTH)) {
		RTK_LOGW(NOTAG, "[at_sktcliconf] Unknown client_auth_mode\r\n");
		error_no = 3;
		goto end;
	}
...
void at_sktsrvconf(void *arg)
{
	int argc = 0, error_no = 0;
	char *argv[MAX_ARGC] = {0};
	int server_ca_enable = FALSE;

	if (arg == NULL) {
		RTK_LOGI(NOTAG, "[at_sktsrvconf] Input parameter is NULL\r\n");
		error_no = 1;
		goto end;
	}
	argc = parse_param(arg, argv);
