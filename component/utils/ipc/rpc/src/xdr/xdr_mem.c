...
{
	caddr_t	align = (caddr_t)((long)(xdrs->x_private + 3) & 0xfffffffc);
	long	diff = align - xdrs->x_private + 4;

	if ((long) xdrs->x_handy < diff) {
		return FALSE;
	}
	xdrs->x_handy -= diff;
	*lp = (int32_t) ntohl((*((int32_t *)(align))));
	xdrs->x_private += diff;
	return TRUE;
}
...
xdrmem_getint32(XDR *xdrs, int32_t *ip)
{
	...
	xdrs->x_handy -= diff;
	*ip = (int32_t) ntohl((*((int32_t *)(align))));
	xdrs->x_private += diff;
	return TRUE;
}
