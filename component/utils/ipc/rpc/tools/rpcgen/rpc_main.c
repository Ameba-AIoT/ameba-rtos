		 ...
		 const char *outfile)
{
	...
	if (infile && (include = extendfile(infile, ".h"))) {
		char *filename = strrchr(include, '/') == NULL ? NULL : strrchr(include, '/') + 1;
		if (filename == NULL) {
			filename = include;
		}
...
//  fprintf (fout, "#include \"xdr/rpc.h\"\n");
	...
	fprintf(fout, "#include \"rpc_struct.h\"\n\n");
	char *extendFile = extendfile(infile, "_data.h");
	char *filename = strrchr(extendFile, '/') == NULL ? NULL : strrchr(extendFile, '/') + 1;
	if (filename == NULL) {
		filename = extendFile;
	}
	fprintf(fout, "#include \"%s\"\n", filename);
		 ...
		 int extend, const char *outfile, int nomain, int netflag)
{
	...
	if (infile && (include = extendfile(infile, ".h"))) {
		...
		fprintf(fout, "#include <rpc_server.h>\n");
		...
		for (l = defined; l != NULL; l = l->next) {
			...
			if (def->def_kind == DEF_PROGRAM) {
				for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
					...
					fprintf(fout, "\n\tREG_STRUCT * pReg = (struct REG_STRUCT *)RPC_Register(rnode, %s, %s,\n", def->def_name, vp->vers_num);
					fprintf(fout, "\t\t(void (*)(struct RPC_STRUCT *, int32_t, RPC_Mutex*, char*))");
					pvname(def->def_name, vp->vers_num);
					fprintf(fout, ");\n");
					//f_print(fout, "\tRPC_InitReceiverLoop(pReg);\n");
					f_print(fout, "\treturn pReg;\n");
					fprintf(fout, "}\n");
				}
			}
		}
		 ...
		 const char *outfile)
{
	...
	fprintf(fout, "#include <rpc_client.h>\n\n");
	//fprintf(fout, "#include <osal_c/osal_mem.h>\n\n");
