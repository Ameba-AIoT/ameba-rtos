	...
	do {
		...
		do {
			...
			get_type(&plist->res_prefix, &plist->res_type,
					 DEF_PROGRAM);
			//ycyang: Get function's type, store in &plist->res_type.
			//ycyang: result:  1st:(null,long)  2nd:(struct BB)

			if (streq(plist->res_type, "opaque")) {
				error("illegal result type");
			}
			scan(TOK_IDENT, &tok);
			//ycyang: function name, 1st is PRINTNAME, 2nd is ITEST_PRINT..
			...
			//ycyang: get this function's 1st argument
			//        and store in &dec
			...
			/* get args */
			//ycyang: according to "," get next argument if exists.
...
get_prog_declaration(declaration *dec, defkind dkind, int num /* arg number */)
{
	...
	if (dkind == DEF_PROGRAM) {  //ycyang: Function's 1st argument
		peek(&tok);
	...
	get_type(&dec->prefix, &dec->type, dkind);
	//ycyang: 1st:(null, long)   2nd:(struct AA)

	dec->rel = REL_ALIAS;
	if (peekscan(TOK_IDENT, &tok)) {	/* optional name of argument */
		strcpy(name, tok.str);
	}
	//ycyang:  1st: name= "number"
	else {
		sprintf(name, "%s%d", ARGNAME, num);    /* default name of argument */
	}
	//ycyang:  2nd: name= "arg1"
