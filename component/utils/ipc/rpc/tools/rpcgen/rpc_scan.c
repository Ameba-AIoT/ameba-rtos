...
get_token(token *tokp)
{
	...
	for (;;) {
		if (*where == 0) {
			for (;;) {
				...
				if (commenting) {
					...
					docppline(curline, &linenum,
							  &infilename);
					//if(strncmp(curline, "#include", 8) == 0)
					if (strncmp(curline, ".insert", 7) == 0) { // add by ycyang
						break;
					}
			...
			where += 2;
			commenting++;
		} else if (strncmp(where, ".insert", 7) == 0) {
			fprintf(fout, "#include %s", where + 8);
			tokp->kind = TOK_INCLUDE;
			*((char *)where) = 0;
		} else {
			break;
		}
	 ...
	 */
	//if (strncmp(where, "#include", 8) == 0) {
	/*
	  if (strncmp(where, ".insert", 7) == 0) {
	     fprintf(fout, "#include %s", where+8);
	     tokp->kind = TOK_INCLUDE;
	     *((char *)where) =0;
	  }
