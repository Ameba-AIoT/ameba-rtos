					...
					//pprocdef (proc, vp->vers_num, "CLNT_STRUCT *", 0, 0);
					{
						...
						for (ii = 0; ii < 2; ++ii) {
							...
							if (0 == ii) {
								...
								f_print(fout, "\n");
								f_print(fout, "{\n");
								//f_print(fout, "    printf (\"RPC HOOK: ");
								//pvname_svc(proc->proc_name, vp->vers_num);
								//f_print(fout, "() is called!\\n\");\n");
								//f_print(fout, "    printf (\"RPC HOOK: p_");
								//pvname_svc(proc->proc_name, vp->vers_num);
								//f_print(fout, ":%%p\\n\", p_");
								//pvname_svc(proc->proc_name, vp->vers_num);
								//f_print(fout, ");\n");
								...
								f_print(fout, "        p_");
								pvname_svc(proc->proc_name, vp->vers_num);
								f_print(fout, "(pParam, pRpcStruct, pRes);\n");
...
write_program(const definition *def, const char *storage)
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		...
		if (Cflag) {
			...
			//f_print (fout, "register SVCXPRT *%s)\n", TRANSP);
			f_print(fout, "(RPC_STRUCT * rpc, int32_t opt, RPC_Mutex *pMutex, char * buf)\n");
		} else {
			//ycyang: not come in here~
			f_print(fout, "(%s, %s)\n", RQSTP, TRANSP);
			f_print(fout, "	struct svc_req *%s;\n", RQSTP);
		...
		f_print(fout, "\tswitch (rpc->procedure_id) {\n");
		...
		// f_print (fout, "\t\tRPC_EMERG(\"unknown procedure:pid = %%d, procedure_id = %%d\\n\", (int)gettid(), (int)rpc->procedure_id);\n");
		// f_print (fout, "#endif\n");
		...
		//edit by ycyang
		f_print(fout, "\tif (!RPC_GetArgs (rpc, (xdrproc_t) _xdr_argument, (caddr_t) &argument, opt, buf)) {\n");
		f_print(fout, "\t\tprintf(\"get args error\\n\");\n");
		f_print(fout, "\t\tRPC_MutexUnlock(pMutex);\n");
		// f_print (fout, "#ifdef IS_SCPU\n");
		// f_print (fout, "\t\tRPC_EMERG(\"RPC_GetArgs error:pid = %%d, procedure_id = %%d\\n\", (int)gettid(), (int)rpc->procedure_id);\n");
		// f_print (fout, "#endif\n");
		f_print(fout, "\t\treturn;\n");
		f_print(fout, "\t}\n\n");
		...
		f_print(fout, "\tif (rpc->task_id != 0) {\n");
//      f_print (fout, "\t\tif (result != NULL)\n");
		f_print(fout, "\t    RPC_SendReply(rpc->task_id, rpc->context, (char *)&retval, ReplyParaSize,\n\t\t\t (xdrproc_t) _xdr_result, opt);\n");
//      f_print (fout, "\t\telse\n");
...
//	   sp, TRANSP, transp, inetdflag ? "sock" : "RPC_ANYSOCK");
	...
	for (l = defined; l != NULL; l = l->next) {
		...
		for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
			...
			f_print(fout, "\treturn (struct REG_STRUCT *)RPC_Register(rnode, %s, %s,\n", def->def_name, vp->vers_name);
			f_print(fout, "\t\t(void (*)(struct RPC_STRUCT *, int32_t, pthread_mutex_t*))");
			pvname(def->def_name, vp->vers_num);
			f_print(fout, ");\n");
