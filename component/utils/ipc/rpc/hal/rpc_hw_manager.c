...
{
	(void)opt;
	int32_t res = IPC_Message_Queue_Recv(IPC_ID_NUM - id - 1, buf, (uint32_t *)&size);
	return (res == RPC_SUCCESS) ? size : res;
}

static int32_t WriteBuffer(int32_t id, int32_t opt, uint8_t *buf, int size)
{
	int32_t res = IPC_Message_Queue_Send(IPC_ID_NUM - id - 1, buf, (uint32_t)size, ((opt & BLOCK_MODE) ? WAIT_FOREVER : WAIT_TIME_OUT));
	return (res == RPC_SUCCESS) ? size : res;
}
