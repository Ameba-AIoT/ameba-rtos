...
void RPCServerTask(void *param)
{
	...
	while (count < IMQ_TEST_COUNT1) {
		...
		size = IMQ_BUF_SIZE1;
		int32_t ret = IPC_Message_Queue_Recv(QUEUE_NP_TO_AP, buf, &size);
		...
		//if (count % 20 == 0)
		{
			printf("%x recv %d packets, %d bytes, ret=%d\n", QUEUE_NP_TO_AP, (int)count, (int)total_size, (int)ret);
		}
	}
...
void RPCClientTask(void *param)
{
	...
	while (count < IMQ_TEST_COUNT2) {
		...
		int32_t ret = IPC_Message_Queue_Send(QUEUE_NP_TO_AP, buf, IMQ_BUF_SIZE2, WAIT_FOREVER);
		//if (count % 11 == 0)
		{
