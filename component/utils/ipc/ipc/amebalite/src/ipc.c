...
typedef enum {
	IPC_ID_RPC_KR4_TO_KM4	= IPC_ID_NUM - 1,
	IPC_ID_RPC_KR4_TO_DSP	= IPC_ID_NUM - 2,
	IPC_ID_RPC_KM4_TO_DSP	= IPC_ID_NUM - 3,
	IPC_ID_RPC_KM4_TO_KR4	= IPC_ID_NUM - 4,
	IPC_ID_RPC_DSP_TO_KR4	= IPC_ID_NUM - 5,
	IPC_ID_RPC_DSP_TO_KM4	= IPC_ID_NUM - 6,
} RPC_ID;
...
static const IMQ_INIT_TABLE ipc_msg_config[] = {
	//ID						DIR
	{IPC_ID_RPC_KR4_TO_KM4, 	IPC_KR4_TO_KM4}, // Internal used for RPC KR4 -> KM4
	{IPC_ID_RPC_KR4_TO_DSP, 	IPC_KR4_TO_DSP}, // Internal used for RPC KR4 -> DSP
	{IPC_ID_RPC_KM4_TO_DSP, 	IPC_KM4_TO_DSP}, // Internal used for RPC KM4 -> DSP
	{IPC_ID_RPC_KM4_TO_KR4, 	IPC_KM4_TO_KR4}, // Internal used for RPC KM4 -> KR4
	{IPC_ID_RPC_DSP_TO_KR4, 	IPC_DSP_TO_KR4}, // Internal used for RPC DSP -> KR4
	{IPC_ID_RPC_DSP_TO_KM4, 	IPC_DSP_TO_KM4}, // Internal used for RPC DSP -> KM4

	{IPC_ID_KR4_TO_KM4, 		IPC_KR4_TO_KM4}, // User for KR4 -> KM4
	{IPC_ID_KM4_TO_KR4, 		IPC_KM4_TO_KR4}, // User for KM4 -> KR4
	{IPC_ID_KR4_TO_DSP, 		IPC_KR4_TO_DSP}, // User for KR4 -> KM4
	{IPC_ID_DSP_TO_KR4, 		IPC_DSP_TO_KR4}, // User for KM4 -> KR4
	{IPC_ID_DSP_TO_KM4, 		IPC_DSP_TO_KM4}, // User for KR4 -> KM4
	{IPC_ID_KM4_TO_DSP, 		IPC_KM4_TO_DSP}, // User for KM4 -> KR4
};
...
int32_t IPC_Message_Queue_Recv(uint32_t id, uint8_t *buf, uint32_t *size)
{
	...
	do {
		...
		if (readable_size < *size) {
			rtos_sema_take(g_wake_sema[id], 0xFFFFFFFF);
			DCache_Invalidate((uint32_t)info, sizeof(MessageInfo));
		}
...
static void IPC_Message_Handler(void *data, uint32_t irq_status, uint32_t chan_num)
{
	...
	if (g_wake_sema[p_recv_msg->msg]) {
		rtos_sema_give(g_wake_sema[p_recv_msg->msg]);
	}
}

IPC_TABLE_DATA_SECTION
const IPC_INIT_TABLE   rpc_message_handlers[] = {
	{IPC_USER_POINT, IPC_Message_Handler, (void *) &g_ipc_irq_data0, NULL, (void *) NULL, IPC_DIR_MSG_RX_0, IPC_RX_TRAN_0},
	{IPC_USER_POINT, IPC_Message_Handler, (void *) &g_ipc_irq_data1, NULL, (void *) NULL, IPC_DIR_MSG_RX_1, IPC_RX_TRAN_1},
};
