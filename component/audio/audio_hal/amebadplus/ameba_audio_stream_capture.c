...
static void ameba_audio_stream_rx_sport_init(CaptureStream **stream, StreamConfig config, uint32_t device)
{
	...
	cstream->stream.sp_initstruct.SP_SelChLen = SP_TXCL_32;
	...
	case PLL_CLOCK_45P1584M:
		RCC_PeriphClockSource_SPORT(cstream->stream.sport_dev_addr, CKSL_I2S_CPUPLL);
	...
	case I2S_CLOCK_XTAL40M:
		...
		RCC_PeriphClockSource_SPORT(cstream->stream.sport_dev_addr, CKSL_I2S_XTAL40M);
		break;
	}

	uint32_t Tmp;
	if (AUDIO_HW_IN_MCLK_MULITIPLIER > 0) {
		if ((AUDIO_HW_IN_MCLK_MULITIPLIER == 200) || (AUDIO_HW_IN_MCLK_MULITIPLIER == 4)) {
			Tmp = cstream->stream.sport_dev_addr->SP_MCLK;
			Tmp &= ~(SP_MASK_MCLK_MI | SP_MASK_MCLK_NI | SP_BIT_MCLK_MI_NI_UPDATE);
			if (AUDIO_HW_IN_MCLK_MULITIPLIER == 200) {
				/* XTAL:40M MCLK:200k */
				Tmp |= (SP_MCLK_MI(200) | SP_MCLK_NI(1) | SP_BIT_MCLK_MI_NI_UPDATE);
				cstream->stream.sport_dev_addr->SP_MCLK = Tmp;
			} else {
				/* XTAL:40M MCLK:4k */
			...
			AUDIO_SP_SetMclkDiv(cstream->stream.sport_dev_num, Clock_Params.MCLK_NI, Clock_Params.MCLK_MI);
		}
	}
...
static void ameba_audio_stream_rx_adc_mic_configure(StreamConfig config)
{
	...
	for (; i <= adc_num; i++) {
		...
		case AMEBA_AUDIO_DMIC2:
			Pinmux_Config(AUDIO_HW_DMIC_DATA0_PIN, PINMUX_FUNCTION_DMIC_DATA);
			if (!dmic_inited) {
...
int  ameba_audio_stream_rx_get_position(Stream *stream, uint64_t *captured_frames, struct timespec *tstamp)
{
	...
	*captured_frames = now_counter;
	...
	nsec = ameba_audio_fast_get_p1_ns();
...
int  ameba_audio_stream_rx_get_time(Stream *stream, int64_t *now_ns, int64_t *audio_ns)
{
	...
	for (int32_t i = 0 ; i < g_time_cnt; i++) {
		rtos_critical_enter();
		int32_t T1 = SYSTICK_CURRENT;
		tsf_l = ameba_audio_fast_get_p1_l();
		int32_t T2 = SYSTICK_CURRENT;

		if (T2 > T1) {
			delta_tsf = T1 + (SYSTICK_RELOAD - T2);
		} else {
			delta_tsf = T1 - T2;
		}

		if (delta_tsf > 200) { //600ns threhold.
			rtos_critical_exit();
			continue;
		}
		//find correct tsf.
		if (tsf_l - last_tsf_l == 1) {

			ameba_audio_stream_rx_latch_phase();
			int32_t T4 = SYSTICK_CURRENT;
			rtos_critical_exit();

			tsf_h = ameba_audio_fast_get_p1_h();

			delta_counter = ameba_audio_stream_rx_get_counter();
			phase = ameba_audio_stream_rx_get_phase();

			if (T4 > T2) {
				delta_time = T2 + (SYSTICK_RELOAD - T4);
			} else {
				delta_time = T2 - T4;
			}

			usec = ((u64)tsf_h) << 32 | tsf_l;
			nsec = usec * 1000LL + delta_time * 1000000000LL / configCPU_CLOCK_HZ + 150;

			//phase has 600ns drift, +300ns here to make drift -300~300ns.
			now_counter = cstream->stream.total_counter + delta_counter;
			audio = (int64_t)((double)((double)now_counter + (double)phase / (double)32) / (double)cstream->stream.config.rate * (double)1000000000) + 300;
			found = true;
			break;
		}
		rtos_critical_exit();
...
Stream *ameba_audio_stream_rx_init(uint32_t device, StreamConfig config)
{
	...
	HAL_AUDIO_INFO("ameba_audio_stream_rx_init device: %" PRId32 "\n", device);

	cstream->stream.config = config;
	cstream->stream.direction = STREAM_IN;
	cstream->stream.is_multi_io = config.is_multi_io;
	cstream->stream.need_sync_start = config.need_sync_start;

	if (device == AMEBA_AUDIO_IN_I2S) {
		cstream->stream.sport_dev_num = config.sport_index;
...
static void ameba_audio_stream_rx_start_in_noirq_mode(Stream *stream)
{
	...
	if (!cstream->stream.start_gdma) {
		...
		cstream->stream.trigger_tstamp = ameba_audio_fast_get_p1_ns();
...
void ameba_audio_stream_rx_stop(Stream *stream)
{
	...
	// 	rtos_sema_take(cstream->stream.sem_gdma_end, RTOS_MAX_TIMEOUT);
	// 	cstream->stream.sem_gdma_end_need_post = false;
	// }
	// if (cstream->stream.extra_gdma_cnt != cstream->stream.extra_gdma_irq_cnt) {
	// 	cstream->stream.extra_sem_gdma_end_need_post = true;
	// 	rtos_sema_take(cstream->stream.extra_sem_gdma_end, RTOS_MAX_TIMEOUT);
	// 	cstream->stream.extra_sem_gdma_end_need_post = false;
	// }
