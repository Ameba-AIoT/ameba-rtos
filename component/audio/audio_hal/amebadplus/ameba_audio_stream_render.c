...
int  ameba_audio_stream_tx_get_time(Stream *stream, int64_t *now_ns, int64_t *audio_ns)
{
	...
	if (rstream->stream.state != STATE_STARTED) {
		//in xrun
		*now_ns = ameba_audio_fast_get_p0_ns();
		*audio_ns = 0;
		return -8;
	}

	for (int i = 0; i < LATCH_RETRY_CNT; i++) {
		int64_t tsf = ameba_audio_get_wmac_ns();
		int64_t audio = ameba_audio_get_audio_ns();
		int64_t tsf2 = ameba_audio_get_wmac_ns();
		int64_t audio2 = ameba_audio_get_audio_ns();

		if (tsf == tsf2 && audio == audio2) {
			found = true;

			if (audio == 0 && rstream->stream.state == STATE_STARTED) {
				//no latch interrupt callback now.
				*now_ns = rstream->stream.trigger_tstamp;
				*audio_ns = 0;
			} else {
				//normal play, and already has latch interrupt callback.
				*now_ns = tsf;
				*audio_ns = audio;
			}
			break;
...
void ameba_audio_stream_tx_start(Stream *stream, int32_t state)
{
	...
	if (rstream->stream.state == STATE_XRUN_NOTIFIED || rstream->stream.state == STATE_XRUN  || rstream->stream.state == STATE_STANDBY) {
		ameba_audio_start_latch_audio();
		ameba_audio_sport_start();
		rstream->stream.trigger_tstamp = ameba_audio_fast_get_p0_ns();
	}
