...
StreamControl *ameba_audio_get_ctl(void)
{
	...
	if (g_control_instance == NULL) {
		...
		g_control_instance->mic_category_for_adc[3] = AMEBA_AUDIO_AMIC3;
		g_control_instance->gain_for_micbst[0] = MICBST_GAIN_30DB;
		g_control_instance->gain_for_micbst[1] = MICBST_GAIN_30DB;
		g_control_instance->gain_for_micbst[2] = MICBST_GAIN_5DB;
	...
	rtos_mutex_give(s_stream_ctl_instance_lock);
...
int32_t ameba_audio_ctl_set_tx_volume(StreamControl *control, float left, float right)
{
	...
	if (control->volume_for_dac != l) {
		HAL_AUDIO_INFO("set tx volume to 0x%lx", l);
		control->volume_for_dac = l;
...
int32_t ameba_audio_ctl_set_tx_mute(StreamControl *control, bool muted, bool should_zdet, bool user_set)
{
	...
	if (ameba_audio_is_audio_ip_in_use(CODEC)) {
		if (muted) {
			...
			if (should_zdet) {
				AUDIO_CODEC_SetDACVolume(DAC_L, control->volume_for_dac);
			}
		}
	}
...
int32_t ameba_audio_ctl_set_mic_usage(StreamControl *control, uint32_t mic_usage)
{
	...
	case AMEBA_AUDIO_CAPTURE_USAGE_AMIC:
		...
		control->mic_category_for_adc[3] = AMEBA_AUDIO_AMIC3;
		break;
...
int32_t ameba_audio_ctl_set_mic_category_for_adc(StreamControl *control, uint32_t channel, uint32_t mic_category)
{
	...
	if (channel >= MAX_AD_NUM) {
		HAL_AUDIO_ERROR("ops, channel(%lu) out of range", channel);
		return HAL_OSAL_ERR_INVALID_OPERATION;
	}
	...
	if (control->mic_category_for_adc[channel] != mic_category) {
		HAL_AUDIO_INFO("ADC: %" PRIu32 " use mic: %" PRIu32 "", channel, mic_category);
...
int32_t ameba_audio_ctl_get_mic_category_for_adc(StreamControl *control, uint32_t channel)
{
	if (control == NULL) {
		HAL_AUDIO_ERROR("ops, %s fail, control null", __func__);
		return HAL_OSAL_ERR_INVALID_OPERATION;
	...
	return control->mic_category_for_adc[channel];
}

int32_t ameba_audio_ctl_set_adc_volume(StreamControl *control, uint32_t channel, uint32_t volume)
{
	if (control == NULL) {
		HAL_AUDIO_ERROR("ops, %s fail, control null", __func__);
		return HAL_OSAL_ERR_INVALID_OPERATION;
	...
	control->volume_for_adc[channel] = volume;

	if (ameba_audio_is_audio_ip_in_use(CODEC)) {
		AUDIO_CODEC_SetADCVolume(ameba_audio_stream_get_adc_idx(channel + 1), volume);
	}

	return HAL_OSAL_OK;
}

int32_t ameba_audio_ctl_get_adc_volume(StreamControl *control, uint32_t channel)
{
	if (control == NULL) {
		HAL_AUDIO_ERROR("ops, %s fail, control null", __func__);
		return HAL_OSAL_ERR_INVALID_OPERATION;
	...
	return control->volume_for_adc[channel];
}
...
float ameba_audio_ctl_pll_clock_tune(StreamControl *control, uint32_t rate, float ppm, uint32_t action)
{
	...
	if (rate % 8000 == 0) {
		tuned_ppm = PLL_I2S_98P304M_ClkTune(CKSL_I2S_CPUPLL, ppm, action);
	} else {
		tuned_ppm = PLL_I2S_45P158M_ClkTune(CKSL_I2S_CPUPLL, ppm, action);
	}

	return tuned_ppm;
}

int32_t ameba_audio_ctl_set_adc_mute(StreamControl *control, uint32_t channel, bool muted)
{
	...
	control->mute_for_adc[channel] = muted;

	if (ameba_audio_is_audio_ip_in_use(CODEC)) {
		adc_state = muted ? MUTE : UNMUTE;
		AUDIO_CODEC_SetADCMute(ameba_audio_stream_get_adc_idx(channel + 1), adc_state);
	}

	return HAL_OSAL_OK;
}

bool ameba_audio_ctl_get_adc_mute(StreamControl *control, uint32_t channel)
{
	if (control == NULL) {
		HAL_AUDIO_ERROR("ops, %s fail, control null", __func__);
		return false;
	...
	return control->mute_for_adc[channel];
}