...
struct AudioHwAdapter *GetAudioHwAdapterFuncs()
{
	...
	adap = (struct PrimaryAudioHwAdapter *)rtos_mem_zmalloc(sizeof(struct PrimaryAudioHwAdapter));
	if (!adap) {
		return NULL;
	}
...
void CloseAudioHwAdapter(struct AudioHwAdapter *adapter)
{
	...
	rtos_mutex_delete(adap->lock);

	if (adapter != NULL) {
		rtos_mem_free(adapter);
		//adapter = NULL;
	}
}
