...
#define DUMP_BUFS                 0
#define HAL_LITTLEFS_DUMP         0

#if DUMP_BUFS
#define DUMP_FRAME                44100
char buf_in_dump[DUMP_FRAME * 4];
static uint32_t buf_in_size_dumped = 0;
char buf_out_dump[DUMP_FRAME * 2];
static uint32_t buf_out_size_dumped = 0;
#endif

#if HAL_LITTLEFS_DUMP
#include <fcntl.h>
static int s_lfs_fd = 0;
static char *s_lfs_name = "lfs://halaudio.raw";
#endif
...
#if HAL_LITTLEFS_DUMP
		if (s_lfs_fd > 0) {
			HAL_AUDIO_INFO("standby close lfs");
...
static int PrimaryGetPresentTime(const struct AudioHwRender *stream, int64_t *now_ns, int64_t *audio_ns)
{
	...
	if (out->out_pcm) {
		int64_t written_to_driver_frames = out->written_to_driver;
...
#if HAL_LITTLEFS_DUMP
	HAL_AUDIO_INFO("open lfs");
	s_lfs_fd = (int)fopen(s_lfs_name, "w+");
#endif
								  ...
								  size_t bytes, bool block)
{
	...
	if (out->out_pcm) {
		if (out->channel_count == 2 && AUDIO_HW_ENABLE_MIX) {

#if DUMP_BUFS
			if (buf_in_size_dumped + bytes <= DUMP_FRAME * 4) {
				memcpy(buf_in_dump + buf_in_size_dumped, buffer, bytes);
				buf_in_size_dumped += bytes;
			}
#endif

			out->buffer = (char *)rtos_mem_calloc(bytes / 2, 1);
			ProcessMix(buffer, (int16_t *)out->buffer, bytes / frame_size);

#if DUMP_BUFS
			if (buf_out_size_dumped + bytes / 2 <= DUMP_FRAME * 2) {
				memcpy(buf_out_dump + buf_out_size_dumped, out->buffer, bytes / 2);
				buf_out_size_dumped += bytes / 2 ;
			}
#endif

			ret = ameba_audio_stream_tx_write(out->out_pcm, (void *)out->buffer, bytes / 2, block);
			ret *= 2;
			rtos_mem_free(out->buffer);
		} else {
			ret = ameba_audio_stream_tx_write(out->out_pcm, (void *)buffer, bytes, block);
			//int64_t dump_start = DTimestamp_Get();
#if HAL_LITTLEFS_DUMP
			if (s_lfs_fd > 0) {
				fwrite(buffer, 1, bytes, (FILE *)s_lfs_fd);
				//HAL_AUDIO_INFO("fwrite takes time:%lld", DTimestamp_Get() - dump_start);
			}
#endif
		}
	} else {
		...
		const struct AudioHwSampleAttributes *attrs, uint32_t flags)
{
	...
	out = (struct PrimaryAudioHwRender *)rtos_mem_zmalloc(sizeof(struct PrimaryAudioHwRender));
	if (!out) {
		return NULL;
	}
...
#if DUMP_BUFS && AUDIO_HW_ENABLE_MIX
	HAL_AUDIO_INFO("buf_in_dump:%p, buf_out_dump:%p", buf_in_dump, buf_out_dump);
#endif
