 ...
 *
 * @brief Provides audio basic functions.
 *
 * @since 1.0
 * @version 1.0
 */
...
static inline size_t GetAudioBytesPerSample(enum AudioHwFormat format)
{
	size_t size = 0;
...
static inline bool AudioIsLinearPCM(enum AudioHwFormat format)
{
	return ((format & AUDIO_HW_FORMAT_MAIN_MASK) == AUDIO_HW_FORMAT_PCM);
}

static inline bool AudioCheckValidFormat(enum AudioHwFormat format)
{
