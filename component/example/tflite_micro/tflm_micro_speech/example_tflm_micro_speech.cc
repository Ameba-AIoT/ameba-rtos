...
TfLiteStatus GenerateFeatures(const int16_t *audio_data,
    ...
    while (remaining_samples >= kAudioSampleDurationCount &&
        ...
        TF_LITE_ENSURE_STATUS(GenerateSingleFeature(
            audio_data, kAudioSampleDurationCount,
...
TfLiteStatus micro_speech_test(void) {
    ...
    TF_LITE_MICRO_TEST(NoTest) {
        TestAudioSample("no", g_no_1000ms_audio_data,
                        g_no_1000ms_audio_data_size);
    }

    TF_LITE_MICRO_TEST(YesTest) {
        TestAudioSample("yes", g_yes_1000ms_audio_data,
