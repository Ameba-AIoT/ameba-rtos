...
#define MEM2MEM		0	//memory to memory access
#define MEM2QSPI	1	//memory to qspi access

#define ACCESS		MEM2MEM
...
static void Prepare_PPE(void)
{
	...
	/* src addr is QSPI fifo register */
...
static void ppe_as_dma(void *para)
{
	UNUSED(para);
	int i = 0;
	int index = 0;
...
static void ppe_as_dma(void *para)
{
	...
	/*initial result layer*/
	PPE_Result_Layer.src_addr					= (u32)DST;
	PPE_Result_Layer.width						= WIDTH;
	PPE_Result_Layer.height						= HEIGHT;
	PPE_Result_Layer.format 					= PPE_RGB888;
	PPE_Result_Layer.line_len					= PPE_Result_Layer.width;
	...
	while (1) {
		...
		DelayMs(500);
		DCache_Invalidate((u32)DST, MEM_SIZE);

		/*Verify the data of result layer*/
		for (u32 counter = 0; counter < (PPE_Result_Layer.width * PPE_Result_Layer.height * 3); counter++) {
			if (PIX_DATA[counter] == DST[counter]) {
				//printf("expected output byte[%x] = %x \r\n",counter,PIC_OUTPUT_LAYER[counter]);
				//printf("result output byte[%x] = %x \r\n",counter,SOURCE_PIC_1[counter]);
				continue;
			} else {
				printf("[error] has occured!\r\n");
				//printf("expected output byte[%x] = %x \r\n", counter, PIX_DATA[counter]);
				//printf("result 	output byte[%x] = %x \r\n", counter, DST[counter]);
			}
		}

		printf("ppe transfer over\n");
...
int main(void)
{
	if (rtos_task_create(NULL, "ppe_as_dma", (rtos_task_t)ppe_as_dma, NULL, (2048), (1)) != SUCCESS) {
		printf("Cannot create ppe_as_dma demo task\n\r");
	}
