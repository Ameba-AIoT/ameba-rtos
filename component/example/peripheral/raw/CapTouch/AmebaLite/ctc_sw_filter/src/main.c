...
u32 ctc_irq_handler(void *para)
{
	...
	u32 IntStatus;
	u32 valid_cnt;
	static u32 fifo_last = 0xF000;
	static u32 fifo_read_cnt = 0;

	IntStatus = CapTouch_GetISR(CAPTOUCH_DEV);

	if (IntStatus & CT_BIT_AFIFO_OVERLVL_INTR) {
		/* get FIFO valid cnt */
		valid_cnt = CT_GET_AFIFO_VALID_CNT(CAPTOUCH_DEV->CT_RAW_CODE_FIFO_STATUS);

		for (i = 0; i < valid_cnt; i++) {
			fifo_data = CAPTOUCH_DEV->CT_RAW_CODE_FIFO_READ;
			/* store the second data after ch_num varies as valid data for further filtering */
			/* the first data after ch_num varies has error and should be ignored */
			if ((CT_GET_AFIFO_RD_CH_NUM(fifo_data) != CT_GET_AFIFO_RD_CH_NUM(fifo_last)) && (i < valid_cnt - 1)) {
				fifo_last = fifo_data;
				fifo_data = CAPTOUCH_DEV->CT_RAW_CODE_FIFO_READ;
				valid_cnt--;
				/* avoid that ch_num varies after the first data */
				if (CT_GET_AFIFO_RD_CH_NUM(fifo_data) != CT_GET_AFIFO_RD_CH_NUM(fifo_last)) {
					fifo_last = fifo_data;
					fifo_data = CAPTOUCH_DEV->CT_RAW_CODE_FIFO_READ;
					valid_cnt--;
				}
				...
				for (j = 0; j < CTC_EN_CH; j++) {
					if (CT_GET_AFIFO_RD_CH_NUM(fifo_data) == ctc_flt[j].ctc_ch_idx) {
						ctc_flt[j].raw_buf[buffer_switch % 2][ctc_flt[j].buf_index++] = (u16)(CT_GET_AFIFO_RD_DATA(fifo_data));
						break;
					}
				}
			}
	...
	if (IntStatus & CT_BIT_AFIFO_OVERFLOW_INTR) {
		...
		CAPTOUCH_DEV->CT_RAW_CODE_FIFO_STATUS |= CT_BIT_AFIFO_CLEAR; // clear captoutch fifo
	}
...
void ctc_board_init(void)
{
	...
	/* set sample clock: xtal2m/8 */
	CapTouch_SetClkPara(CAPTOUCH_DEV, CTC_CLK_XTAL, 8);
	...
	for (i = 0; i < CT_CHANNEL_NUM; i++) {
		if (ctc_ch_config[i].CT_CHEnable == ENABLE) {
			...
			if (_PB_1 == ctc_ch_pad[i]) {
				Pinmux_Swdoff();
			}

			PAD_PullCtrl(ctc_ch_pad[i], GPIO_PuPd_NOPULL);
			PAD_InputCtrl(ctc_ch_pad[i], DISABLE);
			Pinmux_Config(ctc_ch_pad[i], PINMUX_FUNCTION_AUXIN);
		}
	}

	if (ch_idx != CTC_EN_CH) {
		RTK_LOGS(NOTAG, RTK_LOG_WARN, "\nWarning:\n");
		RTK_LOGS(NOTAG, RTK_LOG_WARN, "\tPlease modify CTC_EN_CH to match enabled chan num!!!\n");
		assert_param(0);
	}
	...
	CapTouch_SetFifoLevel(CAPTOUCH_DEV, (SAMPLE_NUM + 1)* CTC_EN_CH - 1); // 0~63, default: 0
	/* enable AFIFO_OVERLVL interrupt */
	/* disable PRESS/RELEASE interrupt */
	CapTouch_INTConfig(CAPTOUCH_DEV, CT_BIT_AFIFO_OVERLVL_INTR_EN | CT_BIT_AFIFO_OVERFLOW_INTR_EN, ENABLE);
