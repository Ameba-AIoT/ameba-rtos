...
static void flash_test_task(void *param)
{
	...
	printf("flash ID is: %02x-%02x-%02x \n", flash_info[0], flash_info[1], flash_info[2]);
	if (flash_info[0] != 0xef) {
		printf("Not Winbond flash. Please modify the setting of the status register in this example according to the actual Flash data sheet \n");
		while (1);
	}
	...
	for (loop = 0; loop < 8; loop++) {
		...
		for (i = 0; i < length; i++) {
			if (write_data1[i] != buff[i]) {
				printf("Error : Write Data is = %x, Read Data is %x\n", write_data1[i], buff[i]);
				result1 = 0;
			}
		}
		if (result1 == 1) {
		...
		printf("Lock\n");
		result2 = 1;
		printf("Status Register Before Setting: 0x%02x\n", 0xff & flash_get_status(&flash));
		...
		for (i = 0; i < length; i++) {
			if (write_data1[i] != buff[i]) {
				printf("Error : Write Data1 is = %x, Write Data2 is = %x, Read Data is %x\n", write_data1[i], write_data2[i], buff[i]);
				result2 = 0;
			}
		}
		if (result2 == 1) {
		...
		printf("Unlock\n");
		result3 = 1;
		printf("Status Register After Setting: 0x%02x\n", 0xff & flash_get_status(&flash));
		...
		printf("Status Register After Reset: 0x%02x\n", 0xff & flash_get_status(&flash));
