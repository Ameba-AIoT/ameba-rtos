...
void example_vfs_usbh_thread(void *param)
{
	rtos_time_delay_ms(3000);
	char filename[] = "vfs_file_1";
	char content[] = "VFS usbh module works normally !!!";
	char path[128] = {0};
	char buffer[64] = {0};
	char *prefix;
	FILE *finfo;
	...
	res = vfs_user_register("usbh", VFS_FATFS, VFS_INF_USBH, VFS_REGION_4, VFS_RW);
	if (res == 0) {
		RTK_LOGS(NOTAG, RTK_LOG_INFO, "VFS-USBH Init Success \n");
	} else {
		RTK_LOGS(NOTAG, RTK_LOG_ERROR, "VFS-USBH Init Fail \n");
		goto exit;
	}
...
void example_vfs_usbh(void)
{
	if (rtos_task_create(NULL, ((const char *)"example_vfs_usbh_thread"), example_vfs_usbh_thread, NULL, 4096 * 4, 1) != RTK_SUCCESS) {
		RTK_LOGS(NOTAG, RTK_LOG_ERROR, "\n\r%s rtos_task_create(example_vfs_usbh_thread) failed", __FUNCTION__);
	}
}
