...
#include "example_atcmd_host_spi.h"
...
void example_atcmd_host_spi(void)
{
	...
	if (xTaskCreate((void *)atcmd_host_spi_task, ((const char *)"atcmd_host_spi_task"), 1024 / sizeof(portSTACK_TYPE), NULL, 5, NULL) != pdPASS) {
		RTK_LOGS(NOTAG, RTK_LOG_ALWAYS, "\n\r%s rtos_task_create(atcmd_host_spi_task) failed", __FUNCTION__);
	}
