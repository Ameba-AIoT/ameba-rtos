...
/* ---------------------------- Static Variables ---------------------------- */
static int a2dp_sample_rate = 48000;
static int a2dp_channels = 2;
static int a2dp_bits = 16;
...
static void a2dp_task_entry(void *ctx)
{
	...
	BT_LOGA("[A2DP Demo] a2dp_task_entry\r\n");
	while (a2dp_task.run) {
#if 0
...
static uint16_t rtk_bt_a2dp_sbc_parse_decoder_struct(rtk_bt_a2dp_codec_t *pa2dp_codec, rtk_bt_sbc_decode_t *psbc_decoder_t)
{
	...
	BT_LOGD("[A2DP] parse_decoder %d-%d-%d %d \r\n",
			psbc_decoder_t->sampling_frequency,
			channels,
			BT_AUDIO_FORMAT_PCM_16_BIT,
			psbc_decoder_t->block_length);
...
static uint16_t rtk_bt_a2dp_sbc_parse_encoder_struct(rtk_bt_a2dp_codec_t *pa2dp_codec, rtk_bt_sbc_encode_t *psbc_encoder_t)
{
	...
	a2dp_sample_rate = psbc_encoder_t->sample_rate;
	a2dp_channels = psbc_encoder_t->channel_mode == SBC_CHANNEL_MODE_MONO ? 1 : 2;
	a2dp_bits = psbc_encoder_t->blocks;

	BT_LOGD("[A2DP] parse_encoder %d-%d-%d \r\n",
			psbc_encoder_t->sample_rate,
			psbc_encoder_t->channel_mode,
			psbc_encoder_t->blocks);
...
char g_urls[MAX_URL_NUM][MAX_URL_SIZE];
int g_url_num = 0;

#define MAX_PAUSE_RESUME_TIME 10
int g_pause_at[MAX_PAUSE_RESUME_TIME] = {0};
int g_resume_at[MAX_PAUSE_RESUME_TIME] = {0};
int g_pause_resume_num = 0;

#define MAX_REWIND_TIME 10
int g_rewind_at[MAX_REWIND_TIME] = {0};
int g_rewind_to[MAX_REWIND_TIME] = {0};
int g_rewind_num = 0;

#define MAX_PAUSE_REWIND_TIME 10
int g_pause_rewind_at[MAX_PAUSE_REWIND_TIME] = {0};
int g_pause_rewind_to[MAX_PAUSE_REWIND_TIME] = {0};
int g_pause_rewind_num = 0;
...
{
	printf("Usage: player_a2dp [OPTION...]\n"
...
ssize_t RTPlayer_AudioSinkCallback(int16_t *data, size_t len)
{
	...
	BT_LOGD("[A2DP] tx %d %d \r\n", src_a2dp_credits, a2dp_demo_send_data_seq);

	ssize_t written = 0;
	if (src_a2dp_credits) {
		penc_codec_buffer_t = rtk_bt_audio_data_encode(RTK_BT_AUDIO_CODEC_SBC, a2dp_demo_codec_entity, data, len);
...
void OnStateChanged(const RTPlayerCallback *listener, const RTPlayer *player, int state)
{
...
void OnInfo(const RTPlayerCallback *listener, const RTPlayer *player, int info, int extra)
{
...
void OnError(const RTPlayerCallback *listener, const RTPlayer *player, int error, int extra)
{
	printf("OnError (%p %p), (%d, %d)\n", player, listener, error, extra);
	g_playing_status = IDLE;
}

long long GetNowUs(void) //1us=1000ms=1000000s
{
	return rtos_time_get_current_system_time_us();
}

void StartPlay(struct RTPlayer *player, char *url, int *pause_array, int *resume_array, int *rewind_at, int *rewind_to, int *pause_rewind_at,
			   int *pause_rewind_to)
{
	printf("start to play: %s\n", url);
	int32_t ret = AUDIO_OK;
	for (int i = 0; i < g_pause_resume_num; i++) {
		printf("pause at %dms, resume at %dms\n", pause_array[i], resume_array[i]);
	}

	for (int i = 0; i < g_rewind_num; i++) {
		printf("rewind at %dms, rewind to %dms\n", rewind_at[i], rewind_to[i]);
	}

	for (int i = 0; i < g_pause_rewind_num; i++) {
		printf("pause at %dms, rewind to %dms\n", pause_rewind_at[i], pause_rewind_to[i]);
	}
	...
	long long startUs = GetNowUs();
	printf("startUs is %lld ms\n", startUs / 1000);
	int i, j, k;
	i = j = k = 0;

	while (g_playing_status == PLAYING || g_playing_status == PAUSED || g_playing_status == REWIND_COMPLETE) {
		rtos_time_delay_ms(1000);

		// pause resume test
		if (i < g_pause_resume_num) {
			long long now_us = GetNowUs();
			if (g_playing_status == PLAYING && pause_array[i] > 0 && ((now_us - startUs) / 1000 > pause_array[i])) {
				printf("enter pause at %ds.\n", pause_array[i]);
				RTPlayer_Pause(player);
				pause_array[i] = -1;
				startUs = GetNowUs();
			}
			if (g_playing_status == PAUSED && resume_array[i] > 0 && ((now_us - startUs) / 1000 > resume_array[i])) {
				printf("enter resume at %ds.\n", resume_array[i]);
				g_playing_status = PLAYING;
		...
		if (j < g_rewind_num) {
			...
			RTPlayer_GetCurrentTime(player, &position);
			printf("GetCurrentTime %.2f secs\n", position / 1E3);
			if (rewind_at[j] >= 0 && position >= rewind_at[j]) {
				printf("enter rewind at %.2f secs, rewind to %dms\n", position / 1E3, rewind_to[j]);
				RTPlayer_Rewind(player, rewind_to[j]);
				rewind_at[j] = -1;
			}
			if (g_playing_status == REWIND_COMPLETE) {
				g_playing_status = PLAYING;
				j++;
			}
		}
		...
		// pause rewind test
		if (k < g_pause_rewind_num) {
			long long now_us = GetNowUs();
			if (g_playing_status == PLAYING && pause_rewind_at[k] > 0 && ((now_us - startUs) / 1000 > pause_rewind_at[k])) {
				printf("ps enter pause at %ds.\n", pause_rewind_at[k]);
				RTPlayer_Pause(player);
				pause_rewind_at[k] = -1;
				startUs = GetNowUs();
			}
			if (g_playing_status == PAUSED && pause_rewind_to[k] >= 0) {
				printf("ps enter rewind to %ds.\n", pause_rewind_to[k]);
				RTPlayer_Rewind(player, pause_rewind_to[j]);
			}
			if (g_playing_status == REWIND_COMPLETE) {
				printf("ps rewind completed, start play.\n");
				g_playing_status = PLAYING;
				RTPlayer_Start(player);
				j++;
			}
...
int player_a2dp_test(int argc, char **argv)
{
	printf("start player a2dp\n");

	g_url_num = 0;
	g_pause_resume_num = 0;
	g_rewind_num = 0;
	g_pause_rewind_num = 0;
	...
	while (argc > 0 && *argv) {
		...
		if (strcmp(*argv, "-?") == 0 || strcmp(*argv, "--help") == 0) {
			...
			argv++;
			memset(g_urls[g_url_num], 0, MAX_URL_SIZE);
			if (!strncasecmp("http://", *argv, 7) || !strncasecmp("https://", *argv, 8)) {
				snprintf(g_urls[g_url_num], MAX_URL_SIZE, "%s", *argv);
			} else {
				snprintf(g_urls[g_url_num], MAX_URL_SIZE, "%s", *argv);
			}
			g_url_num++;
			...
			argv++;
			if (*argv) {
				g_pause_at[g_pause_resume_num] = atoi((const char *)*argv);
			}
		} else if (strcmp(*argv, "-r") == 0) {
			argv++;
			if (*argv) {
				g_resume_at[g_pause_resume_num] = atoi((const char *)*argv);
			}
			g_pause_resume_num ++;
		} else if (strcmp(*argv, "-ra") == 0) {
			argv++;
			if (*argv) {
				g_rewind_at[g_rewind_num] = atoi((const char *)*argv);
			}
		} else if (strcmp(*argv, "-rt") == 0) {
			argv++;
			if (*argv) {
				g_rewind_to[g_rewind_num] = atoi((const char *)*argv);
			}
			g_rewind_num ++;
		} else if (strcmp(*argv, "-pra") == 0) {
			argv++;
			if (*argv) {
				g_pause_rewind_at[g_pause_rewind_num] = atoi((const char *)*argv);
			}
		} else if (strcmp(*argv, "-prt") == 0) {
			argv++;
			if (*argv) {
				g_pause_rewind_to[g_pause_rewind_num] = atoi((const char *)*argv);
			}
			g_pause_rewind_num ++;
		}
		if (*argv) {
	...
	for (int i = 0; i < g_url_num; i++) {
		printf("Usage: g_urls[%d]: %s\n", i, g_urls[i]);
	}

	for (int i = 0; i < g_pause_resume_num; i++) {
		printf("Usage: pause at %ds, resume at %ds\n", g_pause_at[i], g_resume_at[i]);
	}

	RTPlayerCallback *callback = (RTPlayerCallback *)malloc(sizeof(RTPlayerCallback));
	if (!callback) {
		printf("malloc RTPlayerCallback fail.\n");
		return -1;
	}
	...
	g_player = RTPlayer_CreateEx(AUDIO_SINK_TYPE_BT);
	RTPlayer_SetCallback(g_player, callback);

	// invoke bt audio params
	Parcel *request = Parcel_Create();
	Parcel_WriteInt32(request, 1111);
	Parcel_WriteInt32(request, a2dp_sample_rate);
	Parcel_WriteInt32(request, a2dp_channels);
	Parcel_WriteInt32(request, a2dp_bits);
	Parcel_WriteInt32(request, 1024);
	RTPlayer_Invoke(g_player, request, NULL);
	Parcel_Destroy(request);

	for (int i = 0; i < g_url_num; i++) {
		int pause_array[g_pause_resume_num], resume_array[g_pause_resume_num];
		for (int j = 0; j < g_pause_resume_num; j ++) {
			pause_array[j] = g_pause_at[j];
			resume_array[j] = g_resume_at[j];
		}

		int rewind_at[g_rewind_num], rewind_to[g_rewind_num];
		for (int j = 0; j < g_rewind_num; j ++) {
			rewind_at[j] = g_rewind_at[j];
			rewind_to[j] = g_rewind_to[j];
		}

		int pause_rewind_at[g_pause_rewind_num], pause_rewind_to[g_pause_rewind_num];
		for (int j = 0; j < g_pause_rewind_num; j ++) {
			pause_rewind_at[j] = g_pause_rewind_at[j];
			pause_rewind_to[j] = g_pause_rewind_to[j];
		}

		StartPlay(g_player, g_urls[i], pause_array, resume_array, rewind_at, rewind_to, pause_rewind_at, pause_rewind_to);
	}
...
u32 player_a2dp_thread(u16 argc, u8 *argv[])
{
	printf("player_a2dp_thread\n");
	(void) argc;
	(void) argv;
	int night_run = 0;

	if (strcmp((const char *)*argv, "-nt") == 0) {
		argv++;
		night_run = atoi((const char *)*argv);
		argv++;
	}
	printf("night_run: %d\n", night_run);

	unsigned int heap_size = rtos_mem_get_free_heap_size();
	printf("heap_size (%d-0x%x)\n", (int)heap_size, (unsigned int)heap_size);

	int count = 0;
	do {
		count ++;
		printf("Player test start......%d\n", count);

		MEM_DEBUG_INFO_INIT();

		player_a2dp_test(argc, (char **)argv);

		rtos_time_delay_ms(1 * 1000);

		MEM_DEBUG_INFO_DUMP();
		printf("Player test done......%d\n", count);
...
const COMMAND_TABLE cutils_test_cmd_table[] = {
	{
		//player_a2dp -nt 1 -F http://m4a-player.test.upcdn.net/3_441k_2_3min.mp3 -ra 9000 -rt 129000
		(const u8 *)"player_a2dp", 1, player_a2dp_thread, (const u8 *)"\t player_a2dp\n"
	},
};