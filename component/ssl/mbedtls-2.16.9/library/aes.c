						  ...
						  unsigned char output[16])
{
	...
	if (use_hw_crypto_func) {
		...
		if (mode == MBEDTLS_AES_DECRYPT) {
			...
			rtl_crypto_aes_ecb_decrypt(input, 16, NULL, 0, output_buf_aligned);
		} else {
			memcpy(key_buf_aligned, ctx->enc_key, ((ctx->nr - 6) * 4));
			rtl_crypto_aes_ecb_init(key_buf_aligned, ((ctx->nr - 6) * 4));
			rtl_crypto_aes_ecb_encrypt(input, 16, NULL, 0, output_buf_aligned);
		}
		...
		if (length > 0) {
			...
			if (mode == MBEDTLS_AES_DECRYPT) {
				...
				while ((length - length_done) > RTL_CRYPTO_FRAGMENT) {
					...
					rtl_crypto_aes_cbc_decrypt(input + length_done, RTL_CRYPTO_FRAGMENT, iv_buf_aligned, 16, output_buf_aligned + length_done);
					memcpy(iv_buf_aligned, iv_tmp, 16);
					length_done += RTL_CRYPTO_FRAGMENT;
				}

				memcpy(iv_tmp, (input + length - 16), 16);
				rtl_crypto_aes_cbc_decrypt(input + length_done, length - length_done, iv_buf_aligned, 16, output_buf_aligned + length_done);
				memcpy(iv, iv_tmp, 16);
			} else {
				memcpy(key_buf_aligned, ctx->enc_key, ((ctx->nr - 6) * 4));
				rtl_crypto_aes_cbc_init(key_buf_aligned, ((ctx->nr - 6) * 4));
				...
				while ((length - length_done) > RTL_CRYPTO_FRAGMENT) {
					rtl_crypto_aes_cbc_encrypt(input + length_done, RTL_CRYPTO_FRAGMENT, iv_buf_aligned, 16, output_buf_aligned + length_done);
					memcpy(iv_buf_aligned, (output_buf_aligned + length_done + RTL_CRYPTO_FRAGMENT - 16), 16);
					length_done += RTL_CRYPTO_FRAGMENT;
				}

				rtl_crypto_aes_cbc_encrypt(input + length_done, length - length_done, iv_buf_aligned, 16, output_buf_aligned + length_done);
							 ...
							 unsigned char *output)
{
	...
	if (use_hw_crypto_func) {
		...
		if (length > 0) {
			...
			memcpy(iv_buf_aligned, iv, 16);
			memcpy(key_buf_aligned, ctx->enc_key, ((ctx->nr - 6) * 4));
			rtl_crypto_aes_cfb_init(key_buf_aligned, ((ctx->nr - 6) * 4));

			if (mode == MBEDTLS_AES_DECRYPT) {
				while ((length - length_done) > RTL_CRYPTO_FRAGMENT) {
					rtl_crypto_aes_cfb_decrypt(input + length_done, RTL_CRYPTO_FRAGMENT, iv_buf_aligned, 16, output_buf_aligned + length_done);
					memcpy(iv_buf_aligned, (input + length_done + RTL_CRYPTO_FRAGMENT - 16), 16);
					length_done += RTL_CRYPTO_FRAGMENT;
				}

				rtl_crypto_aes_cfb_decrypt(input + length_done, length - length_done, iv_buf_aligned, 16, output_buf_aligned + length_done);
				memcpy(iv, (input + length - 16), 16);
			} else {
				while ((length - length_done) > RTL_CRYPTO_FRAGMENT) {
					rtl_crypto_aes_cfb_encrypt(input + length_done, RTL_CRYPTO_FRAGMENT, iv_buf_aligned, 16, output_buf_aligned + length_done);
					memcpy(iv_buf_aligned, (output_buf_aligned + length_done + RTL_CRYPTO_FRAGMENT - 16), 16);
					length_done += RTL_CRYPTO_FRAGMENT;
				}

				rtl_crypto_aes_cfb_encrypt(input + length_done, length - length_done, iv_buf_aligned, 16, output_buf_aligned + length_done);
				memcpy(iv, (output_buf_aligned + length - 16), 16);
			}
			memcpy(output, output_buf_aligned, length);
			mbedtls_free(output_buf);
						  ...
						  unsigned char *output)
{
	...
	if (use_hw_crypto_func) {
		...
		if (length > 0) {
			...
			memcpy(iv_buf_aligned, iv, 16);
			memcpy(key_buf_aligned, ctx->enc_key, ((ctx->nr - 6) * 4));
			rtl_crypto_aes_ofb_init(key_buf_aligned, ((ctx->nr - 6) * 4));

			while ((length - length_done) > RTL_CRYPTO_FRAGMENT) {
				rtl_crypto_aes_ofb_encrypt(input + length_done, RTL_CRYPTO_FRAGMENT, iv_buf_aligned, 16, output_buf_aligned + length_done);
				length_done += RTL_CRYPTO_FRAGMENT;
				for (i = 0; i < RTL_CRYPTO_FRAGMENT; i += 16) {
					mbedtls_aes_crypt_ecb(ctx, MBEDTLS_AES_ENCRYPT, iv_buf_aligned, iv_buf_aligned);
				}
				rtl_crypto_aes_ofb_init(key_buf_aligned, ((ctx->nr - 6) * 4));
			}

			rtl_crypto_aes_ofb_encrypt(input + length_done, length - length_done, iv_buf_aligned, 16, output_buf_aligned + length_done);
			memcpy(output, output_buf_aligned, length);
			mbedtls_free(output_buf);
		}
						  ...
						  unsigned char *output)
{
	...
	if (use_hw_crypto_func) {
		...
		if (length > 0) {
			...
			memcpy(key_buf_aligned, ctx->enc_key, ((ctx->nr - 6) * 4));
			rtl_crypto_aes_ctr_init(key_buf_aligned, ((ctx->nr - 6) * 4));

			while ((length - length_done) > RTL_CRYPTO_FRAGMENT) {
				rtl_crypto_aes_ctr_encrypt(input + length_done, RTL_CRYPTO_FRAGMENT, iv_buf_aligned, 16, output_buf_aligned + length_done);
				length_done += RTL_CRYPTO_FRAGMENT;
				for (j = 0; j < RTL_CRYPTO_FRAGMENT; j += 16) {
					for (i = 16; i > 0; i--) {
						if (++iv_buf_aligned[i - 1] != 0) {
							break;
						}
					}
				}
			...
			rtl_crypto_aes_ctr_encrypt(input + length_done, length - length_done, iv_buf_aligned, 16, output_buf_aligned + length_done);
			memcpy(output, output_buf_aligned, length);
			mbedtls_free(output_buf);
		}
