							  ...
							  unsigned char *tag)
{
	...
	if (use_hw_crypto_func && (length <= RTL_CRYPTO_FRAGMENT)) {
		...
		if ((ret = rtl_crypto_aes_gcm_init(key_buf_aligned, (ctx->cipher_ctx.key_bitlen / 8))) != 0) {
			return ret;
		}
		if (mode == MBEDTLS_GCM_DECRYPT) {
			if ((ret = rtl_crypto_aes_gcm_decrypt(input, length, iv_buf_aligned, add, add_len, output_buf_aligned, tag_buf_aligned)) != 0) {
				return ret;
			}
		} else {
			if ((ret = rtl_crypto_aes_gcm_encrypt(input, length, iv_buf_aligned, add, add_len, output_buf_aligned, tag_buf_aligned)) != 0) {
				return ret;
			}
		}

		memcpy(output, output_buf_aligned, length);
		memcpy(tag, tag_buf_aligned, tag_len);
