...
rtos_task_t g_tcp_server_task = NULL;
rtos_task_t g_tcp_client_task = NULL;
rtos_task_t g_udp_client_task = NULL;
rtos_task_t g_udp_server_task = NULL;
rtos_task_t g_timer_sync_server_task = NULL;
rtos_task_t g_timer_sync_client_task = NULL;
...
rtos_sema_t	timer_sync_ok_sema = NULL;
u64 sync_time = 0;
u8 timer_sync_ok = 0;
...
int tcp_client_func(struct iperf_data_t iperf_data)
{
	...
	if (iperf_data.total_size == 0) {
		...
		while (((end_time - start_time) <= (1000 * iperf_data.time)) && (!g_tcp_terminate)) {
...
//Restart:
	...
	while (!g_tcp_terminate) {
		...
		if ((iperf_data.report_interval != DEFAULT_REPORT_INTERVAL) && ((end_time - report_start_time) >= (1000 * iperf_data.report_interval)) &&
			((end_time - report_start_time) <= (1000 * (iperf_data.report_interval + 1)))) {
...
int udp_client_func(struct iperf_data_t iperf_data)
{
	...
	if (iperf_data.total_size == 0) {
		...
		report_start_time = start_time;
		client_hdr.mAmount = htonl(~(iperf_data.time * 100) + 1);
		rtos_sema_take(timer_sync_ok_sema, RTOS_MAX_DELAY);
		printf("start udp tx\n");
		while (((end_time - start_time) <= (1000 * iperf_data.time)) && (!g_udp_terminate)) {
...
rtos_sema_t udp_server_rfoff_sema = NULL;
...
#else
	rtos_sema_give(udp_server_rfoff_sema);
#endif
}
...
int udp_server_timeout_start(size_t delay)
{
	...
	do {
		if (rtos_timer_change_period(recv_timeout_timer, delay, 500) == pdFALSE) {
		...
		// started.
		if (rtos_timer_start(recv_timeout_timer, 500) != pdPASS) {
...
void udp_server_rfoff_thread(void)
{
	...
	do {
		rtos_sema_take(udp_server_rfoff_sema, 0xFFFFFFFF);
...
{
	rtos_timer_create(&recv_timeout_timer, "recv_timeout_timer", NULL, (0x7fffffff), TRUE, udp_server_timeout_handler);
...
{
	rtos_timer_stop(recv_timeout_timer, 5000);
	rtos_timer_delete(recv_timeout_timer, 5000);

	udp_server_stop = 1;
	rtos_sema_give(udp_server_rfoff_sema);
	rtos_sema_delete(udp_server_rfoff_sema);
}
...
void tdma_timer_sync_server(void *param)
{
	...
	timer_server_buffer = rtos_mem_malloc(sizeof(sync_hdr));
...
Exit3:
	if (timer_server_buffer) {
		rtos_mem_free(timer_server_buffer);
...
void tdma_timer_sync_client(void *param)
{
	...
	// for internal tese
	timer_client_buffer = rtos_mem_malloc(sizeof(sync_hdr));
...
Exit2:
	printf("\n\r%s: Close client socket", __func__);
	if (timer_client_buffer) {
		rtos_mem_free(timer_client_buffer);
...
int udp_server_func(struct iperf_data_t iperf_data)
{
	...
	if (time_boundary) {
		...
		while (((end_time - start_time) <= (1000 * client_hdr.mAmount)) && (!g_udp_terminate)) {
			...
			if (strcmp(argv[argv_count - 1], "-s") == 0) {
				...
				if (argc == 2) {
					...
					if (g_tcp_server_task) {
						...
						printf("\n\rTCP server stopped!\n");
						rtos_task_delete(g_tcp_server_task);
						g_tcp_server_task = NULL;
					}
...
void cmd_udp(int argc, char **argv)
{
	...
	g_udp_bidirection = 0;
	/* Create a Semaphone */
	rtos_sema_create_binary(&timer_sync_ok_sema);
	while (argv_count <= argc) {
		//first operation
		if (argv_count == 2) {
			if (strcmp(argv[argv_count - 1], "-s") == 0) {
				...
				if (argc == 2) {
					...
					if (g_udp_server_task) {
						...
						printf("\n\rUDP server stopped!\n");
						rtos_task_delete(g_udp_server_task);
						g_udp_server_task = NULL;
					}
					...
					if (g_timer_sync_server_task) {
						...
						rtos_task_delete(g_timer_sync_server_task);
						g_timer_sync_server_task = NULL;
					}
					...
					if (g_timer_sync_client_task) {
						...
						rtos_task_delete(g_timer_sync_client_task);
						g_timer_sync_client_task = NULL;
					}
