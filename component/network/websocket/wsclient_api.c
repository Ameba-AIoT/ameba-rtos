...
uint32_t wsclient_sendblocktime = 0;
...
static void ws_dispatchBinary(wsclient_context **wsclient)
{
	...
	while (1) {
		...
		if ((uint64_t)wsc->rx_len < ws.header_size + ws.N) {
			if (ws.header_size + ws.N > (uint64_t)max_data_len) {
				WSCLIENT_ERROR("Got message size exceed the max buf %llu, %d\n", ws.header_size + ws.N, max_data_len);
			}
		...
		if (ws.opcode == TEXT_FRAME || ws.opcode == BINARY_FRAME || ws.opcode == CONTINUATION) {
			...
			WSCLIENT_DEBUG("string:::%s\n", rcvData + wsc->wsclient_reallength);
			WSCLIENT_DEBUG("string len = %d\n", (int)ws.N);
			...
			WSCLIENT_DEBUG("ws.fin = %d, ws.N = %d\n", ws.fin, ws.N);
			if (ws.fin != 0) {
...
void ws_poll(int timeout, wsclient_context **wsclient)   // timeout in milliseconds
{
	...
	if (timeout != 0) {
		...
		if (rtos_queue_peek(wsc->ready_send_buf, (void *)&tmp_buf, 0) == SUCCESS) {
			if (tmp_buf->tx_len > 0) {
	...
	while (1) {
		ret = wsc->fun_ops.client_read(wsc, (char *)&wsc->rxbuf[wsc->rx_len], (max_data_len - wsc->rx_len) > 1500 ? 1500 : (max_data_len - wsc->rx_len));
		if (ret == 0) {
			break;
		} else if (ret < 0) {
			closesocket(wsc->sockfd);
...
//send buffer in ready queue
	while (rtos_queue_peek(wsc->ready_send_buf, (void *)&tmp_buf, 0) == SUCCESS) {
...
int ws_sendBinary(uint8_t *message, int message_len, int use_mask, wsclient_context *wsclient)
{
	...
	ret = ws_sendData(BINARY_FRAME, (size_t)message_len, (uint8_t *)message, use_mask, wsclient);
	return ret;
}
...
int ws_send(char *message, int message_len, int use_mask, wsclient_context *wsclient)
{
	...
	ret = ws_sendData(TEXT_FRAME, (size_t)message_len, (uint8_t *)message, use_mask, wsclient);
	return ret;
}
