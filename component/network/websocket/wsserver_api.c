			...
			if (FD_ISSET(ws_server_sock, &u.read_fds)) {
				...
				if ((client_sock = accept(ws_server_sock, (struct sockaddr *) &client_addr, &client_addr_size)) >= 0) {
					...
					if (conn == NULL) {
						...
						if (rtos_task_create(&conn->task.task,  "ws_server_conn", ws_server_conn_thread, conn, ws_server_stack_bytes,  WS_SERVER_PRIORITY) != SUCCESS) {
...
int ws_server_start(uint16_t port, uint8_t max_conn, uint32_t stack_bytes, uint8_t secure)
{
	...
	if (rtos_task_create(&ws_server_tsk.task, "ws_server", ws_server_thread, NULL, ws_server_stack_bytes, WS_SERVER_PRIORITY) != SUCCESS) {
