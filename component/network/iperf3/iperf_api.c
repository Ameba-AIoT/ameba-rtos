...
usage_long(FILE *f)
{
	...
	RTK_LOGI("NOTAG", usage_longstr, UDP_RATE / (1024 * 1024), DURATION, DEFAULT_TCP_BLKSIZE / 1024, DEFAULT_UDP_BLKSIZE / 1024);
}
...
#if 0
	int len = strlen(server_hostname) + 1;
	ipt->server_hostname = (char *)malloc(len);

	if (ipt->server_hostname == NULL) {
		printf("\niperf_set_test_server_hostname : malloc fail\n");
		return NULL;
	}
	return (char *) memcpy(ipt->server_hostname, server_hostname, len);
#endif
}
...
#if 0
	int len = strlen(tmp_template) + 1;
	ipt->tmp_template = (char *)malloc(len);

	if (ipt->tmp_template == NULL) {
		printf("\iperf_set_test_template : malloc fail\n");
		return NULL;
	}
	return (char *) memcpy(ipt->tmp_template, tmp_template, len);
#endif
}
...
iperf_on_test_start(struct iperf_test *test)
{
	if (test->json_output) {
		cJSON_AddItemToObject(test->json_start, "test_start",
							  iperf_json_printf("protocol: %s  num_streams: %d  blksize: %d  omit: %d  duration: %d  bytes: %d  blocks: %d  reverse: %d  tos: %d", test->protocol->name,
												(int64_t) test->num_streams, (int64_t) test->settings->blksize, (int64_t) test->omit, (int64_t) test->duration, (int64_t) test->settings->bytes,
												(int64_t) test->settings->blocks, test->reverse ? (int64_t)1 : (int64_t)0, test->settings->tos));
	} else {
		if (test->verbose) {
			if (test->settings->bytes) {
				...
				iperf_printf(test, test_start_blocks, test->protocol->name, test->num_streams, test->settings->blksize, test->omit, test->settings->blocks,
							 test->settings->tos);
			} else {
...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
	...
	while ((flag = getopt_long(argc, argv, "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
		...
		case OPT_NO_FQ_SOCKET_PACING:
			...
			RTK_LOGI("NOTAG", "Warning:  --no-fq-socket-pacing is deprecated\n");
	...
	*/
	...
	if (test->json_output) {
		if (test->duration && test->stats_interval && test->reporter_interval) {
...
iperf_exchange_parameters(struct iperf_test *test)
{
	...
	if (test->role == 'c') {
		...
		if ((s = test->protocol->listen_iperf(test)) < 0) {
			if (iperf_set_send_state(test, SERVER_ERROR) != 0) {
				return -1;
			}
			err = htonl(i_errno);
...
get_parameters(struct iperf_test *test)
{
	...
	if (j == NULL) {
		...
		if ((j_p = cJSON_GetObjectItem(j, "len")) != NULL) {
			...
			if (test->settings->blksize > DEFAULT_BLKSIZE_MAX) {
				printf("blksize is too large, this test may fail,blksize: %d \n", test->settings->blksize);
				printf("Please set the appropriate size by using '-l ' option\n");
				r = -1;
			}
		}
...
connect_msg(struct iperf_stream *sp)
{
	...
	if (sp->test->json_output) {
		cJSON_AddItemToArray(sp->test->json_connected, iperf_json_printf("socket: %d  local_host: %s  local_port: %d  remote_host: %s  remote_port: %d",
...
iperf_defaults(struct iperf_test *testp)
{
	...
	tcp->id = Ptcp;
	tcp->name = "TCP";
	tcp->accept_iperf = iperf_tcp_accept;
	tcp->listen_iperf = iperf_tcp_listen;
	tcp->connect_iperf = iperf_tcp_connect;
	tcp->send_iperf = iperf_tcp_send;
	tcp->recv_iperf = iperf_tcp_recv;
	tcp->init = NULL;
	...
	udp->id = Pudp;
	udp->name = "UDP";
	udp->accept_iperf = iperf_udp_accept;
	udp->listen_iperf = iperf_udp_listen;
	udp->connect_iperf = iperf_udp_connect;
	udp->send_iperf = iperf_udp_send;
	udp->recv_iperf = iperf_udp_recv;
	...
	sctp->id = Psctp;
	sctp->name = "SCTP";
	sctp->accept_iperf = iperf_sctp_accept;
	sctp->listen_iperf = iperf_sctp_listen;
	sctp->connect_iperf = iperf_sctp_connect;
	sctp->send_iperf = iperf_sctp_send;
	sctp->recv_iperf = iperf_sctp_recv;
...
iperf_print_intermediate(struct iperf_test *test)
{
	...
	SLIST_FOREACH(sp, &test->streams, streams) {
		...
		if (irp) {
			...
			if (test->debug) {
				printf("interval_len %f bytes_transferred %lu\n", interval_len, (long unsigned int)irp->bytes_transferred);
			}
		...
		if (sp) {
			...
			if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
				if (test->sender && test->sender_has_retransmits) {
					...
					if (test->json_output) {
						cJSON_AddItemToObject(json_interval, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  omitted: %b",
											  (double) start_time, (double) end_time, (double) irp->interval_duration, (int64_t) bytes, bandwidth * 8, (int64_t) retransmits,
											  irp->omitted));    /* XXX irp->omitted or test->omitting? */
					} else {
						iperf_printf(test, report_sum_bw_retrans_format, start_time, end_time, ubuf, nbuf, retransmits,
									 irp->omitted ? report_omitted : "");    /* XXX irp->omitted or test->omitting? */
					}
				} else {
					/* Interval sum, TCP without retransmits. */
					if (test->json_output) {
						cJSON_AddItemToObject(json_interval, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b",
				...
				if (test->sender) {
					if (test->json_output) {
						cJSON_AddItemToObject(json_interval, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  packets: %d  omitted: %b",
											  (double) start_time, (double) end_time, (double) irp->interval_duration, (int64_t) bytes, bandwidth * 8, (int64_t) total_packets, test->omitting));
					} else {
					...
					if (test->json_output) {
						cJSON_AddItemToObject(json_interval, "sum",
											  iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  omitted: %b",
																(double) start_time, (double) end_time, (double) irp->interval_duration, (int64_t) bytes, bandwidth * 8, (double) avg_jitter * 1000.0, (int64_t) lost_packets,
...
iperf_print_results(struct iperf_test *test)
{
	...
	if (sp) {
		...
		SLIST_FOREACH(sp, &test->streams, streams) {
			...
			if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
				if (test->sender_has_retransmits) {
					...
					if (test->json_output) {
						cJSON_AddItemToObject(json_summary_stream, "sender",
											  iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  max_snd_cwnd:  %d  max_rtt:  %d  min_rtt:  %d  mean_rtt:  %d",
																(int64_t) sp->socket, (double) start_time, (double) sender_time, (double) sender_time, (int64_t) bytes_sent, bandwidth * 8,
																(int64_t) sp->result->stream_retrans, (int64_t) sp->result->stream_max_snd_cwnd, (int64_t) sp->result->stream_max_rtt, (int64_t) sp->result->stream_min_rtt,
																(int64_t)((sp->result->stream_count_rtt == 0) ? 0 : sp->result->stream_sum_rtt / sp->result->stream_count_rtt)));
					} else if (test->role == 's' && !test->sender) {
						if (test->verbose) {
					...
					if (test->json_output) {
						cJSON_AddItemToObject(json_summary_stream, "sender", iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f",
											  (int64_t) sp->socket, (double) start_time, (double) sender_time, (double) sender_time, (int64_t) bytes_sent, bandwidth * 8));
					} else if (test->role == 's' && !test->sender) {
						if (test->verbose) {
							iperf_printf(test, report_sender_not_available_format, sp->socket);
				...
				if (test->json_output) {
					...
					cJSON_AddItemToObject(json_summary_stream, "udp",
										  iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  out_of_order: %d",
															(int64_t) sp->socket, (double) start_time, (double) sender_time, (double) sender_time, (int64_t) bytes_sent, bandwidth * 8, (double) sp->jitter * 1000.0,
															(int64_t)(sp->cnt_error - sp->omitted_cnt_error), (int64_t)(packet_count - sp->omitted_packet_count), (double) lost_percent,
															(int64_t)(sp->outoforder_packets - sp->omitted_outoforder_packets)));
				} else {
					/*
					...
					if (test->role == 's' && !test->sender) {
						...
						iperf_printf(test, report_bw_udp_format, sp->socket, start_time, sender_time, ubuf, nbuf, 0.0, 0, (sender_packet_count - sp->omitted_packet_count), (double) 0,
									 report_sender);
					}
				...
				if (fstat(sp->diskfile_fd, &sb) == 0) {
					...
					if (test->json_output) {
						cJSON_AddItemToObject(json_summary_stream, "diskfile",
											  iperf_json_printf("sent: %d  received: %d  size: %d  percent_sent: %d  percent_received: %d  filename: %s", (int64_t) bytes_sent, (int64_t) bytes_received,
																(int64_t) sb.st_size, (int64_t) percent_sent, (int64_t) percent_received, test->diskfile_name));
					} else if (test->sender) {
			...
			if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
				...
				if (test->json_output) {
					cJSON_AddItemToObject(json_summary_stream, "receiver", iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f",
										  (int64_t) sp->socket, (double) start_time, (double) receiver_time, (double) end_time, (int64_t) bytes_received, bandwidth * 8));
				} else if (test->role == 's' && test->sender) {
					if (test->verbose) {
				...
				if (! test->json_output) {
					...
					if (test->role == 's' && test->sender) {
						...
						iperf_printf(test, report_bw_udp_format, sp->socket, start_time, receiver_time, ubuf, nbuf, sp->jitter * 1000.0, (sp->cnt_error - sp->omitted_cnt_error),
									 (receiver_packet_count - sp->omitted_packet_count), lost_percent, report_receiver);
					}
				}
			}
		}
	...
	if (test->num_streams > 1 || test->json_output) {
		...
		if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
			if (test->sender_has_retransmits) {
				...
				if (test->json_output) {
					cJSON_AddItemToObject(test->json_end, "sum_sent", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d",
										  (double) start_time, (double) sender_time, (double) sender_time, (int64_t) total_sent, bandwidth * 8, (int64_t) total_retransmits));
				} else if (test->role == 's' && !test->sender) {
					if (test->verbose) {
						iperf_printf(test, report_sender_not_available_format, sp->socket);
					}
				...
				if (test->json_output) {
					cJSON_AddItemToObject(test->json_end, "sum_sent", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (double) start_time,
										  (double) sender_time, (double) sender_time, (int64_t) total_sent, bandwidth * 8));
				} else if (test->role == 's' && !test->sender) {
					if (test->verbose) {
						iperf_printf(test, report_sender_not_available_format, sp->socket);
					}
			...
			unit_snprintf(nbuf, UNIT_LEN, bandwidth, test->settings->unit_format);
			if (test->json_output) {
				cJSON_AddItemToObject(test->json_end, "sum_received", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (double) start_time,
									  (double) receiver_time, (double) receiver_time, (int64_t) total_received, bandwidth * 8));
			} else if (test->role == 's' && test->sender) {
				if (test->verbose) {
					iperf_printf(test, report_receiver_not_available_format, sp->socket);
				}
			...
			if (test->json_output) {
				cJSON_AddItemToObject(test->json_end, "sum",
									  iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f",
														(double) start_time, (double) receiver_time, (double) receiver_time, (int64_t) total_sent, bandwidth * 8, (double) avg_jitter * 1000.0, (int64_t) lost_packets,
														(int64_t) total_packets, (double) lost_percent));
			} else {
				/*
				...
				if (!(test->role == 's' && test->sender)) {
					...
					unit_snprintf(nbuf, UNIT_LEN, bandwidth, test->settings->unit_format);
					iperf_printf(test, report_sum_bw_udp_format, start_time, receiver_time, ubuf, nbuf, avg_jitter * 1000.0, lost_packets, receiver_total_packets, lost_percent,
								 "receiver");
				}
			}
		}
	...
	if (test->json_output) {
		cJSON_AddItemToObject(test->json_end, "cpu_utilization_percent",
							  iperf_json_printf("host_total: %f  host_user: %f  host_system: %f  remote_total: %f  remote_user: %f  remote_system: %f", (double) test->cpu_util[0],
												(double) test->cpu_util[1], (double) test->cpu_util[2], (double) test->remote_cpu_util[0], (double) test->remote_cpu_util[1],
		...
		if (test->verbose) {
			iperf_printf(test, report_cpu, report_local, test->sender ? report_sender : report_receiver, test->cpu_util[0], test->cpu_util[1], test->cpu_util[2],
						 report_remote, test->sender ? report_receiver : report_sender, test->remote_cpu_util[0], test->remote_cpu_util[1], test->remote_cpu_util[2]);
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
	...
	if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
		if (test->sender && test->sender_has_retransmits) {
			...
			if (test->json_output) {
				cJSON_AddItemToArray(json_interval_streams,
									 iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  snd_cwnd:  %d  rtt:  %d  rttvar: %d  pmtu: %d  omitted: %b",
													   (int64_t) sp->socket, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8,
													   (int64_t) irp->interval_retrans, (int64_t) irp->snd_cwnd, (int64_t) irp->rtt, (int64_t) irp->rttvar, (int64_t) irp->pmtu, irp->omitted));
			} else {
			...
			if (test->json_output) {
				cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b",
									 (int64_t) sp->socket, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, irp->omitted));
			} else {
		...
		if (test->sender) {
			if (test->json_output) {
				cJSON_AddItemToArray(json_interval_streams,
									 iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  packets: %d  omitted: %b", (int64_t) sp->socket, (double) st,
													   (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, (int64_t) irp->interval_packet_count, irp->omitted));
			} else {
			...
			if (test->json_output) {
				cJSON_AddItemToArray(json_interval_streams,
									 iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  omitted: %b",
													   (int64_t) sp->socket, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, (double) irp->jitter * 1000.0,
													   (int64_t) irp->interval_cnt_error, (int64_t) irp->interval_packet_count, (double) lost_percent, irp->omitted));
			} else {
				iperf_printf(test, report_bw_udp_format, sp->socket, st, et, ubuf, nbuf, irp->jitter * 1000.0, irp->interval_cnt_error, irp->interval_packet_count,
							 lost_percent, irp->omitted ? report_omitted : "");
			}
		}
	}
...
{
	IPERF3_DBG_INFO("iperf_new_stream \n");
	...
	if (sp->buffer == NULL) {
		...
		free(sp->result);
		free(sp);
		IPERF3_DBG_INFO("sp->buffer=null\n");
		return NULL;
	}

	IPERF3_DBG_INFO("iperf_new_stream sp->buffer !=null\n");
	memset(sp->buffer, 0, test->settings->blksize);
	for (int i = 0; i < test->settings->blksize; i++) {
		sp->buffer[i] = (char)(i % 10);
	}

	/* Set socket */
	sp->socket = s;

	sp->snd = test->protocol->send_iperf;
	sp->rcv = test->protocol->recv_iperf;
#if 0
...
{
	IPERF3_DBG_INFO("iperf_json_finish\n");
	if (test->title) {
	...
	if (test->json_output_string == NULL) {
		printf("iperf_json_finish,cJSON_Print fail\n");
