...
#if IPERF3_DBG_ON
#define IPERF3_DBG(fmt, args...)    printf("\n\r%s: " fmt, __FUNCTION__, ## args)
#define IPERF3_DBG_INFO(x)     printf(x)
#else
#define IPERF3_DBG(fmt, args...)
#define IPERF3_DBG_INFO(x)
#endif

extern unsigned char g_server_terminate;
extern unsigned char g_client_terminate;
extern struct task_struct g_server_task;
extern struct task_struct g_client_task;
...
struct protocol {
	int       id;
	char      *name;
	int (*accept_iperf)(struct iperf_test *);
	int (*listen_iperf)(struct iperf_test *);
	int (*connect_iperf)(struct iperf_test *);
	int (*send_iperf)(struct iperf_stream *);
	int (*recv_iperf)(struct iperf_stream *);
