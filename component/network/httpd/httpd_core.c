		...
		if (httpd_thread_mode == HTTPD_THREAD_SINGLE) {
			...
			for (i = 0; i < httpd_max_conn; i ++) {
				if (httpd_connections[i].sock >= 0) {
					FD_SET(httpd_connections[i].sock, &u.read_fds);
				}
			}
		}

		if (select(httpd_max_sock + 1, &u.read_fds, NULL, NULL, &timeout)) {
			if (FD_ISSET(httpd_sock, &u.read_fds)) {
				...
				if ((client_sock = accept(httpd_sock, (struct sockaddr *) &client_addr, &client_addr_size)) >= 0) {
					...
					if (conn == NULL) {
						...
						if (httpd_thread_mode == HTTPD_THREAD_MULTIPLE) {
							if (rtos_task_create(NULL, ((const char *) "httpd_conn"), httpd_conn_thread, conn, httpd_stack_bytes, httpd_priority) != SUCCESS) {
				...
				for (i = 0; i < httpd_max_conn; i ++) {
					if ((httpd_connections[i].sock >= 0) && (FD_ISSET(httpd_connections[i].sock, &u.read_fds))) {
...
int httpd_accept(int server_sock, struct httpd_conn **client_conn, int accept_timeout)
{
	...
	timeout.tv_usec = 0;
	FD_ZERO(&u.read_fds);
	FD_SET(server_sock, &u.read_fds);

	if (select(server_sock + 1, &u.read_fds, NULL, NULL, &timeout)) {
		if (FD_ISSET(server_sock, &u.read_fds)) {
			struct sockaddr_in client_addr;
			unsigned int client_addr_size = sizeof(client_addr);
			int client_sock;
			struct httpd_conn *conn = NULL;
...
int httpd_start(uint16_t port, uint8_t max_conn, uint32_t stack_bytes, uint8_t thread_mode, uint8_t secure)
{
	...
	if (rtos_task_create(NULL, ((const char *) "httpd_server"), httpd_server_thread, NULL, httpd_stack_bytes, httpd_priority) != SUCCESS) {
