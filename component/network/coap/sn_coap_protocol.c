...
static void                  sn_coap_protocol_linked_list_blockwise_payload_store(struct coap_s *handle, sn_nsdl_addr_s *addr_ptr, uint16_t stored_payload_len,
		uint8_t *stored_payload_ptr);
static uint8_t              *sn_coap_protocol_linked_list_blockwise_payload_search(struct coap_s *handle, sn_nsdl_addr_s *src_addr_ptr,
...
static sn_coap_hdr_s        *sn_coap_handle_blockwise_message(struct coap_s *handle, sn_nsdl_addr_s *src_addr_ptr, sn_coap_hdr_s *received_coap_msg_ptr,
		void *param);
	...
	if (returned_dst_coap_msg_ptr->msg_type == COAP_MSG_TYPE_CONFIRMABLE ||
		...
		if (sn_coap_protocol_linked_list_duplication_info_search(handle, src_addr_ptr, returned_dst_coap_msg_ptr->msg_id) == -1) {
			...
			if (stored_duplication_msgs_count >= handle->sn_coap_duplication_buffer_size) {
				...
				sn_coap_protocol_linked_list_duplication_info_remove(handle, stored_duplication_info_ptr->addr_ptr, stored_duplication_info_ptr->port,
...
static sn_coap_hdr_s *sn_coap_protocol_copy_header(struct coap_s *handle, sn_coap_hdr_s *source_header_ptr)
{
	...
	if (source_header_ptr->options_list_ptr) {
		...
		if (source_header_ptr->options_list_ptr->proxy_uri_ptr) {
			...
			memcpy(destination_header_ptr->options_list_ptr->proxy_uri_ptr, source_header_ptr->options_list_ptr->proxy_uri_ptr,
				   source_header_ptr->options_list_ptr->proxy_uri_len);
		}
		...
		if (source_header_ptr->options_list_ptr->uri_host_ptr) {
			...
			memcpy(destination_header_ptr->options_list_ptr->uri_host_ptr, source_header_ptr->options_list_ptr->uri_host_ptr,
				   source_header_ptr->options_list_ptr->uri_host_len);
		}
		...
		if (source_header_ptr->options_list_ptr->location_path_ptr) {
			...
			memcpy(destination_header_ptr->options_list_ptr->location_path_ptr, source_header_ptr->options_list_ptr->location_path_ptr,
		...
		if (source_header_ptr->options_list_ptr->location_query_ptr) {
			...
			memcpy(destination_header_ptr->options_list_ptr->location_query_ptr, source_header_ptr->options_list_ptr->location_query_ptr,
				   source_header_ptr->options_list_ptr->location_query_len);
		}
		...
		if (source_header_ptr->options_list_ptr->uri_query_ptr) {
			...
			memcpy(destination_header_ptr->options_list_ptr->uri_query_ptr, source_header_ptr->options_list_ptr->uri_query_ptr,
				   source_header_ptr->options_list_ptr->uri_query_len);
		}
