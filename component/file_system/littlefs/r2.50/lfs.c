									 ...
									 int (*cb)(void *data, lfs_tag_t tag, const void *buffer), void *data)
{
	...
	uint32_t revs[2] = {0, 0};
	uint32_t fmt_flag = 0;    //realtek add to judge first mount or not
	int r = 0;
	for (int i = 0; i < 2; i++) {
		int err = lfs_bd_read(lfs,
	...
	for (int i = 0; i < 2; i++) {
		...
		while (true) {
			...
			if (!lfs_tag_isvalid(tag)) {
				...
				//realtek add to judge first mount or not
				if (tag == 0xffffffff) {
					fmt_flag = 1;
				}
				break;
						   ...
						   lfs_mdir_t *source, uint16_t begin, uint16_t end)
{
	...
	if (tired && lfs_pair_cmp(dir->pair, (const lfs_block_t[2]) {
	0, 1
}) != 0) {
...
relocate:
		...
		if (lfs_pair_cmp(dir->pair, (const lfs_block_t[2]) {
		0, 1
	}) == 0) {
									...
									lfs_mdir_t *source, uint16_t begin, uint16_t end)
{
	...
	&& lfs_pair_cmp(dir->pair, (const lfs_block_t[2]) {
	0, 1
}) == 0) {
						...
						lfs_size_t pos, lfs_block_t *block, lfs_off_t *off)
{
	...
	lfs_off_t current = lfs_ctz_index(lfs, &(lfs_off_t) {
		size - 1
	});
							...
							int (*cb)(void *, lfs_block_t), void *data)
{
	...
	lfs_off_t index = lfs_ctz_index(lfs, &(lfs_off_t) {
		size - 1
	});
									 ...
									 const void *buffer, lfs_size_t size)
{
	...
	if (!(file->flags & LFS_F_WRITING) && file->pos > file->ctz.size) {
		...
		while (file->pos < pos) {
			lfs_ssize_t res = lfs_file_flushedwrite(lfs, file, &(uint8_t) {
				0
			}, 1);
			if (res < 0) {
				return res;
								   ...
								   lfs_soff_t off, int whence)
{
	...
	if (
		...
		int oindex = lfs_ctz_index(lfs, &(lfs_off_t) {
			file->pos
		});
		lfs_off_t noff = npos;
...
static int lfs_file_rawtruncate(lfs_t *lfs, lfs_file_t *file, lfs_off_t size)
{
	...
	if (size < oldsize) {
		...
		// fill with zeros
		while (file->pos < size) {
			res = lfs_file_rawwrite(lfs, file, &(uint8_t) {
				0
			}, 1);
			if (res < 0) {
				return (int)res;
...
static int lfs_rawformat(lfs_t *lfs, const struct lfs_config *cfg)
{
	int err = 0;
	{
		...
		err = lfs_dir_fetch(lfs, &root, (const lfs_block_t[2]) {
			0, 1
		});
		if (err) {
			goto cleanup;
...
static int lfs1_moved(lfs_t *lfs, const void *e)
{
	...
	// skip superblock
	lfs1_dir_t cwd;
	int err = lfs1_dir_fetch(lfs, &cwd, (const lfs_block_t[2]) {
		0, 1
	});
	if (err) {
		return err;
					  ...
					  const struct lfs_config *cfg)
{
	int err = 0;
	{
		...
		err = lfs1_dir_fetch(lfs, &dir, (const lfs_block_t[2]) {
			0, 1
		});
		if (err && err != LFS_ERR_CORRUPT) {
			goto cleanup;
...

	{
		...
		while (!lfs_pair_isnull(dir1.d.tail)) {
			...
			while (true) {
				...
				uint16_t id;
				err = lfs_dir_find(lfs, &dir2, &(const char *) {
					name
				}, &id);
				if (!(err == LFS_ERR_NOENT && id != 0x3ff)) {
		...
		err = lfs1_dir_fetch(lfs, &dir1, (const lfs_block_t[2]) {
			0, 1
		});
		if (err) {
			goto cleanup;
		...
		// sanity check that fetch works
		err = lfs_dir_fetch(lfs, &dir2, (const lfs_block_t[2]) {
			0, 1
		});
		if (err) {
			goto cleanup;
