/**
  ******************************************************************************
  * @file    vfs_second_nand_spi.h
  * @author
  * @date    2023-12
  * @brief   This file is the header file for vfs_second_nand_spi.c
  ******************************************************************************
  * @attention
  *
...
#ifndef _VFS_SECOND_NAND_SPI_H
#define _VFS_SECOND_NAND_SPI_H
...
#define SPI1_MOSI	_PB_14
#define SPI1_MISO	_PB_15
#define SPI1_SCLK	_PB_13
#define SPI1_CS	_PB_16
...
/* Baudrate must be <= half of the SPI IpClk (100 MHz on the supported SoCs), and
 * must divide it evenly or spi_frequency() rounds down (see spi_api.c).
 *
 * Bring-up history on hand-wired (dupont) boards: at any speed the link was only
 * as good as the wiring.  Once /CS had a pull-up, VCC was decoupled and grounds
 * were solid, 100 kHz then 10 MHz read/programmed byte-exact.  25 MHz is the
 * fastest validated on a clean layout; on flying leads step up and re-test the
 * self-test (SECOND_NAND_FTL_SelfTest) before trusting a higher value. */
#define SPI_SCLK_FREQ	10000000

/* One transfer carries cmd/addr + a full page including the spare area.  Round up
 * to a cache-line multiple: the DMA buffers are cache-line aligned and the driver
 * must own every line it touches, otherwise the post-RX DCache_Invalidate would
 * discard unrelated dirty data sharing the tail line. */
#define SPI_NAND_MAX_XFER	(2048 + 128 + 4)
#define SPI_BUFFER_SIZE		(((SPI_NAND_MAX_XFER) + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1))
...
/* Winbond BUF: 1 = buffer read mode (the read column address is honoured),
 * 0 = continuous read mode (column ignored, output always starts at byte 0).
 * This driver addresses the spare area by column, so BUF must stay 1. */
#define NAND_CFG_BUF_READ		BIT(3)
