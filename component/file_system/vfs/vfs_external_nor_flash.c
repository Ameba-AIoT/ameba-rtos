...
static const char *TAG = "EXT-FLASH";
...
void external_flash_wait_idle(void)
{
	int count = 0;
...
*/
int32_t external_flash_read_stream(uint32_t address, uint32_t length, char *rx_buffer)
{
	int32_t ret = 1;
	char *cmd = NULL;
	char *temp = NULL;
...
*/
int32_t external_flash_fast_read_stream(uint32_t address, uint32_t length, char *rx_buffer)
{
	int32_t ret = 0;
	char *cmd = NULL;
	char *temp = NULL;
...
*/
int32_t external_flash_erase_sector(uint32_t address)
{
	int32_t ret = 0;
	char *status = NULL;
	char *cmd = NULL;
...
*/
int32_t external_flash_write_stream(uint32_t address, uint32_t length, char *tx_buffer)
{
	int32_t ret = 0;
	char *status = NULL;
...
void external_flash_get_id(void)
{
	int32_t ret;
	char device_id[CMD_LENGTH_FOUR] = {0};
	char device_cmd[CMD_LENGTH_FOUR] = {0};
	...
	if (ret != HAL_OK) {
		RTK_LOGW(TAG, "%s: state(0x%lx) is not ready\r\n", __func__, ret);
		rtos_mutex_give(device_lock);
		return;
	}
...
void external_flash_spi_init(int hz)
{
	...
	spi_irq_hook(&spi_master, (spi_irq_handler)external_flash_tr_done_callback, (uint32_t)&spi_master);
...
void external_flash_spi_free(void)
{
	spi_free(&spi_master);
}

void external_flash_flush_rx_fifo(void)
{
	spi_flush_rx_fifo(&spi_master);
}
