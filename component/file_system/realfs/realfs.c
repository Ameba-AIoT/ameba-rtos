 ...
 */
/* Block-device (non-edev) free blocks kept off-limits to user-DATA writes so a
 * delete can still COW its bucket metadata on a full volume.  A delete allocates
 * one block (the new bucket head) before freeing the file's blocks; without a
 * reserve a 100%-full volume can never be emptied (same failure mode as the
 * edev segment reserve).  Metadata ops (ea_meta_op) bypass it.  Cheap: a handful
 * of blocks. */
#define RFS_META_BLK_RESERVE  8u
					 ...
					 int max_runs, int *nout)
{
	...
	*nout = 0;
	...
	uint32_t reserve = fs->ea_meta_op ? 0u : RFS_META_BLK_RESERVE;
...
int realfs_delete(struct realfs *fs, const char *path)
{
	...
	/* Allow delete to use the metadata reserve (edev: vblk/segment; block-device:
	 * free blocks): delete always frees more than it allocates (net negative),
	 * but the alloc precedes the free, so it needs the reserve to bridge the gap
	 * on a full volume.  MUST clear on ALL paths. */
	fs->ea_meta_op = 1;
	txn_begin(fs);
...
/* Free data segments kept off-limits to user-DATA writes so that metadata ops
 * (delete/rename) and GC relocation always have a segment to write into.
 * Without this a 100%-full volume can never be emptied: delete must COW its
 * metadata to a fresh page and GC must relocate live pages, both of which need
 * a free segment.  2 is the safe minimum -- delete may consume one for its
 * metadata while GC still needs one to relocate a victim's live pages; a value
 * of 1 can leave GC with none right after a delete.  Costs ~2 erase blocks of
 * capacity. */
#define EA_USER_SEG_RESERVE 2u
...
static int ea_open_fresh_seg(struct realfs *fs)
{
	...
	/* Keep EA_USER_SEG_RESERVE free segments in reserve for metadata/GC: a
	 * user-DATA write (neither a metadata op nor GC relocation) must stop with
	 * ENOSPC rather than consume the last reserved segments.  The write's txn is
	 * rolled back by the caller, leaving the reserve intact for a later delete. */
	if (!fs->ea_meta_op && !fs->ea_gc_active &&
		fs->ea_free_segs <= EA_USER_SEG_RESERVE) {
		return REALFS_ERR_NO_SPACE;
	}
...
static int ea_reserve_space(struct realfs *fs)
{
	...
	while (fs->ea_free_segs <= EA_GC_RESERVE && guard-- > 0) {
		/* GC relocation must be allowed to use the user-write reserve segments;
		 * otherwise reclaiming a full volume would deadlock on its own reserve. */
		fs->ea_gc_active = 1;
		int rc = ea_gc_one(fs);
		fs->ea_gc_active = 0;
