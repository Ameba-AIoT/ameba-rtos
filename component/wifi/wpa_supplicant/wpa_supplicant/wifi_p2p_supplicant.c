...
static void wifi_p2p_notify_dev_scanned(u8 *addr, u8 role, char *name, u8 channel, int rssi)
{
	at_printf(P2P_EVENT_DEVICE_SCANNED MACSTR, MAC2STR(addr));
	...
	if (role == P2P_ROLE_GO) {
		at_printf(" ssid=%s oper_ch=%d rssi=%d\n", name, channel, rssi);
	} else {
		at_printf(" name=%s listen_ch=%d\n", name, channel);
	}
}
...
static int wifi_p2p_default_channels(struct p2p_channels *chan)
{
	...
	if (band_type & RTW_SUPPORT_BAND_5G) {
...
exit:
	if (res == RTK_FAIL) {
		RTK_LOGE(TAG_WLAN_P2P, "Enqueue cmd 0x%08x fail\n ", (void *)cmd_hdl);
	}
	return res;
}
...
static void wifi_p2p_cmd_resource_free(void)
{
...
void wifi_p2p_cmd_process_task(void *param)
{
	...
	wifi_p2p_cmd_resource_free();
	rtos_sema_give(g_p2p_cmdpriv.terminate_sema);
...
s32 wifi_p2p_scan_report_each_cb(struct rtw_scan_result *scanned_ap_info, void *user_data, u8 *ies, u32 ie_len)
{
	...
	u8 new_node = TRUE, free_scan_info = TRUE;
	struct wpabuf *p2p_ie = NULL;
	struct p2p_message *msg = NULL;
	s32 res = 0;
	...
	if (ies && ie_len) {
		/* find p2p IE and Capability attribute */
		p2p_ie = ieee802_11_vendor_ie_concat(ies + _FIXED_IE_LENGTH_, ie_len - _FIXED_IE_LENGTH_, P2P_IE_VENDOR_TYPE);
		if (p2p_ie == NULL) {
			p2p_role = P2P_ROLE_DISABLE;
		} else {
			msg = rtos_mem_zmalloc(sizeof(struct p2p_message));
			if (p2p_parse_p2p_ie(p2p_ie, msg)) {
				p2p_role = P2P_ROLE_DISABLE;
			} else if (msg->capability && (msg->capability[1] & P2P_GROUP_CAPAB_GROUP_OWNER)) {
	...
	if (p2p_role != P2P_ROLE_DISABLE) {
		...
		list_for_each(iterator, &g_p2p_context.p2p_scan_report_list) {
			...
			if (memcmp(item->p_scanned_dev_info->bssid.octet, scanned_ap_info->bssid.octet, 6) == 0) {
				new_node = FALSE;
				if (item->p_scanned_dev_info->signal_strength < scanned_ap_info->signal_strength) {
					/* replace old node */
					rtos_mem_free((void *)item->p_scanned_dev_info);
					item->p_scanned_dev_info = scanned_ap_info;
					item->role = p2p_role;
		...
		if (new_node) {
			...
			node->p_scanned_dev_info = scanned_ap_info;
			node->role = p2p_role;
			node->ies = ies;
			node->ie_len = ie_len;
			free_scan_info = FALSE;
...
exit:
	if (free_scan_info) {
		rtos_mem_free(ies);
		rtos_mem_free(scanned_ap_info);
	}

	return res;
}

static s32 wifi_p2p_scan_done_cb(u32 ap_num, void *user_data)
{
	struct p2p_scan_report_node *item;
	struct list_head *iterator;
	...
	if (p2p->state == P2P_SEARCH) {
		...
		list_for_each(iterator, &g_p2p_context.p2p_scan_report_list) {
			item = list_entry(iterator, struct p2p_scan_report_node, list);
			p_scanned_dev_info = item->p_scanned_dev_info;
			wifi_p2p_notify_dev_scanned(p_scanned_dev_info->bssid.octet, item->role, (char *)p_scanned_dev_info->ssid.val,
										p_scanned_dev_info->channel, p_scanned_dev_info->signal_strength);
		}
		rtos_mutex_give(g_p2p_context.scan_report_lock);
	}

	RTK_LOGD(TAG_WLAN_P2P, "scan done!\n");
						 ...
						 const u8 *req_dev_types, const u8 *dev_id, u16 pw_id)
{
	...
	case P2P_SCAN_SOCIAL:
		/* scan social channel: 1, 6, 11 of 2.4G */
	...
	case P2P_SCAN_SPECIFIC:
		/* scan one specific channel */
		p2p_freq_to_channel(freq, &reg_class, &channel);
		scan_param.channel_list_num = 1;
		scan_param.channel_list = &channel;
		scan_param.options = RTW_SCAN_ACTIVE;
		break;
	case P2P_SCAN_SOCIAL_PLUS_ONE:
		/* scan social channels and one specific channel */
	...
	scan_param.scan_user_callback = wifi_p2p_scan_done_cb;
...
static struct p2p_scan_report_node *wifi_p2p_get_dev_scan_res(u8 *dest)
{
	...
	list_for_each(iterator, &g_p2p_context.p2p_scan_report_list) {
		...
		if (!memcmp(p_scanned_dev_info->bssid.octet, dest, ETH_ALEN)) {
			RTK_LOGD(TAG_WLAN_P2P, "Find dest dev, ch=%d\n", p_scanned_dev_info->channel);
			rtos_mutex_give(g_p2p_context.scan_report_lock);
...
s32 wifi_p2p_cancel_remain_on_ch(void)
{
	if (g_p2p_context.roch_onging) {
		...
		RTK_LOGD(TAG_WLAN_P2P, "roc cancel\n");
		wifi_scan_abort();
	}
	return 0;
}
...
static s32 wifi_p2p_roc_done_callback(u32 ap_num, void *user_data)
{
	...
	RTK_LOGD(TAG_WLAN_P2P, "roc done\n");
	g_p2p_context.roch_onging = FALSE;
...
static void wifi_p2p_remain_on_channel(u8 wlan_idx, u8 channel, u32 wait_time)
{
	...
	RTK_LOGD(TAG_WLAN_P2P, "roc: %d ms, ch:%d\n", wait_time, channel);
	...
	ret = wifi_scan_networks(&scan_param, 0);
	if (ret < 0) {
		RTK_LOGE(TAG_WLAN_P2P, "roc fail, ret:%d\n", ret);
	}
								...
								size_t data_len, unsigned int wait_time, int *scheduled)
{
	...
	RTK_LOGD(TAG_WLAN_P2P, "Send action(ch=%d, wait=%d ms)\n", channel, wait_time);
						...
						int new_device)
{
	...
	RTK_LOGD(TAG_WLAN_P2P, P2P_EVENT_DEVICE_FOUND MACSTR
			 " p2p_dev_addr=" MACSTR
...
{
	(void) ctx;
	RTK_LOGI(TAG_WLAN_P2P, P2P_EVENT_DEVICE_LOST"p2p_dev_addr=" MACSTR "\n", MAC2STR(dev_addr));
}
...
static void wifi_p2p_find_stopped(void *ctx)
{
	...
	wifi_scan_abort();
	at_printf(P2P_EVENT_FIND_STOPPED"\n");
}
...
static void wifi_p2p_prov_disc_resp(void *ctx, const u8 *peer, u16 config_methods)
{
	...
	if (config_methods & WPS_CONFIG_DISPLAY) {
		...
		RTK_LOGI(TAG_WLAN_P2P, "P2P-PROV-DISC-PBC-RESP " MACSTR"\n", MAC2STR(peer));
	}
}
									...
									const char *deferred_session_resp)
{
	...
	RTK_LOGE(TAG_WLAN_P2P, "P2P-PROV-DISC-FAILURE " MACSTR " status=%d\n", MAC2STR(peer), status);
}
...
static int wifi_p2p_start_go_neg(const u8 *peer_addr,
								 ...
								 const u8 *ssid, size_t ssid_len, unsigned int pref_freq, u8 pd)
{
	if (persistent_group) {
		persistent_group = 2;
	}
...
static void wifi_p2p_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
	...
	if (res->status) {
		RTK_LOGE(TAG_WLAN_P2P, P2P_EVENT_GO_NEG_FAILURE" status=%d\n", res->status);
		return;
	}

	RTK_LOGI(TAG_WLAN_P2P, P2P_EVENT_GO_NEG_SUCCESS"\n");

	if (res->role_go) {
		RTK_LOGI(TAG_WLAN_P2P, "Start GO!\n");
		g_p2p_context.role = P2P_R_GO;
		wifi_p2p_set_role(P2P_R_GO);
	} else {
		RTK_LOGI(TAG_WLAN_P2P, "Start Client!\n");
							...
							u8 go_intent)
{
	...
	struct p2p_scan_report_node *scanned_dev_info;

	(void) ctx;

	RTK_LOGD(TAG_WLAN_P2P, P2P_EVENT_GO_NEG_REQUEST MACSTR" dev_passwd_id=%u,go_intent=%u\n", MAC2STR(src), dev_passwd_id, go_intent);
	...
	scanned_dev_info = wifi_p2p_get_dev_scan_res((u8 *)src);
	if (!scanned_dev_info) {
		RTK_LOGE(TAG_WLAN_P2P, "Device "MAC_FMT" not found!\n", MAC_ARG(src));
		return;
	}
	channel = scanned_dev_info->p_scanned_dev_info->channel;

	dev = p2p_get_device(g_p2p_context.p2p, src);
	if (dev) {
		dev->listen_freq = p2p_channel_to_freq(g_p2p_context.p2p->cfg->reg_class, channel);
	}
	RTK_LOGD(TAG_WLAN_P2P, "Listen ch of "MAC_FMT" is %d!\n", MAC_ARG(src), channel);
}
...
static void wifi_p2p_idle_update(void *ctx, int idle)
{
	...
	if (idle) {
		/* If the group is idle (no associated stations), remove group after some idle time? */
		//wifi_p2p_group_delete(g_p2p_context.p2p, P2P_GROUP_REMOVAL_IDLE_TIMEOUT);
		//wifi_p2p_drv_deinit();
	}
}
...
struct p2p_group *wifi_p2p_group_init(struct p2p_go_neg_results *params)
{
	...
	cfg->max_clients = wifi_user_config.ap_sta_num;
...
static int wifi_p2p_group_delete(enum p2p_group_removal_reason removal_reason)
{
	const char *reason;
	...
	if (g_p2p_context.role == P2P_R_GO) {
		...
		RTK_LOGI(TAG_WLAN_P2P, P2P_EVENT_GROUP_REMOVED " reason=%s\n", reason);
...
int wifi_p2p_group_notify_assoc(u8 *buf, u16 buf_len)
{
	...
	if (p2p_wpa_buf) {
		...
		RTK_LOGI(TAG_WLAN_P2P, AP_STA_CONNECTED MAC_FMT"\n", MAC_ARG(addr));
	}
	return ret;
}
...
int wifi_p2p_group_notify_disassoc(u8 *addr)
{
	...
	RTK_LOGI(TAG_WLAN_P2P, AP_STA_DISCONNECTED MAC_FMT"\n", MAC_ARG(addr));
...
{
	if (!success) {
		RTK_LOGE(TAG_WLAN_P2P, P2P_EVENT_GROUP_FORMATION_FAILURE"\n");

		/* exit cmd_ap_wps or cmd_wps */
		os_xqueue_send(queue_for_credential, NULL, 0);

		/* deinit to P2P DEVICE */
		wifi_p2p_drv_deinit(P2P_GROUP_REMOVAL_FORMATION_FAILED);
		return;
	}

	RTK_LOGI(TAG_WLAN_P2P, P2P_EVENT_GROUP_FORMATION_SUCCESS"\n");
...
int wifi_p2p_drv_deinit(enum p2p_group_removal_reason removal_reason)
{
	...
	} else if (g_p2p_context.role == P2P_R_GO) {
		/* delete goup */
		wifi_p2p_group_delete(removal_reason);

		/* deinit GO interface */
		dhcps_deinit(pnetif);
		wifi_stop_ap();

		/* Switch back port0 & port1 mac addr */
...
int wifi_p2p_disconnect(void)
{
	...
	wifi_p2p_drv_deinit(P2P_GROUP_REMOVAL_REQUESTED);
...
static void wifi_p2p_group_formation_failed(void *ctx)
{
	...
	RTK_LOGI(TAG_WLAN_P2P, "Group Formation failed\n");
	os_cancel_timer(&g_p2p_context.group_formation_timer);
	if (p2p) {
...
void wifi_p2p_wps_success(const u8 *peer_addr, int registrar)
{
	...
	RTK_LOGD(TAG_WLAN_P2P, "WPS Success\n");
...
void wifi_p2p_wps_failed(void)
{
	...
	RTK_LOGD(TAG_WLAN_P2P, "WPS Failed\n");
...
static void wifi_p2p_group_formation_timeout(void *timeout_ctx)
{
	...
	RTK_LOGE(TAG_WLAN_P2P, "Group Formation timeout\n");
	if (p2p) {
		p2p_group_formation_failed(p2p);
	}
	wifi_p2p_group_formation_completed(0);
...
static int wifi_p2p_start_go(char *ssid, char *passphrase, u8 channel)
{
	struct netif *pnetif = &xnetif[NETIF_WLAN_AP_INDEX];
	...
	if (wifi_start_ap(&softAP_config) != RTK_SUCCESS) {
		RTK_LOGE(TAG_WLAN_P2P, "start go failed\n");
		return -1;
	}
...
static int wifi_p2p_start_wps_go(struct p2p_go_neg_results *params, char *pin)
{
	u8 channel, reg_class;
	u16 wps_config = 0;

	p2p_freq_to_channel(params->freq, &reg_class, &channel);
	...
	wifi_start_ap_wps_thread(wps_config, pin);

	return 0;
}

static int wifi_p2p_start_wps_enrollee(struct p2p_go_neg_results *res, char *pin)
{
	u8 channel, reg_class;
	int ret = 0;
	u8 *ies;
	...
	RTK_LOGD(TAG_WLAN_P2P, "start wps enrollee\n");
	p2p_freq_to_channel(res->freq, &reg_class, &channel);
	...
	ie_len = p2p_assoc_req_ie(g_p2p_context.p2p, dev_node->p_scanned_dev_info->bssid.octet, ies, ie_len, 1, NULL);
	...
	wps_start(wps_config, pin, channel, (char *)res->ssid);
...
exit:
	...
	if (ret != 0) {
		RTK_LOGE(TAG_WLAN_P2P, "Group formation failed! "MAC_FMT"\n", MAC_ARG(res->peer_interface_addr));
		wifi_p2p_group_formation_failed(g_p2p_context.p2p);
	}
	return ret;
}

static int wifi_p2p_start_wps(void *res, char *pin)
{
	int ret = -1;
	...
	if (g_p2p_context.role == P2P_R_GO) {
		ret = wifi_p2p_start_wps_go((struct p2p_go_neg_results *)res, pin);
	} else if (g_p2p_context.role == P2P_R_CLIENT) {
		ret = wifi_p2p_start_wps_enrollee((struct p2p_go_neg_results *)res, pin);
	} else {
		RTK_LOGE(TAG_WLAN_P2P, "GO Nego Failed!\n");
		p2p_flush(g_p2p_context.p2p);
	}
	return ret;
}

static void wifi_p2p_init_auto_gc_params(void *nego_res, u8 *dev_addr, struct rtw_scan_result *scanned_peer_info)
{
	struct p2p_go_neg_results *params = (struct p2p_go_neg_results *)nego_res;
	struct p2p_data *p2p = g_p2p_context.p2p;

	params->role_go = 0;
	params->wps_method = WPS_PBC;
	params->ssid_len = scanned_peer_info->ssid.len;
	os_memcpy(params->ssid, scanned_peer_info->ssid.val, params->ssid_len);
	os_memcpy(params->peer_device_addr, dev_addr, 6);
	os_memcpy(params->peer_interface_addr, scanned_peer_info->bssid.octet, 6);
	...
	p2p->op_channel = scanned_peer_info->channel;
	if (p2p->op_channel >= 149) {
		p2p->op_reg_class = 124;
	} else if (p2p->op_channel >= 36) {
...
static void wifi_p2p_init_auto_go_params(void *res, u8 *passphrase, u8 channel)
{
	struct p2p_data *p2p = g_p2p_context.p2p;
...
{
	struct p2p_go_neg_results *neg_params = NULL;
	struct p2p_data *p2p = g_p2p_context.p2p;
	struct p2p_config *cfg = p2p->cfg;

	if (g_p2p_context.role == P2P_R_DISABLE || g_p2p_context.p2p == NULL) {
		return -1;
	}

	neg_params = (struct p2p_go_neg_results *) rtos_mem_zmalloc(sizeof(struct p2p_go_neg_results));
	if (!neg_params) {
		return -1;
	}

	/* set dev_name, manufacturer, model_name, model_number, serial_number */
	os_memcpy(cfg->dev_name, param->dev_name, os_strlen(param->dev_name) + 1);
	os_memcpy(cfg->manufacturer, param->manufacturer, os_strlen(param->manufacturer) + 1);
	os_memcpy(cfg->model_name, param->model_name, os_strlen(param->model_name) + 1);
	os_memcpy(cfg->model_number, param->model_number, os_strlen(param->model_number) + 1);
	os_memcpy(cfg->serial_number, param->serial_number, os_strlen(param->serial_number) + 1);

	/* set primary dev type */
	p2p_set_pri_dev_type(p2p, param->pri_dev_type);
	os_memcpy(g_ap_device_type, param->pri_dev_type, WPS_DEV_TYPE_LEN);

	/* set ssid */
	p2p->ssid_len = os_strlen((const char *)param->ssid.val);
	os_memcpy(p2p->ssid, (const char *)param->ssid.val, p2p->ssid_len);

	/* set config method: in PIN mode advertise Keypad only, so the peer is told
	 * "enter the PIN shown on this device" and pops up a PIN input box (advertising
	 * Display would instead make the peer generate and show its own PIN). */
	if (param->wps_method == WPS_PIN_DISPLAY) {
		p2p_set_config_methods(g_p2p_context.p2p, WPS_CONFIG_KEYPAD);
	} else {
		p2p_set_config_methods(g_p2p_context.p2p, WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD | WPS_CONFIG_PUSHBUTTON);
	}

	wifi_p2p_init_auto_go_params(neg_params, (u8 *)param->password, param->channel);
	wifi_p2p_start_go((char *)neg_params->ssid, neg_params->passphrase, param->channel);

	os_memcpy(g_p2p_context.go_dev_addr, g_p2p_context.p2p->cfg->dev_addr, ETH_ALEN);
	g_p2p_context.group = wifi_p2p_group_init(neg_params);

	if (neg_params) {
		rtos_mem_free(neg_params);
	}

	/* In PIN mode the PIN is generated and displayed per Provision Discovery
	 * request in wifi_p2p_prov_disc_req(), so nothing needs to be started here.
	 * In PBC mode arm the registrar right away. */
	if (param->wps_method != WPS_PIN_DISPLAY) {
...
/**
 * wifi_p2p_connect - Request P2P Group Formation to be started
 ...
 */
static int wifi_p2p_connect(const u8 *peer_addr,
					 enum p2p_wps_method wps_method,
					 ...
					 int go_intent, int freq, int persistent_id, u8 pd)
{
	...
	if (join || auto_join) {
		...
		if (p2p_get_interface_addr(p2p, peer_addr, iface_addr) < 0) {
			os_memcpy(iface_addr, peer_addr, ETH_ALEN);
			p2p_get_dev_addr(p2p, peer_addr, dev_addr);
		}
		if (wifi_p2p_join(iface_addr, dev_addr, wps_method, auto_join) < 0) {
			return -1;
		}
		return ret;
	}
	...
	if (freq > 0) {
		if (!p2p_supported_freq(p2p, freq)) {
			RTK_LOGE(TAG_WLAN_P2P, "pref ch (%u MHz) not support\n", freq);
			return -3;
		}
		pref_freq = freq;
	}
	...
	if (wifi_p2p_start_go_neg(peer_addr, wps_method,
							  ...
							  persistent_group, NULL, 0, pref_freq, pd) < 0) {
		return -1;
	}
	return ret;
}

void wifi_p2p_connect_task(void *param)
{
	(void) param;

	struct p2p_scan_report_node *dev_node = NULL;
	struct p2p_connect_params *params = g_p2p_context.connect_param;
	struct p2p_data *p2p = g_p2p_context.p2p;
	int ret = 0;
	int count = 1000;
	int freq;
	...
	if (g_p2p_context.role != P2P_R_DEVICE || p2p == NULL || params == NULL) {
		goto Exit;
	}
	...
	/* get peer info from scan result */
	dev_node = wifi_p2p_get_dev_scan_res(params->dest);
	if (dev_node == NULL) {
		RTK_LOGE(TAG_WLAN_P2P, "Can't find device "MAC_FMT"!\n", MAC_ARG(params->dest));
		goto Exit;
	}
	freq = p2p_channel_to_freq(p2p->cfg->reg_class, dev_node->p_scanned_dev_info->channel);
	ret = p2p_add_device(p2p, dev_node->p_scanned_dev_info->bssid.octet,
						 freq, NULL,
						 dev_node->p_scanned_dev_info->signal_strength,
						 dev_node->ies + _FIXED_IE_LENGTH_,
						 dev_node->ie_len - _FIXED_IE_LENGTH_,
						 1) ;
	if (ret < 0) {
		RTK_LOGE(TAG_WLAN_P2P, "Add device "MAC_FMT" failed!\n", MAC_ARG(params->dest));
		goto Exit;
	}
	...
	/* PIN to use during provisioning or %NULL to indicate PBC mode */
	if (params->pin) {
		os_strncpy(pin, params->pin, sizeof(pin));
	} else if (params->config_method == WPS_PIN_DISPLAY) {
		val = wps_generate_pin();
		os_snprintf(pin, sizeof(pin), "%08d", val);
		/* wps_generate_pin() always returns an 8-digit PIN, and os_snprintf
		 * (DiagSnPrintf) does not null-terminate, so terminate after digit 8. */
		pin[8] = '\0';
		at_printf("P2P: Randomly generated PIN: %s\n", pin);
	} else {
		pin[0] = '\0';
	}

	/* get preference operating channel */
	freq = p2p_channel_to_freq(p2p->cfg->op_reg_class, p2p->cfg->op_channel);

	/* start p2p connect */
	ret = wifi_p2p_connect(params->dest,
						   params->config_method, 0, 0,
						   params->join, 0,
						   params->go_intent, freq, 0,
						   params->pd_before_go_neg);
	if (ret == 0) {
		if (!params->join) {
			/* wait for go nego complete and start wps */
			if (os_xqueue_receive(g_p2p_context.queue_for_p2p_nego, nego_res, params->timeout_sec) == RTK_SUCCESS) {
				wifi_p2p_start_wps(nego_res, pin);
			} else {
				RTK_LOGE(TAG_WLAN_P2P, "wait for nego result timeout!\n");
			}
		} else {
			/* join directly as GC: skip GO nego and start wps */
			wifi_p2p_init_auto_gc_params(nego_res, params->dest, dev_node->p_scanned_dev_info);
			wifi_p2p_start_wps(nego_res, pin);
		}
	} else {
		RTK_LOGE(TAG_WLAN_P2P, "connect fail, ret=%d!\n", ret);
	}
...
Exit:
	...
	g_p2p_context.connect_param = NULL;
	rtos_task_delete(NULL);
}
...
void wifi_p2p_join_status_hdl(u8 *evt_info)
{
	...
	if (join_status == RTW_JOINSTATUS_FAIL || join_status == RTW_JOINSTATUS_DISCONNECT) {
		...
		if (!get_wps_phase()) {
			...
			wifi_p2p_drv_deinit(0);
		}
	}
}

static void wifi_p2p_free_scan_list(void)
{
	...
	while (list_empty(&g_p2p_context.p2p_scan_report_list) != true) {
		...
		if (p_node->p_scanned_dev_info) {
			rtos_mem_free(p_node->p_scanned_dev_info);
		}
...
void wifi_p2p_find(u32 timeout)
{
	...
	wifi_p2p_free_scan_list();

	p2p_find(g_p2p_context.p2p, timeout,  P2P_FIND_START_WITH_FULL, 0, NULL, NULL, 0, 0, NULL, 0);
}
...
void wifi_p2p_show_peers(void)
{
	...
	list_for_each(iterator, &g_p2p_context.p2p_scan_report_list) {
		...
		at_printf("[%d]:"MAC_FMT", ", count++, MAC_ARG(p_scanned_dev_info->bssid.octet));
		if (item->role == P2P_R_DEVICE) {
			at_printf("%s, listen_ch %d\n", "P2P_DEV", p_scanned_dev_info->channel);
		} else {
			at_printf("%s, %s, oper_ch %d, %d\n", "P2P_GO", p_scanned_dev_info->ssid.val, p_scanned_dev_info->channel, p_scanned_dev_info->signal_strength);
		}
	}
	rtos_mutex_give(g_p2p_context.scan_report_lock);
}
...
{
	g_p2p_context.connect_param = rtos_mem_zmalloc(sizeof(struct p2p_connect_params));
	if (g_p2p_context.connect_param == NULL) {
		return;
	}

	memcpy(g_p2p_context.connect_param, params, sizeof(struct p2p_connect_params));

	if (rtos_task_create(NULL, ((const char *)"p2p_connect"), wifi_p2p_connect_task, NULL, 2500, 3) != RTK_SUCCESS) {
		RTK_LOGE(TAG_WLAN_P2P, "create p2p_connect thread failed\n");
	}
}
...
int wifi_p2p_init(u8 *dev_addr, u8 listen_ch, u8 op_ch, char *ssid_postfix)
{
	...
	if (NULL == g_wps_context.wps) {
		RTK_LOGE(TAG_WLAN_P2P, "wps dev init fail!\n");
		ret = -1;
		goto exit;
	}
	...
	RTK_LOGD(TAG_WLAN_P2P, "Own listen ch: %d\n", p2pcfg->channel);
	...
	RTK_LOGD(TAG_WLAN_P2P, "oper ch: %d:%d\n", p2pcfg->op_reg_class, p2pcfg->op_channel);

	os_memcpy(p2pcfg->country, "XX\x04", 3);

	wifi_p2p_default_channels(&p2pcfg->channels);
	...
	p2pcfg->ssid_postfix_len = os_strlen(ssid_postfix);
	os_memcpy(p2pcfg->ssid_postfix, ssid_postfix, p2pcfg->ssid_postfix_len);
	...
	rtos_sema_create_static(&g_p2p_cmdpriv.terminate_sema, 0, 0xFFFFFFFF);
	g_p2p_cmdpriv.blocked = 0;

	rtos_task_create(NULL, "p2p_cmd_task", wifi_p2p_cmd_process_task, NULL, 1024 * 2, 2);
...
/**
 * wifi_p2p_deinit - Deinitialize per-interface P2P data
 */
void wifi_p2p_deinit(void)
{
	...
	/* wait connect task complete */
	while (g_p2p_context.connect_param && (count-- > 0)) {
		os_sleep(1, 0);
	}
	...
	/* delete cmd task*/
	g_p2p_cmdpriv.blocked = 1;

	rtos_sema_give(g_p2p_cmdpriv.wakeup_sema);
	rtos_sema_take(g_p2p_cmdpriv.terminate_sema, 0xFFFFFFFF);

	rtos_mutex_delete_static(g_p2p_cmdpriv.lock);
	rtos_sema_delete_static(g_p2p_cmdpriv.wakeup_sema);
	rtos_sema_delete_static(g_p2p_cmdpriv.terminate_sema);

	/* free scan result */
	wifi_p2p_free_scan_list();

	rtos_sema_delete_static(g_p2p_context.roc_ready_sema);
	rtos_mutex_delete_static(g_p2p_context.scan_report_lock);
