...
extern void wpas_wsc_registrar_retrans_timeout(void *timeout_ctx);
static void *eap_wsc_init(char *identity, char identity_len)
{
	struct eap_wsc_data *data;
	int registrar;
	struct wps_config cfg;
	//char identity[] =WSC_ID_REGISTRAR;
	//char identity_len = WSC_ID_REGISTRAR_LEN;
	wpa_printf(MSG_DEBUG, "eap_wsc_init()identity_len=%d", identity_len);
	wpa_hexdump_key(MSG_DEBUG, "identity", identity, identity_len);
	if (identity && identity_len == WSC_ID_REGISTRAR_LEN &&
		os_memcmp(identity, WSC_ID_REGISTRAR, WSC_ID_REGISTRAR_LEN)) {
		registrar = 0;    /* Supplicant is Registrar */
	} else if (identity && identity_len == WSC_ID_ENROLLEE_LEN &&
			   os_memcmp(identity, WSC_ID_ENROLLEE, WSC_ID_ENROLLEE_LEN)) {
	...
	os_init_timer(&data->retrans_timer,
				  NULL,
				  wpas_wsc_registrar_retrans_timeout,
				  data,
				  "wps_retrans_timer");
	data->last_reqdata = NULL;
	...
	wpa_printf(MSG_DEBUG, "eap_wsc_init,data->state:%d,START:%d", data->state, START);
	return data;
}


static void eap_wsc_reset(void *priv)
{
	struct eap_wsc_data *data = (struct eap_wsc_data *)priv;
	//eloop_cancel_timeout(eap_wsc_ext_reg_timeout, sm, data);
	wpa_printf(MSG_DEBUG, "eap_wsc_reset");
	wpabuf_free(data->in_buf);
	wpabuf_free(data->out_buf);
	wpabuf_free(data->last_reqdata);
	os_del_timer(&data->retrans_timer);
	if (data->wps) {
		wps_deinit(data->wps);
		os_free(data, 0);
	}
	wpa_printf(MSG_DEBUG, "eap_wsc_reset end");
}


static struct wpabuf *eap_wsc_build_start(struct eap_wsc_data *data, u8 id)
{
	...
	wpa_printf(MSG_INFO, "WPS: Send WSC Start");
	wpabuf_put_u8(reqbuf, WSC_Start); /* Op-Code */
	wpabuf_put_u8(reqbuf, 0); /* Flags */
...
static struct wpabuf *eap_wsc_buildReq(void *priv, u8 id)
{
	struct eap_wsc_data *data = priv;

	wpa_printf(MSG_DEBUG, "EAP-WSC:eap_wsc_buildReq():data->state:%d(start=1,mesg=2)", data->state);
	switch (data->state) {
	case START:
		return eap_wsc_build_start(data, id);
	case MESG:
...
static void eap_wsc_process(void *priv, struct wpabuf *respData)
{
	struct eap_wsc_data *data = priv;
	...
	wpa_printf(MSG_DEBUG, "EAP-server-WSC: eap_wsc_process");
	//eloop_cancel_timeout(eap_wsc_ext_reg_timeout, sm, data);
...
static int eap_wsc_getTimeout(void *priv)
{
	struct eap_wsc_data *wsc_data = priv;

	/* Recommended retransmit times: retransmit timeout 5 seconds,
	 * per-message timeout 15 seconds, i.e., 3 tries. */
	wsc_data->MaxRetrans = 2; /* total 3 attempts */
	return 5;
}
int eap_server_wsc_register(void)
{
	...
	wsc_server_eap.init = eap_wsc_init;
	wsc_server_eap.reset = eap_wsc_reset;
	wsc_server_eap.buildReq = eap_wsc_buildReq;
	//wsc_server_eap.check = eap_wsc_check;
	wsc_server_eap.process = eap_wsc_process;
	//wsc_server_eap.isDone = eap_wsc_isDone;
	//wsc_server_eap.isSuccess = eap_wsc_isSuccess;
	wsc_server_eap.getTimeout = eap_wsc_getTimeout;
