									   ...
									   int mandatory)
{
	...
	wpa_printf(MSG_DEBUG, "config_methods - val = %u", val);
		...
		int mandatory)
{
	if (primary_dev_type == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: Primary Device Type attribute missing");
			return -1;
		}
		return 0;
	}
...
static int wps_validate_assoc_state(const u8 *assoc_state, int mandatory)
{
	...
	wpa_printf(MSG_DEBUG, "assoc_state - val = %u", val);
	if (val > 4) {
...
static int wps_validate_config_error(const u8 *config_error, int mandatory)
{
	...
	wpa_printf(MSG_DEBUG, "config_error - val = %u", val);
	if (val > 18) {
										...
										int mandatory)
{
	...
	wpa_printf(MSG_DEBUG, "dev_password_id - val = %u", val);
									 ...
									 int mandatory)
{
	...
	if (manufacturer == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: Manufacturer attribute missing");
			return -1;
		}
		return 0;
	}
	if (len > 0 && manufacturer[len - 1] == 0) {
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid Manufacturer attribute value - len = %u", len);
		//some APs set length to a fixed number(max num in spec), the last byte equals to 0 if the value is not long enough, jira: https://jira.realtek.com/browse/CIIOT-8283
		//JIRA: https://jira.realtek.com/browse/CIIOT-8283
		//return -1;
	}
	return 0;
}
								   ...
								   int mandatory)
{
	...
	if (len > 0 && model_name[len - 1] == 0) {
		wpa_hexdump_ascii(MSG_DEBUG, "WPS-STRICT: Invalid Model Name "
						  "attribute value", model_name, len);
		//some APs set value to 0 with len=1
		//some APs set length to a fixed number(max num in spec), the last byte equals to 0 if the value is not long enough
		//JIRA: https://jira.realtek.com/browse/CIIOT-8283
		//return -1;
	}
	return 0;
									 ...
									 int mandatory)
{
	...
	if (len > 0 && model_number[len - 1] == 0) {
		wpa_hexdump_ascii(MSG_DEBUG, "WPS-STRICT: Invalid Model Number "
									  ...
									  int mandatory)
{
	...
	if (len > 0 && serial_number[len - 1] == 0) {
		wpa_hexdump_ascii(MSG_DEBUG, "WPS-STRICT: Invalid Serial "
...
static int wps_validate_auth_type_flags(const u8 *flags, int mandatory)
{
	...
	wpa_printf(MSG_DEBUG, "auth_type_flags()= %u", val);
	if ((val & ~WPS_AUTH_TYPES)) {
		//remove required check for WPS_AUTH_WPA2PSK
		//JIRA: https://jira.realtek.com/browse/CIIOT-8283
	...
	if ((val & ~WPS_AUTH_TYPES) || val == 0 ||
		((num_bits_set(val) > 1) &&
		 ((val != (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK)) &&
		  (val != (WPS_AUTH_WPA2PSK | WPS_AUTH_WPA3))))) {
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid Authentication Type "
				   "attribute value 0x%04x", val);
		return -1;
	}
...
static int wps_validate_encr_type_flags(const u8 *flags, int mandatory)
{
	...
	val = WPA_GET_BE16(flags);
	wpa_printf(MSG_DEBUG, "encr_type_flags - val = %u", val);
	if ((val & ~WPS_ENCR_TYPES)) {
		//remove required check for WPS_ENCR_AES
		//JIRA: https://jira.realtek.com/browse/CIIOT-8283
...
static int wps_validate_encr_type(const u8 *type, int mandatory)
{
	...
	if ((val & ~WPS_ENCR_TYPES) || val == 0 ||
		...
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid Encryption Type attribute value 0x%04x", val);
		return -1;
	}
	return 0;
}
...
static int wps_validate_conn_type_flags(const u8 *flags, int mandatory)
{
	...
	if ((*flags & ~(WPS_CONN_ESS | WPS_CONN_IBSS))) {
		//remove required check for WPS_CONN_ESS
		//JIRA: https://jira.realtek.com/browse/CIIOT-8283
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid Connection Type Flags attribute value 0x%02x", *flags);
		return -1;
	}
	return 0;
}
...
static int wps_validate_encr_settings(const u8 *encr_settings, size_t len,
									  int mandatory)
{
	wpa_printf(MSG_DEBUG, "encr_settings_len = 0x%x", len);
...
static int wps_validate_r_snonce1(const u8 *nonce, int mandatory)
{
	if (nonce == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: R-SNonce1 attribute missing");
			return -1;
		}
		return 0;
	}
...
static int wps_validate_key_wrap_auth(const u8 *auth, int mandatory)
{
	if (auth == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: Key Wrap Authenticator attribute missing");
			return -1;
		}
		return 0;
	}
...
static int wps_validate_ssid(const u8 *ssid, size_t ssid_len, int mandatory)
{
	if (ssid == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: SSID attribute missing");
			return -1;
		}
		return 0;
	}
	if (ssid_len == 0 || ssid[ssid_len - 1] == 0) {
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid SSID attribute value - ssid_len = %u", ssid_len);
		return -1;
	}
	return 0;
}
...
static int wps_validate_network_key_index(const u8 *idx, int mandatory)
{
	if (idx == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: Network Key Index attribute missing");
			return -1;
		}
		return 0;
	}
...
static int wps_validate_network_idx(const u8 *idx, int mandatory)
{
	if (idx == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: Network Index attribute missing");
			return -1;
		}
		return 0;
	}
									...
									const u8 *encr_type, int mandatory)
{
	if (key == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: Network Key attribute missing");
			return -1;
		}
		return 0;
	}
	if (((encr_type == NULL || WPA_GET_BE16(encr_type) != WPS_ENCR_WEP) &&
		 key_len > 8 && key_len < 64 && key[key_len - 1] == 0) || key_len > 64) {
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid Network Key attribute value - key_len = %u", key_len);
		return -1;
	}
	return 0;
}
...
static int wps_validate_network_key_shareable(const u8 *val, int mandatory)
{
	if (val == NULL) {
		if (mandatory) {
			wpa_printf(MSG_INFO, "WPS-STRICT: Network Key Shareable attribute missing");
			return -1;
		}
		return 0;
	}
	if (*val > 1) {
		...
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid Network Key Shareable attribute value 0x%x", *val);
		return -1;
	}
	return 0;
}
...
int wps_validate_m2(const struct wpabuf *tlvs)
{
	...
	if (wps_parse_msg(tlvs, &attr) < 0) {
		wpa_printf(MSG_INFO, "WPS-STRICT: Failed to parse attributes in M2");
		return -1;
	}

	//1217 not verify by validate method
	//wps2 = attr.version2 != NULL;
	if (wps_validate_version(attr.version, 1) ||
		wps_validate_msg_type(attr.msg_type, 1) ||
		wps_validate_enrollee_nonce(attr.enrollee_nonce, 1) ||
...
int wps_validate_m4_encr(const struct wpabuf *tlvs, int wps2)
{
	...
	if (tlvs == NULL) {
		wpa_printf(MSG_INFO, "WPS-STRICT: No TLVs in M4 encrypted settings");
		return -1;
	}
	if (wps_parse_msg(tlvs, &attr) < 0) {
		wpa_printf(MSG_INFO, "WPS-STRICT: Failed to parse attributes "
	...
	if (wps_validate_r_snonce1(attr.r_snonce1, 1) ||
		...
		wpa_printf(MSG_INFO, "WPS-STRICT: Invalid M4 encrypted settings");
#ifdef WPS_STRICT_WPS2
		if (wps2) {
			return -1;
		}
...
int wps_validate_m8_encr(const struct wpabuf *tlvs, int ap, int wps2)
{
	...
	if (tlvs == NULL) {
		wpa_printf(MSG_INFO, "WPS-STRICT: No TLVs in M8 encrypted settings");
		return -1;
	}
	if (wps_parse_msg(tlvs, &attr) < 0) {
		wpa_printf(MSG_INFO, "WPS-STRICT: Failed to parse attributes in M8 encrypted settings");
		return -1;
	}
