		...
		const u8 *addr)
{
	...
	for (i = 0; i < WPS_MAX_AUTHORIZED_MACS; i++)
		if (os_memcmp(reg->authorized_macs[i], addr, ETH_ALEN)) {
		...
		const u8 *addr)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Remove authorized MAC " MACSTR,
			   MAC2STR(addr));
	for (i = 0; i < WPS_MAX_AUTHORIZED_MACS; i++) {
		if (os_memcmp(reg->authorized_macs, addr, ETH_ALEN)) {
			break;
		}
	}
		...
		const u8 *addr)
{
	...
	for (dev = reg->devices; dev; dev = dev->next) {
		if (os_memcmp(dev->dev.mac_addr, addr, ETH_ALEN)) {
			return dev;
		}
	}
	return NULL;
...
#define WPS_STRDUP(n) \
	os_free(dst->n,0); \
	dst->n = src->n? (char*)os_strdup(src->n) : NULL
		...
		const u8 *addr, const u8 *uuid_e)
{
	...
	while (pbc) {
		if (os_memcmp(pbc->addr, addr, ETH_ALEN) &&
			os_memcmp(pbc->uuid_e, uuid_e, WPS_UUID_LEN)) {
			if (prev) {
				prev->next = pbc->next;
			} else {
		...
		const u8 *p2p_dev_addr)
{
	...
	wpa_printf(MSG_DEBUG, "wps_registrar_remove_pbc_session");
	pbc = reg->pbc_sessions;
	while (pbc) {
		if (os_memcmp(pbc->uuid_e, uuid_e, WPS_UUID_LEN)  ||
							  ...
							  const u8 *addr, const u8 *uuid_e)
{
	...
	for (pbc = reg->pbc_sessions; pbc; pbc = pbc->next) {
		...
		if (first &&
			os_memcmp(pbc->uuid_e, first->uuid_e, WPS_UUID_LEN)) {
			wpa_printf(MSG_DEBUG, "WPS: Same Enrollee");
			continue; /* same Enrollee */
		}
		if (uuid_e == NULL ||
			os_memcmp(uuid_e, pbc->uuid_e, WPS_UUID_LEN) == FALSE) {
...
{
	struct wps_registrar *reg = (struct wps_registrar *)os_zalloc(sizeof(struct wps_registrar));
	if (reg) {
 ...
 */
void *
wps_registrar_init(void *priv, void *pcfg)
{
	struct wps_registrar *reg = NULL;
	struct wps_context *wps = (struct wps_context *)priv;
	const struct wps_registrar_config *cfg = (const struct wps_registrar_config *)pcfg;
...
{
	struct wps_registrar *reg = (struct wps_registrar *)priv;
	if (reg == NULL) {
		return;
	}
	...
	dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
		if (dev_pw && pin->pin &&
			(dev_pw_len != pin->pin_len ||
			 os_memcmp(dev_pw, pin->pin, dev_pw_len) == FALSE)) {
...
int wps_registrar_invalidate_pin(struct wps_registrar *reg, const u8 *uuid)
{
	...
	dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
		if (os_memcmp(pin->uuid, uuid, WPS_UUID_LEN)) {
			wpa_hexdump(MSG_DEBUG, "WPS: Invalidated PIN for UUID",
						pin->uuid, WPS_UUID_LEN);
			wps_registrar_remove_pin(reg, pin);
			return 0;
									   ...
									   const u8 *uuid, size_t *pin_len)
{
	...
	dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
		if (!pin->wildcard_uuid &&
			os_memcmp(pin->uuid, uuid, WPS_UUID_LEN)) {
			found = pin;
			break;
		}
	}
								...
								const u8 *p2p_dev_addr)
{
	...
	wpa_printf(MSG_INFO, "WPS: Button pushed - PBC mode started");
...
int wps_registrar_wps_cancel(void *priv)
{
	struct wps_registrar *reg = (struct wps_registrar *)priv;
	if (reg->pbc) {
		wpa_printf(MSG_INFO, "WPS: PBC is set - cancelling it");
								...
								int p2p_wildcard)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "WPS: UUID-E from Probe Request", attr.uuid_e,
					WPS_UUID_LEN);

#ifdef WPS_WORKAROUNDS
	if (reg->pbc_ignore_start.sec &&
		os_memcmp(attr.uuid_e, reg->pbc_ignore_uuid, WPS_UUID_LEN)) {
						 ...
						 struct wpabuf *probe_resp_ie)
{
	if (reg->set_ie_cb == NULL) {
		wpa_printf(MSG_INFO, "WPS: wps_cb_set_ie is NULL ");
		return -1;
	} else {
		wpa_printf(MSG_INFO, "WPS: wps_cb_set_ie ");
	}
...
static void wps_cb_set_sel_reg(struct wps_registrar *reg)
{
	...
	wpa_printf(MSG_DEBUG, "WPS:wps_cb_set_sel_reg()reg->selected_registrar:%d", reg->selected_registrar);
...
static int wps_set_ie(struct wps_registrar *reg)
{
	...
	wpa_printf(MSG_INFO, "WPS: Build Beacon IEs");
	...
	if (wps_build_version(beacon) ||
		...
		(wps_build_rf_bands(&reg->wps->dev, beacon)) || //reg->dualband &&
	...
	wpa_printf(MSG_INFO, "WPS: Build Probe Response IEs");
	...
	if (wps_build_version(probe) ||
		...
		(wps_build_rf_bands(&reg->wps->dev, probe)) ||//reg->dualband &&
	...
	wpa_hexdump_key(MSG_MSGDUMP, "Beacon WPS IE:", wpabuf_head(beacon), wpabuf_len(beacon));
	wpa_hexdump_key(MSG_MSGDUMP, "Probe Response WPS IE:", wpabuf_head(probe), wpabuf_len(probe));
	...
	wpa_printf(MSG_DEBUG, "WPS: reg->cb_ctx:%p,reg->set_ie_cb:%p", reg->cb_ctx, reg->set_ie_cb);
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
		...
		os_free(wps->new_psk, 0);
		wps->new_psk = wpa_base64_encode(r, sizeof(r), &wps->new_psk_len);
		if (wps->new_psk == NULL) {
			return -1;
		}
...
static struct wpabuf *wps_build_m2(struct wps_data *wps)
{
	...
	wpa_printf(MSG_INFO, "WPS: Building Message M2");
	msg = wpabuf_alloc(1000);
	if (msg == NULL) {
		return NULL;
	}
...
static struct wpabuf *wps_build_m2d(struct wps_data *wps)
{
	...
	wpa_printf(MSG_INFO, "WPS: Building Message M2D");
	msg = wpabuf_alloc(1000);
	if (msg == NULL) {
		return NULL;
	}
...
static struct wpabuf *wps_build_m4(struct wps_data *wps)
{
	...
	wpa_printf(MSG_INFO, "WPS: Building Message M4");
...
static struct wpabuf *wps_build_m6(struct wps_data *wps)
{
	...
	wpa_printf(MSG_INFO, "WPS: Building Message M6");
...
static int wps_process_registrar_nonce(struct wps_data *wps, const u8 *r_nonce)
{
	...
	if (os_memcmp(wps->nonce_r, r_nonce, WPS_NONCE_LEN) == FALSE) {
...
static int wps_registrar_p2p_dev_addr_match(struct wps_data *wps)
{
	...
	if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN) == FALSE) {
...
static int wps_registrar_skip_overlap(struct wps_data *wps)
{
	...
	if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN)) {
...
static enum wps_process_res wps_process_m1(struct wps_data *wps,
		struct wps_parse_attr *attr)
{
	wpa_printf(MSG_INFO, "WPS: Received M1");
	...
	if (wps->dev_pw_id == DEV_PW_PUSHBUTTON) {
		if ((wps->wps->registrar->force_pbc_overlap ||
			...
			wpa_printf(MSG_DEBUG, "WPS: PBC overlap - deny PBC "
					   "negotiation");
			wpa_printf(MSG_DEBUG, "force_pbc_overlap:%d", wps->wps->registrar->force_pbc_overlap);
			wps->state = SEND_M2D;
...
static enum wps_process_res wps_process_m3(struct wps_data *wps,
		...
		struct wps_parse_attr *attr)
{
	wpa_printf(MSG_INFO, "WPS: Received M3");
		...
		struct wps_parse_attr *attr)
{
	...
	wpa_printf(MSG_INFO, "WPS: Received M5");
		...
		struct wps_parse_attr *attr)
{
	...
	wpa_printf(MSG_INFO, "WPS: Received M7");
		...
		const struct wpabuf *msg)
{
	...
	case WPS_M3:
		...
		if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK) {
			wpa_printf(MSG_INFO, "M3:WPS_FAILURE or State = SEND_WSC_NACK");
		}
		break;
	case WPS_M5:
		...
		if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK) {
			wpa_printf(MSG_INFO, "M5:WPS_FAILURE or State = SEND_WSC_NACK");
		}
		break;
	case WPS_M7:
		...
		if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK) {
			wpa_printf(MSG_INFO, "M7:WPS_FAILURE or State = SEND_WSC_NACK");
		}
		break;
	default:
		...
		const struct wpabuf *msg)
{
	...
	wpa_printf(MSG_INFO, "WPS: Received WSC_ACK");
		...
		const struct wpabuf *msg)
{
	...
	wpa_printf(MSG_INFO, "WPS: Received WSC_Done");
...
u32 wps_registrar_process_msg(void *priv, u32 op_code, const void *pmsg)
{
	...
	struct wps_data *wps = (struct wps_data *)priv;
	const struct wpabuf *msg = (const struct wpabuf *)pmsg;
	wpa_printf(MSG_DEBUG, "WPS: Processing received message (len=%lu "
			   "op_code=%d)",
			   (unsigned long) wpabuf_len(msg), op_code);

	switch ((enum wsc_op_code)op_code) {
	case WSC_MSG:
	...
	case WSC_Done:
		...
		if (ret == WPS_FAILURE) {
			...
			//	       wps->config_error,
			//	       wps->error_indication, wps->mac_addr_e);
			wpa_printf(MSG_ERROR, "WPS: WPS_FAILURE");
		}
		return ret;
	default:
...
void wps_registrar_selected_registrar_changed(struct wps_registrar *reg,
		u16 dev_pw_id)
{
	wpa_printf(MSG_INFO, "WPS: Selected registrar information changed. reg=%p", reg);
	...
	wpa_hexdump(MSG_INFO, "WPS: Authorized MACs union (start with own)",
	...
	if (reg->selected_registrar) {
		...
		wpa_printf(MSG_INFO, "WPS: Internal Registrar selected "
				   "(pbc=%d)", reg->pbc);
		reg->sel_reg_config_methods_override = methods;
	} else {
		wpa_printf(MSG_INFO, "WPS: Internal Registrar not selected");
	}
