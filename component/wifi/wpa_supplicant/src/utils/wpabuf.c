...
int wpabuf_resize(struct wpabuf **_buf, size_t add_len)
{
	...
	if (buf->used + add_len > buf->size) {
		...
		if (buf->flags & WPABUF_FLAG_EXT_DATA) {
			nbuf = os_realloc(buf->buf, buf->used, buf->used + add_len);
			if (nbuf == NULL) {
				return -1;
			}
			...
			os_memset(nbuf + sizeof(struct wpabuf_trace) +
					  ...
					  add_len);
#else /* WPA_TRACE */
			nbuf = os_realloc((u8 *)buf, (sizeof(struct wpabuf) + buf->used),
							  (sizeof(struct wpabuf) + buf->used + add_len));
			if (nbuf == NULL) {
				return -1;
			}
...
struct wpabuf *wpabuf_alloc(size_t len)
{
	...
	buf->size = len;
	buf->buf = (u8 *)(buf + 1);  // set the real buffer address
	return buf;
}
