...
static void random_extract(u8 *out)
{
	...
	rt_hmac_sha1(dummy_key, sizeof(dummy_key), (u8 *) pool,
				 sizeof(pool), hash);
	...
	rt_hmac_sha1(dummy_key, sizeof(dummy_key), (u8 *) buf,
				 sizeof(buf), hash);
	/*
...
int random_get_bytes(void *buf, size_t len)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "random from os_get_random",
					buf, len);
	...
	wpa_hexdump_key(MSG_DEBUG, "mixed random", buf, len);
...
{
	os_free(random_entropy_file, 0);
	if (entropy_file) {
		random_entropy_file = (char *)os_strdup(entropy_file, strlen(entropy_file));
	} else {
