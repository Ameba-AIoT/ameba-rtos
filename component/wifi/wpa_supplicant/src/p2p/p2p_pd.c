...
do { \
	...
	if (msg->persistent_dev) {
		channel_list = 1;
		config_timeout = 1;
		if (memcmp(msg->persistent_dev, addr, ETH_ALEN) == 0) {
			intended_addr = 1;
			operating_channel = 1;
		}
	}
							   ...
							   const u8 *data, size_t len, int rx_freq)
{
	...
	if (!msg->status) {
		...
		if (memcmp(p2p->cfg->dev_addr, msg->adv_mac, ETH_ALEN)) {
			p2p_dbg(p2p,
	...
	if (p2p->p2ps_prov->adv_id != adv_id ||
		memcmp(p2p->p2ps_prov->adv_mac, msg->adv_mac, ETH_ALEN)) {
		p2p_dbg(p2p,
	...
	if (p2p->p2ps_prov->session_id != session_id ||
		memcmp(p2p->p2ps_prov->session_mac, msg->session_mac, ETH_ALEN)) {
...
#define P2PS_PD_RESP_CHECK(_val, _attr) \
	...
	if (memcmp(p2p->p2ps_prov->session_mac, msg->session_mac,
			   ETH_ALEN)) {
		p2p_dbg(p2p,
	...
	if (memcmp(p2p->p2ps_prov->adv_mac, msg->adv_mac, ETH_ALEN) != 0) {
		p2p_dbg(p2p,
								...
								const u8 *data, size_t len)
{
	...
	if (p2p->user_initiated_pd &&
		memcmp(p2p->pending_pd_devaddr, sa, ETH_ALEN) == 0) {
