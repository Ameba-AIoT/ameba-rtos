...
static void p2p_state_timeout(void *timeout_ctx);
...
static void p2p_process_presence_resp(struct p2p_data *p2p, const u8 *da,
									  ...
									  size_t len);
#endif
static void p2p_ext_listen_timeout(void *timeout_ctx);
static void p2p_scan_timeout(void *timeout_ctx);
...
void p2p_set_timeout(struct p2p_data *p2p, unsigned int sec, unsigned int usec)
{
	...
	os_cancel_timer(&p2p->watch_state_timer);

	/* if timeout value=0, this timer will not be triggered. */
	if ((sec * 1000 + usec / 1000) == 0) {
		os_set_timer(&p2p->watch_state_timer, 1);
	} else {
		os_set_timer(&p2p->watch_state_timer, (sec * 1000 + usec / 1000));
	}
}
...
void p2p_go_neg_failed(struct p2p_data *p2p, int status)
{
	...
	os_cancel_timer(&p2p->neg_wait_timer);
		...
		const u8 *addr)
{
	struct p2p_device *dev;
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (memcmp(dev->interface_addr, addr, ETH_ALEN) == 0) {
			return dev;
		}
	}
	return NULL;
								 ...
								 struct os_reltime *rx_time)
{
	...
	info = rtos_mem_zmalloc(sizeof(struct p2p_group_info));
	if (!info) {
		return -1;
	}
	 ...
	 */
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (memcmp(dev->member_in_go_iface, go_interface_addr,
				   ETH_ALEN) == 0) {
	...
	for (c = 0; c < info->num_clients; c++) {
		...
		if (memcmp(cli->p2p_device_addr, p2p->cfg->dev_addr,
				   ETH_ALEN) == 0) {
								...
								const struct p2p_message *msg)
{
	if (dev->info.wfd_subelems && msg->wfd_subelems) {
		...
		return memcmp(dev->info.wfd_subelems->buf,
				   ...
				   size_t ies_len, int scan_res)
{
	...
	if (!is_zero_ether_addr(p2p->peer_filter) &&
		memcmp(p2p_dev_addr, p2p->peer_filter, ETH_ALEN) != 0) {
	...
	if (memcmp(addr, p2p_dev_addr, ETH_ALEN) != 0) {
		os_memcpy(dev->interface_addr, addr, ETH_ALEN);
	}
	if (msg->ssid &&
		msg->ssid[1] <= sizeof(dev->oper_ssid) &&
		(msg->ssid[1] != P2P_WILDCARD_SSID_LEN ||
		 memcmp(msg->ssid + 2, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN)
		 != 0)) {
...
static void p2p_find_timeout(void *timeout_ctx)
{
...
void p2p_notify_scan_trigger_status(struct p2p_data *p2p, int status)
{
	if (status != 0) {
		...
		os_cancel_timer(&p2p->scan_timer);
		os_set_timer(&p2p->scan_timer, P2P_SCAN_TIMEOUT * 1000);
	}
}
...
static int p2ps_gen_hash(struct p2p_data *p2p, const char *str, u8 *hash)
{
	...
	if (wifi_rom_sha256_vector(1, &adv_array, &adv_len, buf)) {
		rtos_mem_free(str_buf);
		return 0;
	}
			 ...
			 u8 seek_count, const char **seek, int freq)
{
	...
	if (req_dev_types && num_req_dev_types) {
		p2p->req_dev_types = os_malloc(num_req_dev_types * WPS_DEV_TYPE_LEN);
		if (p2p->req_dev_types == NULL) {
			return -1;
		}
		os_memcpy(p2p->req_dev_types, req_dev_types, num_req_dev_types * WPS_DEV_TYPE_LEN);
	...
	os_cancel_timer(&p2p->find_timer);
	p2p->last_p2p_find_timeout = timeout;
	if (timeout) {
		os_set_timer(&p2p->find_timer, timeout * 1000);
	}
						...
						unsigned int force_freq, unsigned int pref_freq, int go)
{
	...
	if (force_freq || pref_freq) {
		if (p2p_prepare_channel_pref(p2p, force_freq, pref_freq, go) < 0) {
			return -1;
		}
	} else {
	...
	if (go) {
		...
		p2p_channels_union_inplace(&p2p->channels, &p2p->cfg->cli_channels);
	}
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
	...
	intersection = rtos_mem_zmalloc(sizeof(struct p2p_channels));
	if (!intersection) {
		return;
	}
...
static void p2p_go_neg_start(void *timeout_ctx)
{
	struct p2p_data *p2p = timeout_ctx;
...
#ifdef CONFIG_P2P_INVITE
static void p2p_invite_start(void *timeout_ctx)
{
	struct p2p_data *p2p = timeout_ctx;
...
{
	if (memcmp(dev_type, req_dev_type, WPS_DEV_TYPE_LEN) == 0) {
		return 1;
	}
	if (memcmp(dev_type, req_dev_type, 2) == 0 &&
...
static int p2p_service_find_asp(struct p2p_data *p2p, const u8 *hash)
{
	...
	any_wfa = memcmp(hash, p2p->wild_card_hash, P2PS_HASH_LEN) == 0;

	adv_data = p2p->p2ps_adv_list;
	while (adv_data) {
		if (memcmp(hash, adv_data->hash, P2PS_HASH_LEN) == 0) {
...
p2p_reply_probe(struct p2p_data *p2p, const u8 *addr, const u8 *dst,
	...
	if (dst && !is_broadcast_ether_addr(dst) &&
		memcmp(dst, p2p->cfg->dev_addr, ETH_ALEN) != 0)
	{
	...
	if (elems.ssid == NULL || elems.ssid_len != P2P_WILDCARD_SSID_LEN ||
		memcmp(elems.ssid, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN) !=
		0)
	{
	...
	if (msg.device_id &&
		memcmp(msg.device_id, p2p->cfg->dev_addr, ETH_ALEN) != 0)
	{
...
p2p_probe_req_rx(struct p2p_data *p2p, const u8 *addr, const u8 *dst,
...
#if 0 //not supported for limited RAM size
	...
	if ((p2p->state == P2P_CONNECT || p2p->state == P2P_CONNECT_LISTEN) &&
		p2p->go_neg_peer &&
		memcmp(addr, p2p->go_neg_peer->info.p2p_device_addr, ETH_ALEN)
		== 0 &&
		!(p2p->go_neg_peer->flags & P2P_DEV_WAIT_GO_NEG_CONFIRM))
	{
		/* Received a Probe Request from GO Negotiation peer */
		p2p_dbg(p2p, "Found GO Negotiation peer - try to start GO negotiation from timeout");
		os_cancel_timer(&p2p->neg_timer);
		os_set_timer(&p2p->neg_timer, 1);
		return res;
	}
	...
	if ((p2p->state == P2P_INVITE || p2p->state == P2P_INVITE_LISTEN) &&
		...
		memcmp(addr, p2p->invite_peer->info.p2p_device_addr, ETH_ALEN)
		== 0)
	{
		/* Received a Probe Request from Invite peer */
		p2p_dbg(p2p, "Found Invite peer - try to start Invite from timeout");
#ifdef CONFIG_P2P_INVITE
		os_cancel_timer(&p2p->invite_timer);
		os_set_timer(&p2p->invite_timer, 1);
#endif
		return res;
	}
...
void p2p_wps_success_cb(struct p2p_data *p2p, const u8 *mac_addr)
{
	...
	if (memcmp(mac_addr, p2p->go_neg_peer->intended_addr, ETH_ALEN) != 0) {
...
struct p2p_data *p2p_init(const struct p2p_config *cfg)
{
	...
	os_init_timer(&p2p->scan_timer, p2p, p2p_scan_timeout, p2p, "scan_timer");
	os_init_timer(&p2p->find_timer, p2p, p2p_find_timeout, p2p, "find_timer");
	os_init_timer(&p2p->watch_state_timer, p2p, p2p_state_timeout, p2p, "watch_state_timer");
	os_init_timer(&p2p->neg_wait_timer, p2p, p2p_go_neg_wait_timeout, p2p, "go_nego_wait_timer");
	os_init_timer(&p2p->neg_timer, p2p, p2p_go_neg_start, p2p, "neg_timer");
#ifdef CONFIG_P2P_INVITE
	os_init_timer(&p2p->invite_timer, p2p, p2p_invite_start, p2p, "invite_timer");
#endif
	os_init_timer(&p2p->ext_listen_timer, p2p, p2p_ext_listen_timeout, p2p, "ext_listen_timer");
...
void p2p_deinit(struct p2p_data *p2p)
{
	...
	os_del_timer(&p2p->scan_timer);
	os_del_timer(&p2p->find_timer);
	os_del_timer(&p2p->watch_state_timer);
	os_del_timer(&p2p->neg_wait_timer);
	os_del_timer(&p2p->neg_timer);
#ifdef CONFIG_P2P_INVITE
	os_del_timer(&p2p->invite_timer);
#endif
	os_del_timer(&p2p->ext_listen_timer);

	p2p_flush(p2p);
	p2p_free_req_dev_types(p2p);
	os_free(p2p->cfg->dev_name, 0);
	os_free(p2p->cfg->manufacturer, 0);
	os_free(p2p->cfg->model_name, 0);
	os_free(p2p->cfg->model_number, 0);
	os_free(p2p->cfg->serial_number, 0);
	os_free(p2p->cfg->pref_chan, 0);
	os_free(p2p->groups, 0);
	p2ps_prov_free(p2p);
#ifdef CONFIG_P2P_SD
	wpabuf_free(p2p->sd_resp);
#endif
	p2p_remove_wps_vendor_extensions(p2p);
	os_free(p2p->no_go_freq.range, 0);
									 ...
									 int rx_freq)
{
	...
	for (g = 0; g < p2p->num_groups; g++) {
		if (memcmp(da, p2p_group_get_interface_addr(p2p->groups[g]),
				   ETH_ALEN) == 0) {
			group = p2p->groups[g];
			break;
		}
...
static void p2p_ext_listen_timeout(void *timeout_ctx)
{
	...
	if (p2p->ext_listen_interval) {
		...
		os_set_timer(&p2p->ext_listen_timer, p2p->ext_listen_interval_sec * 1000 + p2p->ext_listen_interval_usec / 1000);
	}
				   ...
				   unsigned int interval)
{
	...
	os_cancel_timer(&p2p->ext_listen_timer);
					  ...
					  const struct p2p_channel *pref_chan)
{
	...
	if (pref_chan) {
		...
		os_memcpy(n, pref_chan, num_pref_chan * sizeof(struct p2p_channel));
	} else {
		n = NULL;
	}
...
void p2p_go_neg_wait_timeout(void *timeout_ctx)
{
	struct p2p_data *p2p = timeout_ctx;
