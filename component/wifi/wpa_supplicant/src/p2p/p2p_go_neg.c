							...
							const u8 *channel_list, size_t channel_list_len)
{
	...
	if (pos[2] != 0x04 && memcmp(pos, p2p->cfg->country, 2) != 0) {
...
const char *p2p_wps_method_str(enum p2p_wps_method wps_method)
{
	switch (wps_method) {
	case WPS_PIN_DISPLAY:
		return "Display";
						  ...
						  u8 *status)
{
	...
	tmp = rtos_mem_zmalloc(sizeof(struct p2p_channels));
	if (!tmp) {
		ret = -1;
		goto exit;
	}
							...
							const u8 *data, size_t len, int rx_freq)
{
	...
	if (memcmp(msg->p2p_device_addr, sa, ETH_ALEN) != 0) {
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		if (dev->go_neg_req_sent &&
			memcmp(sa, p2p->cfg->dev_addr, ETH_ALEN) > 0) {
...
fail:
	...
	if (status == P2P_SC_SUCCESS) {
		...
		if (os_memcmp_p2p(sa, p2p->cfg->dev_addr, ETH_ALEN) < 0) {
			/*
							 ...
							 const u8 *data, size_t len, int rx_freq)
{
	...
	if (*msg->status) {
		...
		if (*msg->status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
			...
			dev->flags |= P2P_DEV_NOT_YET_READY;
			os_cancel_timer(&p2p->neg_wait_timer);
			os_set_timer(&p2p->neg_wait_timer, 120 * 1000);
