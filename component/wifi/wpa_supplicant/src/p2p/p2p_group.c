...
static int p2p_group_remove_member(struct p2p_group *group, const u8 *addr)
{
	...
	prev = NULL;
	while (m) {
		if (memcmp(m->addr, addr, ETH_ALEN) == 0) {
			break;
		}
		prev = m;
		m = m->next;
...
int p2p_group_match_dev_id(struct p2p_group *group, struct wpabuf *p2p)
{
	...
	if (memcmp(msg.device_id, group->p2p->cfg->dev_addr, ETH_ALEN) == 0) {
		return 1;    /* Match with our P2P Device Address */
	}

	for (m = group->members; m; m = m->next) {
		if (memcmp(msg.device_id, m->dev_addr, ETH_ALEN) == 0) {
		...
		const u8 *dev_id)
{
	...
	for (m = group->members; m; m = m->next) {
		if (memcmp(dev_id, m->dev_addr, ETH_ALEN) == 0) {
			return m;
		}
	}
	...
	struct p2p_group *group, const u8 *interface_addr)
{
	...
	for (m = group->members; m; m = m->next) {
		if (memcmp(interface_addr, m->addr, ETH_ALEN) == 0) {
			return m;
		}
	}
...
int p2p_group_is_client_connected(struct p2p_group *group, const u8 *dev_addr)
{
	...
	for (m = group->members; m; m = m->next) {
		if (memcmp(m->dev_addr, dev_addr, ETH_ALEN) == 0) {
			return 1;
		}
	}
								...
								size_t group_id_len)
{
	...
	if (memcmp(group_id, group->p2p->cfg->dev_addr, ETH_ALEN) != 0) {
		return 0;
	}
	return memcmp(group_id + ETH_ALEN, group->cfg->ssid,
