...
typedef unsigned long mp_digit;	// select
typedef u64 mp_word;
...
static int mp_init(mp_int *a)
{
	...
	a->dp = (mp_digit *)XMALLOC(sizeof(mp_digit) * MP_PREC);
	if (a->dp == NULL) {
		return MP_MEM;
	}
...
#if defined(BN_MP_DR_IS_MODULUS_C) || defined(BN_MP_REDUCE_IS_2K_C)
	int dr = 0;
#endif
...
#endif
	if ((mp_init(&x) || mp_init(&y) || mp_init(&u) || mp_init(&v) ||
		 mp_init(&A) || mp_init(&B) || mp_init(&C) || mp_init(&D)) != MP_OKAY) {
		return MP_MEM;
	}
...
{
	int     res;
	//  ===>mp_read_unsigned_bin(), c = 0x1
	// a->used = 0,a->alloc = 8,a->sign = 0
...
static int mp_grow(mp_int *a, int size)
{
	...
	if (a->alloc < size) {
		...
		*/
		tmp = (mp_digit *)XREALLOC(a->dp, sizeof(mp_digit) * a->alloc, sizeof(mp_digit) * size);
		if (tmp == NULL) {
...
static int mp_mul_2d(mp_int *a, int b, mp_int *c)
{
	...
	// c->alloc = 8,c->used = 0,(c->used + b/DIGIT_BIT + 1) = 1
...
#endif
	if ((mp_init(&ta) | mp_init(&tb) | mp_init(&tq) | mp_init(&q)) != MP_OKAY) {
		return MP_MEM;
	}
...
static int fast_s_mp_mul_digs(mp_int *a, mp_int *b, mp_int *c, int digs)
{
	...
	mp_digit *W = rtos_mem_zmalloc(MP_WARRAY * sizeof(mp_digit)); /*realtek modify https://jira.realtek.com/browse/RSWLANDIOT-8148*/
	/*register*/ mp_word  _W;
...
static int mp_init_size(mp_int *a, int size)
{
	...
	/* alloc mem */
	a->dp = (mp_digit *)XMALLOC(sizeof(mp_digit) * size);
	if (a->dp == NULL) {
		return MP_MEM;
	}
...
static int s_mp_mul_high_digs(mp_int *a, mp_int *b, mp_int *c, int digs)
{
	...
	if (((a->used + b->used + 1) < MP_WARRAY) && MIN(a->used, b->used) < (1 << ((CHAR_BIT * sizeof(mp_word)) - (2 * DIGIT_BIT)))) {
