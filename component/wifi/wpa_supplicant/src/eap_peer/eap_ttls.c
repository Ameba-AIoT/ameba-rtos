...
static void eap_ttls_deinit(struct eap_sm *sm, void *priv)
{
	...
	os_free(data->phase2_eap_types, 0);
...
#if defined(EAP_TTLS_EAP) || !defined(CONFIG_FIPS) && (defined(EAP_MSCHAPv2) ||defined(EAP_TTLS_MSCHAPv2) || defined(EAP_TTLS_CHAP)  || defined(EAP_TTLS_PAP))
...
#if !defined(CONFIG_FIPS) && (defined(EAP_MSCHAPv2) ||defined(EAP_TTLS_MSCHAPv2) || defined(EAP_TTLS_CHAP)  || defined(EAP_TTLS_PAP))
...
#if !defined(CONFIG_FIPS) && (defined(EAP_MSCHAPv2) ||defined(EAP_TTLS_MSCHAPv2) || defined(EAP_TTLS_CHAP))
...
#endif /* EAP_TTLS_MSCHAPv2 */

#ifdef EAP_TTLS_MSCHAP
								   ...
								   struct ttls_parse_avp *parse)
{
	...
	if (parse->eapdata == NULL) {
		...
		// modified by Owen
		u8 *neweap = os_realloc(parse->eapdata, parse->eap_len, parse->eap_len + dlen);
		if (neweap == NULL) {
			wpa_printf(MSG_WARNING, "EAP-TTLS: Failed to allocate "
					   "memory for Phase 2 EAP data");
			return -1;
