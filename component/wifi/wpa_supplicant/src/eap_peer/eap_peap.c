...
static void *eap_peap_init(struct eap_sm *sm)
{
	...
	struct eap_peer_config *config = sm->last_config;
	...
	if (eap_peer_tls_ssl_init(sm, &data->ssl, config, EAP_TYPE_PEAP)) {
		...
		DiagPrintf("EAP-PEAP: Failed to initialize SSL\n");
		eap_peap_deinit(sm, data);
		return NULL;
	}
...
// Owen for debug
		...
		wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: buf", buf, sizeof(buf));
		wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Received MAC", pos, SHA1_MAC_LEN);
		wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Expected MAC", mac, SHA1_MAC_LEN);
	...
	if (result_tlv) {
		...
		if (status == EAP_TLV_RESULT_SUCCESS) {
			...
			/*
						...
						wpa_hexdump_key(MSG_DEBUG, "Key", data->key_data, EAP_TLS_KEY_LEN);
						int key_len = 0;
						u8 *key = eap_peap_getKey(sm, data, key_len);
						set_wpa_global_PSK(key);
			*/
	...
	default:
		...
		if (data->phase2_priv == NULL) {
			data->phase2_method = eap_peer_get_eap_method(
									  data->phase2_type.vendor,
									  (enum EapType)data->phase2_type.method);
...
continue_req:
	...
	case EAP_CODE_REQUEST:
		wpa_printf(MSG_DEBUG, "EAP-PEAP: Phase 2 Request");
									   ...
									   const struct wpabuf *reqData)
{
	...
	pos = eap_peer_tls_process_init(sm, &data->ssl, EAP_TYPE_PEAP, ret,
									reqData, &left, &flags);
	if (pos == NULL) {
		wpa_printf(MSG_INFO, "PEAP: eap_peer_tls_process_init() return NULL");
		return NULL;
	}
	...
	//wpa_printf(MSG_DEBUG, "PEAP: before wpabuf_set %s %s %d", msg.buf, pos, left);
	wpabuf_set(&msg, pos, left);
	//wpa_printf(MSG_DEBUG, "PEAP: after wpabuf_set %s %s %d", msg.buf, pos, left);
	...
	if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
		...
		if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
			...
			if (data->session_id) {
				...
				// temp added by Owen for debug
				wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Derived Session-Id", data->session_id, data->id_len);
			} else {
