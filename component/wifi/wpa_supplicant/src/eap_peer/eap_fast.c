								  ...
								  const char *phase1)
{
	...
	if (pos) {
		...
		wpa_printf(MSG_DEBUG, "EAP-FAST: Maximum PAC list length: %d",
				   data->max_pac_list_len);
	}
								   ...
								   struct wpabuf **resp)
{
	...
	if (data->phase2_priv && data->phase2_method &&
		(vendor != data->phase2_type.vendor ||
		 method != (enum EapType)data->phase2_type.method)) {
...
static struct wpabuf *eap_fast_tlv_result(int status, int intermediate)
{
	...
	result->tlv_type = host_to_be16((EAP_TLV_TYPE_MANDATORY |
									 (intermediate ?  EAP_TLV_INTERMEDIATE_RESULT_TLV : EAP_TLV_RESULT_TLV)));
									   ...
									   const struct wpabuf *reqData)
{
	...
	pos = eap_peer_tls_process_init(sm, &data->ssl, EAP_TYPE_FAST, ret,
									reqData, &left, &flags);
	if (pos == NULL) {
		wpa_printf(MSG_INFO, "FAST: eap_peer_tls_process_init() return NULL");
		return NULL;
	}
	...
	// printf("+++FAST: before wpabuf_set %p %p %d\n", msg.buf, pos, left);
	wpabuf_set(&msg, pos, left);
	// printf("+++FAST: after wpabuf_set %p %p %d\n", msg.buf, pos, left);
