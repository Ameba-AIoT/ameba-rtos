...
{
	os_free(pac->pac_opaque, 0);
	os_free(pac->pac_info, 0);
	os_free(pac->a_id, 0);
	os_free(pac->i_id, 0);
	os_free(pac->a_id_info, 0);
	os_free(pac, 0);
}
...
static char *eap_fast_fgets(char *dst, size_t max, const char *f, int *offset)
{
	int c = '\0';
	char *p;
...
static int eap_fast_read_line(struct eap_fast_read_ctx *rc, char **value)
{
	...
	rc->line++;
	if (rc->f) {
		if (eap_fast_fgets(rc->buf, rc->buf_len, rc->f, &rc->offset) == NULL) {
			return -1;
		}
	} else {
		const char *l_end;
								  ...
								  struct eap_fast_read_ctx *rc)
{
	...
	rc->buf_len = 2048;
	rc->buf = (char *)os_malloc(rc->buf_len);
	if (rc->buf == NULL) {
		return -1;
	}
	...
	if (os_strncmp(pac_file, "blob://", 7) == 0) {
		...
		rc->f = pac_file;
		os_memcpy(rc->buf, rc->f, sizeof(rc->f));
		if (rc->f == NULL) {
						   ...
						   size_t len, int txt)
{
	...
	if (*pos - *buf + need > *buf_len) {
		char *nbuf = (char *)os_realloc(*buf, *buf_len, *buf_len + need);
		if (nbuf == NULL) {
			os_free(*buf, 0);
			*buf = NULL;
			return;
					  ...
					  const char *pac_file)
{
	...
	buf_len = 1024;
	pos = buf = (char *) os_zalloc(buf_len);
	if (buf == NULL) {
		return -1;
	}
