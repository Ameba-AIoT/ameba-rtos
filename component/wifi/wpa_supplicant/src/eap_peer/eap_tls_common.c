								  ...
								  struct wpabuf **out_data)
{
	...
	wpa_printf(MSG_DEBUG, "SSL: %d bytes left to be sent out (of total "
			   "%d bytes)",
			   len,
			   wpabuf_len(data->tls_out));
									...
									size_t *len, u8 *flags)
{
	...
	wpa_printf(MSG_DEBUG, "SSL: Received packet(len=%d) - "
			   "Flags 0x%02x", wpabuf_len(reqData),
			   *flags);
