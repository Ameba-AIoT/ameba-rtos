...
static void eap_peer_erp_init(struct eap_sm *sm)
{
	...
	WPA_PUT_BE16(len, 8);
	if (wifi_rom_hmac_sha256_kdf(sm->eapSessionId, sm->eapSessionIdLen, "EMSK",
								 len, sizeof(len),
	...
	if (wifi_rom_hmac_sha256_kdf(emsk, emsk_len,
	...
	if (wifi_rom_hmac_sha256_kdf(erp->rRK, erp->rRK_len,
		...
		size_t *count)
{
	...
	*count = 0;
	// struct eap_method_type !!!!! Carefully!!!!!
	buf = (struct eap_method_type *)os_malloc(mcount * sizeof(struct eap_method_type));
	if (buf == NULL) {
		return NULL;
	}
