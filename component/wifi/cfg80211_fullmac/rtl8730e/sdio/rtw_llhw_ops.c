...
void llhw_send_msg(u32 id, u8 *param, u32 param_len, u8 *ret, u32 ret_len)
{
	...
	dev_dbg(global_idev.fullmac_dev, "-----LINUX CALLING API %d START\n", id);
	...
	if (ret_msg != NULL) {
		...
		if (ret_msg->api_id != id) {
			dev_err(global_idev.fullmac_dev, "Linux API return value id not match!\n");
		}
...
exit:
	...
	dev_dbg(global_idev.fullmac_dev, "-----LINUX API %d CALLING DONE\n", id);
...
void llhw_wifi_on(void)
{
	...
	llhw_send_msg(INIC_API_WIFI_ON, (u8 *)param_buf, sizeof(param_buf), NULL, 0);
}
...
int llhw_wifi_set_mac_addr(u32 wlan_idx, u8 *addr)
{
	...
	llhw_send_msg(INIC_API_WIFI_SET_MAC_ADDR, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_connect(struct _rtw_network_info_t *connect_param, unsigned char block)
{
	...
	llhw_send_msg(INIC_API_WIFI_CONNECT, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_disconnect(void)
{
	...
	llhw_send_msg(INIC_API_WIFI_DISCONNECT, NULL, 0, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_is_connected_to_ap(void)
{
	...
	llhw_send_msg(INIC_API_WIFI_IS_CONNECTED_TO_AP, NULL, 0, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_get_channel(u32 wlan_idx, u8 *ch)
{
	...
	llhw_send_msg(INIC_API_WIFI_GET_CHANNEL, (u8 *)param_buf, sizeof(param_buf), ch, sizeof(u8));
...
int llhw_wifi_init_ap(void)
{
	...
	llhw_send_msg(INIC_API_WIFI_INIT_AP, NULL, 0, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_deinit_ap(void)
{
	...
	llhw_send_msg(INIC_API_WIFI_DEINIT_AP, NULL, 0, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_del_sta(u8 wlan_idx, u8 *mac)
{
	...
	llhw_send_msg(INIC_API_WIFI_DEL_STA, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_start_ap(struct _rtw_softap_info_t *softAP_config)
{
	...
	llhw_send_msg(INIC_API_WIFI_START_AP, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_stop_ap(void)
{
	...
	llhw_send_msg(INIC_API_WIFI_STOP_AP, NULL, 0, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_set_EDCA_params(unsigned int *AC_param)
{
	...
	llhw_send_msg(INIC_API_WIFI_SET_EDCA_PARAM, (u8 *)param_buf, sizeof(param_buf), (u8 *)&ret, sizeof(int));
...
int llhw_wifi_add_key(struct rtw_crypt_info *crypt)
{
	...
	llhw_send_msg(INIC_API_WIFI_ADD_KEY, (u8 *)crypt, sizeof(struct rtw_crypt_info), (u8 *)&ret, sizeof(int));
...
int llhw_wifi_get_chplan(u8 *chplan)
{
	...
	llhw_send_msg(INIC_API_WIFI_GET_CHPLAN, NULL, 0, chplan, sizeof(u8));
...
int llhw_wifi_sae_status_indicate(u8 wlan_idx, u16 status, u8 *mac_addr)
{
	...
	llhw_send_msg(INIC_API_WIFI_SAE_STATUS, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_pmksa_ops(dma_addr_t pmksa_ops_addr)
{
	...
	llhw_send_msg(INIC_API_WPA_PMKSA_OPS, (u8 *)pmksa_ops_addr, sizeof(struct rtw_pmksa_ops_t), (u8 *)&ret, sizeof(int));
...
int llhw_wifi_set_lps_enable(u8 enable)
{
	...
	llhw_send_msg(INIC_API_WIFI_SET_LPS_EN, (u8 *)param_buf, sizeof(param_buf), (u8 *)&ret, sizeof(int));
...
int llhw_wifi_mp_cmd(dma_addr_t cmd_addr, unsigned int cmd_len, dma_addr_t user_addr)
{
	...
	llhw_send_msg(INIC_API_WIFI_MP_CMD, (u8 *)param, size, (u8 *)user_addr, WIFI_MP_MSG_BUF_SIZE);
...
int llhw_wifi_iwpriv_cmd(dma_addr_t cmd_addr, unsigned int cmd_len, dma_addr_t user_addr)
{
	...
	llhw_send_msg(INIC_API_WIFI_IWPRIV_INFO, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_start_nan(u8 master_pref, u8 band_support)
{
	...
	llhw_send_msg(INIC_API_NAN_START, (u8 *)param_buf, sizeof(param_buf), (u8 *)&ret, sizeof(int));
...
int llhw_wifi_stop_nan(void)
{
	...
	llhw_send_msg(INIC_API_NAN_STOP, NULL, 0, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_add_nan_func(struct rtw_nan_func_info_t *func, void *nan_func_pointer)
{
	...
	llhw_send_msg(INIC_API_NAN_ADD_FUNC, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_del_nan_func(u64 cookie)
{
	...
	llhw_send_msg(INIC_API_NAN_DEL_FUNC, (u8 *)param_buf, sizeof(param_buf), (u8 *)&ret, sizeof(int));
...
int llhw_wifi_nan_cfgvendor_cmd(u16 vendor_cmd, const void *data, int len)
{
	...
	llhw_send_msg(INIC_API_NAN_CFGVENFOR, (u8 *)param, size, (u8 *)&ret, sizeof(int));
...
int llhw_wifi_set_pmf_mode(u8 pmf_mode)
{
	...
	llhw_send_msg(INIC_API_WIFI_SET_PMF_MODE, (u8 *)param_buf, sizeof(param_buf), (u8 *)&ret, sizeof(int));
...
int llhw_wifi_set_wps_phase(u8 enable)
{
	...
	llhw_send_msg(INIC_API_WIFI_SET_WPS_PHASE, (u8 *)param_buf, sizeof(param_buf), (u8 *)&ret, sizeof(int));
	return ret;
}
...
int llhw_wifi_set_wpa_mode(rtw_wpa_mode wpa_mode)
{
	...
	llhw_send_msg(INIC_API_WIFI_SET_WPA_MODE, (u8 *)param_buf, sizeof(param_buf), (u8 *)&ret, sizeof(int));
	return ret;
}
