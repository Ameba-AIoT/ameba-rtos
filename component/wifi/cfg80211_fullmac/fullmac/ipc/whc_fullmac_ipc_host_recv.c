...
void whc_fullmac_ipc_host_recv_task_from_msg_recv_pkts(int idx_wlan, struct dev_sk_buff *skb_phy)
{
...
static unsigned int whc_fullmac_ipc_host_recv_interrupt(aipc_ch_t *ch, ipc_msg_struct_t *pmsg)
{
	struct whc_ipc_ex_msg *p_ipc_msg = (struct whc_ipc_ex_msg *)llhw_ipc_fw_phy_to_virt(pmsg->msg);
	...
	if (p_ipc_msg->event_num == IPC_WIFI_EVT_TX_DONE) {
		whc_fullmac_ipc_host_xmit_done(p_ipc_msg->wlan_idx);
		ret = 0;
	} else {
		/* put the ipc message to the queue */
		ret = whc_fullmac_ipc_host_msg_enqueue(p_ipc_msg);
	}
...
struct aipc_ch_ops whc_fullmac_ipc_host_recv_ops = {
	.channel_recv = whc_fullmac_ipc_host_recv_interrupt,
};
