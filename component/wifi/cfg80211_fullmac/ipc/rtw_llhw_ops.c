...
int llhw_wifi_scan(struct _rtw_scan_param_t *scan_param, u32 ssid_length, u8 block)
{
	...
	ret = llhw_ipc_send_msg(INIC_API_WIFI_SCAN_NETWROKS, param_buf, 2);
...
int llhw_wifi_connect(struct _rtw_network_info_t *connect_param, unsigned char block)
{
	...
	ret = llhw_ipc_send_msg(INIC_API_WIFI_CONNECT, param_buf, 2);
...
int llhw_wifi_start_ap(struct _rtw_softap_info_t *softAP_config)
{
	...
	ret = llhw_ipc_send_msg(INIC_API_WIFI_START_AP, param_buf, 2);
	if (buf_vir) {
		rtw_mfree(size, buf_vir, buf_phy);
	}
	return ret;
...
int llhw_wifi_tx_mgnt(u8 wlan_idx, const u8 *buf, size_t buf_len, u8 need_wait_ack)
{
	...
	param_buf[0] = (u32)buf_phy;
	param_buf[1] = (u32)size;
	ret = llhw_ipc_send_msg(INIC_API_WIFI_SEND_MGNT, param_buf, 2);
...
int llhw_wifi_set_gen_ie(unsigned char wlan_idx, char *buf, u16 buf_len, u16 flags)
{
	int ret = 0;
	u32 param_buf[4];
	dma_addr_t dma_data = 0;
...
int llhw_wifi_add_custom_ie(const struct element **elem, u8 num, u16 type)
{
	...
	ret = llhw_ipc_send_msg(INIC_API_WIFI_CUS_IE, param_buf, 4);
