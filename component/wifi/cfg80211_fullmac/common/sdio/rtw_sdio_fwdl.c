...
int rtw_sdio_write_fw(struct inic_sdio *priv, u32 cnt, u8 *pmem)
{
	int r_val;
...
static int rtw_sdio_fwpagewrite(struct inic_sdio *priv, u32 offset, void *pdata, u32 size)
{
	u8 ret = false;
	int try_cnt = 100;
	u8 reg8;
...
static int rtw_sdio_writefw(struct inic_sdio *priv, void *buffer, u32 startaddr, u32 size)
{
	int ret = true;
...
int rtw_sdio_firmware_download(struct inic_sdio *priv, u32 need_checksum)
{
	s32 rtStatus = true;
	u8 write_fw = 0;
	u32 fwdl_start_time;
