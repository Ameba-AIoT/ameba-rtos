 ...
 *                               Common task size
 *********************************************************************************************/
#define WIFI_LITTLE_TASK_SIZE		                  ((432 + MCC_LITTE_TASK_SIZE + NAN_LITTE_TASK_SIZE + 128 + CONTEXT_SAVE_SIZE) / 4)	/* rtw_if_wifi_create_task: size will *4 */

#if defined (CONFIG_WHC_DEV)
#if defined(CONFIG_WHC_INTF_IPC)
#define WIFI_SINGLE_TASK_SIZE		                  ((1424 + WTN_SINGLE_TASK_SIZE + NAN_SINGLE_TASK_SIZE + 128 + CONTEXT_SAVE_SIZE) / 4)	/* max 1648 in lite, rtw_if_wifi_create_task: size will *4 */
#else
...
#define WIFI_DRIVER_RESUME_TASK_SIZE              ((352 + 128 + CONTEXT_SAVE_SIZE) / 4)	/* rtw_if_wifi_create_task: size will *4 */
...
#define WIFI_WHC_IPC_HST_EVT_API_TASK_SIZE        (2192 + EAP_UNBLK_API_SIZE + WPS_UNBLK_API_SIZE + 128 + CONTEXT_SAVE_SIZE)

#if defined(CONFIG_WHC_HOST)
#define WIFI_WHC_IPC_MSG_Q_TASK_SIZE	            (472 + 128 + CONTEXT_SAVE_SIZE)
...
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1024*4)
#else
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1048 + 128 + CONTEXT_SAVE_SIZE)	/* max 1072 in lite */
#endif
