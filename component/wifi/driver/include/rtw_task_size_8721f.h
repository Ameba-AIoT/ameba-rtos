...
#define WIFI_SINGLE_TASK_SIZE		                  ((1344 + WTN_SINGLE_TASK_SIZE + NAN_SINGLE_TASK_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN) / 4)	/* max 1648 in lite, rtw_if_wifi_create_task: size will *4 */
#else
...
#define WIFI_WHC_IPC_HST_EVT_API_TASK_SIZE        (1848 + EAP_UNBLK_API_SIZE + WPS_UNBLK_API_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN)
...
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1024*4)
#else
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1384 + CONTEXT_SAVE_SIZE_WITH_MARGIN)	/* max 1072 in lite */
#endif
