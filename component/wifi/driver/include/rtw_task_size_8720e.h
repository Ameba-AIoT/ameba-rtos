 ...
 *                               Common task size
 *********************************************************************************************/
#define WIFI_LITTLE_TASK_SIZE		                  ((528 + MCC_LITTE_TASK_SIZE + NAN_LITTE_TASK_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN) / 4)	/* rtw_if_wifi_create_task: size will *4 */

#if defined (CONFIG_WHC_DEV)
#if defined(CONFIG_WHC_INTF_IPC)
#define WIFI_SINGLE_TASK_SIZE	                    ((1520 + WTN_SINGLE_TASK_SIZE + NAN_SINGLE_TASK_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN) / 4)	/* rtw_if_wifi_create_task: size will *4 */
#else
...
#define WIFI_DRIVER_RESUME_TASK_SIZE              ((480 + CONTEXT_SAVE_SIZE_WITH_MARGIN) / 4)	/* rtw_if_wifi_create_task: size will *4 */
...
#define WIFI_WHC_IPC_HST_API_TASK_SIZE            (308 + CONTEXT_SAVE_SIZE_WITH_MARGIN)	// for psp overflow when update group key: jira: https://jira.realtek.com/browse/RSWLANQC-1027
#define WIFI_WHC_IPC_HST_EVT_API_TASK_SIZE        (1800 + EAP_UNBLK_API_SIZE + WPS_UNBLK_API_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN)

#if defined(CONFIG_WHC_HOST)
#define WIFI_WHC_IPC_MSG_Q_TASK_SIZE	            (468 + CONTEXT_SAVE_SIZE_WITH_MARGIN)
...
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1024*4)
#else
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1360 + CONTEXT_SAVE_SIZE_WITH_MARGIN)
#endif
