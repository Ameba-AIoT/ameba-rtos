 ...
 *                               Common task size
 *********************************************************************************************/
#define WIFI_LITTLE_TASK_SIZE		                  ((400 + MCC_LITTE_TASK_SIZE + NAN_LITTE_TASK_SIZE + 128 + CONTEXT_SAVE_SIZE) / 4)	/* rtw_if_wifi_create_task: size will *4 */

#if defined (CONFIG_WHC_DEV)
#if defined(CONFIG_WHC_INTF_IPC)
#define WIFI_SINGLE_TASK_SIZE		                  ((1360 + WTN_SINGLE_TASK_SIZE + NAN_SINGLE_TASK_SIZE + 128 + CONTEXT_SAVE_SIZE) / 4)	/* max 1648 in lite, rtw_if_wifi_create_task: size will *4 */
#else
...
#define WIFI_WHC_IPC_HST_API_TASK_SIZE            (440 + 128 + CONTEXT_SAVE_SIZE)	// for psp overflow when update group key: jira: https://jira.realtek.com/browse/RSWLANQC-1027
#define WIFI_WHC_IPC_HST_EVT_API_TASK_SIZE        (2272 + EAP_UNBLK_API_SIZE + WPS_UNBLK_API_SIZE + 128 + CONTEXT_SAVE_SIZE)

#if defined(CONFIG_WHC_HOST)
#define WIFI_WHC_IPC_MSG_Q_TASK_SIZE              (576 + 128 + CONTEXT_SAVE_SIZE)
...
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1024*5)
#elif defined(CONFIG_PLATFORM_ZEPHYR)
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1024*4)
#else
#define WIFI_WHC_IPC_DEV_API_TASK_SIZE            (1280 + 128 + CONTEXT_SAVE_SIZE)	/* max 1072 in lite */
#endif
...
#endif /*#ifndef RTW_TASK_SIZE_8730E_H */
