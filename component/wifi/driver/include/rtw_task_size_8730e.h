...
#define MCC_LITTE_THREAD_SIZE                     616  /* enable MCC need 616 */
#endif
 ...
 *                               Common task size
 *********************************************************************************************/
#define WIFI_TASK_SIZE_LITTLE_THREAD		          ((368 + MCC_LITTE_THREAD_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN) / 4)	/* rtw_if_wifi_create_task: size will *4 */

#if defined (CONFIG_WHC_DEV)
#if defined(CONFIG_WHC_INTF_IPC)
#define WIFI_TASK_SIZE_SINGLE_THREAD		          ((1360 + WTN_SINGLE_THREAD_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN) / 4)	/* max 1648 in lite, rtw_if_wifi_create_task: size will *4 */
#else
...
#define WIFI_TASK_SIZE_WHC_IPC_HST_API            (440 + CONTEXT_SAVE_SIZE_WITH_MARGIN)	// for psp overflow when update group key: jira: https://jira.realtek.com/browse/RSWLANQC-1027
#define WIFI_TASK_SIZE_WHC_IPC_HST_EVT_API        (1856 + EAP_UNBLK_API_SIZE + WPS_UNBLK_API_SIZE + CONTEXT_SAVE_SIZE_WITH_MARGIN)

#if defined(CONFIG_WHC_HOST)
#define WIFI_TASK_SIZE_WHC_IPC_MSG_Q              (576 + CONTEXT_SAVE_SIZE_WITH_MARGIN)
...
#define WIFI_TASK_SIZE_WHC_IPC_DEV_API            (1024*4)
#else
#define WIFI_TASK_SIZE_WHC_IPC_DEV_API            (1280 + CONTEXT_SAVE_SIZE_WITH_MARGIN)	/* max 1072 in lite */
#endif
