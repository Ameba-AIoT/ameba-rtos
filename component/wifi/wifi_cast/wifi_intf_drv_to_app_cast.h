...
/**
 * @brief Wifi cast send type.
 */
typedef enum {
	SEND_ETHERNET = 0, /*pkt with ethernet hdr*/
	SEND_RAW,		/*pkt with 802.11 hdr*/
	SEND_RELAY,		/*pkt with 802.11 hdr and it's for relay(different src addr)*/
} wifi_cast_send_type_t;
...
typedef struct wifi_cast_node {
	...
	unsigned char 	src_mac[ETH_ALEN]; 	   /* src address */
...
typedef struct wifi_cast_node_info {
	...
	wifi_cast_send_type_t	send_type;
	wifi_cast_frame_type_t	cast_frame_type;
...
#define WIFI_CAST_ERROR_CHECK(x) \
	do {                                      \
        ...
        if (err_rc_ != WIFI_CAST_OK) {                              \
            DiagPrintf("%s, filename: %d, line: %d, err: %d\n", __FUNCTION__, __FILE__, __LINE__,        \
                                    err_rc_);                 \
        }                                                               \
    } while(0)
