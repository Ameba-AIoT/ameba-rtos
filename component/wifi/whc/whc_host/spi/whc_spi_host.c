...
struct inic_spi_host_priv_t spi_host_priv = {0};

extern struct event_priv_t event_priv;

extern void inic_host_api_task(void);
void(*bt_inic_spi_recv_host_ptr)(uint8_t *buffer, uint16_t len);

bool inic_spi_rxgdma_init(
	u8 Index,
...
void inic_spi_host_rx_handler(u8 *buf)
{
	struct inic_msg_info *msg_info = (struct inic_msg_info *)buf;
	char *data = (char *)(buf + sizeof(struct inic_msg_info) + msg_info->pad_len);
...
int rtw_spi_recv_process(void)
{
	...
	case INIC_WIFI_EVT_RECV_PKTS:
		inic_spi_host_rx_handler(recv_msg);
		break;
	...
	case INIC_WIFI_EVT_API_RETURN:
		if (event_priv.b_waiting_for_ret) {
			...
			ret_msg = (struct inic_api_info *)(recv_msg + SIZE_RX_DESC);
...
void flush_rx_fifo(void)
{
	...
	while ((INIC_SPI_DEV->SPI_SR & (SPI_BIT_RFNE))) {
		rx_fifo_level =  INIC_SPI_DEV->SPI_RXFLR & SPI_MASK_RXTFL;
		for (i = 0; i < rx_fifo_level; i++) {
			value = INIC_SPI_DEV->SPI_DRx[0];
		}
	}
...
void inic_spi_rx_req_task(void)
{
	for (;;) {
...
static int inic_spi_dev_rdy_handler(int irq, void *context)
{
	(void)irq;
...
static int inic_spi_rx_req_handler(int irq, void *context)
{
	(void)irq;
	(void)context;
...
static void inic_spi_setup_gpio(void)
{
	...
	GPIO_UserRegIrq(GPIO_InitStruct.GPIO_Pin, inic_spi_rx_req_handler, &GPIO_InitStruct);
	...
	GPIO_Init(&GPIO_InitStruct);
	GPIO_UserRegIrq(GPIO_InitStruct.GPIO_Pin, inic_spi_dev_rdy_handler, &GPIO_InitStruct);
	GPIO_INTConfig(GPIO_InitStruct.GPIO_Pin, ENABLE);
...
void inic_spi_dma_tx_done_cb(void)
{
	struct inic_txbuf_info_t *inic_tx = container_of(spi_host_priv.txbuf_info, struct inic_txbuf_info_t, txbuf_info);
...
void inic_spi_rxdma_irq_task(void *pData)
{
	...
	for (;;) {
		...
		rtw_spi_recv_process();
	}
}

static void inic_spi_host_spi_init(void)
{
	SSI_InitTypeDef SSI_InitStructMaster;

	u8 index = (INIC_SPI_DEV == SPI0_DEV) ? 0 : 1;
...
//	PAD_PullCtrl(_PB_26, GPIO_PuPd_UP);
	...
	SSI_SetRole(INIC_SPI_DEV, SSI_MASTER);
	...
	SSI_Init(INIC_SPI_DEV, &SSI_InitStructMaster);

	spi_host_priv.rx_buf = rtos_mem_zmalloc(SPI_BUFSZ);
	DCache_CleanInvalidate((u32)spi_host_priv.rx_buf, SPI_BUFSZ);
	inic_spi_rxgdma_init(index, &(spi_host_priv.SSIRxGdmaInitStruct), (void *)INIC_SPI_RXDMA, (IRQ_FUN) inic_spi_rxdma_irq_handler, spi_host_priv.rx_buf,
						 SPI_BUFSZ);
	...
	if (rtos_task_create(NULL, "SPI_RXDMA_IRQ_TASK", inic_spi_rxdma_irq_task, (void *)&spi_host_priv, WIFI_STACK_INIC_SPI_RXDMA_IRQ_TASK, 7) != SUCCESS) {
		RTK_LOGE(TAG_WLAN_INIC, "Create SPI_RXDMA_IRQ_TASK Err!!\n");
		return;
	}

	if (rtos_task_create(NULL, "SPI_TXDMA_IRQ_TASK", inic_spi_txdma_irq_task, (void *)&spi_host_priv, WIFI_STACK_INIC_SPI_TXDMA_IRQ_TASK, 7) != SUCCESS) {
...
retry:
	...
	if (!spi_host_priv.txdma_initialized) {
		inic_spi_txdma_init(index, &(spi_host_priv.SSITxGdmaInitStruct), INIC_SPI_TXDMA, inic_spi_txdma_irq_handler, (u8 *)(pbuf->buf_addr), SPI_BUFSZ);
...
static void inic_spi_host_init(void)
{
	/* init host priv */
	...
	if (rtos_task_create(NULL, (const char *const)"SPI_RX_REQ_TASK", (rtos_task_function_t)inic_spi_rx_req_task, NULL, WIFI_STACK_SIZE_INIC_RX_REQ_TASK,
						 6) != SUCCESS) {
		RTK_LOGE(TAG_WLAN_INIC, "Create SPI_RXDMA_IRQ_TASK Err!!\n");
		return;
	}
	...
	if (SUCCESS != rtos_task_create(NULL, (const char *const)"inic_api_host_task", (rtos_task_function_t)inic_host_api_task, NULL,
