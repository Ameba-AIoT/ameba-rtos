...
void whc_host_deliver_rxbuf_to_user(uint8_t *pbuf);

/* Shared HISR dispatch (one pass): read/clear HISR, handle TX-avail and
 * RX-request. Called by each port's whc_sdio_host_irqhdl_task wrapper.
 * Additional required port macros:
 *   - whc_sem_give(sem)              : give a sema handle/object
 */
void whc_sdio_host_isr_process(struct whc_sdio *priv);
 ...
 *   - whc_msleep(ms)
 * plus whc_sdio_host_query_txbd_status() and hal rtw_write_port(). */
