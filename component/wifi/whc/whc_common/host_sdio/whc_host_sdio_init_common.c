...
void whc_sdio_host_init_txavailbd_threshold(struct whc_sdio *priv)
{
	uint16_t txBDTh_l;
	uint16_t txBDTh_h;
	uint32_t freeBDNum;
...
uint32_t whc_sdio_host_init(struct whc_sdio *priv)
{
	...
	if (whc_sdio_host_enable_func(priv) == FALSE) {
		return FALSE;
	}
...
#ifdef CONFIG_SDIO_TX_ENABLE_AVAL_INT
	whc_sdio_host_init_txavailbd_threshold(priv);
#endif

	priv->txbd_wptr = (uint16_t)rtw_read8(priv, SPDIO_REG_TXBD_WPTR);
	whc_sdio_host_query_txbd_status(priv);

	if (whc_sdio_host_get_tx_max_size(priv) == FALSE) {
		return FALSE;
	}

	whc_sdio_host_init_interrupt(priv);
