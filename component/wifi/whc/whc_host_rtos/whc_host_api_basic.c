...
s32 wifi_connect(struct rtw_network_info *connect_param, u8 block)
{
	...
	union rtw_event_info evt_info = {0};
...
error:
	...
	if (rtw_join_status == RTW_JOINSTATUS_FAIL && no_need_indicate == 0) {
		evt_info.join_status.fail_reason = result;
		wifi_indication(RTW_EVENT_JOIN_STATUS, (u8 *)&evt_info, sizeof(union rtw_event_info), RTW_JOINSTATUS_FAIL);
	}
