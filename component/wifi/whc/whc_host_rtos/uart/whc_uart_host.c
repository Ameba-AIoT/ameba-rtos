...
int whc_uart_host_recv_process(void)
{
	...
	uart_host_priv.rx_buf = rtos_mem_zmalloc(UART_RX_MAX_SIZE);
	DCache_CleanInvalidate((u32)uart_host_priv.rx_buf, UART_RX_MAX_SIZE);
	...
	case WHC_WIFI_EVT_API_CALL:
		buf = rtos_mem_zmalloc(UART_RX_MAX_SIZE);
		memcpy(buf, recv_msg, UART_RX_MAX_SIZE);
	...
	default:
		...
		//size = *(u16 *)pbuf;
		whc_host_pkt_rx_to_user(recv_msg, (u32)UART_RX_MAX_SIZE);
		break;
#endif
	}
...
void whc_uart_host_rxdma_irq_task(void *pData)
{
	...
	for (;;) {
		...
		DCache_Invalidate((u32)uart_host_priv.rx_buf, UART_RX_MAX_SIZE);
...
{
	if (irq == RxIrq) {
		UART_INTConfig(WHC_UART_DEV, RUART_BIT_ERBI | RUART_BIT_ELSI | RUART_BIT_ETOI, status);
	} else {
		// Tx irq seems no need
		UART_INTConfig(WHC_UART_DEV, RUART_BIT_ETBEI, status);
	}
}
...
u32 whc_uart_host_irq(void *param)
{
	u32 uart_irq;
	(void)param;
	GDMA_InitTypeDef *RxGDMA_InitStruct = &uart_host_priv.UARTRxGdmaInitStruct;
	u32 ret = 0;
	uart_irq = UART_LineStatusGet(WHC_UART_DEV);

	/* rx FIFO not empty */
	if ((uart_irq & RUART_BIT_RXFIFO_INT) || (uart_irq & RUART_BIT_TIMEOUT_INT)) {
		whc_uart_irq_set(RxIrq, DISABLE);
		UART_INT_Clear(WHC_UART_DEV, RUART_BIT_TOICF);
		GDMA_Cmd(RxGDMA_InitStruct->GDMA_Index, RxGDMA_InitStruct->GDMA_ChNum, ENABLE);
	}

	/* rx FIFO not empty */
	if (uart_irq & RUART_BIT_TX_EMPTY) {
		whc_uart_irq_set(TxIrq, DISABLE);
...
u32 whc_uart_host_dma_rx_done_cb(void *pData)
{
	GDMA_InitTypeDef *GDMA_InitStruct;
	u32 int_status;
...
static void whc_uart_host_drv_init(void)
{
	...
	rtos_mutex_create_static(&whc_uart_priv->tx_lock);
	rtos_mutex_create_static(&whc_uart_priv->rx_lock);
	rtos_mutex_create_static(&whc_uart_priv->host_send);
...
#else
	whc_uart_struct->RxFifoTrigLevel = UART_RX_FIFOTRIG_LEVEL_HALF;
#endif
	...
	whc_uart_priv->rx_buf = rtos_mem_zmalloc(UART_RX_MAX_SIZE);

	DCache_Invalidate((u32)whc_uart_priv->rx_buf, UART_RX_MAX_SIZE);
	...
	whc_uart_host_rxdma_init(whc_uart_idx, &whc_uart_priv->UARTRxGdmaInitStruct, WHC_UART_DEV, (IRQ_FUN)whc_uart_host_dma_rx_done_cb, whc_uart_priv->rx_buf, 0);

	/* setup tx dma */
	UART_TXDMAConfig(WHC_UART_DEV, WHC_UART_TX_BURST_SIZE);

	/* enable tx dma when start trans */
...
void whc_uart_host_send_data(struct whc_buf_info *pbuf)
{
	...
	if (!uart_host_priv.txdma_initialized) {
		...
		whc_uart_host_txdma_init(uart_host_priv.uart_idx, GDMA_InitStruct, WHC_UART_DEV, (IRQ_FUN)whc_uart_host_txdma_irq_handler, (u8 *)pbuf->buf_addr, len);
	...
	/* re-enable txfifo empty int */
	whc_uart_irq_set(TxIrq, ENABLE);
}