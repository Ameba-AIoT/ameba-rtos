...
struct whc_spdio_priv_t sdio_priv = {0};

void rtw_pending_q_resume(void);
void (*bt_inic_sdio_recv_ptr)(uint8_t *buffer, uint16_t len);

static u32 whc_spdio_dev_suspend(u32 expected_idle_time, void *param)
{
...
static u32 whc_spdio_dev_resume(u32 expected_idle_time, void *param)
{
	UNUSED(expected_idle_time);
	UNUSED(param);
	if (pmu_get_sleep_type() == SLEEP_PG) {
...
static char whc_spdio_dev_rpwm_cb(void *priv, u16 value)
{
	UNUSED(priv);
...
static char whc_spdio_dev_tx_done_cb(void *priv, void *pbuf)
{
	UNUSED(priv);
...
static char whc_spdio_dev_rx_done_cb(void *priv, void *pbuf, u8 *pdata, u16 size, u8 type)
{
...
void whc_spdio_dev_init(void)
{
	u32 i;
	struct sk_buff *skb = NULL;
	struct spdio_t *dev = &sdio_priv.dev;
#ifdef WHC_SPDIO_USE_GPIO_INT
	...
	dev->pSDIO = SDIO_WIFI;
	dev->device_rx_done_cb = whc_spdio_dev_rx_done_cb;
	dev->device_tx_done_cb = whc_spdio_dev_tx_done_cb;
	dev->rpwm_cb = whc_spdio_dev_rpwm_cb;
	...
	pmu_register_sleep_callback(PMU_WHC_WIFI, (PSM_HOOK_FUN)whc_spdio_dev_suspend, NULL, (PSM_HOOK_FUN)whc_spdio_dev_resume, NULL);
	...
	whc_spdio_dev_set_rxreq_pin(DEV_RX_IDLE);
#endif

	RTK_LOGI(TAG_WLAN_INIC, "SPDIO device init done!\n");
	return;
...
void whc_spdio_dev_trigger_rx_handle(void)
{
...
void whc_spdio_dev_send(u8 *buf, u16 len, void *buf_alloc, u8 is_skb)
{
	...
	whc_spdio_dev_set_rxreq_pin(DEV_RX_IDLE);
	whc_spdio_dev_set_rxreq_pin(DEV_RX_REQ);
#endif
...
u8 whc_spdio_dev_bus_is_idle(void)
{
	/*Not yet implemented*/
	return TRUE;
}
