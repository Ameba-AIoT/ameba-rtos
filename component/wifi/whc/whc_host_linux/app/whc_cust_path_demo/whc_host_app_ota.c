...
void whc_host_ota_init(struct whc_host_ota_info *ota_info)
{
	...
	*(uint32_t *)ptr = WHC_WIFI_TEST;
	...
	/* tell dev prepare */
	whc_host_api_send_nl_payload(buf, buf_len);
...
{
	uint8_t notify_buf[8] = {0};
	uint8_t *ptr = notify_buf;
	uint32_t buf_len = 0;
	struct whc_host_ota_hdr *ota_hdr;
	...
	*(uint32_t *)ptr = WHC_WIFI_TEST;
	...
	ota_hdr = (struct whc_host_ota_hdr *)ptr;
	ota_hdr->type = WHC_WIFI_TEST_OTA;
	ota_hdr->subtype = WHC_OTA_RMESH_INIT;
	ota_hdr->len = 0;
	ptr += sizeof(struct whc_host_ota_hdr);
	buf_len += sizeof(struct whc_host_ota_hdr);
	printf("[WHC] notify dev: WHC_WIFI_TEST_OTA + WHC_OTA_RMESH_INIT\n");
	whc_host_api_send_nl_payload(notify_buf, buf_len);
}
