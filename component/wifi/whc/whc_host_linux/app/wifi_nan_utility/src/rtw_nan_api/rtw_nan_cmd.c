...
extern char *rtw_iw;
extern char *rtw_nan_r3;
extern char *nan_intf;
					 ...
					 char *fname)
{
	...
	/* send iw cmd */
	sprintf(cmd, "%s %s vendor recv 0x%x 0x%x %s > %s",
			rtw_iw, if_name, OUI_REALTEK, cmd_id, tmp_file, cmdreply_file);
	printf("[rtw_cmd]%s !\n", cmd);
	system(cmd);
...
void nandow_parse_cmd_reply(struct nan_customer_nandow *nandow_test, char *cmdreply_file)
{
	...
	char *para = (char *)&nandow_test->para;
							   ...
							   char *iw_path)
{
	...
	FILE *file = NULL;
	char cmd[512] = {0};
	unsigned int no_input = 0xff;
	int ret = RTW_RET_STATUS_ERROR;
	...
	// close file
	fclose(outfile);
	if ((file = fopen(iw_path, "r"))) {
		sprintf(cmd, "%s %s vendor send 0x%x 0x%x %s", iw_path, if_name, OUI_REALTEK, cmd_id, tmp_file);
	...
	return ret ? RTW_RET_STATUS_ERROR : RTW_RET_STATUS_SUCCESS;
}


bool _nan_get_wdev(char *intf, char *wdev)
{
	FILE *fp = NULL;
	char temp[50] = {0}, cmd[512] = {0};
	uint32_t num = 0, wdev_value_offset = 4;

	if (intf == NULL) {
		INFO_PRINT("[rtw_cmd] error unknow interface!\n");
		return false;
	}
	...
	while (fscanf(fp, "%49s", temp) != EOF) {
		if (strstr(temp, "wdev") != 0) {
			fscanf(fp, "%49s", temp);
			strcpy(wdev, temp);
		}
		if ((strstr(temp, "NAN") != 0) && (wdev != NULL)) {
			/* if find type NAN that means get target wdev*/
			DEBUG_PRINT("[rtw_cmd] find wdev(%s)\n", wdev);
			num ++;
			break;
		}
	}
...
bool _nan_check_intf(char *intf)
{
	FILE *fp = NULL;
	char temp[50] = {0};
	char cmd[512] = {0};
...
bool _nan_check_phy(uint32_t phy_num)
{
	FILE *fp;
	char temp[50];
	char phy[50];
...
RTW_RET_STATUS rtw_nan_api_get_capability(char *intf, uint16_t *nan_cap)
{
	...
	if (_nan_check_intf(intf) == false) {
		INFO_PRINT("[rtw_cmd] %s, can not find intf[%s]\n", __func__, intf);
...
RTW_RET_STATUS rtw_nan_api_init(uint16_t phy_num, char *intf)
{
	...
	if (_nan_check_phy(phy_num) == false) {
		INFO_PRINT("[rtw_cmd] can not find phy%u !!\n", phy_num);
		return RTW_RET_STATUS_INVALID_INPUT;
	}
	if (intf == NULL) {
		INFO_PRINT("[rtw_cmd] unknow interface!\n");
		return RTW_RET_STATUS_INVALID_INPUT;
	}

	/* 1. add nan interface */
	if (_nan_check_intf(intf) == true) {
		INFO_PRINT("[rtw_cmd] %s exist!!\n", intf);
	} else {
		INFO_PRINT("[rtw_cmd] %s interface add\n", intf);
		sprintf(cmd, "%s phy%u interface add %s type __nan",
				rtw_iw, phy_num, intf);
		system(cmd);
	}

	/* 2. bring up nan interface */
	INFO_PRINT("[rtw_cmd] %s interface up\n", intf);
	sprintf(cmd, "ifconfig %s up", intf);
	system(cmd);

	/* 3. start event thread and iw event */
...
RTW_RET_STATUS rtw_nan_api_deinit(uint16_t phy_num)
{
	...
	/* 1. terminate event thread and kill iw */
	rtw_nan_evt_deinit();

	/* 2. delete nan wdev */
	if (_nan_get_wdev(nan_intf, wdev) == true) {
		INFO_PRINT("[rtw_cmd] delete wdev = %s \n", wdev);
		sprintf(cmd, "%s wdev %s interface del", rtw_iw, wdev);
		system(cmd);
	} else {
		INFO_PRINT("[rtw_cmd] phy%d can not find %s!!\n", phy_num, nan_intf);
	}
