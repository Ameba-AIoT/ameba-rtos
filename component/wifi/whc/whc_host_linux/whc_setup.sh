...
case $choice in
	1)
		echo "WPAoH host with cfg80211S select"
		sed -i 's/CONFIG_WHC_WIFI_API_PATH = n/CONFIG_WHC_WIFI_API_PATH = y/g' Makefile
		;;
	2)
		echo "WPAoD host without cfg80211 select"
...
if [ "$1" == "spi" ]; then
	...
	sed -i 's/CONFIG_WHC_HCI_SPI = n/CONFIG_WHC_HCI_SPI = y/g' Makefile
elif [ "$1" == "usb" ]; then
	sed -i 's/CONFIG_WHC_HCI_SDIO = y/CONFIG_WHC_HCI_SDIO = n/g' Makefile
	sed -i 's/CONFIG_WHC_HCI_SPI = y/CONFIG_WHC_HCI_SPI = n/g' Makefile
	sed -i 's/CONFIG_WHC_HCI_USB = n/CONFIG_WHC_HCI_USB = y/g' Makefile
elif [ "$1" == "sdio" ]; then
	sed -i 's/CONFIG_WHC_HCI_SDIO = n/CONFIG_WHC_HCI_SDIO = y/g' Makefile
	sed -i 's/CONFIG_WHC_HCI_SPI = y/CONFIG_WHC_HCI_SPI = n/g' Makefile
	sed -i 's/CONFIG_WHC_HCI_USB = y/CONFIG_WHC_HCI_USB = n/g' Makefile
fi
...
if [[ "$1" == "spi" ]]; then
	...
	sed -i 's/#define CALCULATE_FREE_TXBD 1/#undef CALCULATE_FREE_TXBD/g' ./common/autoconf.h
	sed -i 's/#define CONFIG_AMEBAGREEN2 1/#undef CONFIG_AMEBAGREEN2/g' ./common/autoconf.h
	sed -i 's/#define CONFIG_AMEBALITE 1/#undef CONFIG_AMEBALITE/g' ./common/autoconf.h
    ...
    case $choice in
        ...
        3)
            ...
            sed -i 's/#undef CONFIG_AMEBALITE/#define CONFIG_AMEBALITE 1/g' ./common/autoconf.h
            ;;
        *)
            echo "Invalid selection"
            ;;
