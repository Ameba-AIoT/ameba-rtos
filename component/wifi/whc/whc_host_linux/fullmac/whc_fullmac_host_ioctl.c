...
#ifdef CONFIG_WOWLAN
static int whc_fullmac_host_wowlan_ctrl(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	int ret = 0;
...
static int whc_fullmac_host_wowlan_set_pattern(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
#ifdef TODO
	...
	if (!check_fwstate(pmlmepriv, WIFI_ASOC_STATE) &&
		...
		goto _whc_fullmac_host_wowlan_set_pattern_exit;
	}
...
_whc_fullmac_host_wowlan_set_pattern_exit:
	return ret;
#else
	return 0;
#endif
...
static int whc_fullmac_host_priv_set(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wdata, char *extra)
{
...
#ifdef CONFIG_WOWLAN
	case MP_WOW_ENABLE:
		whc_fullmac_host_wowlan_ctrl(dev, info, wdata, extra);
		break;
	case MP_WOW_SET_PATTERN:
		whc_fullmac_host_wowlan_set_pattern(dev, info, wdata, extra);
		break;
#endif
	default:
		return -EIO;
...
static int whc_fullmac_host_priv_get(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wdata, char *extra)
{
	struct iw_point *wrqu = (struct iw_point *)wdata;
	u32 subcmd = wrqu->flags;
...
static const struct iw_priv_args whc_fullmac_host_private_args[] = {
...
static iw_handler whc_fullmac_host_private_handler[] = {
	whc_fullmac_host_priv_set,					/* 0x00 */
	whc_fullmac_host_priv_get,					/* 0x01 */
};

#ifdef CONFIG_WIRELESS_EXT
struct iw_handler_def whc_fullmac_host_handlers_def = {
	.standard = rtw_handlers,
	.num_standard = sizeof(rtw_handlers) / sizeof(iw_handler),

#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 33)) || defined(CONFIG_WEXT_PRIV)
	.private = whc_fullmac_host_private_handler,
	.private_args = whc_fullmac_host_private_args,
	.num_private = sizeof(whc_fullmac_host_private_handler) / sizeof(iw_handler),
	.num_private_args = sizeof(whc_fullmac_host_private_args) / sizeof(struct iw_priv_args),
#endif
