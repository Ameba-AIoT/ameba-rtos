...
static int whc_fullmac_host_del_station(struct wiphy *wiphy, struct net_device *ndev, struct station_del_parameters *params)
{
	...
	dev_dbg(global_idev.pwhc_dev, "[fullmac]:%s %x:%x:%x:%x:%x:%x", __func__,
	...
	if (global_idev.is_need_4way[wlan_idx] && global_idev.is_4way_ongoing[wlan_idx] > 0) {
		...
		if (0 == global_idev.is_4way_ongoing[wlan_idx]) {// no client ongoing 4-way process, then notify NP end. otherwise, not notify 4-way end.
			...
			dev_dbg(global_idev.pwhc_dev, "fullmac host indicate 4-way end\n");
		}
	}
...
void whc_fullmac_host_sta_assoc_indicate(char *buf, int buf_len)
{
	...
	if (global_idev.is_need_4way[wlan_idx]) {
		...
		global_idev.is_4way_ongoing[wlan_idx]++;
		dev_dbg(global_idev.pwhc_dev, "fullmac host indicate 4-way start, ongoing 4way client cnt=%d \n", global_idev.is_4way_ongoing[wlan_idx]);
	}
}
