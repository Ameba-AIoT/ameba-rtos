...
#ifdef CONFIG_P2P
static int whc_fullmac_host_p2p_ndev_open(struct net_device *pnetdev)
{
...
static int whc_fullmac_host_p2p_ndev_close(struct net_device *pnetdev)
{
	struct wireless_dev *wdev;
	...
	if (wdev->iftype == NL80211_IFTYPE_P2P_CLIENT) {
		if (global_idev.p2p_global.pd_wlan_idx == 1) {//GC is up and use netdev1 with port0
			whc_fullmac_host_p2p_gc_intf_revert(1);
		}
...
const struct net_device_ops whc_fullmac_host_ndev_ops_p2p = {
	.ndo_init = rtw_ndev_init,
	.ndo_uninit = rtw_ndev_uninit,
	.ndo_open = whc_fullmac_host_p2p_ndev_open,
	.ndo_stop = whc_fullmac_host_p2p_ndev_close,
...
int whc_fullmac_host_ndev_p2p_register(enum nl80211_iftype type, const char *name, u8 wlan_idx)
{
	...
	ndev->netdev_ops = &whc_fullmac_host_ndev_ops_p2p;
									 ...
									 struct wireless_dev **p2p_wdev, enum nl80211_iftype type)
{
	...
	if (type == NL80211_IFTYPE_P2P_GO || type == NL80211_IFTYPE_P2P_CLIENT) {
		...
		whc_fullmac_host_ndev_p2p_register(type, name, wlan_idx); /*step3: alloc/register new netdev and wdev*/
...
void whc_fullmac_host_p2p_iface_free(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	if (iftype == NL80211_IFTYPE_P2P_DEVICE) {
		...
		if (iftype == NL80211_IFTYPE_P2P_CLIENT) {
			whc_fullmac_host_p2p_gc_intf_revert(1); /* switch back port0 and port1's mac addr*/
		}
