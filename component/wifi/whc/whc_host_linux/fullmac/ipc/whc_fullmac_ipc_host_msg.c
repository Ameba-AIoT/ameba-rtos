...
static int whc_fullmac_ipc_host_enqueue_ipc_msg_node(struct ipc_msg_q_priv *priv, struct ipc_msg_node *p_node)
{
	spin_lock(&(priv->lock));
...
static struct ipc_msg_node *whc_fullmac_ipc_host_dequeue_ipc_msg_node(struct ipc_msg_q_priv *priv)
{
...
static void whc_fullmac_ipc_host_msg_q_task(struct work_struct *data)
{
	...
	if (priv->b_queue_working) {
		...
		while (1) {
			p_node = whc_fullmac_ipc_host_dequeue_ipc_msg_node(priv);
			if (p_node == NULL) {
				break;
			}
			/* haddle the message */
...
int whc_fullmac_ipc_host_msg_q_init(struct device *pdev, void (*task_hdl)(u8 event_num, u32 msg_addr, u8 wlan_idx))
{
	...
	INIT_WORK(&(msg_priv->msg_work), whc_fullmac_ipc_host_msg_q_task);
...
int whc_fullmac_ipc_host_msg_enqueue(struct whc_ipc_ex_msg *p_ipc_msg)
{
	...
	ret = whc_fullmac_ipc_host_enqueue_ipc_msg_node(priv, p_node);
...
void whc_fullmac_ipc_host_msg_q_deinit(void)
{
	...
	dma_free_coherent(global_idev.ipc_dev, sizeof(struct whc_ipc_ex_msg), msg_priv->p_inic_ipc_msg, msg_priv->ipc_msg_phy_addr);
