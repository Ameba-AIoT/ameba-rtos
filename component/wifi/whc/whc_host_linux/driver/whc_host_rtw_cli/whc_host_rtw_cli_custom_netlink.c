...
int whc_host_nl_custom_api(struct sk_buff *skb, struct genl_info *info)
{
	...
	cmd = nla_get_u32(info->attrs[WHC_ATTR_API_ID]);
	if (cmd == API_WIFI_SEND_TO_DEV) {
		...
		whc_host_do_scan(skb, info);
	} else if (cmd == API_WIFI_SCAN_RESULT) {
		whc_host_scan_result();
	} else if (cmd == API_WIFI_GET_JOIN_EVENT) {
		whc_host_get_join_status(skb, info);
	} else if (cmd == API_WIFI_MP) {
		whc_host_do_mp_cmd(skb, info);
#if defined(CONFIG_WHC_WIFI_API_PATH)
	} else if (cmd == API_WIFI_DBG) {
...
int whc_host_send_to_user_multi(u8 *buf, u16 size, u16 api_id, u32 chuck_index, u8 last_chuck)
{
	struct sk_buff *skb;
	void *reply;
	...
	ret = genlmsg_multicast(&whc_nl_family, skb, 0, WHC_MCGRP_EVENT, GFP_KERNEL);
	if (ret < 0) {
		return ret;
	}
...
int whc_host_send_to_user(u8 *buf, u16 size, u16 api_id)
{
	struct sk_buff *skb;
	void *reply;
	struct genlmsghdr *genlhdr;
...
int whc_host_send_rxbuf_to_user(u8 *buf, u16 size)
{
	...
	ret = whc_host_send_to_user(buf, size, 0);
