...
int whc_host_set_user_config(struct wifi_user_conf *pwifi_usrcfg)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_USR_CFG, param_buf, 1);
...
void whc_host_wifi_on(void)
{
	...
	whc_ipc_host_send_msg(WHC_API_WIFI_ON, param_buf, 1);
}
...
int whc_host_set_mac_addr(u32 wlan_idx, u8 *addr)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_MAC_ADDR, param_buf, 3);
...
int whc_host_send_scan(struct rtw_scan_param *scan_param, u32 ssid_length)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SCAN_NETWROKS, param_buf, 2);
...
{
	return whc_ipc_host_send_msg(WHC_API_WIFI_SCAN_ABORT, NULL, 0);
}
...
int whc_host_send_connect(struct rtw_network_info *connect_param)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_CONNECT, param_buf, 2);
	rtw_mfree(size, buf_vir, buf_phy);
...
int whc_host_event_disconnect(u16 reason_code)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_DISCONNECT, param_buf, 3);
...
int whc_host_set_channel(u32 wlan_idx, u8 ch)
{
	...
	param_buf[0] = wlan_idx;
	param_buf[1] = (u32)ch;
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_CHANNEL, param_buf, 2);
...
int whc_host_init_ap(void)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_INIT_AP, NULL, 0);
	return ret;
}
...
int whc_host_deinit_ap(void)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_DEINIT_AP, NULL, 0);
	return ret;
}
...
int whc_host_ap_del_client(u8 wlan_idx, u8 *mac)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_AP_DEL_CLIENT, param_buf, 2);
	return ret;
}
...
int whc_host_start_ap(struct rtw_softap_info *softAP_config)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_START_AP, param_buf, 2);
	if (buf_vir) {
		rtw_mfree(size, buf_vir, buf_phy);
	}
	return ret;
...
int whc_host_stop_ap(void)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_STOP_AP, NULL, 0);
...
int whc_host_set_EDCA_params(struct rtw_edca_param *pedca_param)
{
	...
	param_buf[0] = (u32)phy_addr;
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_EDCA_PARAM, param_buf, 1);
...
int whc_host_add_key(struct rtw_crypt_info *crypt)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_ADD_KEY, param_buf, 1);
...
int whc_host_wpa_4way_status_indicate(struct rtw_wpa_4way_status *rpt_4way)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WPA_4WAY_REPORT, param_buf, 1);
...
int whc_host_tx_mgnt(u8 wlan_idx, const u8 *buf, size_t buf_len, u8 need_wait_ack)
{
	...
	param_buf[0] = (u32)buf_phy;
	param_buf[1] = (u32)size;
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SEND_MGNT, param_buf, 2);
...
int whc_host_sae_status_indicate(u8 wlan_idx, u16 status, u8 *mac_addr)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SAE_STATUS, param_buf, 3);
...
int whc_host_get_traffic_stats(u8 wlan_idx, dma_addr_t stats_traffic)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_GET_TRAFFIC_STATS, param_buf, 2);
...
int whc_host_get_phy_stats(u8 wlan_idx, const u8 *mac_addr, union rtw_phy_stats *stats)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_GET_PHY_STATS, param_buf, 3);
...
int whc_host_get_setting(unsigned char wlan_idx, dma_addr_t setting_phy)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_GET_SETTING, param_buf, 2);
...
int whc_host_channel_switch(dma_addr_t csa_param_phy)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_AP_CH_SWITCH, param_buf, 1);
	return ret;
}
...
int whc_host_pmksa_ops(dma_addr_t pmksa_ops_phy)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WPA_PMKSA_OPS, param_buf, 1);
...
int whc_host_set_lps_enable(u8 enable)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_LPS_EN, param_buf, 1);
	return ret;
}
...
int whc_host_mp_cmd(dma_addr_t cmd_phy, unsigned int cmd_len, dma_addr_t user_phy)
{
	...
	return whc_ipc_host_send_msg(WHC_API_WIFI_MP_CMD, param_buf, 4);
}
...
int whc_host_iwpriv_cmd(dma_addr_t cmd_phy, unsigned int cmd_len, unsigned char *cmd, unsigned char *user_buf)
{
	...
	param_buf[1] = (u32)cmd_len;
	param_buf[2] = (u32)1;
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_IWPRIV_INFO, param_buf, 3);
...
int whc_host_init_nan(void)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_NAN_INIT, NULL, 0);
	return ret;
}
...
int whc_host_deinit_nan(void)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_NAN_DEINIT, NULL, 0);
	return ret;
}
...
int whc_host_start_nan(u8 master_pref, u8 band_support)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_NAN_START, param_buf, 2);
	return ret;
}
...
int whc_host_stop_nan(void)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_NAN_STOP, NULL, 0);
	return ret;
}
...
int whc_host_add_nan_func(struct rtw_nan_func_t *func, void *nan_func_pointer)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_NAN_ADD_FUNC, param_buf, 4);
	if (buf_vir) {
		rtw_mfree(size, buf_vir, buf_phy);
	}
	return ret;
...
int whc_host_del_nan_func(u64 cookie)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_NAN_DEL_FUNC, param_buf, 2);
	return ret;
}
...
int whc_host_nan_cfgvendor_cmd(u16 vendor_cmd, const void *data, int len)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_NAN_CFGVENFOR, param_buf, 3);
...
void whc_host_set_p2p_role(enum rtw_p2p_role role)
{
	...
	whc_ipc_host_send_msg(WHC_API_P2P_ROLE, param_buf, 1);
}
...
int whc_host_set_p2p_remain_on_ch(unsigned char wlan_idx, u8 enable)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_P2P_REMAIN_ON_CH, param_buf, 2);
	return ret;
}
#endif
...
void whc_host_sme_auth(dma_addr_t auth_data_phy)
{
	...
	whc_ipc_host_send_msg(WHC_API_WIFI_SME_AUTH, param_buf, 1);
}
...
int whc_host_sme_set_assocreq_ie(u8 *ie, size_t ie_len, u8 wpa_rsn_exist)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SME_SET_ASSOCREQ_IE, param_buf, 3);
...
int whc_host_set_pmf_mode(u8 pmf_mode)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_PMF_MODE, param_buf, 1);
	return ret;
}
...
int whc_host_set_wpa_mode(u8 wpa_mode)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_WPA_MODE, param_buf, 1);
	return ret;
}
...
int whc_host_set_owe_param(struct rtw_owe_param_t *owe_param)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_OWE_PARAM, param_buf, 1);
...
int whc_host_set_gen_ie(unsigned char wlan_idx, char *buf, u16 buf_len, u16 flags)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_GEN_IE, param_buf, 4);
...
int whc_host_add_custom_ie(const struct element **elem, u8 num, u16 type)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_CUS_IE, param_buf, 4);
...
int whc_host_del_custom_ie(unsigned char wlan_idx)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_CUS_IE, param_buf, 2);
...
int whc_host_set_edcca_mode(struct rtw_edcca_param_t *p)
{
	...
	case RTW_EDCCA_DISABLE:
		param_buf[0] = buf_phy;
		ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_EDCCA_PARAM, param_buf, 1);
		break;
	default:
...
int whc_host_get_edcca_mode(u8 *edcca_mode)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_GET_EDCCA_MODE, param_buf, 1);
...
int whc_host_get_ant_info(u8 *antdiv_mode, u8 *curr_ant)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_GET_ANTENNA_INFO, param_buf, 2);
...
int whc_host_set_country_code(char *cc)
{
	...
	param_buf[0] = (u32)phy_addr;
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_SET_COUNTRY_CODE, param_buf, 1);
...
int whc_host_get_country_code(struct rtw_country_code_table *table)
{
	...
	param_buf[0] = (u32)phy_addr;
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_GET_COUNTRY_CODE, param_buf, 1);
...
int whc_host_dev_driver_is_mp(u8 *is_mp)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_DRIVE_IS_MP, NULL, 0);
	if (ret < 0) {
		*is_mp = 0;
	} else {
		*is_mp = ret;
...
int whc_host_set_promisc_enable(u32 enable, u8 mode)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_PROMISC_INIT, param_buf, 3);
...
int whc_host_ft_status_indicate(struct rtw_kvr_param_t *kvr_param, u16 status)
{
	...
	ret = whc_ipc_host_send_msg(WHC_API_WIFI_FT_STATUS, param_buf, 2);
	if (phy_addr) {
