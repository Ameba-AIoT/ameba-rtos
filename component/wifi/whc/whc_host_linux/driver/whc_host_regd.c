...
static struct ieee80211_regdomain *_whc_host_regd_get(u8 chplan)
{
...
static void _whc_host_regd_set_country_code(struct wiphy *wiphy, u8 *country)
{
	int ret = 0;
	...
	if (ret == 0) {
		...
		regd = _whc_host_regd_get(table.channel_plan);
...
int whc_host_regd_update(struct rtw_country_code_table *ptab)
{
	int ret = 0;
	struct ieee80211_regdomain *regd = NULL;
	struct wiphy *wiphy = global_idev.pwiphy_global;

	regd = _whc_host_regd_get(ptab->channel_plan);
...
void whc_host_regd_notifier(struct wiphy *wiphy, struct regulatory_request *request)
{
	...
	case NL80211_REGDOM_SET_BY_COUNTRY_IE:
		_whc_host_regd_set_country_code(wiphy, request->alpha2);
		break;
	default:
		/* do nothing */
		break;
...
int whc_host_regd_init(void)
{
	...
	because _whc_host_regd_set_country_code must be called after wifi on, whc_host_regd_init assignement should be moved to whc_host_regd_init */
	wiphy->reg_notifier = whc_host_regd_notifier;
