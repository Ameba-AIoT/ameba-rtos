...
#ifdef CONFIG_NAN
	is_nan_data = whch_host_nan_is_nan_frame(precv_frame);
#endif
	pda = get_da(ptr);
	psa = get_sa(ptr);
...
#ifdef CONFIG_NAN
				if (iface_type == WHC_NAN_PORT && whch_host_nan_is_nan_frame(precv_frame)) {
...
void whc_host_recv_wlanhdr_to_ethhdr(union recv_frame *precvframe)
{
	...
	if (!memcmp(pattrib->dst, g_nanpriv.nmi.mac, ETH_ALEN)) {
		memcpy(ptr, global_idev.pndev[precvframe->u.hdr.iface_type]->dev_addr, ETH_ALEN);
	}
#endif
