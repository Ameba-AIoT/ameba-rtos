...
static int whc_host_nl_send_buf_to_dev(struct genl_info *info);
...
static struct whc_host_netlink_command_entry netlink_cmd_table[] = {
	{API_WIFI_INFO_INIT, whc_host_nl_init},
	{API_WIFI_SEND_TO_DEV, whc_host_nl_send_buf_to_dev},
...
__attribute__((weak)) int whc_host_send_rxbuf_to_user(u8 *buf, u16 size)
{
	...
	if (size == 0) {
		printk("%s: size err!", __FUNCTION__);
		return -1;
	}
