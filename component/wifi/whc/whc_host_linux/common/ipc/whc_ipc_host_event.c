...
static void whc_host_scan_report_indicate(struct event_priv_t *event_priv, struct whc_ipc_dev_req_msg *p_ipc_msg)
{
...
static void whc_host_event_promisc_pkt_hdl(struct event_priv_t *event_priv, struct whc_ipc_dev_req_msg *p_ipc_msg)
{
	struct device *pdev = NULL;
...
void whc_host_event_task(struct work_struct *data)
{
	...
	case WHC_API_SCAN_USER_CALLBACK:
		...
		whc_host_scan_done_indicate(p_recv_msg->param_buf[0], NULL);
	...
	case WHC_API_PROMISC_CALLBACK:
		whc_host_event_promisc_pkt_hdl(event_priv, p_recv_msg);
		break;
	...
	case WHC_API_CFG80211_SCAN_REPORT:
		whc_host_scan_report_indicate(event_priv, p_recv_msg);
		break;
#ifdef CONFIG_NAN
