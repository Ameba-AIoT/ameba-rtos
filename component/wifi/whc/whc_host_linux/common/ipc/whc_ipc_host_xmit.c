...
static struct dev_sk_buff *whc_ipc_host_find_one_free_skb(u32 *skb_index, bool *b_dropped)
{
...
int whc_host_xmit_entry(int idx, struct sk_buff *pskb)
{
	...
	if ((skb->busy) && ((skb = whc_ipc_host_find_one_free_skb(&skb_index, &b_dropped)) == NULL)) {
	...
	skb->no_free = 1;
	skb->len = 0;
	skb->list.prev = skb->list.next = (void *)(skb_phy + offsetof(struct dev_sk_buff, list));
