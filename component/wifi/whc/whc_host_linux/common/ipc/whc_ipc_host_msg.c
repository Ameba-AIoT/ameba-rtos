...
static int whc_ipc_host_enqueue_ipc_msg_node(struct ipc_msg_q_priv *priv, struct ipc_msg_node *p_node)
{
	spin_lock(&(priv->lock));
...
static struct ipc_msg_node *whc_ipc_host_dequeue_ipc_msg_node(struct ipc_msg_q_priv *priv)
{
...
static void whc_ipc_host_msg_q_task(struct work_struct *data)
{
	...
	if (priv->b_queue_working) {
		...
		while (1) {
			p_node = whc_ipc_host_dequeue_ipc_msg_node(priv);
			if (p_node == NULL) {
				break;
			}
			/* haddle the message */
...
int whc_ipc_host_msg_q_init(struct device *pdev, void (*task_hdl)(u8 event_num, u32 msg_addr, u8 wlan_idx))
{
	...
	INIT_WORK(&(msg_priv->msg_work), whc_ipc_host_msg_q_task);
...
int whc_ipc_host_msg_enqueue(struct whc_ipc_ex_msg *p_ipc_msg)
{
	...
	ret = whc_ipc_host_enqueue_ipc_msg_node(priv, p_node);
