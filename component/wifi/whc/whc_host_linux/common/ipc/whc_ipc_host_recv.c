...
recv_done:
	...
	whc_ipc_host_send_packet(&ipc_msg);
...
static unsigned int whc_ipc_host_recv_interrupt(aipc_ch_t *ch, ipc_msg_struct_t *pmsg)
{
	...
	if (p_ipc_msg->event_num == IPC_WIFI_EVT_TX_DONE) {
		whc_ipc_host_xmit_done(p_ipc_msg->wlan_idx);
		ret = 0;
	} else {
		/* put the ipc message to the queue */
		ret = whc_ipc_host_msg_enqueue(p_ipc_msg);
	}
...
struct aipc_ch_ops whc_ipc_host_recv_ops = {
	.channel_recv = whc_ipc_host_recv_interrupt,
};
