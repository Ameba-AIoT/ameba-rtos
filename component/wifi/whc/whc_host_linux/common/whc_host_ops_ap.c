...
static struct wireless_dev *whc_host_add_virtual_intf(struct wiphy *wiphy, const char *name,
		unsigned char name_assign_type, enum nl80211_iftype type, struct vif_params *params)
{
	int ret = 0;
	struct wireless_dev *wdev = NULL;

	dev_info(global_idev.pwhc_dev, "%s: name:%s, type:%d.", __func__, name, type);
	...
	case NL80211_IFTYPE_P2P_DEVICE:
		ret = whc_host_p2p_iface_alloc(wiphy, name, &wdev, type);
	...
	default:
		ret = -ENODEV;
		dev_err(global_idev.pwhc_dev, "%s: Unsupported interface type.", __func__);
		break;
	}

	dev_info(global_idev.pwhc_dev, "%s: wdev:%p, ret:%d.", __func__, wdev, ret);
...
static int whc_host_del_virtual_intf(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	int ret = 0;
...
static int whc_host_set_mac_acl(struct wiphy *wiphy, struct net_device *ndev, const struct cfg80211_acl_data *params)
{
	return 0;
}
#endif /* CONFIG_RTW_MACADDR_ACL && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0)) */

static int whc_host_add_station(struct wiphy *wiphy, struct net_device *ndev, const u8 *mac, struct station_parameters *params)
{
	return 0;
}

static int whc_host_del_station(struct wiphy *wiphy, struct net_device *ndev, struct station_del_parameters *params)
{
	int ret = 0;
	...
	dev_dbg(global_idev.pwhc_dev, "%s(): not to del sta when AP SME is offloaded\n", __func__);
	return 0;
#endif

	dev_dbg(global_idev.pwhc_dev, "[whc]:%s %x:%x:%x:%x:%x:%x", __func__,
	...
	ret = whc_host_ap_del_client(wlan_idx, (u8 *) mac_phy);
...
static int whc_host_change_station(struct wiphy *wiphy, struct net_device *ndev, const u8 *mac, struct station_parameters *params)
{
	return 0;
}

static int whc_host_dump_station(struct wiphy *wiphy, struct net_device *ndev, int idx, u8 *mac, struct station_info *sinfo)
{
	return -ENOENT;
}

static int whc_host_change_bss(struct wiphy *wiphy, struct net_device *ndev, struct bss_parameters *params)
{
	return 0;
}

static int whc_host_set_txq_params(struct wiphy *wiphy, struct net_device *ndev, struct ieee80211_txq_params *params)
{
	int ret = 0;
	...
	ret = whc_host_set_EDCA_params(&edca_param);

	return ret;
}

#ifdef CONFIG_SUPPLICANT_SME
static int whc_host_probe_client(struct wiphy *wiphy, struct net_device *ndev, const u8 *peer, u64 *cookie)
{
	return 0;
}
...
static int whc_host_change_beacon(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_beacon_data *info)
#else
static int whc_host_change_beacon(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_ap_update *info)
#endif
{
#ifdef CONFIG_P2P
	...
	if (target_ptr) {
		whc_host_update_custom_ie((u8 *)target_ptr, (global_idev.p2p_global.beacon_wps_ie_idx + 1), (RTW_CUS_IE_BEACON | RTW_CUS_IE_PROBERSP));
	}
	...
	if (target_ptr) {
		whc_host_update_custom_ie((u8 *)target_ptr, (global_idev.p2p_global.beacon_p2p_ie_idx + 1), (RTW_CUS_IE_BEACON | RTW_CUS_IE_PROBERSP));
	}
#endif
...
static int whc_host_channel_switch_ops(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_csa_settings *params)
{
	int ret;
	...
	dev_dbg(global_idev.pwhc_dev, "switch to channel: %d", ieee80211_frequency_to_channel(params->chandef.chan->center_freq));
	dev_dbg(global_idev.pwhc_dev, "switch to freq: %d", params->chandef.chan->center_freq);
	dev_dbg(global_idev.pwhc_dev, "channel switch count: %d", params->count);
	...
	ret = whc_host_channel_switch(csa_param_phy);
...
void whc_host_sta_assoc_indicate(char *buf, int buf_len)
{
	...
	if (global_idev.is_need_4way[wlan_idx]) {
		...
		global_idev.is_4way_ongoing[wlan_idx]++;
		dev_dbg(global_idev.pwhc_dev, "whc host indicate 4-way start, ongoing 4way client cnt=%d \n", global_idev.is_4way_ongoing[wlan_idx]);
	}
}

static int whc_host_start_ap_ops(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_ap_settings *settings)
{
	int ret = 0;
...
#endif
	u8 wlan_idx = rtw_netdev_idx(ndev);
	dev_dbg(global_idev.pwhc_dev, "=>"FUNC_NDEV_FMT" - Start Softap\n", FUNC_NDEV_ARG(ndev));
	...
	dev_dbg(global_idev.pwhc_dev, "wpa_versions=%d\n", settings->crypto.wpa_versions);
	dev_dbg(global_idev.pwhc_dev, "n_ciphers_pairwise=%d\n", settings->crypto.n_ciphers_pairwise);
	dev_dbg(global_idev.pwhc_dev, "ciphers_pairwise=0x%x\n", settings->crypto.ciphers_pairwise[0]);
	dev_dbg(global_idev.pwhc_dev, "n_akm_suites=%d\n", settings->crypto.n_akm_suites);
	dev_dbg(global_idev.pwhc_dev, "akm_suites=0x%x\n", settings->crypto.akm_suites[0]);
	dev_dbg(global_idev.pwhc_dev, "cipher_group=0x%x\n", settings->crypto.cipher_group);
#if (KERNEL_VERSION(6, 6, 0) > LINUX_VERSION_CODE)
	dev_dbg(global_idev.pwhc_dev, "wep_tx_key=%d\n", settings->crypto.wep_tx_key);
#endif
	dev_dbg(global_idev.pwhc_dev, "sae_pwd_len=%d\n", settings->crypto.sae_pwd_len);
	dev_dbg(global_idev.pwhc_dev, "hidden_ssid=%d\n", settings->hidden_ssid);
	...
	if (settings->privacy) {
		if (settings->crypto.n_ciphers_pairwise > 1 || settings->crypto.n_akm_suites > 1) {
			dev_dbg(global_idev.pwhc_dev, "wpa mixed mode, not support right now!\n");
			return -EPERM;
		}
		if ((settings->crypto.wpa_versions == 2) && ((u8)settings->crypto.akm_suites[0] == 0x08)) {
			...
			dev_dbg(global_idev.pwhc_dev, "wpa_versions=1, not support right now!\n");
			return -EPERM;
		} else {
			softAP_config->security_type = RTW_SECURITY_WEP_PSK;
			dev_err(global_idev.pwhc_dev, "ERR: AP in WEP security mode is not supported!!");
			return -EPERM;
		}

		dev_dbg(global_idev.pwhc_dev, "wlan_idx:%d, is_need_4way=%d, \n", wlan_idx, global_idev.is_need_4way[wlan_idx]);
	...
	if (settings->beacon.assocresp_ies && settings->beacon.assocresp_ies_len > 0) {
		...
		if (target_ptr) {
			whc_host_set_gen_ie(1, (u8 *)target_ptr, ((u16)target_ptr->datalen + 2), P2PWPS_ASSOC_RSP_IE);
		}
	}
#endif

	ret = whc_host_start_ap(softAP_config);
	...
	if (settings->beacon.beacon_ies_len) {
		...
		if (elem_num != 0) {
			...
			if (!pelem) {
				dev_dbg(global_idev.pwhc_dev, "%s: malloc pelem failed.", __func__);
				return -ENOMEM;
			}
			...
			whc_host_add_custom_ie(pelem, elem_num, (RTW_CUS_IE_BEACON | RTW_CUS_IE_PROBERSP));
...
static int whc_host_stop_ap_ops(struct wiphy *wiphy, struct net_device *ndev
							   ...
							   )
{
	int ret = 0;
	u8 is_mp = 0;
	dev_dbg(global_idev.pwhc_dev, "=>"FUNC_NDEV_FMT" - Stop Softap\n", FUNC_NDEV_ARG(ndev));
...
void whc_host_ops_ap_init(void)
{
	...
	ops->channel_switch = whc_host_channel_switch_ops;
	ops->add_virtual_intf = whc_host_add_virtual_intf;
	ops->del_virtual_intf = whc_host_del_virtual_intf;
	ops->start_ap = whc_host_start_ap_ops;
	ops->change_beacon = whc_host_change_beacon;
	ops->stop_ap = whc_host_stop_ap_ops;
#if defined(CONFIG_RTW_MACADDR_ACL) && (CONFIG_RTW_MACADDR_ACL == 1)
	ops->set_mac_acl = whc_host_set_mac_acl;
#endif
	ops->add_station = whc_host_add_station;
	ops->del_station = whc_host_del_station;
	ops->change_station = whc_host_change_station;
	ops->dump_station = whc_host_dump_station;
	ops->change_bss = whc_host_change_bss;
	ops->set_txq_params = whc_host_set_txq_params;
	ops->dump_survey = whc_host_dump_survey_params;
#ifdef CONFIG_SUPPLICANT_SME
	/*
	 ...
	 */
	ops->probe_client = whc_host_probe_client;
#endif	/* CONFIG_SUPPLICANT_SME */
}
