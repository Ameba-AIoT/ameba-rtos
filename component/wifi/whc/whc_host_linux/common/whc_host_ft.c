...
static struct whc_host_ft_priv_t lacal_ft_priv = {0};

static int _whc_host_ft_auth_tx(void);

static void _whc_host_ft_auth_timer_hdl(struct timer_list *t)
{
...
static int _whc_host_ft_auth_tx(void)
{
	struct whc_host_ft_priv_t *ft_priv = &lacal_ft_priv;
...
static int _whc_host_ft_auth_resp_process(u8 *pframe, u32 pkt_len)
{
...
static int _whc_host_ft_auth_start(void)
{
	struct whc_host_ft_priv_t *ft_priv = &lacal_ft_priv;
	int ret = 0;
	...
	timer_setup(&lacal_ft_priv.ft_auth_timer, _whc_host_ft_auth_timer_hdl, 0);
	mod_timer(&lacal_ft_priv.ft_auth_timer, jiffies + msecs_to_jiffies(REAUTH_TO));
...
static int _whc_host_ft_rx_mgnt(char *evt_info)
{
	...
	if (ieee80211_is_auth(mgmt->frame_control)) {
		ret = _whc_host_ft_auth_rx(evt_rpt_frm->frame, evt_rpt_frm->frame_len);
...
static void _whc_host_ft_rx_join_status(unsigned int join_status)
{
...
int whc_host_ft_event(u32 event, char *evt_info, unsigned int join_status)
{
	...
	case RTW_EVENT_FT_AUTH_START:
		_whc_host_ft_auth_start();
		break;
	case RTW_EVENT_FT_RX_MGNT:
		_whc_host_ft_rx_mgnt(evt_info);
		break;
	case RTW_EVENT_JOIN_STATUS:
		_whc_host_ft_rx_join_status(join_status);
		break;
	default:
		ret = -EINVAL;
...
void whc_host_ft_init(void)
{
	...
	dev_dbg(global_idev.pwhc_dev, "%s===>\n", __func__);
	memset(ft_priv, 0, sizeof(struct whc_host_ft_priv_t));
	_FT_SET_STATUS(_FT_UNASSOCIATED_STA);
}
