...
static int whc_host_ops_get_station(struct wiphy *wiphy, struct net_device *ndev, const u8 *mac, struct station_info *sinfo)
{
	int ret = 0;
	...
	if (!mac) {
		dev_err(global_idev.pwhc_dev, "[whc]: %s !mac", __func__);
		return -ENOENT;
	}

	dev_dbg(global_idev.pwhc_dev, "[whc]: %s,MAC => %02x:%02x:%02x:%02x:%02x:%02x\r\n",
...
static int whc_host_ops_change_iface(struct wiphy *wiphy, struct net_device *ndev, enum nl80211_iftype type, struct vif_params *params)
{
...
static int whc_host_scan_ops(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	int i;
	int ret = 0;
...
static void whc_host_abort_scan(struct wiphy *wiphy, struct wireless_dev *wdev)
{
...
static int whc_host_join_ibss(struct wiphy *wiphy, struct net_device *ndev,
...
static int whc_host_leave_ibss(struct wiphy *wiphy, struct net_device *ndev)
{
	dev_dbg(global_idev.pwhc_dev, "[whc]: %s", __func__);
	return 0;
}
...
static int whc_host_connect_ops(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_connect_params *sme)
{
	int ret = 0;
	u8 *buf = NULL, *ptr = NULL;
	size_t size = 0;
...
#ifdef CONFIG_IEEE80211R
		whc_host_ft_set_bssid(sme->bssid);
#endif
...
static int whc_host_disconnect_ops(struct wiphy *wiphy, struct net_device *ndev, u16 reason_code)
{
	int ret = 0;
	u8 is_mp = 0;
...
static int whc_host_set_power_mgmt(struct wiphy *wiphy, struct net_device *ndev, bool enabled, int timeout)
{
	u8 is_mp = 0;
...
static int whc_host_set_monitor_channel(struct wiphy *wiphy, struct cfg80211_chan_def *chandef)
{
	...
	dev_dbg(global_idev.pwhc_dev, "[whc]: %s, channel %d", __func__, chandef->center_freq1);
	...
	if (ret < 0) {
		dev_err(global_idev.pwhc_dev, "[whc] set channel failed(%d).", ret);
		return ret;
	}
									...
									struct cfg80211_chan_def *chandef)
{
	...
	if (wdev == global_idev.p2p_global.pd_pwdev) {/*P2P Device intf not have ndev, wlanidx need fetch from other way*/
		...
		if (pnetdev) {
			...
			dev_err(global_idev.pwhc_dev, "[whc]: %s, cannot find wlan idx.", __func__);
			return -EINVAL;
		}
...
static int whc_host_external_auth_status(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_external_auth_params *params)
{
	u8 is_mp = 0;
...
static int whc_sme_host_assoc(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_assoc_request *req)
{
	...
	conn_param->auth_type = sme_priv->auth_type;	/* used by whc_host_connect_ops() */
	 ...
	 * atomic.
	 */
	if (whc_host_connect_ops(wiphy, ndev, conn_param) < 0) {
...
static int whc_sme_host_deauth(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_deauth_request *req)
{
	...
	whc_host_disconnect_ops(wiphy, ndev, req->reason_code);
...
static s32 whc_host_remain_on_channel(struct wiphy *wiphy, struct wireless_dev *wdev, struct ieee80211_channel *channel, unsigned int duration,
									  u64 *cookie)
{
#ifdef CONFIG_P2P
	int ret = 0;
...
static s32 whc_host_cancel_remain_on_channel(struct wiphy *wiphy, struct wireless_dev *wdev, u64 cookie)
{
#ifdef CONFIG_P2P
...
static int whc_host_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev, struct cfg80211_mgmt_tx_params *params, u64 *cookie)
{
...
void whc_host_mgmt_frame_register(struct wiphy *wiphy, struct wireless_dev *wdev, u16 frame_type, bool reg)
{
	//dev_dbg(global_idev.pwhc_dev, "[whc]: %s reg = %d", __func__, reg);
#ifdef CONFIG_P2P
	int p2p_idx = -1;
...
void whc_host_ops_sta_init(void)
{
	...
	ops->change_virtual_intf = whc_host_ops_change_iface;
	ops->get_station = whc_host_ops_get_station;
	ops->scan = whc_host_scan_ops;
	ops->abort_scan = whc_host_abort_scan;
	ops->set_wiphy_params = whc_host_set_wiphy_params;
	ops->connect = whc_host_connect_ops;
	ops->disconnect = whc_host_disconnect_ops;
	ops->join_ibss = whc_host_join_ibss;
	ops->leave_ibss = whc_host_leave_ibss;
	ops->set_tx_power = whc_host_set_txpower;
	ops->get_tx_power = whc_host_get_txpower;
	ops->set_power_mgmt = whc_host_set_power_mgmt;
	ops->set_monitor_channel = whc_host_set_monitor_channel;
	ops->get_channel = whc_host_get_channel_ops;
	ops->remain_on_channel = whc_host_remain_on_channel;
	ops->cancel_remain_on_channel = whc_host_cancel_remain_on_channel;
	ops->mgmt_tx = whc_host_mgmt_tx;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0))
	ops->mgmt_frame_register = whc_host_mgmt_frame_register;
#else
	ops->update_mgmt_frame_registrations = whc_host_update_mgmt_frame_register;
#endif
	ops->external_auth = whc_host_external_auth_status;
#ifdef CONFIG_SUPPLICANT_SME
