...
#endif
{
	...
	if (copy_from_user(&cmd, rq->ifr_data, sizeof(struct rtw_priv_ioctl))) {
		dev_err(global_idev.pwhc_dev, "[fullmac]: %s copy_from_user failed\n", __func__);
		ret = -EFAULT;
		goto out;
	}
...
static void rtw_set_rx_mode(struct net_device *dev)
{
	...
	if ((rtw_netdev_flags(dev) ^ dev->flags) & IFF_PROMISC) {
		if (dev->flags & IFF_PROMISC) {
			dev_dbg(global_idev.pwhc_dev, "[fullmac]: %s enable promisc mode!\n", __func__);
			whc_host_set_promisc_enable(1, RTW_PROMISC_FILTER_ALL_PKT);
		} else {
			dev_dbg(global_idev.pwhc_dev, "[fullmac]: %s disable promisc mode!\n", __func__);
