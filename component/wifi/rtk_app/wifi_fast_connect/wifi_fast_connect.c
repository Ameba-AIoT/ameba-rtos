...
__weak int write_fast_connect_data_to_flash(unsigned int offer_ip, unsigned int server_ip)
{
	...
	(void) server_ip;
#endif
	struct fast_conn_write_flash *data = (struct fast_conn_write_flash *)rtos_mem_zmalloc(sizeof(struct fast_conn_write_flash));

	if (data == NULL) {
		RTK_LOGS(NOTAG, RTK_LOG_ERROR, "\r\n %s():malloc fail", __func__);
		return RTK_FAIL;
	}

	/* STEP1: get current connect info from wifi driver*/
	if (wifi_get_setting(STA_WLAN_INDEX, &(data->setting)) || data->setting.mode == RTW_MODE_AP) {
		RTK_LOGS(NOTAG, RTK_LOG_ERROR, "\r\n %s():wifi_get_setting fail or ap mode", __func__);
		rtos_mem_free((u8 *)data);
		return RTK_FAIL;
	}
	...
	case RTW_SECURITY_WPA3_OWE:
		memcpy(data->wifi_data_to_flash.psk_essid, data->setting.ssid, strlen((const char *)data->setting.ssid));
		data->wifi_data_to_flash.security_type = RTW_SECURITY_OPEN;
		break;
	case RTW_SECURITY_WEP_PSK:
	case RTW_SECURITY_WEP_SHARED:
		memcpy(data->wifi_data_to_flash.psk_essid, data->setting.ssid, strlen((const char *)data->setting.ssid));
		memcpy(data->wifi_data_to_flash.psk_passphrase, data->setting.password, strlen((const char *)data->setting.password));
		data->channel |= (data->setting.key_idx) << 28;
		data->wifi_data_to_flash.security_type = RTW_SECURITY_WEP_SHARED;
		break;
	...
	case RTW_SECURITY_WPA2_WPA3_MIXED:
		...
		rtw_psk_get_psk_info(&(data->PSK_info));
		memcpy(data->wifi_data_to_flash.psk_essid, data->PSK_info.psk_essid, sizeof(data->wifi_data_to_flash.psk_essid));
		memcpy(data->wifi_data_to_flash.psk_passphrase, data->PSK_info.psk_passphrase, sizeof(data->wifi_data_to_flash.psk_passphrase));
		memcpy(data->wifi_data_to_flash.wpa_global_PSK, data->PSK_info.wpa_global_PSK, sizeof(data->wifi_data_to_flash.wpa_global_PSK));
		data->wifi_data_to_flash.security_type = data->setting.security_type;
		data->wifi_data_to_flash.is_wps_trigger = data->setting.is_wps_trigger;
		break;

	default:
		break;
	}
	memcpy(&(data->wifi_data_to_flash.channel), &(data->channel), 4);
#if defined(CONFIG_FAST_DHCP) && CONFIG_FAST_DHCP
	data->wifi_data_to_flash.offer_ip = offer_ip;
	data->wifi_data_to_flash.server_ip = server_ip;
#endif

	/* STEP2: get last time fast connect info from flash*/
	memset(&(data->read_data), 0xff, sizeof(struct wlan_fast_reconnect));
#ifdef CONFIG_PLATFORM_ZEPHYR
	settings_load_one("wlan_data", (uint8_t *) & (data->read_data), sizeof(struct wlan_fast_reconnect));
#else
	rt_kv_get("wlan_data", (uint8_t *) & (data->read_data), sizeof(struct wlan_fast_reconnect));
#endif

	/* STEP3: wirte new connect info to flash if different content: SSID, Passphrase, Channel, Security type*/
	if (memcmp((u8 *) & (data->wifi_data_to_flash), (u8 *) & (data->read_data), sizeof(struct wlan_fast_reconnect)) != 0) {
#if defined(CONFIG_FAST_DHCP) && CONFIG_FAST_DHCP
...
#endif
#ifdef CONFIG_PLATFORM_ZEPHYR
		settings_save_one("wlan_data", (uint8_t *) & (data->wifi_data_to_flash), sizeof(struct wlan_fast_reconnect));
#else
		rt_kv_set("wlan_data", (uint8_t *) & (data->wifi_data_to_flash), sizeof(struct wlan_fast_reconnect));
#endif
	}
...
{
	int ret;
	struct do_fast_conn *data = (struct do_fast_conn *)rtos_mem_zmalloc(sizeof(struct do_fast_conn));
	if (data == NULL) {
		RTK_LOGS(NOTAG, RTK_LOG_ERROR, "\r\n %s():malloc fail", __func__);
		return RTK_FAIL;
	}
#ifndef CONFIG_ENABLE_WPS
	data->wifi_retry_connect = WIFI_RETRYCOUNT;	// For fast wifi connect retry
#else
	data->wifi_retry_connect = 0;	// For fast wifi connect retry times in WPS function
#endif
	...
	memset(&(data->fast_reconn_data), 0xff, sizeof(struct wlan_fast_reconnect));
#ifndef CONFIG_PLATFORM_ZEPHYR
	ret = rt_kv_get("wlan_data", (uint8_t *) & (data->fast_reconn_data), sizeof(struct wlan_fast_reconnect));
#else
	ret = settings_load_one("wlan_data", (uint8_t *) & (data->fast_reconn_data), sizeof(struct wlan_fast_reconnect));
#endif
	if (ret < 0) {
	...
	empty_data = (struct wlan_fast_reconnect *)rtos_mem_zmalloc(sizeof(struct wlan_fast_reconnect));
	if (empty_data) {
		memset(empty_data, 0xff, sizeof(struct wlan_fast_reconnect));
		if (memcmp(empty_data, &(data->fast_reconn_data), sizeof(struct wlan_fast_reconnect)) == 0) {
			DiagPrintf("[FAST_CONNECT] Fast connect profile is empty, abort fast connection\n");
			rtos_mem_free((u8 *)empty_data);
			rtos_mem_free((u8 *)data);
			return 0;
		}
		rtos_mem_free((u8 *)empty_data);
	...
	data->PSK_INFO.security_type = data->fast_reconn_data.security_type;
	memcpy(data->PSK_INFO.psk_essid, data->fast_reconn_data.psk_essid, sizeof(data->fast_reconn_data.psk_essid));
	memcpy(data->PSK_INFO.psk_passphrase, data->fast_reconn_data.psk_passphrase, sizeof(data->fast_reconn_data.psk_passphrase));
	memcpy(data->PSK_INFO.wpa_global_PSK, data->fast_reconn_data.wpa_global_PSK, sizeof(data->fast_reconn_data.wpa_global_PSK));

	if (wifi_check_fast_connect_data(&(data->fast_reconn_data)) < 0) {
		DiagPrintf("[FAST_CONNECT] Fast connect profile is wrong, abort fast connection\n");
		rtos_mem_free((u8 *)data);
		return -1;
	}
	data->PSK_INFO.index = STA_WLAN_INDEX;
	rtw_psk_set_psk_info(&(data->PSK_INFO));

	data->channel = data->fast_reconn_data.channel;
	data->key_id = data->channel >> 28;
	data->channel &= 0xff;
	data->security_type = data->fast_reconn_data.security_type;
...
WIFI_RETRY_LOOP:
	...
	//SSID
	data->wifi.ssid.len = strlen((char *)(data->fast_reconn_data.psk_essid));
	if (data->wifi.ssid.len > 32) {
		DiagPrintf("[FAST_CONNECT] SSID length can't exceed 32\n\r");
		rtos_mem_free((u8 *)data);
		return -1;
	}
	strncpy((char *)data->wifi.ssid.val, (char *)(data->fast_reconn_data.psk_essid), sizeof(data->wifi.ssid.val) - 1);

#ifdef CONFIG_WPS
	data->wifi.is_wps_trigger = data->fast_reconn_data.is_wps_trigger;
#endif

	switch (data->security_type) {
	case RTW_SECURITY_WEP_PSK:
	case RTW_SECURITY_WEP_SHARED:
		data->wifi.password = (unsigned char *)(data->fast_reconn_data.psk_passphrase);
		data->wifi.password_len = strlen((char *)(data->fast_reconn_data.psk_passphrase));
		data->wifi.key_id = data->key_id;
		break;
	case RTW_SECURITY_WPA_TKIP_PSK:
	case RTW_SECURITY_WPA2_TKIP_PSK:
	case RTW_SECURITY_WPA_WPA2_TKIP_PSK:
...
#if defined(CONFIG_FAST_DHCP) && CONFIG_FAST_DHCP
	offer_ip = data->fast_reconn_data.offer_ip;
	server_ip = data->fast_reconn_data.server_ip;
#endif

	ret = wifi_connect(&(data->wifi), 1);
	if (ret != RTK_SUCCESS) {
		data->wifi_retry_connect--;
		if (data->wifi_retry_connect > 0) {
