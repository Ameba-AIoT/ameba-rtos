...
/* IPC arch: the whole event (header + any variable-length attribute list) is
 * delivered in @buf of length @len by the WHC NAN event callback, so the
 * fixed-size events memcpy from @buf and the variable-length ones cast @buf
 * directly. No file polling / hex parsing / heap alloc is needed. */
void _evt_handler(int evt_id, void *buf, uint32_t len)
{
	if (buf == NULL || len == 0) {
		return;
	}
	...
	case RTW_VENDOR_EVT_NAN_DATA_COMFIRM: {
		...
		memcpy(&evt_data, buf, sizeof(evt_data) < len ? sizeof(evt_data) : len);
	...
	case RTW_VENDOR_EVT_NAN_DISC_RESULT_RECEIVED: {
		struct rtw_nan_discovery_result_received_event_data *p_evt_data =
			(struct rtw_nan_discovery_result_received_event_data *)buf;
	...
	case RTW_VENDOR_EVT_NAN_DATAPATH_REQ_RECEIVED: {
		struct rtw_nan_datapath_request_received_event_data *p_evt_data =
			(struct rtw_nan_datapath_request_received_event_data *)buf;

		_print_datapath_req_recv_evt(p_evt_data);

		if (g_nan_evt_cb) {
			g_nan_evt_cb(NAN_EVT_DATAPATH_RECV_REQ, (void *)p_evt_data, len);
		}
	}
	break;
	case RTW_VENDOR_EVT_NAN_DATAPATH_RSP_RECEIVED: {
		struct rtw_nan_datapath_response_received_event_data *p_evt_data =
			(struct rtw_nan_datapath_response_received_event_data *)buf;

		_print_datapath_rsp_recv_evt(p_evt_data);

		if (g_nan_evt_cb) {
			g_nan_evt_cb(NAN_EVT_DATAPATH_RECV_RSP, (void *)p_evt_data, len);
		}
	}
	break;
	...
	case RTW_VENDOR_EVT_NAN_FOLLOW_UP_RECEIVED: {
		struct rtw_nan_follow_up_received_event_data *p_evt_data =
			(struct rtw_nan_follow_up_received_event_data *)buf;

		_print_datapth_follow_up_recv_evt(p_evt_data);

		if (g_nan_evt_cb) {
			g_nan_evt_cb(NAN_EVT_DATAPATH_RECV_FOLLOW_UP, (void *)p_evt_data, len);
		}
	}
	break;
...
/* WHC NAN event callback (registered via whc_host_nan_register_evt_cb).
 * Runs on the WHC host event task; @buf holds the binary event from the NP
 * core, already copied into a host-owned buffer. */
static void nan_ipc_evt_cb(uint16_t event_id, void *buf, uint32_t len)
{
	_evt_handler((int)event_id, buf, len);
}

int rtw_nan_evt_init()
{
	INFO_PRINT("[rtw_event] %s\n", __func__);
	/* IPC arch: NAN events arrive from the NP core via the WHC IPC path
	 * instead of polling an 'iw event' log file. */
	whc_host_nan_register_evt_cb(nan_ipc_evt_cb);
...
{
	whc_host_nan_register_evt_cb(NULL);
	g_nan_evt_cb = NULL;
