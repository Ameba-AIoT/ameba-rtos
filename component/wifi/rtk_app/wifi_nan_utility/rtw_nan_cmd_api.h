...
/* RTOS/firmware-independent copy: does NOT include the host "stack_api.h".
 * RTW_RET_STATUS comes from rtw_nan_struct.h; the logging macros below are
 * firmware-safe (printf, no stderr) so this port shares no OS-dependent host
 * headers. */
#include <stdio.h>
...
#if DEBUG_LEVEL > 1
#define INFO_PRINT(...)  do{ printf( __VA_ARGS__ ); } while(0)
