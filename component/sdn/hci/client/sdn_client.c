...
bool _rtk_bt_should_disable_shared_power(void)
{
#if defined(CONFIG_WLAN) && CONFIG_WLAN
	return (!(wifi_is_running(STA_WLAN_INDEX) || wifi_is_running(SOFTAP_WLAN_INDEX)));
#else
	/* WLAN may be not built in or running on another core.
	 * Since this is wifi SoC, it is unlikely that WLAN is not built in.
	 * So keep shared power on just in case WLAN is running on another core. */
	return false;
#endif
}
...
#if BT_LL_FEATURE_C2H_FLOWCTRL
extern struct c2h_flowctrl gbt_c2h_flowctrl;
void bt_hci_c2h_flowctrl_host_completed(uint8_t count)
{
	rtos_critical_enter(RTOS_CRITICAL_BT);
	if (gbt_c2h_flowctrl.enable) {
		gbt_c2h_flowctrl.acl_avail += count;
	}
	rtos_critical_exit(RTOS_CRITICAL_BT);
...
#if BT_LL_FEATURE_C2H_FLOWCTRL
			bool acl_skip = false;
			list_for_each_entry(pdata_buf, &g_sdn_client_intf.tx.busy_list, list, struct sdn_data_buf) {
				if (pdata_buf->msg_type == SDN_MSG(SDN_INTF_BT, BT_HCI_H4_ACL) && (acl_skip || !bt_hci_c2h_flowctrl_acl_avail())) {
					acl_skip = true;
					continue;
				}
				/* ToDo sco check */
			...
			if (pdata_buf->protocol == SDN_INTF_COEX) {
...
#endif
				{
					...
					if (sdn_uart_is_on()) {
						...
						sdn_client_tx_buf_complete(pdata_buf); /* bridge tx is local/synchronous, return buf directly */
					} else
#endif
					{
						sdn_c2h(pdata_buf);
...
#endif
#if defined(CONFIG_WPAN_DRIVER_VHDLC_PLATFORM) && CONFIG_WPAN_DRIVER_VHDLC_PLATFORM
	if (g_sdn_client_intf.tx.wpan_pool) {
		rtos_mem_free(g_sdn_client_intf.tx.wpan_pool);
		g_sdn_client_intf.tx.wpan_pool = NULL;
	}
#endif
}
...
#endif
#if defined(CONFIG_WPAN_DRIVER_VHDLC_PLATFORM) && CONFIG_WPAN_DRIVER_VHDLC_PLATFORM
	g_sdn_client_intf.tx.wpan_pool = _alloc_data_list(&g_sdn_client_intf.tx.wpan_list, SDN_INTF_MAX_154_LEN,
