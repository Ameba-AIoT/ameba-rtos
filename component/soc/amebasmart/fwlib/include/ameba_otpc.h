...
#define OTPC_MASK_CRC_MAGIC      ((u32)0x0000FFFF << 0)            /*!< R 0x0  magic data read from valid CRC entry*/
...
/** @defgroup OTPC_OTP_PARAM
  * @brief
  * @{
  */
#define OTPC_BIT_UDZ1_RD_PROT ((u32)0x00000001 << 7) /*!< R/WH 0x0  otp physical 0x380-=0x3FF read protect 0: can be read 1: can't be read Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
#define OTPC_BIT_UDZ1_WR_PROT ((u32)0x00000001 << 6) /*!< R/WH 0x0  otp physical 0x380-=0x3FF write protect 0: can be write 1: can't be write Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
#define OTPC_BIT_UDZ0_RD_PROT ((u32)0x00000001 << 5) /*!< R/WH 0x0  otp physical 0x400-=0x4FF read protect 0: can be read 1: can't be read Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
#define OTPC_BIT_UDZ0_WR_PROT ((u32)0x00000001 << 4) /*!< R/WH 0x0  otp physical 0x400-=0x4FF write protect 0: can be write 1: can't be write Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
#define OTPC_BIT_PCR_PROT_K3  ((u32)0x00000001 << 3) /*!< R/WH 0x0  Retention Mem last forth 256 bit protect 0: read and write ,  1 : read only Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
#define OTPC_BIT_PCR_PROT_K2  ((u32)0x00000001 << 2) /*!< R/WH 0x0  Retention Mem last third 256 bit protect 0: read and write ,  1 : read only Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
#define OTPC_BIT_PCR_PROT_K1  ((u32)0x00000001 << 1) /*!< R/WH 0x0  Retention Mem last second 256 bit protect 0: read and write ,  1 : read only Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
#define OTPC_BIT_PCR_PROT_K0  ((u32)0x00000001 << 0) /*!< R/WH 0x0  Retention Mem last 256 bit protect (located in last 256bit of retention memory) 0: read and write ,  1 : read only Note:After this bit write to 1, it can't be write back to 0 , unless system reset*/
/** @} */
...
/** @defgroup OTPC_MTP_definitions
  * @{
  */
#define OTP_MTP_USER_START						0x170 /*!< logical map MTP user start address*/
#define OTP_MTP_USER_END							0x19F /*!< logical map MTP user end address*/
#define OTP_MTP_USER_LEN							(OTP_MTP_USER_END - OTP_MTP_USER_START+1) /*!< logical map MTP user section len in byte 0x170~0x19F*/
/** @} */
...
#define OTP_REGPATCH_BASE							0x42008000
#define OTP_REGPATCH_END							(OTP_REGPATCH_BASE + 0x9FF)
#define OTP_IS_REGPATCH(x)							((x >= OTP_REGPATCH_BASE) && (x < OTP_REGPATCH_END))

#define OTP_PADPATCH_BASE							0x42008A00
