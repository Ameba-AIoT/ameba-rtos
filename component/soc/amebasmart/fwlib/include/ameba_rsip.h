 ...
 * @{
 **/
#define RSIP_BIT_LOCK_STATUS          ((u32)0x00000001 << 8)          /*!<R 0x0  Indicate whether the setting is locked. If it is locked, only Clr_intr and Maintenance registers can be set. */
...
#define RSIP_BIT_INTR_ERR_APB         ((u32)0x00000001 << 4)          /*!<R 0x0  Indicate whether the setting configuration failed. When a transaction is in progress, all settings except clear interruption are invalid. */
#define RSIP_BIT_INTR_ERR_RESP        ((u32)0x00000001 << 3)          /*!<R 0x0  Indicate the error response is received from slave. */
#define RSIP_BIT_INTR_ERR_TAG         ((u32)0x00000001 << 2)          /*!<R 0x0  Indicate a GCM tag mismatch error occurred. */
#define RSIP_BIT_KEY_INIT_DONE        ((u32)0x00000001 << 1)          /*!<R 0x0  Indicate whether the initialization of the key is completed. Only used in GCM and MIX mode. */
 ...
 * @{
 **/
#define RSIP_MASK_LINE_SIZE           ((u32)0x00000007 << 12)          /*!<R -  Cache line size 0: 16B = 1 AES block 1: 32B = 2 AES blocks 2: 64B = 4 AES blocks 3: 128B = 8 AES blocks Indicate how many 128-bit blocks are used to calculate the tag cache line size (byte) = 16 * 2BLOCK_SIZE */
#define RSIP_LINE_SIZE(x)             (((u32)((x) & 0x00000007) << 12))
#define RSIP_GET_LINE_SIZE(x)         ((u32)(((x >> 12) & 0x00000007)))
#define RSIP_BIT_KEY_SOURCE           ((u32)0x00000001 << 11)          /*!<R -  Indicate the key source 0: from dedicated key port (only support this configuration currently) 1: from APB register */
#define RSIP_BIT_SUPPORT_GCM          ((u32)0x00000001 << 10)          /*!<R -  Indicate whether GCM mode is supported. */
#define RSIP_BIT_SUPPORT_MIX          ((u32)0x00000001 << 9)          /*!<R -  Indicate whether MIX mode is supported. */
#define RSIP_BIT_SUPPORT_CTR          ((u32)0x00000001 << 8)          /*!<R -  Indicate whether CTR mode is supported. */
...
#define RSIP_BIT_GCM_KEY_INIT         ((u32)0x00000001 << 0)          /*!<W 0x0  Only used in GCM mode. Whenever the key is changed, write 1 to initialize the GMAC key. When keys are updated by external port, SEC engine will trigger this bit automatically. The status of completion can be determined by reading bit 1 of the status register. */
/** @} */
 ...
 * @{
 **/
#define RSIP_MASK_LOCK                ((u32)0x000000FF << 0)          /*!<W 0x0  When setting 0xDC to this field, all registers except Clr_intr and Maintenance are not allowed to be written. */
