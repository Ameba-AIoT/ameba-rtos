...
void xlat_mmap_print(__attribute__((__unused__)) const mmap_region_t *mmap)
{
	/* Empty */
}

void xlat_tables_print(__attribute__((__unused__)) xlat_ctx_t *ctx)
{
	/* Empty */
}
...
void xlat_mmap_print(const mmap_region_t *mmap)
{
	...
	while (mm->size != 0U) {
		printf(" VA:0x%lx  PA:0x%llx  size:0x%x  attr:0x%x  granularity:0x%x\n",
	...
	while (table_idx < table_entries) {
		...
		if ((desc & DESC_MASK) == INVALID_DESC) {
			...
			if (((desc & DESC_MASK) == TABLE_DESC) &&
				...
				printf("%sVA:0x%lx PA:0x%llx size:0x%x ",
