...
void AUDIO_CODEC_SetMicBstChnMute(u32 amic_sel, u32 type, u32 newstate)
{
	...
	if (newstate == UNMUTE) {
		if (amic_sel == AMIC1) {
			...
			if (type == LINEIN) {
				RTK_LOGS(TAG, RTK_LOG_ERROR, "AMIC3 NOT SUPPORT LINEIN MODE\n");
				return;
			}
		} else if (amic_sel == AMIC4) {
			AUD->AUD_MICBST_CTL1 &= ~AUD_MASK_MICBST2_MUTE_R;
			if (type == LINEIN) {
				RTK_LOGS(TAG, RTK_LOG_ERROR, "AMIC4 NOT SUPPORT LINEIN MODE\n");
				return;
			}
		} else {
			AUD->AUD_MICBST_CTL1 &= ~ AUD_MASK_MICBST3_MUTE;
			if (type == LINEIN) {
				RTK_LOGS(TAG, RTK_LOG_ERROR, "AMIC5 NOT SUPPORT LINEIN MODE\n");
				return;
			}
		}
	} else {
...
/**
  ...
  *			   @arg MUTE: mute
  */
void AUDIO_CODEC_SetSideToneMute(u32 channel, u32 newstate)
{
	assert_param(IS_CODEC_ST_SEL(channel));
	assert_param(IS_CODEC_MUTE_CTR(newstate));
	AUDIO_TypeDef *audio_base = AUDIO_CODEC_GetAddr();
	...
	if (channel == ST_L) {
		if (newstate == UNMUTE) {
			audio_base->CODEC_DAC_L_CONTROL_1 &= ~AUD_BIT_DAC_L_DMIX_MUTE_SIDETONE;
		} else {
			audio_base->CODEC_DAC_L_CONTROL_1 |= AUD_BIT_DAC_L_DMIX_MUTE_SIDETONE;
		}
	} else {
		if (newstate == UNMUTE) {
			audio_base->CODEC_DAC_R_CONTROL_1 &= ~AUD_BIT_DAC_R_DMIX_MUTE_SIDETONE;
		} else {
			audio_base->CODEC_DAC_R_CONTROL_1 |= AUD_BIT_DAC_R_DMIX_MUTE_SIDETONE;
		}
	}
