...
void DDR_PHY_ZQ_SET_CAL(u32 set_num, u32 NOCD_ZPROG_val)
{
	...
	RTK_LOGI(TAG,  "!!!DDRPHY_PAD_ZCTRL_STATUS = 0x%08lX\n", ddr_phy->DDRPHY_PAD_ZCTRL_STATUS);
...
{
	DDRPHY_TypeDef *ddr_phy = DDRPHY_DEV;
	RTK_LOGI(TAG,  "DDRPHY_PAD_RZCTRL_STATUS = 0x%08lX\n", ddr_phy->DDRPHY_PAD_RZCTRL_STATUS);
	RTK_LOGI(TAG,  "DDRPHY_PAD_ZCTRL_STATUS = 0x%08lX\n", ddr_phy->DDRPHY_PAD_ZCTRL_STATUS);
	RTK_LOGI(TAG,  "DDRPHY_PAD_ZCTRL_RESULT = 0x%08lX\n", ddr_phy->DDRPHY_PAD_ZCTRL_RESULT);
...
void bstc_other_sram_fill(u32 sram_base_addr)
{
	...
	for (loop_cnt = 0; loop_cnt < data_entry; loop_cnt++) {
		...
		SCAN_PRINT("> BSTC_FILL_ADDR = 0x%08lX, BSTC_WDSRAM_DATA = 0x%lx%lx%lx%lx\n",
...
void DDR_PHY_BSTC_STARK(void)
{
	...
	RTK_LOGI(TAG,  "CR_BEXPC %p = 0x%08lX\n", &(ddrc->DDRC_BEXPC), ddrc->DDRC_BEXPC);
	...
	for (loop_cnt = 0; loop_cnt < BSTC_CMD_NUM; loop_cnt++) {
		DiagPrintf("Read CMD 0x%08lX = 0x%08lX\n", bstc_addr, HAL_READ32(bstc_addr, 0));
	...
	for (loop_cnt = 0; loop_cnt < data_entry; loop_cnt++) {
		...
		DiagPrintf("> BSTC_WDSRAM_ADDR = 0x%08lX, BSTC_WDSRAM_DATA = 0x%lx%lx%lx%lx\n",
				   bstc_addr, bstc_data->data_3, bstc_data->data_2, bstc_data->data_1, bstc_data->data_0);
		bstc_addr += 0x10; // push 128-bit
	}
#endif
	...
	for (loop_cnt = 0; loop_cnt < data_entry; loop_cnt = loop_cnt + 1) {
		...
		bstc_rd_128(bstc_addr, bstc_data);
		DiagPrintf("> BSTC_WMSKSRAM_ADDR = 0x%08lX, BSTC_WMSKSRAM_DATA = 0x%lx%lx%lx%lx\n",
				   bstc_addr, bstc_data->data_3, bstc_data->data_2, bstc_data->data_1, bstc_data->data_0);
		bstc_addr += 0x10; // push 128-bit
	}
#endif
	...
	for (loop_cnt = 0; loop_cnt < data_entry; loop_cnt++) {
		...
		DiagPrintf("> BSTC_RGSRAM_ADDR = 0x%08lX, BSTC_RGSRAM_DATA = 0x%lx%lx%lx%lx\n",
				   bstc_addr, bstc_data->data_3, bstc_data->data_2, bstc_data->data_1, bstc_data->data_0);
		bstc_addr += 0x10; // push 128-bit
	}
#endif
	...
	if (bsram0_cmd_lvl != BSTC_CMD_EX_LVL) {
		RTK_LOGE(TAG, "[ERR] BSTC CMD sram filled error! Expect cmd lvl = 0x%08lX, Actual cmd lvl= 0x%08lX\n", BSTC_CMD_EX_LVL, bsram0_cmd_lvl);
	} else {
		RTK_LOGI(TAG,  "[INFO] BSTC CMD sram valid data filled correct! cmd lvl = 0x%08lX\n", bsram0_cmd_lvl);
	}

	if (bsram0_wd_lvl != BSTC_WD_EX_LVL) {
		RTK_LOGE(TAG, "[ERR] BSTC WD sram filled error! Expect wd lvl = 0x%08lX, Actual wd lvl= 0x%08lX\n", BSTC_WD_EX_LVL, bsram0_wd_lvl);
	} else {
		RTK_LOGI(TAG,  "[INFO] BSTC WR sram valid data filled correct! WD lvl = 0x%08lX\n", bsram0_wd_lvl);
	}

	if (bsram1_rg_lvl != BSTC_WD_EX_LVL) {
		RTK_LOGE(TAG, "[ERR] BSTC RG sram filled error! Expect rg lvl= 0x%08lX, Actual rg lvl= 0x%08lX\n", BSTC_WD_EX_LVL, bsram1_rg_lvl);
	} else {
		RTK_LOGI(TAG,  "[INFO] BSTC RG sram valid data filled correct! RG lvl = 0x%08lX\n", bsram1_rg_lvl);
	}
	...
	for (loop_cnt = 0; loop_cnt < data_entry; loop_cnt++) {
		bstc_rd_128(bstc_addr, bstc_data);
		RTK_LOGI(TAG,  "> BSTC_RDSRAM_ADDR = 0x%08lX, BSTC_RDSRAM_DATA = 0x%08lX%lx%lx%lx\n", bstc_addr, bstc_data->data_3, bstc_data->data_2,
	...
	RTK_LOGI(TAG,  "BSTC ERROR BIT: 0x%08lX\n", DDRC_GET_ERR_BIT(ddrc->DDRC_BER));
...
void sram_sg(void)
{
	...
	SCAN_PRINT("CR_CCR %p = 0x%08lX\n", &(ddrc->DDRC_CCR), ddrc->DDRC_CCR);

	// Clean err_bit
	ddrc->DDRC_BCR = DDRC_BIT_CRR;
	SCAN_PRINT("CR_BCR %p = 0x%08lX\n", &(ddrc->DDRC_BCR), ddrc->DDRC_BCR);

	// Set BEXPC when single mode or reload mode
	// (CR_LOOP=0) or (CR_LOOP=1 & CR_RELOAD=1)
	ddrc->DDRC_BEXPC = (DDRC_RD_EXPC_CNT(BSTC_RD_EX_LVL) |	DDRC_WD_EXPC_CNT(BSTC_WD_EX_LVL) | DDRC_CMD_EXPC_CNT(BSTC_CMD_EX_LVL));
	SCAN_PRINT("CR_BRXPC %p = 0x%08lX\n", &(ddrc->DDRC_BEXPC), ddrc->DDRC_BEXPC);
	...
	if (err_cnt != 0 || DDRC_GET_ERR_BIT(ddrc->DDRC_BER)) {
		RTK_LOGE(TAG, "[ERR] BSTC error, err_cnt = %lu\n", err_cnt);
		RTK_LOGE(TAG, "BSTC ERROR BIT: 0x%08lX\n", DDRC_GET_ERR_BIT(ddrc->DDRC_BER));
	} else {
...
void tx_MR_wrlvl(void)
{
	...
	for (dqs_pi = 0; dqs_pi < 32; dqs_pi++) {
		...
		//RTK_LOGI(TAG,  "CK0 PI = 0x%08lX\n", DDRPHY_GET_DPI_POST_PI_SEL0(ddr_phy->DDRPHY_PLL_PI0));
		//RTK_LOGI(TAG,  "DQS0 PI = 0x%08lX\n", DDRPHY_GET_DPI_POST_PI_SEL2(ddr_phy->DDRPHY_PLL_PI0));
		//RTK_LOGI(TAG,  "DQS1 PI = 0x%08lX\n", DDRPHY_GET_DPI_POST_PI_SEL3(ddr_phy->DDRPHY_PLL_PI0));
		//RTK_LOGI(TAG,  "DQS2 PI = 0x%08lX\n", DDRPHY_GET_DPI_POST_PI_SEL4(ddr_phy->DDRPHY_PLL_PI1));
		//RTK_LOGI(TAG,  "DQS3 PI = 0x%08lX\n", DDRPHY_GET_DPI_POST_PI_SEL5(ddr_phy->DDRPHY_PLL_PI1));
		RTK_LOGI(TAG,  "DQSPI=%02d, DQ0~31 = 0x%08lX\n", dqs_pi, ddr_phy->DDRPHY_DQ_PAT_IN0); // dq_pat_in
...
#define SCAN_PER_SLICE 0
	...
	for (dqsen = dqsen_init[0] ; dqsen < 12; dqsen++) {
		...
		//printf("~~~~~ dqs_int ~~~~~~~\n");
		DiagPrintf("dqs_int: 0x%08lX, ddr_error_bit: 0x%08lX\n", ddr_phy->DDRPHY_INT_STATUS_0, ddr_error_bit);
	...
	//This is result
	RTK_LOGI(TAG,  "DDRPHY_READ_CTRL_0_SETx[0]: 0x%08lX\n", ddr_phy->DDRPHY_READ_CTRL_0_SETx[0]);
	RTK_LOGI(TAG,  "DDRPHY_READ_CTRL_0_SETx[1]: 0x%08lX\n", ddr_phy->DDRPHY_READ_CTRL_0_SETx[1]);
...
void rx_delaytap_write(int Ref_Te[])
{
	...
	//This is result
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_0_SETx[0] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_0_SETx[0]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_1_SETx[0] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_1_SETx[0]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_0_SETx[1] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_0_SETx[1]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_1_SETx[1] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_1_SETx[1]);

	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_2_SETx[0] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_2_SETx[0]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_3_SETx[0] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_3_SETx[0]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_2_SETx[1] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_2_SETx[1]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_3_SETx[1] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_3_SETx[1]);

	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_1_DBI_SETx[0] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_1_DBI_SETx[0]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_1_DBI_SETx[1] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_1_DBI_SETx[1]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_3_DBI_SETx[0] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_3_DBI_SETx[0]);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_DQS_IN_DLY_3_DBI_SETx[1] = 0x%08lX;\n", ddr_phy->DDRPHY_DQS_IN_DLY_3_DBI_SETx[1]);
...
void scan_rxfifo(void)
{
	...
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_READ_CTRL_1 = 0x%08lX;\n", ddr_phy->DDRPHY_READ_CTRL_1);
...
void DDR_PHY_Scan_Result_Set(void)
{
	...
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_PLL_PI0 = 0x%08lX;\n", ddr_phy->DDRPHY_PLL_PI0);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_PLL_PI1 = 0x%08lX;\n", ddr_phy->DDRPHY_PLL_PI1);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_PLL_PI2 = 0x%08lX;\n", ddr_phy->DDRPHY_PLL_PI2);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_PLL_PI3 = 0x%08lX;\n", ddr_phy->DDRPHY_PLL_PI3);

	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_PLL_CTL1 = 0x%08lX;\n", ddr_phy->DDRPHY_PLL_CTL1);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_AFIFO_STR_0 = 0x%08lX;\n", ddr_phy->DDRPHY_AFIFO_STR_0);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_AFIFO_STR_1 = 0x%08lX;\n", ddr_phy->DDRPHY_AFIFO_STR_1);
	RTK_LOGI(TAG,  "ddr_phy->DDRPHY_AFIFO_STR_SEL = 0x%08lX;\n", ddr_phy->DDRPHY_AFIFO_STR_SEL);
}
