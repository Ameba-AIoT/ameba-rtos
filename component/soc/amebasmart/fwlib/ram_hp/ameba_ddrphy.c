...
/*
static const unsigned short pll_sel_table[7][13] = {
...
static const DDRPHY_Tx_Scan_Def ddrphy_tx_scan[] = {
	...
	{0x0f0f001b,	0x00180f0f,	0x13181818,	0x00000013,	0x20000fc1,	0x22222222,		0x000c0012,		0x34000000},//QFN100_DDR2(0x07/0x11)
...
void DDR_PHY_DLL_CLK_DIV(u32 DDR_Freq)
{
	...
	//RTK_LOGD(TAG, "DDRPHY DLL Div %lu %lu \n", DDRPHY_Div_n, DDRPHY_Div_f);
...
#if DDR_AUTOGATING
	//Bit-0 is used to select clock source of mck_mc in AmebaD2. 1'b0: select mck_cmd 1'b1: select PLL internal clock
	...
	//DDR_PHY_PLL_SEL(&(pll_sel_table[DDR_Freq_idx][4]));
...
void DDR_PHY_WRITE_LEVELING_CAL(void)
{
	...
	for (temp = 0; temp < 32; temp++) {
		...
		if (ddr_phy->DDRPHY_DQ_PAT_IN0) {
			DiagPrintf("WRITE_LEVELING_CAL dqs_pi = %lu DQ_PAT_IN0 = %lx\n", temp, ddr_phy->DDRPHY_DQ_PAT_IN0);
			break;
		}
	}
...
void DDR_PHY_LoopBack(void)
{
	...
	RTK_LOGI(TAG, "BIST_2TO1_1 is 0x%lx\n", ddr_phy->DDRPHY_BIST_2TO1_1);
}
#endif
