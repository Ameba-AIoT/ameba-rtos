...
u32 OSC_CalResult_Get(u8 cal_clk)
{
	...
	/* start calibration */
	temp = HAL_READ32(SYSTEM_CTRL_BASE_LP, REG_LSYS_AIP_CTRL0);
	temp &= ~LSYS_MASK_CK_CAL_SEL;
	temp |= (LSYS_CK_CAL_SEL(cal_clk) | LSYS_BIT_CK_CAL_START);
	HAL_WRITE32(SYSTEM_CTRL_BASE_LP, REG_LSYS_AIP_CTRL0, temp);
...
/**
  * @brief  XTAL Init
  * @note This is the special setting of a cut. Special settings are required because the XTAL default parameter is not optimal.
  */
void XTAL_INIT(void)
{
	...
	xtal->XTAL_ANAPAR_XTAL_ON_2 = 0x000029F5; /*0ch*/
	...
	xtal->XTAL_ANAPAR_XTAL_MODE_DEC_ON_0 = 0x988365E0; /*2ch*/
...
void CLK_SWITCH_XTAL(u32 State)
{
	...
	if (State == TRUE) {
		...
		Temp &= ~LSYS_BIT_CKSL_HBUS;		//AHB
		Temp &= ~LSYS_BIT_CKSL_NP;
	} else {
		// select divn_clk from pll
		Temp |= LSYS_BIT_CKSL_PSRAM;
		Temp |= LSYS_BIT_CKSL_HBUS;
		Temp |= LSYS_BIT_CKSL_NP;		//AHB
	}
