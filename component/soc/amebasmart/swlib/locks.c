...
void __retarget_lock_init(_LOCK_T *lock_ptr)
{
	if (*lock_ptr) {
		...
		RTK_LOGS(TAG, "%s, lock_ptr %p is already initialized!!!\n", __func__, *lock_ptr);
		return;
	}
...
void __retarget_lock_acquire(_LOCK_T lock)
{
	...
	if (rtos_critical_is_in_interrupt()) {
		...
		if (ret != pdTRUE) {
			RTK_LOGS(TAG, "%s, lock %p acquire from isr failed!!!\n", __func__, lock);
			return;
		}
		...
		if (ret != pdTRUE) {
			RTK_LOGS(TAG, "%s, lock %p acquire failed!!!\n", __func__, lock);
		}
	}
}
...
void __retarget_lock_release(_LOCK_T lock)
{
	...
	if (rtos_critical_is_in_interrupt()) {
		...
		if (ret != pdTRUE) {
			RTK_LOGS(TAG, "%s, lock %p release from isr failed!!!\n", __func__, lock);
			return;
		}
		portEND_SWITCHING_ISR(task_woken);
	} else {
		...
		if (ret != pdTRUE) {
			RTK_LOGS(TAG, "%s, lock %p release failed!!!\n", __func__, lock);
		}
	}
}
