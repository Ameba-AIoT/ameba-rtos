...
void NAND_TxData(u8 cmd, u32 StartAddr, u32 ByteLen, u8 *pData)
{
	...
	/* Set ADDR length */
	value = spi_flash->USER_LENGTH & ~MASK_USER_ADDR_LENGTH;
	spi_flash->USER_LENGTH = value | USER_ADDR_LENGTH(2);
...
u8 NAND_Page_Read(u32 PageAddr, u32 ByteAddr, u32 ByteLen, u8 *pData)
{
	...
	NAND_Page_Read_FromCache(flash_init_para.FLASH_cur_cmd, ByteAddr, ByteLen, pData);
...
/**
  ...
  * 	  if you want to update paitial area of nand cache please call NAND_Page_Write_Data_Xfer() with
  *			Program load ramdom data command and NAND_Page_Write_Program_Execute
  */
__WEAK HAL_ROM_TEXT_SECTION
u8 NAND_Page_Write(u32 PageAddr, u32 ByteAddr, u32 ByteLen, u8 *pData)
{
	...
	if (flash_init_para.FLASH_cur_bitmode >= SpicQuadOBitMode) {
		NAND_Page_Write_Data_Xfer(NAND_CMD_QPP, ByteAddr, ByteLen, pData);
	} else {
		NAND_Page_Write_Data_Xfer(NAND_CMD_PP, ByteAddr, ByteLen, pData);
	}
