...
/**
  ...
  * @param  IrqNum: TIMx IRQ num.This parameter can be TIMERx_IRQ
  *         where x can be 0-9 corresponding to TIMx
  ...
  */
__weak HAL_ROM_TEXT_SECTION
void RTIM_TimeBaseInit(RTIM_TypeDef *TIMx, RTIM_TimeBaseInitTypeDef *TIM_InitStruct, IRQn_Type IrqNum, IRQ_FUN UserCB,
					   u32 UserCBData)
{
	u32 tmpCR = 0;
	u32 Status;
	...
	if (TIM_InitStruct->TIM_SecureTimer) {
		...
		TIMx->RSVD0 |= TIM_BIT_SEC;
	}

	/* Reset Timer before init */
	/* 1. disable timer CNT */
	/* 2. close and clear all interrupt, or err interrupt will hanppen */
	RTIM_Cmd(TIMx, DISABLE);

	TIMx->DIER = 0x00;
	RTIM_INTClear(TIMx);

	/* Set the Autoreload value */
	if (IS_TIM_40M_TIM(TIMx)) {
		TIMx->ARR = TIM_CNT(TIM_InitStruct->TIM_Period);
	} else {
		TIMx->ARR = TIMBasic_CNT(TIM_InitStruct->TIM_Period);
	}

	/* Set the Prescaler value */
	if (IS_TIM_CCM_TIM(TIMx)) {
...
/**
  ...
  * @param  TIMx: where x can be 0-9 to select the TIMx peripheral.
...
void RTIM_Cmd(RTIM_TypeDef *TIMx, u32 NewState)
{
	...
	if (NewState != DISABLE) {
		...
		if ((TIMx->EN & TIM_BIT_CEN) == 0) {
			TIMx->EN = TIM_BIT_CNT_START;
		}

		/* poll if cnt is running, 3*32k cycles */
		while (1) {
			if (TIMx->EN & TIM_BIT_CEN) {
				break;
			}
		}
	} else {
...
u32 RTIM_GetCount(RTIM_TypeDef *TIMx)
{
	...
	TimerRDCnt = 0;
	TimerCountOld = TIMx->CNT;
	while (1) {
		TimerCountNew = TIMx->CNT;
...
/**
  * @brief  ENABLE/DISABLE  the TIMx's interrupt bits.
  * @param  TIMx: where x can be 0-9 to select the TIM peripheral.
...
/**
  * @brief  Clears the TIMx's all interrupt pending bits.
  * @param  TIMx: where x can be 0-9 to select the TIM peripheral.
  * @retval None
  */
__weak HAL_ROM_TEXT_SECTION
