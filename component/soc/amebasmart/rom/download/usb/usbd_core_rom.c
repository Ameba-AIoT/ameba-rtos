...
static void USBD_GetDescriptor(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
	...
	case USB_DESC_TYPE_DEVICE:
		//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor USB_DESC_TYPE_DEVICE\n");
		pbuf = pdev->pClass->GetDeviceDescriptor(pdev->dev_speed, &len);
		break;

	case USB_DESC_TYPE_CONFIGURATION:
		if (pdev->dev_speed == USBD_SPEED_HIGH) {
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor HS USB_DESC_TYPE_CONFIGURATION\n");
			pbuf = (u8 *)pdev->pClass->GetHSConfigDescriptor(&len);
		} else {
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor FS USB_DESC_TYPE_CONFIGURATION\n");
			pbuf = (u8 *)pdev->pClass->GetFSConfigDescriptor(&len);
		}
		break;
	...
	case USB_DESC_TYPE_STRING:
		...
		case USBD_IDX_LANGID_STR:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor USBD_IDX_LANGID_STR\n");
			pbuf = pdev->pClass->GetLangIDStrDescriptor(pdev->dev_speed, &len);
			break;

		case USBD_IDX_MFC_STR:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor USBD_IDX_MFC_STR\n");
			pbuf = pdev->pClass->GetManufacturerStrDescriptor(pdev->dev_speed, &len);
			break;

		case USBD_IDX_PRODUCT_STR:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor USBD_IDX_PRODUCT_STR\n");
			pbuf = pdev->pClass->GetProductStrDescriptor(pdev->dev_speed, &len);
			break;

		case USBD_IDX_SERIAL_STR:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor USBD_IDX_SERIAL_STR\n");
			pbuf = pdev->pClass->GetSerialStrDescriptor(pdev->dev_speed, &len);
			break;

		default:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor %d USBD_CtlError\n", (u8)(req->wValue));
			//USBD_CtlError(pdev, req);  // This line will cause enumeration failure
			USBD_StallEP(pdev, 0x80U);
			return;
		}
		break;
	case USB_DESC_TYPE_DEVICE_QUALIFIER:
		//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor USB_DESC_TYPE_DEVICE_QUALIFIER\n");
		if (pdev->dev_speed == USBD_SPEED_HIGH) {
			pbuf = (u8 *)pdev->pClass->GetDeviceQualifierDescriptor(&len);
			break;
		} else {
	...
	case USB_DESC_TYPE_OTHER_SPEED_CONFIGURATION:
		//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_GetDescriptor USB_DESC_TYPE_OTHER_SPEED_CONFIGURATION\n");
		if (pdev->dev_speed == USBD_SPEED_HIGH) {
			pbuf   = (u8 *)pdev->pClass->GetOtherSpeedConfigDescriptor(&len);
			break;
		} else {
			USBD_CtlError(pdev, req);
			return;
	...
	if (USBD_ConfigIndex > USBD_MAX_NUM_CONFIGURATION) {
		...
		case USBD_STATE_ADDRESSED:
			if (USBD_ConfigIndex) {
				...
				if (USBD_SetClassConfig(pdev, USBD_ConfigIndex) != HAL_OK) {
					USBD_CtlError(pdev, req);
					return;
				}
				USBD_CtlSendStatus(pdev);
...
static HAL_Status USBD_StallEP(USBD_HandleTypeDef *pdev, u8 ep_addr)
{
	//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StallEP %d\n", ep_addr);
...
static HAL_Status  USBD_StdItfReq(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef  *req)
{
	...
	//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdItfReq: wIndex=0x%04X bmRequest=0x%02X bRequest=0x%02X wLength=0x%04X\n", req->wIndex, req->bmRequest, req->bRequest, req->wLength);
...
static HAL_Status USBD_StdDevReq(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef  *req)
{
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_GET_DESCRIPTOR:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq USB_REQ_GET_DESCRIPTOR\n");
			USBD_GetDescriptor(pdev, req);
			break;

		case USB_REQ_SET_ADDRESS:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq USB_REQ_SET_ADDRESS\n");
			USBD_SetAddress(pdev, req);
			break;

		case USB_REQ_SET_CONFIGURATION:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq USB_REQ_SET_CONFIGURATION\n");
			USBD_SetConfig(pdev, req);
			break;

		case USB_REQ_GET_CONFIGURATION:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq USB_REQ_GET_CONFIGURATION\n");
			USBD_GetConfig(pdev, req);
			break;

		case USB_REQ_GET_STATUS:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq USB_REQ_GET_STATUS\n");
			USBD_GetStatus(pdev, req);
			break;


		case USB_REQ_SET_FEATURE:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq USB_REQ_SET_FEATURE\n");
			USBD_SetFeature(pdev, req);
			break;

		case USB_REQ_CLEAR_FEATURE:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq USB_REQ_CLEAR_FEATURE\n");
			USBD_ClrFeature(pdev, req);
			break;

		default:
			//DBG_PRINTF(MODULE_USB_OTG, LEVEL_TRACE, "USBD_StdDevReq unexpected request: 0x%02X\n", req->bRequest);
			USBD_CtlError(pdev, req);
			break;
		}
		break;
...
HAL_Status USBD_Core_Init(USBD_CDC_ItfTypeDef *fops, USBD_SpeedTypeDef speed)
{
	...
	USBD_PCDHandle.Init.dev_endpoints = USBD_MAX_ENDPOINTS;
	...
	if (ret == HAL_OK) {
		...
		pdev->pClass = (USBD_ClassTypeDef *)&USBD_CDC;
...
HAL_Status USBD_SetupStage(USBD_HandleTypeDef *pdev, u8 *psetup)
{
	...
	default:
		DBG_PRINTF(MODULE_USB_OTG, LEVEL_WARN, "EP0 unexpected request: 0x%08X\n", pdev->request.bmRequest);
	...
	if (epnum == 0U) {
		...
		if (pdev->ep0_state == USBD_EP0_DATA_OUT) {
			if (pep->rem_length > pep->maxpacket) {
				...
				USBD_CtlContinueRx(pdev, pdata, (u16)MIN(pep->rem_length, pep->maxpacket));
			} else {
			...
			if (pdev->ep0_state == USBD_EP0_STATUS_OUT) {
				...
				//USBD_StallEP(pdev, 0U);  // FIXME: This line will cause enumeration failure
			}
		}
		...
		if (pdev->ep0_state == USBD_EP0_DATA_IN) {
			if (pep->rem_length > pep->maxpacket) {
				...
				if ((pep->total_length % pep->maxpacket == 0U) &&
					...
					//USBD_StallEP(pdev, 0x80U);  // FIXME: This line will cause enumeration failure
