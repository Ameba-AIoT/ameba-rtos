...
_WEAK void wifi_set_user_config(void)
{
...
#if (defined(CONFIG_WHC_INTF_SPDIO) || defined(CONFIG_WHC_INTF_USB))
	...
	wifi_user_config.skb_num_np = 21; /* skb_num_np should >= rx_ampdu_num + skb_num_np_rsvd */
	wifi_user_config.skb_num_ap = 20;
...
#ifndef CONFIG_EXTRA_ALLOC_RXBUF
	//RXBD reuse skb, skb_buf_size should be 256-bytes align, and need alloc more skb for rxbuf
	skb_num_np_rsvd += 12;
	wifi_user_config.skb_num_np += 12;
	wifi_user_config.skb_buf_size = (wifi_user_config.skb_buf_size + 255) & ~255;
#endif
