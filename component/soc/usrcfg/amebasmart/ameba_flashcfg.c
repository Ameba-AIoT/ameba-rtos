...
const FlashLayoutInfo_TypeDef Flash_Layout[] = {
	...
	{IMG_APP_OTA1,  CONFIG_FLASH_OTA1_OFFSET, CALC_END_ADDR(CONFIG_FLASH_OTA1_OFFSET, CONFIG_FLASH_OTA1_SIZE)}, //Certificate(4K) + Manifest(4K) + KM0 & KM4 & CA32 Application OTA1 + RDP IMG OTA1
	// + AP IMG OTA1
	...
	{IMG_APP_OTA2,  CONFIG_FLASH_OTA2_OFFSET, CALC_END_ADDR(CONFIG_FLASH_OTA2_OFFSET, CONFIG_FLASH_OTA2_SIZE)}, //Certificate(4K) + Manifest(4K) + KM0 & KM4 & CA32 Application OTA2 + RDP IMG OTA2
	// + AP IMG OTA2
...
*  In each entry, the first item is flash regoin type, the second item is start address, the second item is end address */
FlashLayoutInfo_TypeDef Flash_Layout_Nor_Linux[] = {
	/*Region_Type,		[StartAddr,	EndAddr]		*/
...
*  In each entry, the first item is flash regoin type, the second item is start address, the second item is end address */
FlashLayoutInfo_TypeDef Flash_Layout_Nand_Linux[] = {
	/*Region_Type,		[StartAddr,	EndAddr]		*/
	{IMG_BOOT, 			0x08000000, 0x0801FFFF}, //Boot Manifest(4K) + KM4 Bootloader(124K)
	//Users should modify below according to their own memory
	{IMG_APP_OTA1, 		0x08020000, 0x0813FFFF}, //Certificate(4K) + Manifest(4K) + KM0 & KM4 & CA32 Application OTA1 + RDP IMG OTA1
...
const FlashLayoutInfo_TypeDef *flash_get_layout_struct(void)
{
	const FlashLayoutInfo_TypeDef *pLayout;
...
{
	u32 i = 0;
	const FlashLayoutInfo_TypeDef *pLayout = flash_get_layout_struct();
	...
	RTK_LOGE(TAG, "flash_get_layout_info by type %d Fail\n", type);
}
...
void flash_get_layout_info_by_addr(u32 addr, u32 *start, u32 *end)
{
	...
	RTK_LOGE(TAG, "flash_get_layout_info by addr 0x%x Fail\n", addr);
}