...
static psa_status_t its_flash_fs_file_write_crypto(
                                      struct its_flash_fs_ctx_t *fs_ctx,
                                      const struct its_block_meta_t *block_meta,
                                      const struct its_file_meta_t *file_meta,
                                      size_t offset,
                                      ...
                                      const uint8_t *buf)
{
	/*Actually offset must be zero here for all requests shall be handled in one iteration */
	psa_status_t err;
	u8 Key[32];

	ALIGNMTO(CACHE_LINE_SIZE) u8 enc_tag[CACHE_LINE_ALIGMENT(GCM_TAG_SIZE)];
	ALIGNMTO(CACHE_LINE_SIZE) u8 enc_buf[CACHE_LINE_ALIGMENT(ITS_BUF_SIZE)];
	...
	/* Check that the new data is contained within the file's max size */
	if (its_utils_check_contained_in(file_meta->max_size, offset, size) != PSA_SUCCESS) {
		return PSA_ERROR_INVALID_ARGUMENT;
	}

	/* Only ITS protected by GCM */
	if (!fs_ctx_is_its(fs_ctx) || (size == 0)) {
		return its_flash_fs_file_write_aligned_data(fs_ctx, block_meta, file_meta, offset, size, buf);
	}
	...
	do {
		...
		/* treat file_meta->id as IV, DerivedKey_Bkup as aad */
		err = rtl_crypto_aes_gcm_encrypt(buf, size, file_meta->id,
										DerivedKey_Bkup, sizeof(DerivedKey_Bkup), enc_buf, enc_tag);
		if (err != PSA_SUCCESS) {
			break;
		}

		err = its_flash_fs_file_write_aligned_data(fs_ctx, block_meta, file_meta, offset, size, enc_buf);

		memcpy((void *)file_meta->hash, (const void *)enc_tag, GCM_TAG_SIZE);
	} while (0);
...
static psa_status_t its_flash_fs_file_read_crypto(
                                        struct its_flash_fs_ctx_t *fs_ctx,
                                        const struct its_file_meta_t *file_meta,
                                        size_t offset,
                                        size_t size,
                                        uint8_t *buf)
{
	...
	ALIGNMTO(CACHE_LINE_SIZE) u8 dec_buf[CACHE_LINE_ALIGMENT(ITS_BUF_SIZE)];

	/* Only ITS protected by GCM */
	if (!fs_ctx_is_its(fs_ctx) || (size == 0)) {
		return its_flash_fs_dblock_read_file(fs_ctx, file_meta, offset, size, buf);
	}
	...
	do {
		...
		err = its_flash_fs_dblock_read_file(fs_ctx, file_meta, 0, file_meta->max_size, dec_buf);
		if (err != PSA_SUCCESS) {
			break;
		}

		err = rtl_crypto_aes_gcm_decrypt(dec_buf, file_meta->max_size, file_meta->id,
										DerivedKey_Bkup,  sizeof(DerivedKey_Bkup), dec_buf, dec_tag);
		if (err != PSA_SUCCESS) {
			break;
		}

		err = memcmp(file_meta->hash, dec_tag, GCM_TAG_SIZE);
		if (err != PSA_SUCCESS) {
			break;
		}

		memcpy(buf, (const void *)&dec_buf[offset], size);
	} while (0);
                                     ...
                                     const uint8_t *data)
{
    ...
    if (data_size != 0) {
        ...
        err = its_flash_fs_file_write_crypto(fs_ctx, &block_meta,
                                                   &file_meta, offset,
                                                   data_size, data);
        if (err != PSA_SUCCESS) {
            return PSA_ERROR_GENERIC_ERROR;
                                    ...
                                    uint8_t *data)
{
    ...
    err = its_flash_fs_file_read_crypto(fs_ctx, &tmp_metadata, offset, size,
                                        data);
    if (err != PSA_SUCCESS) {
        return PSA_ERROR_GENERIC_ERROR;
    }
