                            ...
                            struct q_useful_buf_c *measurement_desc)
{
    ...
    case PSA_ALG_HMAC(PSA_ALG_SHA_256):
        measurement_desc->ptr = "hmac-256";
        measurement_desc->len = 8; /* Not including the null-terminator. */
        break;
    case PSA_ALG_HMAC(PSA_ALG_SHA_384):
        measurement_desc->ptr = "hmac-384";
        measurement_desc->len = 8; /* Not including the null-terminator. */
        break;
    case PSA_ALG_HMAC(PSA_ALG_SHA_512):
        measurement_desc->ptr = "hmac-512";
        measurement_desc->len = 8; /* Not including the null-terminator. */
        break;
    default:
        /* Algorithm not supported. */
...
#else
#ifndef VERSION_MAX_SIZE
#define VERSION_MAX_SIZE 4
#endif
#ifndef SIGNER_ID_MAX_SIZE
#define SIGNER_ID_MAX_SIZE 64
#endif
            Q_USEFUL_BUF_MAKE_STACK_UB(signer_id_buf, SIGNER_ID_MAX_SIZE);
            Q_USEFUL_BUF_MAKE_STACK_UB(sw_version_buf, VERSION_MAX_SIZE);
            signer_id_buf.len   = SIGNER_ID_MAX_SIZE;
            ...
            if (tfm_core_read_measurement(&measurement_algo, signer_id_buf.ptr, signer_id_buf.len, &signer_id_buf.len,
                                           sw_version_buf.ptr, sw_version_buf.len, &sw_version_buf.len)) {
                return PSA_ATTEST_ERR_GENERAL;
            }

            if (get_measurement_description(measurement_algo, &measurement_desc)) {
                return PSA_ATTEST_ERR_GENERAL;
            }
