...
/*Dplus do not support MMIO, and MPU_S is not enough*/
#ifndef AMEBADPLUS_TODO
#undef CONFIG_TFM_MMIO_REGION_ENABLE
...
REGION_DECLARE(Image$$, VENEER_ENTRY, $$Base);
REGION_DECLARE(Image$$, VENEER_ENTRY, $$Limit);
...
const struct mpu_armv8m_region_cfg_t region_cfg[] = {
	...
	/* Veneer region */
	{
		...
		(uint32_t) &REGION_NAME(Image$$, VENEER_ENTRY, $$Limit),
		MPU_ARMV8M_MAIR_ATTR_CODE_IDX,
		MPU_ARMV8M_XN_EXEC_OK,
		MPU_ARMV8M_AP_RO_PRIV_UNPRIV,
		MPU_ARMV8M_SH_NONE
	},
	/* TFM Core unprivileged code region & RO region*/
	{
		0, /* will be updated before using */
