...
#define ENTROPY_SEED_SIZE	(64u)
#define IMPLEMENTATION_ID_SIZE (8u)

//static u8 entropy_seed[64] = {0};
//static u8 boot_seed[32] = {0};
...
{
	u8 implementation_id[IMPLEMENTATION_ID_SIZE];

    switch (id) {
    case PLAT_OTP_ID_HUK:
        memcpy(out, DerivedKey_Bkup, out_len < sizeof(DerivedKey_Bkup) ? out_len : sizeof(DerivedKey_Bkup));
		return TFM_PLAT_ERR_SUCCESS;
    ...
    case PLAT_OTP_ID_BOOT_SEED:
		RandBytes_Get(out, out_len < BOOT_SEED_SIZE ? out_len : BOOT_SEED_SIZE);
		return TFM_PLAT_ERR_SUCCESS;
    ...
    case PLAT_OTP_ID_IMPLEMENTATION_ID:
		...
		memcpy(out, implementation_id, out_len < IMPLEMENTATION_ID_SIZE ? out_len : IMPLEMENTATION_ID_SIZE);
		return TFM_PLAT_ERR_SUCCESS;
#ifdef AMEBA_NONEED
    ...
    case PLAT_OTP_ID_ENTROPY_SEED:
		RandBytes_Get(out, out_len < ENTROPY_SEED_SIZE ? out_len : ENTROPY_SEED_SIZE);
		return TFM_PLAT_ERR_SUCCESS;
    default:
