...
void rtk_log_forward_init(rtk_log_output_fn fn)
{
	...
	if (RTK_SUCCESS != rtos_sema_create(&g_fwd.ring.data_sem, 0, LOG_FWD_RING_SIZE / (sizeof(u16) + 1))) {
		RTK_LOGE(NOTAG, "Create log_fwd data_sem Err\n");
		return;
	}
	...
	if (RTK_SUCCESS != rtos_task_create(NULL, "log_fwd_sender", (rtos_task_function_t)log_fwd_sender_task,
		...
		rtos_sema_delete(g_fwd.ring.data_sem);
		g_fwd.ring.data_sem = NULL;
		return;
	}
