...
typedef struct {
	...
	u32 event_capacity; //==payload_capacity - sizeof(offset):2 - sizeof(crc):1

	const RtkDiagEvent_t *last_event;
	u8 last_event_state;
	u32 last_event_offset; //上个event分片传输的偏移量
...
static int rtk_diag_proto_load_data_from_start(const RtkDiagEvent_t *event, u16 local_offset)
{
...
#endif
	u32 event_size = RTK_DIAG_EVENT_STRUCTURE_REAL_SIZE(event);
	u32 total_event_size = 0;
	if (event_size > g_handler->event_capacity) {
		...
		while (event) {
			...
			RTK_LOGA(NOTAG, "load event: %p, %u, %s, %u\n", g_handler->data_frame->payload + 2 + total_event_size, event_size, event->evt_info, event->evt_len);
