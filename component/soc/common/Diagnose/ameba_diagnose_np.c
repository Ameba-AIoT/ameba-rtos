...
int rtk_diag_init(u16 heap_capacity, u16 sender_buffer_size)
{
	...
	void rtk_diag_create_task(void);
	rtk_diag_create_task();
#endif
	if (g_initialized == 1) {
...
static int rtk_diag_req_add_event(u32 timestamp, u8 evt_level, u16 evt_type, const u8 *evt_info, u16 evt_len)
{
	// RTK_LOGA(NOTAG, "Diagnose event add: %u, %u, %s\n", evt_level, evt_type, (const char*)evt_info);
