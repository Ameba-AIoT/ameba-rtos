...
/* group: Static variables in the group should not be extracted to the doxygen doc */
static uint16_t i2c_target_addr[2];
static uint32_t restart_enable = 0;
static uint32_t master_addr_retry = 1;
...
/* group: MBED_I2C_Exported_Functions */
...
{
	if ((sda < PA_8) || (sda > PB_10) || (sda == PA_19) || (sda == PA_20)) {
		assert_param(0);
	}
	return 0;
}
...
void i2c_init(i2c_t *obj, PinName sda, PinName scl)
{
	...
	if (I2CInitDat[obj->i2c_idx].I2CIdx == 0) {
		I2CInitDat[obj->i2c_idx].I2CIPClk = I2C0_1_IPCLK;//100M
...
/**
  ...
  * @param  obj I2C object defined in application software.
  * @param  address Slave address which will be transmitted.
  * @param  data Pointer to the buffer of data to be transmitted.
  * @param  length Length of data that to be received.
  * @param  stop Reserved.
  * @param  timeout_ms Specify timeout time in units of ms.
  * @return Length of written data.
  * @note
  *         - If the length is 0, function @ref I2C_MasterSendNullData_TimeOut() will be called.
...
int i2c_repeatread(i2c_t *obj, int address, u8 *pWriteBuf, int Writelen, u8 *pReadBuf, int Readlen)
{
	...
	return I2C_MasterRepeatRead(obj->I2Cx, pWriteBuf, Writelen, pReadBuf, Readlen);
}
...
{
	return I2C_SlaveRead(obj->I2Cx, (unsigned char *)data, length);
}
...
{
	return I2C_SlaveWrite(obj->I2Cx, (unsigned char *)data, length);
}
