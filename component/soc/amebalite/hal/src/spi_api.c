...
{
	u32 mpll_clk, hperi_div, ipclk;
	u32 reg_tmp;

	mpll_clk = PLL_ClkGet(CLK_CPU_MPLL);

	reg_tmp = HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_CKD_GRP0);
	hperi_div = LSYS_GET_CKD_HBUS(reg_tmp);

	ipclk = mpll_clk / (hperi_div + 1);
//	printf("mpllclk:%d hperi_div:%d ipclk:%dHz\r\n", mpll_clk, hperi_div, ipclk);
