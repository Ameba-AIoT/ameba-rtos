...
void BOOT_OTA_Extract(void)
{
	...
	if (Ver[ExtractIdx] >= Ver[OverrideIdx]) {
		...
		if (ret == TRUE) {
			...
			bootLzma_buffer_set(__km4_bd_ram_start__);
...
u8 BOOT_OTA_IMG2(void)
{
	...
	RRAM_DEV->OTA_IMG_REMAP[0] = RSIP_BASE->FLASH_MMU[0].RSIP_REMAPxOR;
	RRAM_DEV->OTA_IMG_REMAP[1] = RSIP_BASE->FLASH_MMU[1].RSIP_REMAPxOR;
	RRAM_DEV->OTA_IMG_REMAP[2] = RSIP_BASE->FLASH_MMU[2].RSIP_REMAPxOR;
