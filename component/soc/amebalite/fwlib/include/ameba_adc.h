...
#define ADC_BIT_DAT_OVW_CHx                 ((u32)0x00000001 << 16)          /*!<R 0  This bit indicates that there is a data overwritten situation in bit_adc_data_chx takeing place. A data overwritten situation is that a former conversion data is NOT read before a new conversion is written to data field. It would be cleared to 0 when a read operation of reg_adc_data_chx. 0: There is no data overwritten case. 1: There is a data overwritten case. */
#define ADC_MASK_NUM_CHx                    ((u32)0x0000000F << 12)          /*!<R 0  This field contains the newsest conversion data of channel x which channel in channel list the data belongs to, only valid when bit_adc_ch_unmask=1 */
#define ADC_NUM_CHx(x)                      ((u32)(((x) & 0x0000000F) << 12))
#define ADC_GET_NUM_CHx(x)                  ((u32)(((x >> 12) & 0x0000000F)))
#define ADC_MASK_DATA_CHx                   ((u32)0x00000FFF << 0)          /*!<R 0  This field contains the newsest conversion data of channel x newest data */
...
#define ADC_MASK_CH_NUM_GLOBAL              ((u32)0x0000000F << 12)          /*!<R 0  This field contains the newsest conversion data of channel in the list. which channel in channel list the data belongs to, only valid when bit_adc_ch_unmask=1 */
#define ADC_CH_NUM_GLOBAL(x)                ((u32)(((x) & 0x0000000F) << 12))
#define ADC_GET_CH_NUM_GLOBAL(x)            ((u32)(((x >> 12) & 0x0000000F)))
#define ADC_MASK_DATA_GLOBAL                ((u32)0x00000FFF << 0)          /*!<R 0  This field contains the newsest conversion data of channel in the list. newest data */
...
/** @defgroup ADC_Chn_Pad_Selection ADC Channel Pad Selection
  * @{
  */
#define ADC_CH0_PIN					(_PB_5) /*!< Pin mapping for ADC channel 0. */
#define ADC_CH1_PIN					(_PB_4) /*!< Pin mapping for ADC channel 1. */
#define ADC_CH2_PIN					(_PB_3) /*!< Pin mapping for ADC channel 2. */
#define ADC_CH3_PIN					(_PB_2) /*!< Pin mapping for ADC channel 3. */
#define ADC_CH4_PIN					(_PB_1) /*!< Pin mapping for ADC channel 4. */
#define ADC_CH5_PIN					(_PB_0) /*!< Pin mapping for ADC channel 5. */
/**
  * @}
  */
...
/** @defgroup ADC_Interrupt_Status ADC Interrupt Status
  * @{
  */
/** @brief Bitmask of all channel rising-edge comparison interrupt status bits. */
...
#define ADC_IT_COMPRE_CH_EN(x)					((u32)0x00000001 << ((19+x))) /*!< Comparison rising-edge interrupt enable bit for channel x. */
