...
#ifndef _AMEBA_LEDC_H_
#define _AMEBA_LEDC_H_
 ...
 * @brief LEDC Control Register
 * @{
 **/
#define LEDC_MASK_TOTAL_DATA_LENGTH        ((u32)0x00001FFF << 16)          /*!<R/W 0x0  Total length of transfer data, range from 1 to 8K (unit: 32 bits, only low 24-bit is valid). Actual transfer package is TOTAL_DATA_LENGTH+1. The field is recommended to an integer multiple of (LED_NUM+1). Note: If TOTAL_DATA_LENGTH is greater than (LED_NUM+1) but not integer multiple, the last frame of data will be transferred less than (LED_NUM+1). */
#define LEDC_TOTAL_DATA_LENGTH(x)          (((u32)((x) & 0x00001FFF) << 16))
#define LEDC_GET_TOTAL_DATA_LENGTH(x)      ((u32)(((x >> 16) & 0x00001FFF)))
#define LEDC_BIT_RESET_LED_EN              ((u32)0x00000001 << 10)          /*!<R/W/ES 0x0  Write operation: When software writes 1 to this bit, LEDC FSM turns to CPU_RESET_LED state, and CPU triggers LEDC to transfer a reset to LED. Note: Only when LEDC is in IDLE state, can FSM turn to CPU_RESET_LED state. * When LEDC finishes to send RESET to LED, LEDC FSM returns to the IDLE state, and hardware clears RESET_LED_EN. * Or when software sets SOFT_RESET, LEDC FSM returns to IDLE state, and hardware clears RESET_LED_EN. Read operation: 0: LEDC completes the transmission of LED reset operation. 1: LEDC does not complete the transmission of LED reset operation. */
#define LEDC_BIT_LED_POLARITY              ((u32)0x00000001 << 9)          /*!<R/W 0x0  LED DI level when free. 0: Low level 1: High level */
#define LEDC_MASK_LED_RGB_MODE             ((u32)0x00000007 << 6)          /*!<R/W 0x0  LEDC inputs 24 bits data package in order of {byte2, byte1, byte0}. 000: Output 24 bits data package in order of {byte2, byte1, byte0} 001: Output 24 bits data package in order of {byte2, byte0, byte1} 010: Output 24 bits data package in order of {byte1, byte2, byte0} 011: Output 24 bits data package in order of {byte0, byte2, byte1} 100: Output 24 bits data package in order of {byte1, byte0, byte2} 101: Output 24 bits data package in order of {byte0, byte1, byte2} */
...
#define LEDC_BIT_LED_MSB_BYTE2             ((u32)0x00000001 << 4)          /*!<R/W 0x1  LEDC outputs 24 bits data package in order of {byte2, byte1, byte0}; 1: Output data byte2 MSB first 0: Output data byte2 LSB first */
#define LEDC_BIT_LED_MSB_BYTE1             ((u32)0x00000001 << 3)          /*!<R/W 0x1  LEDC outputs 24 bits data package in order of {byte2, byte1, byte0}; 1: Output data byte1 MSB first 0: Output data byte1 LSB first */
#define LEDC_BIT_LED_MSB_BYTE0             ((u32)0x00000001 << 2)          /*!<R/W 0x1  LEDC outputs 24 bits data package in order of {byte2, byte1, byte0}; 1: Output data byte0 MSB first 0: Output data byte0 LSB first */
#define LEDC_BIT_SOFT_RESET                ((u32)0x00000001 << 1)          /*!<R/WA0 0x0  LEDC software reset. If software writes 1 to this bit, the next cycle hardware will clear LEDC_SOFT_RESET to 0, which will generate a pulse. LEDC soft reset includes all interrupt status register, the control state machine returns to the IDLE state, LEDC FIFO read and write point is cleared to 0, LEDC interrupt is cleared. The related registers are as below: * LEDC_CTRL_REG (LEDC_EN is cleared to 0) * LEDC_DATA_FINISH_CNT_REG (LEDC_DATA_FINISH_CNT is cleared to 0) * LEDC_INT_STS_REG (all interrupt are cleared) Other registers remain unchanged. */
#define LEDC_BIT_EN                        ((u32)0x00000001 << 0)          /*!<R/W/ES 0x0  LEDC enable bit. 0: Disable 1: Enable This bit enable indicates that LEDC can be started when LEDC data has finished transmission, or this bit is cleared to 0 by hardware when software sets LEDC_SOFT_RESET. Software clearing LEDC_EN when LEDC FSM is not IDLE don't affect data transfer. */
/** @} */

/** @defgroup LEDC_LED_T01_Timing_CTRL_REG
 * @brief LED Timing Control Register
 * @{
 **/
...
#define LEDC_MASK_T0H_CNT                  ((u32)0x000000FF << 8)          /*!<R/W 0xc  LED modulate logical "0" high level time. Unit: cycle (40MHz), T0H_TIME = 25ns * T0H_CNT, where T0H_CNT>0 T0H_TIME default value is 300ns. */
 ...
 * @brief LEDC Data Finish Register
 * @{
 **/
#define LEDC_MASK_LED_WAIT_DATA_TIME       ((u32)0x00007FFF << 16)          /*!<R/W 0x2edf  The time that internal FIFO of LEDC is waiting for data. The default value is 300us by default. To avoid to insert LED RESET between two-pixel data, hardware will send WAITDATA_TIMEOUT_INT when wait_time>= min (RESET_TIME, LED_WAIT_DATA_TIME). The adjust range is from 25ns ~ 819us, LED_WAIT_DATA_TIME = 25ns *(N+1) where N is from 0 to 0x7FFF. When the setting time is exceeded, LEDC will send WAITDATA_TIMEOUT_INT interrupt. Under this abnormal situation, software should reset LEDC. */
 ...
 * @brief LED Reset Timing Control Register
 * @{
 **/
 ...
 * @brief LED Wait Time 0 Control Register
 * @{
 **/
#define LEDC_BIT_WAIT_TIME0_EN             ((u32)0x00000001 << 9)          /*!<R/W 0  Wait time0 enable. When set this bit, the controller automatically inserts waiting time before next LED package data. 0: Disable 1: Enable */
#define LEDC_MASK_TOTAL_WAIT_TIME0         ((u32)0x000001FF << 0)          /*!<R/W 0x1FF  Waiting time between 2 LED data, and LEDC outputs Low level during waiting time0. The adjust range is from 25ns to 12.5us, WAIT_TIME0 = 25ns *(N+1) where N is from 0 to 0x1FF. */
 ...
 * @brief LED Wait Time 1 Control Register
 * @{
 **/
#define LEDC_BIT_WAIT_TIME1_EN             ((u32)0x00000001 << 31)          /*!<R/W 0  Wait time1 enable. When set this bit, the controller automatically inserts waiting time before next LED frame data. 0: Disable 1: Enable */
#define LEDC_MASK_TOTAL_WAIT_TIME1         ((u32)0x7FFFFFFF << 0)          /*!<R/W 0x1FFFFFF  Waiting time between 2 frame data, and LEDC outputs Low level during waiting time1. The adjust range is from 25ns to 53s, WAIT_TIME0 = 25ns *(N+1) where N is from 0 to 0x7FFFFFFF. */
 ...
 * @brief LED Data Register
 * @{
 **/
#define LEDC_MASK_DATA                     ((u32)0x00FFFFFF << 0)          /*!<WPD 0  LEDC display data, the lower 24-bit is valid. Writ this register means push data to LEDC FIFO. Register LEDC_EN toggles from 0 to 1 will clear LEDC FIFO. Thus, only data pushed when LEDC_EN=1 will be send to LED. */
#define LEDC_DATA(x)                       (((u32)((x) & 0x00FFFFFF) << 0))
#define LEDC_GET_DATA(x)                   ((u32)(((x >> 0) & 0x00FFFFFF)))
/** @} */

/** @defgroup LEDC_DMA_CTRL_REG
 * @brief LED DMA Control Register
 * @{
 **/
 ...
 * @brief LED Interrupt Control Register
 * @{
 **/
#define LEDC_BIT_GLOBAL_INT_EN             ((u32)0x00000001 << 4)          /*!<R/W 0  Global interrupt enable. 0: Disable 1: Enable */
#define LEDC_BIT_FIFO_OVERFLOW_INT_EN      ((u32)0x00000001 << 3)          /*!<R/W 0  FIFO overflow interrupt enable. When the data written by software is more than internal FIFO level of LEDC, LEDC is in data loss state. 0: Disable 1: Enable */
 ...
 * @brief LED Interrupt Status Register
 * @{
 **/
...
#define LEDC_BIT_WAITDATA_TIMEOUT_INT      ((u32)0x00000001 << 2)          /*!<RW1C 0  Before transfer a frame: When the internal FIFO in LEDC can't get data after the LED_WAIT_DATA_TIME, this bit is set. During transfer of a frame (between two pixel of data): to avoid insert LED RESET between two pixel data, hardware will send WAITDATA_TIMEOUT_INT when wait_time>= min(RESET_TIME,LED_WAIT_DATA_TIME). Clear 0 when software sets LEDC_SOFT_RESET. When new data arrives at WAIT_DATA state, LEDC will continue sending data. Note: Software should note that if the waiting time exceeds the operation of reset time, LEDC may enter refresh state, new data has not been sent. 0: LEDC not timeout 1: LEDC timeout */
#define LEDC_BIT_FIFO_CPUREQ_INT           ((u32)0x00000001 << 1)          /*!<RW1C 0  When FIFO data available is less than the threshold left, the interrupt is reported to CPU. Clear 0 when software sets LEDC_SOFT_RESET. 0: FIFO doesn't request CPU for data 1: FIFO request CPU to transfer data Note: When getting FIFO_CPUREQ_INT, CPU writes 1 data to LEDC Buffer. It is inefficiently compared to DMA mode. */
#define LEDC_BIT_LED_TRANS_FINISH_INT      ((u32)0x00000001 << 0)          /*!<RW1C 0  When the data configured as TOTAL_DATA_LENGTH has been transferred complete, this bit is set. Clear 0 when software sets LEDC_SOFT_RESET. 0: Data transfer has not complete 1: Data transfer complete */
/** @} */

/** @defgroup LEDC_DEBUG_SEL_REG
 * @brief LED Debug Selection Register
 * @{
 **/
#define LEDC_MASK_DEBUG_SEL_DRIVE          ((u32)0x00000003 << 7)          /*!<R/W 0  Drive module debug out selection. */
#define LEDC_DEBUG_SEL_DRIVE(x)            (((u32)((x) & 0x00000003) << 7))
#define LEDC_GET_DEBUG_SEL_DRIVE(x)        ((u32)(((x >> 7) & 0x00000003)))
#define LEDC_MASK_DEBUG_SEL_FIFO           ((u32)0x0000000F << 3)          /*!<R/W 0  FIFO module debug out select. [6:5]: select debug_data of ledc_txfifo, valid range is 0 or 1 [4:3]: select output data byte when [6:5] is 0. 2'b00: byte0 2'b01: byte1 2'b1x: byte2 */
 ...
 *****************************************************************************/
typedef struct {
	__IO uint32_t LEDC_CTRL_REG;                          /*!< LEDC CONTROL REGISTER,  Address offset: 0x000 */
	__IO uint32_t LEDC_LED_T01_TIMING_CTRL_REG;           /*!< LED TIMING CONTROL REGISTER,  Address offset: 0x004 */
	__IO uint32_t LEDC_DATA_FINISH_CNT_REG;               /*!< LEDC DATA FINISH REGISTER,  Address offset: 0x008 */
	__IO uint32_t LEDC_LED_RESET_TIMING_CTRL_REG;         /*!< LED RESET TIMING CONTROL REGISTER,  Address offset: 0x00C */
	__IO uint32_t LEDC_WAIT_TIME0_CTRL_REG;               /*!< LED WAIT TIME 0 CONTROL REGISTER,  Address offset: 0x010 */
	__IO uint32_t LEDC_WAIT_TIME1_CTRL_REG;               /*!< LED WAIT TIME 1 CONTROL REGISTER,  Address offset: 0x014 */
	__O  uint32_t LEDC_DATA_REG;                          /*!< LED DATA REGISTER,  Address offset: 0x018 */
	__IO uint32_t LEDC_DMA_CTRL_REG;                      /*!< LED DMA CONTROL REGISTER,  Address offset: 0x01C */
	__IO uint32_t LEDC_LED_INTERRUPT_CTRL_REG;            /*!< LED INTERRUPT CONTROL REGISTER,  Address offset: 0x020 */
	__IO uint32_t LEDC_LED_INT_STS_REG;                   /*!< LED INTERRUPT STATUS REGISTER,  Address offset: 0x024 */
	__IO uint32_t RSVD0[2];                               /*!<  Reserved,  Address offset:0x28-0x2C */
	__IO uint32_t LEDC_DEBUG_SEL_REG;                     /*!< LED DEBUG SELECTION REGISTER,  Address offset: 0x030 */
	__IO uint32_t RSVD1[47];                              /*!<  Reserved,  Address offset:0x34-0xEC */
	__IO uint32_t LEDC_DUMMY_F0;                          /*!< DUMMY REGISTER,  Address offset: 0xF0 */
	__IO uint32_t LEDC_DUMMY_F4;                          /*!< DUMMY REGISTER,  Address offset: 0xF4 */
} LEDC_TypeDef;
/** @} */
/* AUTO_GEN_END */
...
/** @defgroup LEDC_Reset_Cnt
  * @{
  */
#define LEDC_MAX_RESET_CNT				(0x4000)
#define IS_LEDC_RESET_VAL(NUM)			((NUM > 0) && (NUM <= LEDC_MAX_RESET_CNT))
/**
  * @}
  */

/** @defgroup LEDC_Wait_Data_Time_Cnt
  * @{
  */
#define LEDC_MAX_WAIT_DATA_TIME_CNT		(0x8000)
#define IS_LEDC_WAIT_DATA_TIME_VAL(NUM)	((NUM > 0) && (NUM <= LEDC_MAX_WAIT_DATA_TIME_CNT))
/**
  * @}
  */

/** @defgroup LEDC_Wait_Time0_Cnt
  * @{
  */
#define LEDC_MAX_WAIT_TIME0_CNT			(0x200)
#define IS_LEDC_WAIT_TIME0_VAL(NUM)		((NUM > 0) && (NUM <= LEDC_MAX_WAIT_TIME0_CNT))
/**
  * @}
  */

/** @defgroup LEDC_Wait_Time1_Cnt
  * @{
  */
#define LEDC_MAX_WAIT_TIME1_CNT			(0x80000000)
#define IS_LEDC_WAIT_TIME1_VAL(NUM)		((NUM > 0) && (NUM <= LEDC_MAX_WAIT_TIME1_CNT))
/**
  * @}
  */
