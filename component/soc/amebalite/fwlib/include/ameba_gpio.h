 ...
 * @{
 **/
#define GPIO_MASK_PORT_DATA_DIRECTION  ((u32)0xFFFFFFFF << 0)          /*!<R/W 0  Values written to this register independently control the direction of the corresponding data bit in Portx. The default direction can be configured as input or output after system reset through the GPIO_DFLT_DIR_x parameter. 0: Input (default) 1: Output Valid bits is: PortA[31:0] and PortB[19:0]. */
 ...
 * @{
 **/
#define GPIO_MASK_ID_CODE              ((u32)0xFFFFFFFF << 0)          /*!<R 32'h1  This register is available only if PortxA/B is configured to generate interrupts (GPIO_PORTx_INTR = Include (1)) and interrupt detection is configured to generate on both rising and falling edges of external input signal (GPIO_INT_BOTH_EDGE=Include (1)). */
...
typedef struct {
	...
	__IO uint32_t RSVD3[32];                              /*!<  Reserved,  Address offset:0x7C-0xF8 */
