 ...
 * @{
 **/
#define MASK_WR_CMD                ((u32)0x0000FFFF << 0)          /*!<R/W 0x0202  WRITE_SINGLE (or WR_OCTA_IO) Indicate SPI Flash command value of write command. WR_CMD[15:8] is for WR_OCTA_IO_CMD_2nd_BYTE (only available when configuration of DDR_EN is defined) WR_CMD[7:0] is for WR_OCTA_IO_CMD_1st_BYTE */
 ...
 * @{
 **/
#define MASK_WR_EN_CMD_2ND_BYTE    ((u32)0x000000FF << 8)          /*!<R/W 0x06  Indicate SPI Flash command value of 2nd byte of write enable. */
...
#define BIT_WR_VL_EN				((u32)0x00000001 << 17)
#define RD_VL_EN(x)					((u32)(((x) & 0x00000001) << 16))
#define BIT_RD_VL_EN				((u32)0x00000001 << 16)
#define BIT_DIS_WRAP_ALIGN         ((u32)0x00000001 << 15)          /*!<R/W 0x0  Set to disable wrap_align function (convert wrap to incremental through aligning start address to burst boundary) in auto mode. */
...
#define BIT_WPN_DNUM               ((u32)0x00000001 << 2)          /*!<R/W 0  Indicate the WPn input pin of SPI Flash is connected to spi_sout[2]or [3]. If setting 0, it connects to spi_sdata[2]. Otherwise, it connects to spi_sdata[3]. */
 ...
 * @{
 **/
#define MASK_FSCKDV                ((u32)0x00000FFF << 0)          /*!<R/W 0x01  Indicate the divider of Fast read command when FAST_RD is setting in user mode or using fast read command in auto mode. The frequency of spi_sclk is derived from: Frequency of spi_sclk = Frequency of bus_clk / (2*FSCKDV). */
 ...
 * @{
 **/
#define MASK_FLASH_SIZE            ((u32)0x0000000F << 0)          /*!<R/W FLASH_MEM_SIZE - 12  Indicate the size of flash size to select the target SPI Flash in auto mode. The number is an exponent of 2 (Flash size) -12(The smallest flash size is 4kB). Ex: If flash size is 4MB (22-bit), set FLASH_SIZE = 10 (22-12). */
 ...
 * @{
 **/
#define MASK_WR_DUMMY_LENGTH       ((u32)0x00000FFF << 0)          /*!<R/W 0x0  Indicate delay cycles for transmitting data in auto mode. It is referenced by bus_clk. */
 ...
 * @{
 **/
#define MASK_STFLR                 ((u32)0x0000000F << 0)          /*!<R 0x0  Status FIFO level. Indicate the FIFO entries of valid data in status data. */
...
_LONG_CALL_ void FLASH_WaitBusy_Timeout(u32 WaitType);
_LONG_CALL_ void FLASH_WriteEn_Timeout(void);
...
_LONG_CALL_ void FLASH_CalibrationPLLSel(u8 isCPUPLL);
_LONG_CALL_ u32 FLASH_PLLInit_ClockDiv(FlashDivInt_E Div);
_LONG_CALL_ void FLASH_Calibration_PSPLL_Open(void);
_LONG_CALL_ void FLASH_Calibration_PSPLL_Close(void);
