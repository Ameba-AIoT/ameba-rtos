...
u32 OSC4M_Calibration(u32 ppm_limit)
{
	...
	for (cal_n = 5; cal_n <= 9; cal_n++) {
		...
		//RTK_LOGD(TAG, "[CAL4M]:cal_n %lu H:0x%lx L:0x%lx result:%lu ", cal_n,REGU_GET_VCM_SEL_H(regu->REGU_4MOSC0),REGU_GET_VCM_SEL_L(regu->REGU_4MOSC0),temp);
	...
	for (cal_n = 1; cal_n <= 8; cal_n++) {
		...
		/* read calibration result */
		temp = OSC_CalResult_Get(OSC4M_CAL_CLK);
		//RTK_LOGD(TAG, "[CAL4M]:cal_n %lu R:0x%lx result:%lu ", cal_n,REGU_GET_FREQ_R_SEL(regu->REGU_4MOSC0), temp, target_40m_counter, delta);
...
cal_end:
	...
	if (cur_ppm >= ppm_limit) {
		RTK_LOGI(TAG, "[CAL4M]: PPM: %lu PPM_Limit:%lu \n", cur_ppm, ppm_limit);
		assert_param(0);
	}
...
void XTAL_AACK(void)
{
	...
	if (xtal_mode != XTAL_HP_MODE) {
		RTK_LOGI(TAG, "XTAL is not in HP mode:%lx\n", xtal_mode);
	}
