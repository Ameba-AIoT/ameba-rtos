...
/**
  ...
  *		 @arg AON_BIT_RSTF_WDG4
  *		 @arg AON_BIT_RSTF_WDG3
  *		 @arg AON_BIT_RSTF_WDG2
  *		 @arg AON_BIT_RSTF_WDG1
  *		 @arg AON_BIT_RSTF_IWDG
  *		 @arg AON_BIT_RSTF_HIFI_SYS
  *		 @arg AON_BIT_RSTF_KR4_SYS
  *		 @arg AON_BIT_RSTF_KM4_SYS
...
{
	return (HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_BOOT_REASON_SW) & 0x0007FFFF);
}
...
void System_Reset(void)
{
	...
	/*valid CPU ID or not*/
	if (CPUID == DSP_CPU_ID) {
		Trig |= LSYS_BIT_HIFI_SYS_RST;
	} else if (CPUID == KM4_CPU_ID) {
		Trig |= LSYS_BIT_KM4_SYS_RST;
	} else if (CPUID == KR4_CPU_ID) {
		Trig |= LSYS_BIT_KR4_SYS_RST;
	} else {
		/*invalid CPU ID*/
		return;
	}
