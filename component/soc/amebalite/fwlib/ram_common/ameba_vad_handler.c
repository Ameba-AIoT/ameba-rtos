...
/** @defgroup VAD_HANDLER
* @brief VAD_HANDLER driver modules
* @{
*/

/* Exported constants ------------------------------------------------------------*/
/** @defgroup VAD_HANDLER_Exported_Constants VAD_HANDLER Exported Constants
  * @{
  */

/** @defgroup VAD_HANDLER_Parameter
  * @{
  */

#define VAD_DEV ((VADBUF_TypeDef *)VAD_REG_BASE)
#define PRE_READ_NUM_BLOCK 12800	/*!<The amount of data moved forward from the position where VAD detects voice*/
#define ONE_SAMPLE_BYTES  4

static u32 block_range[4][2] = {{0x20078000, 0x2007FFFC},
	{0x20070000, 0x20077FFC},
	{0x20068000, 0x2006FFFC},
	{0x20060000, 0x20067FFC},
};
...
/* Exported functions ------------------------------------------------------------*/
/** @defgroup VAD_HANDLER_Exported_Functions VAD_HANDLER Exported Functions
  * @{
  */
...
{
	return VAD_DEV -> VADBUF_BUF_CTRL_W_ADDRESS;
}
...
{
	if ((addr >= 0x20060000) && (addr <= 0x20067FFC)) {
		return 3;
	} else if ((addr >= 0x20068000) && (addr <= 0x2006FFFC)) {
		return 2;
	} else if ((addr >= 0x20070000) && (addr <= 0x20077FFC)) {
		return 1;
	} else {
		return 0;
	}
...
/**
  * @brief  In VAD application, we often need audio data before a voice activity is detected. You can tune the period with
            ...
            to around 32000 bytes. Since we calculate the address with "(addr_ref - PRE_READ_NUM_BLOCK + 1)". addr_ref is an even number
            which points to the lower byte of a sample, so here PRE_READ_NUM_BLOCK should be 31999 to make sure we have 32000bytes(500ms)
            in total.
...
u32 get_start_addr_block(u32 addr_ref, u8 channel_index)
{
	...
	if (channel_index == 0) {
		block_index = VADBUF_GET_CODEC0_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
	}

	if (channel_index == 1) {
		block_index = VADBUF_GET_CODEC1_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
	}

	if (channel_index == 2) {
		block_index = VADBUF_GET_CODEC2_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
	}

	if (channel_index == 3) {
		block_index = VADBUF_GET_CODEC3_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
	}
...
{
	return (VAD_DEV -> VADBUF_VAD_ASSERT_W_SRAM_ADDRSS);
}
...
{
	return (VADBUF_GET_CH_W_VAD_ASSERT(VAD_DEV -> VADBUF_VAD_BUF_CTRL0));
}
...
/* delete low 16bit for every samples */
static void vad_data_get_high_bytes(u8 *temp_buf, u32 amount_byte, u8 *buf)
{
	u32 index;

	for (index = 0; index < amount_byte / 4; index++) {
		((u16 *)buf)[index] = (u16)((((u32 *)temp_buf)[index] & 0xffff0000) >> 16);
	}
}
...
u32 get_new_addr_block(u32 addr, u32 byte_num)
{
	...
	if ((block_range[block_index][1] + ONE_SAMPLE_BYTES - addr) >= byte_num) {
		return (addr + byte_num);
	} else {
		return (byte_num - (block_range[block_index][1]  + ONE_SAMPLE_BYTES - addr) + block_range[block_index][0]);
	}
}
...
/**
  ...
  * @param  time_period_ms  the time period of requied audio data
            buf1  memory space malloced by the user to store 16bit audio data
            buf   memory space malloced by the user to store temp audio data,buf length is twice as long as buf1 is.
  */
void get_vad_data(u32 time_period_ms, u8 *buf, u8 *buf1)
{
	u32 block_number = 0;
	u32 i, j;
	if (first_time_flag) {
		addr_ref = get_hit_addr(); //RTK_LOGD(TAG, "hit addr is %lx\n", addr_ref);
		first_time_flag = 0;

		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH0_IN_VADBUF_EN) {
			addr_ch0 = get_start_addr_block(addr_ref, 0);  // get ch0 start address
		}

		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH1_IN_VADBUF_EN) {
			addr_ch1 = get_start_addr_block(addr_ref, 1);
		}

		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH2_IN_VADBUF_EN) {
			addr_ch2 = get_start_addr_block(addr_ref, 2);
		}

		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH3_IN_VADBUF_EN) {
			addr_ch3 = get_start_addr_block(addr_ref, 3);
		}
		addr_ref = get_start_addr_block(addr_ref, 0);  // get ch0 data start address
	} else {
	...
	//RTK_LOGD(TAG, "addr_ref %lx  addr_curr %lx  byte_num %lx  data_stored %lx\r\n", addr_ref, addr_curr, byte_num, data_stored);
	//RTK_LOGD(TAG, "addr_ch0  %lx  addr_ch1  %lx  addr_ch2  %lx  addr_ch3  %lx\r\n", addr_ch0, addr_ch1, addr_ch2, addr_ch3);
	...
	u32 length = byte_num / 4;
	int m = 0;
	for (j = 0; j < length ; j++) {
		for (i = 0 ; i < block_number; i++) {
			((u16 *)buf1)[m] = ((u16 *)buf)[i * length + j];
			m++;
		}
	}
