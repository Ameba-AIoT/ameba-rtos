...
u8 SWR_Calib_Get_Value_AUD(void)
{
	...
	OTP_Read8(SEC_VOL_AUDIO_VREF_1V8_F, &audio_vref_f_data);
	OTP_Read8(SEC_VOL_AUDIO_VREF_1V8_S, &audio_vref_s_data);
...
void SWR_CORE_Vol_Set(u8 vol_type)
{
	...
	/* calib load only need do once when first power-on */
	if (BOOT_Reason() == 0) {
		SWR_Calib_Load();
	}

	/* if current core voltage is 1V, no need raise voltage */
	if ((CORE_VOL_1P0 == vol_type) && (CORE_VOL_0P9 == rram->CORE_Vol_Flag)) {
