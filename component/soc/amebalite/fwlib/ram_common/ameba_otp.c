...
int OTP_Write8(u32 Addr, u8 Data)
{
	...
	if (OTP_ProgramMarginRead8(Addr, &Temp) == RTK_FAIL) {
...
retry:
	...
	/*program*/
	if (_OTP_Write8(Addr, Data) == RTK_FAIL) {
...
int OTP_Read32(u32 Addr, u32 *Data)
{
	...
	*Data = 0;
	...
	for (i = 0; i < 4; i++) {
		if (OTP_Read8(Addr++, &Temp) == RTK_FAIL) {
			goto exit;
		}
...
/**
  ...
  *          - RTK_FAIL: write fail
  */
static int OTP_PG_Packet_Word(u16 offset, u8 len, u8 *pContant)
{
	u32 Idx = 0;
	u32 OTPData;
	u32 IdxTemp;
	...
	while (left_cnts > 0) {
		...
		for (i = offset, j = 0; i < write_pkt; i++, j++) {
			if (pcontant[j] != newdata[i]) {
				...
				wordmap |= BIT(i >> 2);
				byte_change++;
				RTK_LOGS(TAG, RTK_LOG_ERROR, "newdata[%x]= %x\n", i, newdata[i]);
			}
		}
...
next:
		...
		RTK_LOGS(TAG, RTK_LOG_INFO, "next write cycle Base %x cnts %x \n", base, left_cnts);
	}
