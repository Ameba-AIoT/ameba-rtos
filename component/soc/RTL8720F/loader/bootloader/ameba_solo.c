...
void SOLO_Plfm1ResetHandler(void)
{
	...
	/* SOLO: before releasing cpu1, reload the iot image. If an iot OTA committed a
	 * newer slot, BOOT_OTA_LoadNP re-selects it (by cert version), re-verifies it
	 * and re-points REG_LSYS_BOOT_ADDR_KM4NS, so cpu1 boots the updated iot image.
	 * (Runs in the resident bootloader; MMU_ID3 is NP-only so km4tz is undisturbed.) */
	BOOT_OTA_LoadNP();
...
{
	UNUSED(param);
	u32 code = RXI_300->CODE;

	InterruptDis(RXI300_IRQ);

	/* timeout occurs */
	if (code == 0x2C || code == 0x2D || code == 0x2E) {
		RTK_LOGI(TAG, "rxi300_handler timeout: code=%x\n", code);
		/* clear interrupt */
		process_timeout_flow();
		...
		/* Non-timeout error (e.g. code 0x01 = address-hole: a master accessed an
		 * unmapped address). Log the offending address ONCE and clear the error */
		RTK_LOGI(TAG, "rxi300_handler err: code=%x addr=%08x\n", code, (unsigned int)RXI_300->ADR0);
		RXI_300->INT_CLR = RXI300_BIT_ERR_INTR_CLR;
	}
