...
{
	IMAGE_HEADER ImgHdr;
	u32 RSIP_MODE = MANIFEST_RSIP_GET_IMG_MODE(img_index, Manifest->RSIPConfig);

	if (RSIP_MODE == MANIFEST_RSIP_GCM_MODE) {
		_memcpy((void *)&ImgHdr, (void *)ImgPhyAddr, IMAGE_HEADER_LEN);
		SubImgInfo->Addr = ImgPhyAddr;
...
__NO_RETURN void SBOOT_Validate_Fail_Stuck_NS(u32 wdg_fresh)
{
#ifdef RTL8720F_TODO
	u32 paddr;

	/* clear SRAM image and entry register */
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_BOOT_ADDR_KM4TZ, 0);
	_memset((void *)BOOT_LOADER_START, 0, BOOT_LOADER_SIZE);
	DCache_CleanInvalidate(BOOT_LOADER_START, BOOT_LOADER_SIZE);

	/* clear boot rom stack */
	__set_MSP(MSP_RAM_HP);
	for (paddr = MSPLIM_RAM_HP; paddr < MSP_RAM_HP; paddr += sizeof(u32)) {
		WriteEF4Byte(paddr, 0);
	}
	DCache_CleanInvalidate(MSPLIM_RAM_HP, MSP_RAM_HP - MSPLIM_RAM_HP);
#endif

	if (!wdg_fresh) {
		/* change wdg from 64s to 4s to try another OTA. */
		WDG_Timeout(IWDG_DEV, 4096);
...
fih_ret BOOT_OTA_LoadIMGAll(u8 ImgIndex)
{
	...
	RSIP_MMU_Config(MMU_ID3, LogAddr, (u32)__km4ns_app_flash_text_end__, PhyAddr);
	RSIP_MMU_Cmd(MMU_ID3, ENABLE);
	RSIP_MMU_Cache_Clean();

	/*NP IMG2 OTF configurations*/
	FIH_CALL(BOOT_ROM_OTFCheck, fih_rc, LogAddr, (u32)__km4ns_app_flash_text_end__, RSIP_IV1, RSIP_REGION1, RSIP_KEY_NUM1,
...
void BOOT_OTA_Region_Init(void)
{
	...
	flash_get_layout_info(IMG_APP_OTA1, &image_addr, NULL);
	OTA_Region[IMG_CERT][0] = image_addr + IMG_APP_OTA1_SIZE + 1;
	flash_get_layout_info(IMG_APP_OTA2, &image_addr, NULL);
	OTA_Region[IMG_CERT][1] = image_addr + IMG_APP_OTA1_SIZE + 1;
...
Fail:
	...
	SBOOT_Validate_Fail_Stuck_NS(FALSE);
