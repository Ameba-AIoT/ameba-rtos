						  ...
						  u8 RSIP_KEY_ID, u32 GCM_TagBase, u8 RSIPConfig, u8 img_index)
{
	...
	} else if (manifest_value == MANIFEST_RSIP_GCM_MODE) {
		rsip_mode = OTF_GCM_MODE;
	} else {
		/* if manifest setting is invlid, we use OTP rsip setting.
			OTP will set all image to the same mode. */
...
fih_ret BOOT_DecRDPImg(u32 SrcAddr, Manifest_TypeDef *Manifest, SubImgInfo_TypeDef *SubImgInfo, u8 *Cnt)
{
	...
	*Cnt = SubImgCnt + 1;  /* add gcm tag bin */
	...
	BOOT_ROM_CheckGCM(Manifest, SubImgInfo, PhyAddr, MANIFEST_SECURE_IMG3_ID);
	if (SubImgInfo->Len) {
		IMG3_GCM_TagBase = PhyAddr + IMAGE_HEADER_LEN;
		PhyAddr += SubImgInfo->Len;
	}
	SubImgInfo ++;
	...
	FIH_CALL(BOOT_LoadSubImage, fih_rc, SubImgInfo, ImgAddr, SubImgCnt, Img3Label, FALSE);
	...
	FIH_CALL(BOOT_ROM_OTFCheck, fih_rc, LogAddr, (u32)__km4tz_img3_text_end__, RSIP_IV3, RSIP_REGION3, RSIP_KEY_NUM0, IMG3_GCM_TagBase, Manifest->RSIPConfig,
			 MANIFEST_SECURE_IMG3_ID);
	RSIP_MMU_Cache_Clean();
	...
	FIH_CALL(BOOT_LoadSubImage, fih_rc, SubImgInfo, ImgAddr, SubImgCnt, Img3Label, TRUE);
