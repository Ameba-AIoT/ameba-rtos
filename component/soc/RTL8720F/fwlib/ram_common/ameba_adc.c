...
void ADC_StructInit(ADC_InitTypeDef *ADC_InitStruct)
{
	...
	for (i = 0; i < ADC_EXT_CH_NUM; i++) {
		ADC_InitStruct->ADC_Chan[i].ADC_SamplePeriodUs = 6; // 6us, 166.67KHz
	}
}
...
void ADC_Init(ADC_InitTypeDef *ADC_InitStruct)
{
	...
	/* Clear tm_latch related bits */
	/* Clear status bit */
	adc->ADC_TM_POW_CTRL |= ADC_BIT_TM_POW_LAT_ST; // RW1C

	/* Clear en bit */
	value = HAL_READ32(SYSTEM_CTRL_BASE, REG_AON_DUMMY_AIP_INFO0);
	value &= ~AON_BIT_THM_LATCH_EN;
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_AON_DUMMY_AIP_INFO0, value);

	/* Delay 2 Tosc(2M) */
	DelayUs(1);

	/* Disable ADC fen & cke, which is required by DD */
	RCC_PeriphClockCmd(APBPeriph_ADC, APBPeriph_ADC_CLOCK, DISABLE);

	/* Enable ADC fen & cke again */
	...
	if (!vref_init_done) {
		...
		value = ADC_COM->ADC_COM_ADC_VOL_LIST_0;
		value &= ~ADC_COM_MASK_ADC_SEL_VREFP_VOL_0;
		if (vref_sel == ADC_VREF_1P7) {
			value |= ADC_COM_ADC_SEL_VREFP_VOL_0(ADC_VREF_1P7);
		} else {
			value |= ADC_COM_ADC_SEL_VREFP_VOL_0(ADC_VREF_1P65); // default
		}
		ADC_COM->ADC_COM_ADC_VOL_LIST_0 = value;
	...
	for (i = 0; i < ADC_InitStruct->ADC_CvlistLen + 1; i++) {
		chan = ADC_InitStruct->ADC_Cvlist[i];

		/* Set channel sample time */
		value = ADC_COM->ADC_COM_ADC_CLK_HCNTx_DIV[chan];
		value &= ~ADC_COM_MASK_ADC_CLK_HCNTx;
		value |= ADC_COM_ADC_CLK_HCNTx(ADC_InitStruct->ADC_Chan[chan].ADC_SamplePeriodUs * 0x4 - 0x3);
		ADC_COM->ADC_COM_ADC_CLK_HCNTx_DIV[chan] = value;
	}
...
/**
 * @brief Set high level tick count in sample period of specified ADC channel.
 * @param ADC_channel This parameter can be a value of @ref ADC_Chn_Selection.
 * @param HCnt ADC high level tick count, where clock frequency is 4MHz.
 * @return None
 */
void ADC_SetChanHCnt(u8 ADC_channel, u16 HCnt)
{
	...
	assert_param(HCnt < 0x1000);

	/* Set channel sample time */
	value = ADC_COM->ADC_COM_ADC_CLK_HCNTx_DIV[ADC_channel];
	value &= ~ADC_COM_MASK_ADC_CLK_HCNTx;
	value |= ADC_COM_ADC_CLK_HCNTx(HCnt);
	ADC_COM->ADC_COM_ADC_CLK_HCNTx_DIV[ADC_channel] = value;
}

/**
 * @brief Set conversion time.
 * @param ConvMode ADC conversion mode, which can be a value of @ref ADC_Convert_Mode.
 * @return None
 */
void ADC_SetConvTime(u8 ConvMode)
{
	if (ConvMode == ADC_CONV_500NS) {
		ADC_COM->ADC_COM_ADC_CLK_COV &= ~ADC_COM_BIT_ADC_COV_MODE;
	} else {
		ADC_COM->ADC_COM_ADC_CLK_COV |= ADC_COM_BIT_ADC_COV_MODE;
	}
}
 ...
 * 		@arg ADC_BIT_BREAK_TIMEOUT_EN: ADC does not respond for a long time due to CTC occupation.
 ...
 * 		This parameter can be one of the following parameters:
 * 		@arg ADC_OSR_1: 1x, disable oversample.
 * 		@arg ADC_OSR_2: 2x.
 * 		@arg ADC_OSR_4: 4x.
 * 		@arg ADC_OSR_8: 8x.
 * 		@arg ADC_OSR_16: 16x.
...
/**
 * @brief Set threshold ADC sample waiting time.
 * @param TimeOutCnt Waiting count, waiting time = Waiting count * sample period.
 * @return None
 */
void ADC_SetBreakTO(u16 TimeOutCnt)
{
	...
	value = adc->ADC_BRK_CNT;
	value &= ~ADC_MASK_BREAK_TIMEOUT;
	value |= ADC_BREAK_TIMEOUT(TimeOutCnt);
	adc->ADC_BRK_CNT = value;
}
...
u32 ADC_GetInterR(void)
{
	...
	RTK_LOGE(NOTAG, "[NOTE]TBD: EFuse is not ready.\n");
	return 0;
}
