...
/** @defgroup GPIO_INT_MASK
  ...
  * @{
  */
#define GPIO_MASK_INT_MASK   ((u32)0xFFFFFFFF << 0)           /*!< R/W 0x0  Controls whether an interrupt on Portx can create an interrupt for the interrupt controller by not masking it. By default, all interrupts bits are unmasked. Whenever a 1 is written to a bit in this register, it masks the interrupt generation capability for this signal; otherwise interrupts are allowed through. The unmasked status can be read as well as the resultant status after masking. 0: Interrupt bits are unmasked (default) 1: Mask interrupt*/
...
typedef struct {
	...
	__IO uint32_t RSVD2[35]             ;  /*!< Reserved,  Address offset:0x070-0x0FB */
...
/** @defgroup GPIO_IRQ_ENTRY_USED_MAGIC_NUMBER GPIO IRQ Entry Used Magic Number
  * @{
  */
...
_LONG_CALL_ void GPIO_SetDWWrapper(u32 Status);
...
extern GPIO_TypeDef *GPIO_PORTx_S[1];
