...
#define RUART_MASK_RXDMA_BURSTSIZE   ((u32)0x000000FF << 11)           /*!< R/W 0x4  RXDMA burstsize, maximum value is half of rxfifo depth.*/
...
/** @defgroup FCR
  ...
  * @{
  */
#define RUART_MASK_RECVTRG   ((u32)0x00000003 << 6)           /*!< R/W 0x3  Define the Receiver FIFO Interrupt trigger level * 00: 1 byte * 01: 16 bytes (FIFO_DEPTH/4) * 10: 32 bytes (FIFO_DEPTH/2) * 11: 62 bytes (FIFO_DEPTH-2)*/
...
/** @defgroup RXTO_THRS
  * @brief UART rx timeout threshold
  * @{
  */
#define RUART_MASK_R_RXTO_THRS   ((u32)0x0FFFFFFF << 0)           /*!< R/W 0x40  Indicate the timeout threshold of Rx path, unit in one-bit time. Set rxfifo empty timeout interrupt, when  ERETI=1 and rxfifo is empty for longer than R_RXTO_THRS, unit in one-bit time*/
#define RUART_R_RXTO_THRS(x)     (((u32)((x) & 0x0FFFFFFF) << 0))
#define RUART_GET_R_RXTO_THRS(x) ((u32)(((x >> 0) & 0x0FFFFFFF)))
/** @} */

/** @defgroup RXDMA_MAX_LEN
  * @brief UART rxdma max length register
  * @{
  */
#define RUART_MASK_RXDMA_MAX_LEN   ((u32)0x003FFFFF << 0)           /*!< R/W 0x0  If uart is rxdma flow controller, rxdma_last will be asserted when rxdma transmitted bytes equal RXDMA_MAX_LEN*/
#define RUART_RXDMA_MAX_LEN(x)     (((u32)((x) & 0x003FFFFF) << 0))
#define RUART_GET_RXDMA_MAX_LEN(x) ((u32)(((x >> 0) & 0x003FFFFF)))
/** @} */

/** @defgroup RXDMA_REMAIN_LEN
  * @brief UART rxdma remain length register
  * @{
  */
#define RUART_MASK_RXDMA_REMAIN_LEN   ((u32)0x003FFFFF << 0)           /*!< R 0x0  Rxdma transmitted remain bytes , RXDMA_REMAIN_LEN will be set value of RXDMA_MAX_LEN when DMA transmission is completed(rxdma_last=1) or rxdma_en is disabled.*/
#define RUART_RXDMA_REMAIN_LEN(x)     (((u32)((x) & 0x003FFFFF) << 0))
#define RUART_GET_RXDMA_REMAIN_LEN(x) ((u32)(((x >> 0) & 0x003FFFFF)))
/** @} */

/** @defgroup CLR_FLAG
  * @brief UART clear flag register
  * @{
  */
#define RUART_BIT_CLR_DUMMY_FLAG ((u32)0x00000001 << 0) /*!< RW1C 0x0  Indicate the clearing of dummy data*/
/** @} */
...
typedef struct {
	...
	__IO uint32_t RXTO_THRS       ;  /*!< UART RX TIMEOUT THRESHOLD Register,  Address offset:0x060 */
	__IO uint32_t RXDMA_MAX_LEN   ;  /*!< UART RXDMA MAX LENGTH REGISTER,  Address offset:0x064 */
	__I  uint32_t RXDMA_REMAIN_LEN;  /*!< UART RXDMA REMAIN LENGTH REGISTER,  Address offset:0x068 */
	__IO uint32_t CLR_FLAG        ;  /*!< UART CLEAR FLAG REGISTER,  Address offset:0x06C */
	__IO uint32_t RSVD1[32]       ;  /*!< Reserved,  Address offset:0x070-0x0EF */
	__IO uint32_t DUMMY           ;  /*!< UART DUMMY REGISTER,  Address offset:0x0F0 */
} UART_TypeDef;
