...
#define BIT_ECC_DATA_EN  ((u32)0x00000001 << 28)           /*!< R/W 0x0  Indicate the data path should go through ECC in NAND user mode read write.  1. ECC mode. User should program 0x170 and 0x174. The read/write data flow will go through ECC module.  2. Normal mode. The read/write data flow will bypass ECC module. Exists: TAG >= 2.0.11. Available only if configuration of NAND_EN and ECC_EN are defined.*/
...
#define MASK_DDR_EN      ((u32)0x00000007 << 13)           /*!< R/W 0x0  Indicate the DDR mode in CMD_CH/DATA_CH/ADDR_CH.  CTRLR0[15]: CMD_CH (always 2-Byte CMD type)  CTRLR0[14]: DATA_CH  CTRLR0[13]: ADDR_CH  0: Disable; 1: Enable. Available only if configuration of DDR_EN is defined*/
...
#define MASK_TMOD        ((u32)0x00000003 << 8)            /*!< R/W 0x0  Indicate transfer mode.  2'b00: transmit mode, 2'b11: receive mode, others: reserved*/
...
/** @defgroup SSIENR
  ...
  * @{
  */
#define BIT_NAND_WR   ((u32)0x00000001 << 13)           /*!< R/W 0x0  Set to enable SPIC when start NAND auto command flow in user mode write transactions. To start NAND auto command flow user write transaction, please do not set SSIENR.SPIC_EN. If NAND_WR is activated, SPIC will send WEN command, Page Load program data command and Page execute command to NAND Flash. Write: 0 - Not to activate NAND auto command flow user write 1 - Activate NAND auto command flow user write Read: 0 - NAND auto command flow user write is done 1 - NAND auto command flow user write is still in progress Exists: TAG >= 2.0.11.*/
#define BIT_NAND_RD   ((u32)0x00000001 << 12)           /*!< R/W 0x0  Set to enable SPIC when start NAND auto command flow in user read transactions. To start NAND auto command flow user read transaction, please do not set SSIENR.SPIC_EN. If NAND_RD is activated, SPIC will send Page read data command and Read data command to NAND Flash. Write: 0 - Not to activate NAND auto command flow user read 1 - Activate NAND auto command flow user read Read: 0 - NAND auto command flow user read is done 1 - NAND auto command flow user read is still in progress Exists: TAG >= 2.0.11.*/
...
#define BIT_PDEX      ((u32)0x00000001 << 3)            /*!< R/W 0x1  Set to trigger device enter sleep mode or deep power down mode. After setting PDEX_CTRL0, PDEX_CTRL1, PDEX_CTRL2 and TPR2, then set this bit to trigger PDEX flow. Write: 0 - Not to activate PDEX 1 - Activate PDEX Read: 0 - PDEX is still in progress 1 - PDEX is done Exists: TAG >= 2.0.5.*/
...
#define BIT_ATCK_CMD  ((u32)0x00000001 << 1)            /*!< R/W 0x0  Set to enable ATCK_CMD implementation. After setting SPIC would not accept any command until checking Flash is not busy or checking time out. (Use in User Mode, especially for Erase/Program to check status automatically.)*/
#define BIT_SPIC_EN   ((u32)0x00000001 << 0)            /*!< R/W 0x0  Set to enable SPIC and start user mode transaction. TX : Before writing DR, ensure that the ndf of command, address, write dummy cycles, and write data match control register settings. RX : Before reading DR, ensure to polling CR.SR[3].RFNE = 1.  SPIC will set SPIC_EN=1 automatically if user continues to push DR when DR is full in user write mode. SPIC_EN can only be set when BOOT_FIN=1 (boot finished) and USER_MODE=1 (user mode). SPIC_EN is only allowed to set to zero after user mode transaction is finished completely (reading SPIC_EN == 0 after setting SPIC_EN = 1) Write: 0 - Not to activate SPIC user mode transaction 1 - Activate SPIC user mode transaction Read: 0 - SPIC user mode transaction is done       If ATCK_CMD is enabled, transaction is done means the number of write/read data is equal to CR.TX_NDF/RX_NDF and the auto-check is done.       If ATCK_CMD is disabled, transaction is done means the number of write/read data is equal to CR.TX_NDF/RX_NDF. 1 - SPIC user mode transaction is still in progress*/
/** @} */
...
/** @defgroup SER
  ...
  * @{
  */
#define MASK_CSN_SW   ((u32)0x00000007 << 29)           /*!< R/W 0x0  Set to determine the switch of csn0 and csn1~3.  3'b000: no switch 3'b001: csn0 switch with cs1 3'b010: csn0 switch with cs2 3'b100: csn0 switch with cs3 Exists: TAG >= 2.0.8.*/
#define CSN_SW(x)     (((u32)((x) & 0x00000007) << 29))
#define GET_CSN_SW(x) ((u32)(((x >> 29) & 0x00000007)))
...
#define MASK_SCKDV   ((u32)0x00000FFF << 0)            /*!< R/W Check SPIC frequency in configure form * *: (spic frequency) / 20 ex: spic freq=120 MHz, SCKDV= ⌊120/20⌋ = 6  Define spi_sclk divider value. The frequency of spi_sclk is derived from: The frequency of spi_sclk = The frequency of bus_clk/(2*SCKDV).*/
...
/** @defgroup TXFLR
  ...
  * @{
  */
#define MASK_TXFLR   ((u32)0x0000003F << 0)           /*!< R 0x0  Transmit FIFO level. Indicate the FIFO entry level of valid data in normal mode except in receiving data. (or as FIFO_FLR in any mode)*/
...
/** @defgroup RXFLR
  ...
  * @{
  */
#define MASK_RXFLR   ((u32)0x0000003F << 0)           /*!< R 0x0  Receive FIFO level. Indicate the FIFO entries of valid data in receiving data.*/
...
#define BIT_BUSY        ((u32)0x00000001 << 0)  /*!< R 0x0  SPIC busy flag. Set if SPIC is still transmitting to or receiving data from SPI Flash, or write/read data number are not equal to CR.TX_NDF/RX_NDF (TX_FIFO/RX_FIFO are not empty). User should check BUSY=0 before setting CR_CTRLR0[31].USER_MODE .*/
/** @} */
...
#define BIT_ACSIM ((u32)0x00000001 << 11) /*!< R/W 0x0  Auto-check Flash Status raw interrupt mask.  1: spi_acsir_r is not masked.  0: spi_acsir_r is masked.*/
#define BIT_TFSIM ((u32)0x00000001 << 10) /*!< R/W 0x0  User mode transaction finish interrupt mask. 1: spi_tfsir_r is not masked.  0: spi_tfsir_r is masked.*/
...
#define BIT_WBEIM ((u32)0x00000001 << 6)  /*!< R/W 0x1  Write burst error interrupt mask.  1: spi_wbeir_r is not masked.  0: spi_wbeir_r is masked.*/
...
#define BIT_TFSIS ((u32)0x00000001 << 10) /*!< R 0x0  User mode transaction finish interrupt status after masking.  1: spi_tfsir_r is active after masking.  0: spi_tfsir_r is not active after masking.*/
...
#define BIT_ACEIS ((u32)0x00000001 << 8)  /*!< R 0x0  Auto-check timeout error status after masking.  1: spi_acsir_r is active after masking.  0: spi_acsir_r is not active after masking.*/
...
#define BIT_TFSIR ((u32)0x00000001 << 10) /*!< R 0x0  User mode transaction Finish Status raw interrupt status prior to masking 1: spi_tfsir_r is active prior to masking.  0: spi_tfsir_r is not active prior to masking.*/
...
/** @defgroup TXOICR
  ...
  * @{
  */
#define BIT_TXOICR ((u32)0x00000001 << 0) /*!< R/W 0x0  When reading/writing the register, spi_txoir_r would be cleared. Note: Only response value 0 when read.*/
/** @} */
...
/** @defgroup IDR
  ...
  * @{
  */
#define MASK_IDCODE   ((u32)0xFFFFFFFF << 0)           /*!< R If TAG >= 25.0.0: {16'd ENC_DATE, 16'd ID_NUM} ENC_DATE: generated date ID_NUM: SPIC instance number (hardware config) If TAG <= 2.0.14: {16'h CR_version, 16'hFF01} e.g., if there are two SPICs in a chip, the ID should be 0x0203_FF01 for #1 SPIC v2.3 and 0x0203_FF02 for #2 SPIC v2.3  Contain the decimal value of SPIC version.*/
...
/** @defgroup SPIC_VERSION
  ...
  * @{
  */
#define MASK_SPIC_VERSION   ((u32)0xFFFFFFFF << 0)           /*!< R If TAG >= 25.0.0: {32'd TAG_NUM} If TAG <= 2.0.14: {16'd TAG_NUM, 16'd Encryptd_Date} TAG_NUM: version of SPIC rtl design Encrypted_Date: generated date  Contain the decimal value of SPIC version.*/
...
/** @defgroup DM_DRx
  ...
  * @{
  */
#define BIT_DATA_MASK_DR_EXTENDx ((u32)0x00000001 << 1) /*!< W 0x0  It is a extended data buffer for spi_dm[1] signal in user mode when the width of spi_dm is 2-bit (DQ16). The output value of spi_dm[1] is the data that has been pushed to DM_FIFO [1]. If DM_FIFO is empty, the output value of spi_data_en will be (~DM_ACT). Available only if configuration of DM_EN and DQ16 are defined. DM_EN will be defined automatically if DDR_EN and PSRAM_EN are defined*/
#define BIT_DATA_MASK_DRx        ((u32)0x00000001 << 0) /*!< W 0x0  It is a data buffer for spi_dm signal in user mode. The output value of spi_dm is the data that has been pushed to DM_FIFO [0]. If DM_FIFO is empty, the output value of spi_data_en will be (~DM_ACT). Available only if configuration of DM_EN is defined. DM_EN will be defined automatically if DDR_EN and PSRAM_EN are defined*/
/** @} */
...
/** @defgroup READ_DUAL_DATA
  ...
  * @{
  */
#define MASK_RD_DUAL_O_CMD   ((u32)0x000000FF << 0)           /*!< R/W 0x3B  Indicate SPI Flash command value of dual data read command.*/
...
/** @defgroup READ_DUAL_ADDR_DATA
  ...
  * @{
  */
#define MASK_RD_DUAL_IO_CMD   ((u32)0x000000FF << 0)           /*!< R/W 0xBB  Indicate SPI Flash command value of dual address and data read command.*/
...
/** @defgroup READ_QUAD_DATA
  ...
  * @{
  */
#define MASK_RD_QUAD_O_CMD   ((u32)0x000000FF << 0)           /*!< R/W 0x6B  Indicate SPI Flash command value of quad data read command.*/
...
/** @defgroup READ_QUAD_ADDR_DATA
  ...
  * @{
  */
#define MASK_EXIT_PRM_CMD       ((u32)0x000000FF << 24)           /*!< R/W 0xFF  Exit High Performance Read Mode command. Note: Available only if configuration of PRM_EN is defined (SPIC Lite ver).*/
...
#define MASK_EXIT_PRM_DUM_LEN   ((u32)0x000000FF << 8)            /*!< R/W 0x0  If Exit PRM command is more than 1 byte, use EXIT_PRM_DUM_LEN to extend the command. It is referenced by bus_clk. Ex. In qpi mode (4-4-4), FFFFFFFFh data cycle should be issued (4-byte data in 8 spi cycles). EXIT_PRM_CMD only push 1 byte (2 spi cycles), so use EXIT_PRM_DUM_LEN to extend 6 spi cycles. EXIT_PRM_DUM_LEN = 6 (spi_cycle) * (2 * baurd_rate) Note: Available only if configuration of PRM_EN is defined (SPIC Lite ver).*/
#define EXIT_PRM_DUM_LEN(x)     (((u32)((x) & 0x000000FF) << 8))
#define GET_EXIT_PRM_DUM_LEN(x) ((u32)(((x >> 8) & 0x000000FF)))
#define MASK_RD_QUAD_IO_CMD     ((u32)0x000000FF << 0)            /*!< R/W 0xEB  Indicate SPI Flash command value of quad address and data read command.*/
#define RD_QUAD_IO_CMD(x)       (((u32)((x) & 0x000000FF) << 0))
#define GET_RD_QUAD_IO_CMD(x)   ((u32)(((x >> 0) & 0x000000FF)))
/** @} */

/** @defgroup WRITE_SINGLE
  * @brief Page Program Command of SPI Flash
  * @{
  */
#define MASK_WR_CMD   ((u32)0x0000FFFF << 0)           /*!< R/W 0x0202  Indicate SPI Flash command value of write command. WR_CMD[15:8] is for WR_OCTA_IO_CMD_2nd_BYTE (only available when configuration of DDR_EN is defined) WR_CMD[7:0] is for WR_OCTA_IO_CMD_1st_BYTE*/
...
/** @defgroup WRITE_DUAL_DATA
  ...
  * @{
  */
#define MASK_WR_DUAL_I_CMD   ((u32)0x000000FF << 0)           /*!< R/W 0xA2  Indicate SPI Flash command value of dual data write command.*/
...
/** @defgroup WRITE_DUAL_ADDR_DATA
  ...
  * @{
  */
#define MASK_WR_DUAL_II_CMD   ((u32)0x000000FF << 0)           /*!< R/W 0x0  Indicate SPI Flash command value of dual address and data write command.*/
...
/** @defgroup WRITE_QUAD_DATA
  ...
  * @{
  */
#define MASK_WR_QUAD_I_CMD   ((u32)0x000000FF << 0)           /*!< R/W 0x32  Indicate SPI Flash command value of quad data write command.*/
...
/** @defgroup WRITE_QUAD_ADDR_DATA
  ...
  * @{
  */
#define MASK_WR_QUAD_II_CMD   ((u32)0x000000FF << 0)           /*!< R/W 0x38  Indicate SPI Flash command value of quad address and data write command.*/
...
/** @defgroup WRITE_ENABLE
  ...
  * @{
  */
#define MASK_WR_EN_CMD_2ND_BYTE   ((u32)0x000000FF << 8)           /*!< R/W 0x06  Indicate SPI Flash command value of 2nd byte of write enable. Available only if configuration of DDR_EN is defined.*/
#define WR_EN_CMD_2ND_BYTE(x)     (((u32)((x) & 0x000000FF) << 8))
#define GET_WR_EN_CMD_2ND_BYTE(x) ((u32)(((x >> 8) & 0x000000FF)))
#define MASK_WR_EN_CMD_1ST_BYTE   ((u32)0x000000FF << 0)           /*!< R/W 0x06  Indicate SPI Flash command value of 1st byte of write enable.*/
...
#define BIT_INTERVAL_ODD          ((u32)0x00000001 << 30)           /*!< R/W 0x0  Indicate SPI Flash read status with 2-Byte status but checking only odd byte or even byte. 0: even byte (2, 4, 6, ...) 1: odd byte (1, 3, 5, ...) Available only if configuration of SPIC_LITE is not defined (SPIC Full ver).*/
#define MASK_ST_CMD_LEN           ((u32)0x00000003 << 28)           /*!< R/W 0x1  Indicate the number of bytes in read status command. ST_CMD_LEN can be 1~3. Available only if configuration of SPIC_LITE is not defined (SPIC Full ver).*/
...
#define BIT_ST_CMD_DDR_EN         ((u32)0x00000001 << 25)           /*!< R/W 0x0  Indicate the DDR mode in ST_CMD_CH. Available only if configuration of DDR_EN is defined.*/
#define MASK_RD_ST_CMD_3RD_BYTE   ((u32)0x000000FF << 16)           /*!< R/W 0x00  Indicate SPI Flash command value of 3rd byte of read status. Available only if configuration of SPIC_LITE is not defined (SPIC Full ver).*/
#define RD_ST_CMD_3RD_BYTE(x)     (((u32)((x) & 0x000000FF) << 16))
#define GET_RD_ST_CMD_3RD_BYTE(x) ((u32)(((x >> 16) & 0x000000FF)))
#define MASK_RD_ST_CMD_2ND_BYTE   ((u32)0x000000FF << 8)            /*!< R/W 0x05  Indicate SPI Flash command value of 2nd byte of read status. Available only if configuration of SPIC_LITE is not defined (SPIC Full ver).*/
#define RD_ST_CMD_2ND_BYTE(x)     (((u32)((x) & 0x000000FF) << 8))
#define GET_RD_ST_CMD_2ND_BYTE(x) ((u32)(((x >> 8) & 0x000000FF)))
#define MASK_RD_ST_CMD_1ST_BYTE   ((u32)0x000000FF << 0)            /*!< R/W 0x05  Indicate SPI Flash command value of 1st byte of read status.*/
...
#define BIT_RDVL_RWDS        ((u32)0x00000001 << 18)           /*!< R/W 0x0  Set to enable variable read latency according to RWDS. SPIC will set variable read latency according to RWDS during CA phase. SPIC also checks RWDS during dummy phase if RWDS is delayed to dummy phase. If RDVL_RWDS = 0, SPIC will set fix read latency. Only supported by Winbond devices. It's recommended to set or disable RDVL_RWDS and WDVL_RWDS together. Ex: {RDVL_RWDS, WDVL_RWDS} = {1, 1} or {0, 0} Exists: TAG >= 2.0.11.*/
#define BIT_WDVL_RWDS        ((u32)0x00000001 << 17)           /*!< R/W 0x0  Set to enable variable write latency according to RWDS. SPIC will set variable write latency according to RWDS during CA phase. SPIC also checks RWDS during dummy phase if RWDS is delayed to dummy phase. If WDVL_RWDS = 0, SPIC will set fix write latency. Only supported by Winbond devices. It's recommended to set or disable RDVL_RWDS and WDVL_RWDS together. Ex: {RDVL_RWDS, WDVL_RWDS} = {1, 1} or {0, 0}*/
#define BIT_PHY_RD_VALID     ((u32)0x00000001 << 16)           /*!< R/W 0x0  Set to enable variable read latency according to PHY rx_data_valid. SPIC will not sample read data until rx_data_valid = 1. If PHY_RD_VALID = 0, SPIC will sample read data according to internal counter.*/
#define BIT_WRAP_IMPR_DIS    ((u32)0x00000001 << 15)           /*!< R/W 0x0  Set to disable wrap improvement function (convert wrap to incremental through aligning start address to burst boundary) in auto mode. If WRAP_IMPR_DIS = 1, SPIC will chop wrap transation into two incremental transactions.*/
...
#define MASK_RX_FIFO_ENTRY   ((u32)0x0000000F << 8)            /*!< R/W RX_FIFO_ABW  If using SPIC_HAS_DMA (NO_MERGE_FIFO) SPIC, this field indicates the valid entry of RX FIFO. It is an index of 2 and it should be equal or smaller than RX_FIFO_ABW. The user can modify it to profile with a different entry.  Ex: if RX_FIFO entry is 32, RX_FIFO_ENTYR <=5.*/
...
#define BIT_WPN_DNUM         ((u32)0x00000001 << 2)            /*!< R/W 0x0  Indicate the WPn input pin of SPI Flash is connected to spi_sout[2]or [3].  If setting 0, it connects to spi_sdata[2]. Otherwise, it connects to spi_sdata[3].*/
#define BIT_WPN_SET          ((u32)0x00000001 << 1)            /*!< R/W 0x0  To implement write protect function. spi_wen_out and the bit of spi_sout which connects to WPN would be initial to 0.  WPN_SET: If setting 1 is protected. Otherwise, is not protected.*/
#define BIT_SO_DNUM          ((u32)0x00000001 << 0)            /*!< R/W 0x1  Indicate SO input pin of SPI Flash is connected to spi_sout[0]or [1]. It refers to SPI Flash and also the system connection with SPI_Flash (see 6.1.3)  If setting 0, it connects to spi_sout[0] (single-channel connection).  Otherwise, it connects to spi_sout[1] (multi-channels connection).  (Default is configured to 1 to support multi-channel connection)*/
/** @} */
...
/** @defgroup FBAUDR
  ...
  * @{
  */
#define MASK_FSCKDV   ((u32)0x00000FFF << 0)           /*!< R/W FBAUDR_DEF  Indicate the divider of Fast read command when FAST_RD is setting in user mode or using fast read command in auto mode. The frequency of spi_sclk is derived from: Frequency of spi_sclk = Frequency of bus_clk/(2*FSCKDV).*/
...
#define MASK_USER_CMD_LENGTH        ((u32)0x00000003 << 12)           /*!< R/W 0x0  Indicate number of bytes in command phase in user mode. USER_CMD_LENGTH can be 0~3.*/
#define USER_CMD_LENGTH(x)          (((u32)((x) & 0x00000003) << 12))
#define GET_USER_CMD_LENGTH(x)      ((u32)(((x >> 12) & 0x00000003)))
...
#define MASK_IN_PHYSICAL_CYC     ((u32)0x0000000F << 12)           /*!< R/W 0x0  This indicates how many SPIC CLK (bus_clk) cycles from pad to internal SPIC. It is used in both auto and user mode. it must >= 2 for PSRAM DDR DQ8/16 without PHY before TAG 2.0.13.*/
...
/** @defgroup VALID_CMD
  ...
  * @{
  */
#define BIT_ADDR_SWAP     ((u32)0x00000001 << 18) /*!< R/W 0x0  Set to enable address swap based on CR.FLASH_SIZE in auto mode If address swap is enable, memory space is divided by two and address will be swapped between two spaces ex. if CR.FLASH_SIZE is 0x9, axi_axaddr is 0x00000000. The real address transmitted to device will be 0x00800000 Exists: TAG >= 2.0.14.*/
#define BIT_ADDR_PUIN     ((u32)0x00000001 << 17) /*!< R/W 0x0  Set (1) to pull in first address to negedge of spi_clk. (negedge of last cmd) Set (0) indicate first address start at next posedge of last cmd phase. Used for DDR address. It's necessary to additionally add 1 to CR_RD_DUMMY_LENGTH when ADDR_PUIN is 1 Exists: TAG >= 2.0.6.*/
...
#define BIT_WR_DUAL_II    ((u32)0x00000001 << 6)  /*!< R/W VALID_CMD_DEF[6]  Indicate dual address/data write is a valid command to execute. (known as (1-2-2))*/
#define BIT_WR_DUAL_I     ((u32)0x00000001 << 5)  /*!< R/W VALID_CMD_DEF[5]  Indicate dual data write is a valid command to execute. (known as (1-1-2))*/
...
#define BIT_FRD_SINGLE    ((u32)0x00000001 << 0)  /*!< R/W VALID_CMD_DEF[0]  Indicate fast read command is a valid command to execute. (known as (1-1-1)) PSRAM should enable this setting, SPIC will use FBAUDR to do SPI transaction.*/
/** @} */
...
/** @defgroup FLASH_SIZE
  ...
  * @{
  */
#define MASK_FLASH_SIZE   ((u32)0x0000001F << 0)           /*!< R/W If TAG >= 2.0.3: FLASH_MEM_SIZE -15  If TAG <= 2.0.2: FLASH_MEM_SIZE -12  Indicate the size of flash size to select the target SPI Flash in auto mode. (flash size unit is byte) If TAG >= 2.0.3: CR_FLASH_SIZE = log2 (flash size) -15.  Ex: flash size is 4MB, CR_FLASH_SIZE = log2 (4MB) -15 = 22 - 15 = 7 If TAG <= 2.0.2: CR_FLASH_SIZE = log2 (flash size) -12. Ex: flash size is 4MB, CR_FLASH_SIZE = log2 (4MB) -12 = 22 - 12 = 10 Example. TAG >= 2.0.3:  0x9: 16MB 0xa: 32MB 0xb: 64MB*/
...
/** @defgroup TX_NDF
  ...
  * @{
  */
#define MASK_TX_NDF   ((u32)0x00FFFFFF << 0)           /*!< R/W 0x00  Indicate a number of data frames (Unit: Byte). After transmitting command and address, SPIC transmits data continuously until data frames are equal to TX_NDF. TX_NDF can set larger than TX_FIFO size.*/
...
/** @defgroup DEVICE_INFO
  ...
  * @{
  */
#define BIT_USER_DEFINED_ATOM ((u32)0x00000001 << 31)           /*!< R/W 0x0  Enable user defined DEVICE_INFO[30:29].ATOM_SIZE 1: users can program ATOM_SIZE 0: SPIC sets ATOM_SIZE dynamically Exists: TAG >= 2.0.12.*/
#define MASK_ATOM_SIZE        ((u32)0x00000003 << 29)           /*!< R/W 0x0  Indicate the atomic size of read/write transactions 2'b00: 1 byte 2'b01: 2 byte 2'b10: 4 byte 2'b11:  reserved The equation of atomic size: data_channel * data_rate e.g., if DQ16 and DDR, atomic size is (16*2/8) = 4 byte *Minimun atomic size is 1 byte *For exception of this equaltion, refer to PSRAM Format for devices with special requirements For TAG < 2.0.12, ATOM_SIZE is CR_DEV_INFO[7:6] and is set automatically by SPIC*/
#define ATOM_SIZE(x)          (((u32)((x) & 0x00000003) << 29))
#define GET_ATOM_SIZE(x)      ((u32)(((x >> 29) & 0x00000003)))
#define BIT_ATOM_PHASE        ((u32)0x00000001 << 28)           /*!< R/W 0x0  Indicate how many data phases needed to build atom-size data in SPI interface; this setting is depended in device atom size and data channel number. 1'b0 → atom data phase = 2. 1'b1 → atom data phase = 4. e.g. UPSRAM(APS01608E) device's atom size is 4-byte; data channel is DQ8, so (4*8)/8=4. Atom data phase is 4, means at least needed 4 data phase in SPI transaction to build an atom-size data. */
...
/** @defgroup TPR0
  ...
  * @{
  */
#define MASK_CS_TCEM           ((u32)0x000000FF << 24)           /*!< R/W 0x0  Set to chop auto cmd when CSN low pulse width = (CS_TCEM*32) bus_clk. If set CS_TCEM = 0 will disable this function. ex. PSRAM tCEM = 4us, SPIC frequency = 100Mhz.  CS_TCEM = ((4000 ns/10 ns)/32) - ATOMIC_DELAY = 12 (c) - ATOMIC_DELAY ATOMIC_DELAY = (atom_size*8*2*baudr)/(data_channel*(ddr_en+1)). ex. If baud_rate = 3, data_channel = 16, atom_size = 4byte, ddr_en = 1. ATOMIC_DELAY = (((4*8*2*3)/16*2) = 32*2*3/32 = 6 Therefore, ATOMIC_DELAY should be 6 in this example. Note: Available only if configuration of PSRAM_EN is defined.*/
...
#define MASK_CS_ACTIVE_HOLD    ((u32)0x0000000F << 12)           /*!< R/W 0x00  For Flash chip select active hold time after SCLK rising edge (refer to tCHSH) Note: Wait most 16 bus_clk cycles before CS inactive. (Implement CS_ACTIVE_Hold timing only, CS_ACTIVE_Setup timing using D-PHY to shift)*/
#define CS_ACTIVE_HOLD(x)      (((u32)((x) & 0x0000000F) << 12))
#define GET_CS_ACTIVE_HOLD(x)  ((u32)(((x >> 12) & 0x0000000F)))
#define MASK_CS_H_WR_DUM_LEN   ((u32)0x0000003F << 6)            /*!< R/W CS_H_WR_DUM  Dummy cycle between sending write command to SPI Flash. Using the dummy cycles to avoid the timing violation of CS high time (refer to tSHSL or tCSH).*/
#define CS_H_WR_DUM_LEN(x)     (((u32)((x) & 0x0000003F) << 6))
#define GET_CS_H_WR_DUM_LEN(x) ((u32)(((x >> 6) & 0x0000003F)))
#define MASK_CS_H_RD_DUM_LEN   ((u32)0x0000003F << 0)            /*!< R/W CS_H_RD_DUM  Dummy cycle between sending read command to SPI Flash. Using the dummy cycles to avoid the timing violation of CS high time (refer to tSHSL or tCSH).*/
...
/** @defgroup AUTO_LENGTH2
  ...
  * @{
  */
#define MASK_WR_DUMMY_LENGTH   ((u32)0x00000FFF << 0)           /*!< R/W 0x0  Indicate delay cycles for transmitting data in auto mode. It is referenced by bus_clk. Available only if configuration of PSRAM_EN is defined.*/
...
/** @defgroup TPR1
  ...
  * @{
  */
#define MASK_TPWR              ((u32)0x000000FF << 16)           /*!< R/W 0x00  It is used to configure the cycle counts before spi_csn becomes low when spi_pwr turns from low to high. Note: the default is 0x00. Exists: TAG >= 1.0.4.*/
#define TPWR(x)                (((u32)((x) & 0x000000FF) << 16))
#define GET_TPWR(x)            ((u32)(((x >> 16) & 0x000000FF)))
#define MASK_IDLE_WINDOW       ((u32)0x000000FF << 8)            /*!< R/W 0x10  It is used to configure the cycle counts before turn off the spi_pwr when SPIC is IDLE. Note: the default is 0x10. Exists: TAG >= 1.0.4.*/
#define IDLE_WINDOW(x)         (((u32)((x) & 0x000000FF) << 8))
#define GET_IDLE_WINDOW(x)     ((u32)(((x >> 8) & 0x000000FF)))
#define MASK_CS_ACTIVE_SETUP   ((u32)0x0000000F << 0)            /*!< R/W 0x1  It is used to configure the setup time before the first edge (rising edge for mode0 and falling edge for mode3) of spi_sclk(refer to tCSS or tSLCH ). Note1: It can't be configured 0x0 (if you configure 0x0 to this 4 bits field, this field will not be updated) Note2: The default value is 0x1. Exists: TAG >= 1.0.4.*/
#define CS_ACTIVE_SETUP(x)     (((u32)((x) & 0x0000000F) << 0))
#define GET_CS_ACTIVE_SETUP(x) ((u32)(((x >> 0) & 0x0000000F)))
/** @} */

/** @defgroup SLOT_STARVE
  * @brief Slot Starvation Control Register
  * @{
  */
#define MASK_IDLE_TIMEOUT      ((u32)0x0000FFFF << 16)           /*!< R/W 0xF  If the current slot has no command and another slot has command, or both write/read slot have no command and current slot is write slot, to wait IDLE_TIMEOUT cycles and turnaround slot if current slot still has no command. Unit: spic clk cycle Exists: TAG >= 2.0.5.*/
#define IDLE_TIMEOUT(x)        (((u32)((x) & 0x0000FFFF) << 16))
#define GET_IDLE_TIMEOUT(x)    ((u32)(((x >> 16) & 0x0000FFFF)))
#define MASK_BASE_STARVE_CYC   ((u32)0x000000FF << 8)            /*!< R/W 0xFF  This is basic starvation counter cycles for WR_STARVE_CYC and RD_STARVE_CYC. Unit: device clk cycle Exists: TAG >= 2.0.5.*/
#define BASE_STARVE_CYC(x)     (((u32)((x) & 0x000000FF) << 8))
#define GET_BASE_STARVE_CYC(x) ((u32)(((x >> 8) & 0x000000FF)))
#define MASK_WR_STARVE_CYC     ((u32)0x0000000F << 4)            /*!< R/W 0x3  The timeout of write slot would be set when write cmd is no servicing in the interval time= (WR_STARVE_CYC+1) x (BASE_STARVE_CYC+1) x Tsclk Unit: device clk cycle Exists: TAG >= 2.0.5.*/
#define WR_STARVE_CYC(x)       (((u32)((x) & 0x0000000F) << 4))
#define GET_WR_STARVE_CYC(x)   ((u32)(((x >> 4) & 0x0000000F)))
#define MASK_RD_STARVE_CYC     ((u32)0x0000000F << 0)            /*!< R/W 0x0  The timeout of read slot would be set when read cmd is no servicing in the interval time= (RD_STARVE_CYC+1) x (BASE_STARVE_CYC+1) x Tsclk Unit: device clk cycle Exists: TAG >= 2.0.5.*/
...
/** @defgroup PDEX_CTRL0
  ...
  * @{
  */
#define MASK_PDE_CMD0   ((u32)0xFFFFFFFF << 0)           /*!< R/W 0x000000B9  Indicate the [3-0] byte of SPI PDE cmd value. Exists: TAG >= 2.0.5. Available only if configuration of SPI_PDEX_ENABLE is defined.*/
...
/** @defgroup PDEX_CTRL1
  ...
  * @{
  */
#define MASK_PDE_CMD1   ((u32)0xFFFFFFFF << 0)           /*!< R/W 0x00000000  Indicate the [7-4] byte of SPI PDE cmd value. Exists: TAG >= 2.0.5. Available only if configuration of SPI_PDEX_ENABLE is defined.*/
...
#define BIT_CGATE_EN_WRCR    ((u32)0x00000001 << 11) /*!< R/W 0x1  write control register module clock gating enable.*/
#define BIT_CGATE_EN_BPGM_CR ((u32)0x00000001 << 10) /*!< R/W 0x1  bpgm control register module clock gating enable.*/
...
#define BIT_CGATE_EN_ATRD    ((u32)0x00000001 << 5)  /*!< R/W 0x1  Auto read clock gating enable*/
#define BIT_CGATE_EN_ATWR    ((u32)0x00000001 << 4)  /*!< R/W 0x1  Auto write clock gating enable*/
#define BIT_CGATE_EN_USRD    ((u32)0x00000001 << 3)  /*!< R/W 0x1  User read clock gating enable*/
#define BIT_CGATE_EN_USWR    ((u32)0x00000001 << 2)  /*!< R/W 0x1  User write clock gating enable*/
...
/** @defgroup WBID
  ...
  * @{
  */
#define MASK_WBID_MSK   ((u32)0x0000FFFF << 16)           /*!< R/W 0x0  WBID_ORI bit mask. 0: Interrupt bit is masked. 1: WBID_ORI bit enable.*/
...
#define MASK_ADDR_CTRL       ((u32)0x00000007 << 13)           /*!< R/W 0x0  This field can only be active when ADDR_CTRL_BIT is not 0. DCX control bit will be asserted every (ADDR_CTRL(byte)/addr_channel_num) spi cycles. 0: only one DCX control bit in addr phase; 1: DCX control bit for each byte in addr phase; 2: DCX control bit for every 2-byte in addr phase; 3: DCX control bit for every 4-byte in addr phase; 4: DCX control bit for every 8-byte in addr phase; 5: DCX control bit for every 16-byte in addr phase;*/
...
#define BIT_ADDR_TAIL_EN     ((u32)0x00000001 << 9)            /*!< R/W 0x0  0: without tail in addr phase 1: insert tail in addr phase*/
#define BIT_ADDR_HEAD_EN     ((u32)0x00000001 << 8)            /*!< R/W 0x0  0: without header in addr phase 1: insert header in addr phase*/
#define MASK_CMD_CTRL        ((u32)0x00000007 << 5)            /*!< R/W 0x0  This field can only be active when CMD_CTRL_BIT is not 0. DCX control bit will be asserted every (CMD_CTRL(byte)/cmd_channel_num) spi cycles. 0: only one DCX control bit in cmd phase; 1: DCX control bit for each byte in cmd phase; 2: DCX control bit for every 2-byte in cmd phase; 3: DCX control bit for every 4-byte in cmd phase; 4: DCX control bit for every 8-byte in cmd phase; 5: DCX control bit for every 16-byte in cmd phase;*/
...
/** @defgroup DCX_ADDR
  * @brief LCDC addr phase DCX control bit value register
  * @{
  */
#define MASK_ADDR_TAIL_VAL   ((u32)0x0000FFFF << 16)           /*!< R/W 0x0  Value of DCX tail control bit in addr phase.*/
#define ADDR_TAIL_VAL(x)     (((u32)((x) & 0x0000FFFF) << 16))
#define GET_ADDR_TAIL_VAL(x) ((u32)(((x >> 16) & 0x0000FFFF)))
#define MASK_ADDR_HEAD_VAL   ((u32)0x0000FFFF << 0)            /*!< R/W 0x0  Value of DCX header control bit in addr phase. In 4-wire format addr phase, spi_dcx = ADDR_HEAD_VAL[0].*/
...
/** @defgroup PAGE_CTRL0
  * @brief NAND page ctrl Register 0
  * @{
  */
...
/** @defgroup PAGE_CTRL1
  * @brief NAND page ctrl Register 1
  * @{
  */
...
/** @defgroup ST_DRx
  ...
  * @{
  */
#define MASK_ST_DRx   ((u32)0x000000FF << 0)           /*!< R 0x0  It is a data buffer with 8-bit width FIFO. Available only if configuration of NAND_EN is defined*/
...
/** @defgroup STFLR
  ...
  * @{
  */
#define MASK_STFLR   ((u32)0x0000000F << 0)           /*!< R 0x0  Status FIFO level. Indicate the FIFO entries of valid data in status data. Note: Available only if configuration of NAND_EN is defined.*/
...
/** @defgroup PAGE_READ
  ...
  * @{
  */
#define MASK_PAGE_RD_ADDR_LEN   ((u32)0x00000003 << 18)           /*!< R/W 0x3  Indicate number of bytes in address phase in page read command. Available only if configuration of NAND_EN is defined.*/
#define PAGE_RD_ADDR_LEN(x)     (((u32)((x) & 0x00000003) << 18))
#define GET_PAGE_RD_ADDR_LEN(x) ((u32)(((x >> 18) & 0x00000003)))
#define MASK_PAGE_RD_CH         ((u32)0x00000003 << 16)           /*!< R/W 0x0  Indicate channel number of command and address phase in transmitting page read command. 0: single channel 1: dual channels 2: quad channels 3: octal channel Available only if configuration of NAND_EN is defined.*/
#define PAGE_RD_CH(x)           (((u32)((x) & 0x00000003) << 16))
#define GET_PAGE_RD_CH(x)       ((u32)(((x >> 16) & 0x00000003)))
#define MASK_PAGE_RD_CMD        ((u32)0x000000FF << 0)            /*!< R/W 0x13  Indicate SPI Flash command value of page read command Available only if configuration of NAND_EN is defined.*/
...
#define MASK_SEC_Rx_MODE   ((u32)0x00000007 << 4)            /*!< R/W 0x0  SEC region x - function mode/seed selection. Select one of mode/seed from SEC_NONCE0~SEC_NONCE7. Note: Available only if configuration of PSRAM_EN is defined.*/
#define SEC_Rx_MODE(x)     (((u32)((x) & 0x00000007) << 4))
#define GET_SEC_Rx_MODE(x) ((u32)(((x >> 4) & 0x00000007)))
#define BIT_SEC_Rx_EN      ((u32)0x00000001 << 0)            /*!< R/W 0x0  Set to enable SEC region x to scramble write data and descramble read data. Note: Available only if configuration of PSRAM_EN is defined.*/
/** @} */
...
/** @defgroup SEC_IDR
  ...
  * @{
  */
#define MASK_SEC_IDR   ((u32)0xFFFFFFFF << 0)           /*!< R SEC_CONF_NUM, SEC_KEY_NUM, 5'b0, 1'b0, SEC_EN  SEC ID register*/
...
typedef struct {
	...
	__IO uint32_t WRITE_SINGLE                    ;  /*!< PAGE PROGRAM COMMAND OF SPI FLASH Register,  Address offset:0x0F4 */
	...
	__IO uint32_t FLASH_SIZE                      ;  /*!< FLASH SIZE REGISTER,  Address offset:0x124 */
	...
	__IO uint32_t SLOT_STARVE                     ;  /*!< SLOT STARVATION CONTROL REGISTER,  Address offset:0x144 */
	...
	__IO uint32_t WBID                            ;  /*!< WBUF ID CONTROL REGISTER,  Address offset:0x15C */
	...
	__IO uint32_t DCX_ADDR                        ;  /*!< LCDC ADDR PHASE DCX CONTROL BIT VALUE REGISTER,  Address offset:0x168 */
	__IO uint32_t DCX_DATA                        ;  /*!< LCDC DATA PHASE DCX CONTROL BIT VALUE REGISTER,  Address offset:0x16C */
	__IO uint32_t PAGE_CTRL0                      ;  /*!< NAND PAGE CTRL REGISTER 0 Register,  Address offset:0x170 */
	__IO uint32_t PAGE_CTRL1                      ;  /*!< NAND PAGE CTRL REGISTER 1 Register,  Address offset:0x174 */
...
typedef struct {
	...
	u8 FLASH_pinmux;			/*!< Specifies which pinmux is used. PINMUX_EXTERNAL or PINMUX_INTERNAL*/
...
#define FLASH_CMD_WRCR			0x11            /*!< write configuration register*/
