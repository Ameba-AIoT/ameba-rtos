...
void sys_clear_ota_signature(int ImgID)
{
	...
	if (ImgID == OTA_IMGID_BOOT) {
		check_sig[0] = SPIC_CALIB_PATTERN[0];
		check_sig[1] = SPIC_CALIB_PATTERN[1];
...
void sys_recover_ota_signature(int ImgID)
{
	...
	if (ImgID == OTA_IMGID_BOOT) {
		recover_sig[0] = SPIC_CALIB_PATTERN[0];
		recover_sig[1] = SPIC_CALIB_PATTERN[1];
		flash_get_layout_info(IMG_BOOT, &ota1_start_addr, NULL);
		flash_get_layout_info(IMG_BOOT_OTA2, &ota2_start_addr, NULL);
	} else {
	...
	RTK_LOGA(TAG, "[%s] IMGID: %d, current OTA%d Address: 0x%08x, target OTA%d Address: 0x%08x\n", __func__,
			 ImgID,
			 otaCurIdx + 1, Address[otaCurIdx],
			 otaDstIdx + 1, Address[otaDstIdx]);
	/* backup this sector */
