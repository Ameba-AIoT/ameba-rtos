...
static u32 uart_irqhandler(void *Data)
{
	...
	if ((reg_lsr & RUART_BIT_RE_TIMEOUT_INT) && (reg_ier & RUART_BIT_ERETI)) {
		UART_INT_Clear(puart_adapter->UARTx, RUART_BIT_RETICF);
		rx_dma_timeout = 1;

		RTK_LOGI(NOTAG, "Timeout! Got %dByte(s)\n", UART_RxByteCntGet(puart_adapter->UARTx));
		UART_RxByteCntClear(puart_adapter->UARTx);

		if (UART_GetRxFlag(puart_adapter->UartIndex) == STATERX_DMA) {
			uart_dmarecv_irqhandler(puart_adapter);
		}
...
void serial_init(serial_t *obj, PinName tx, PinName rx)
{
	...
	assert_param(tx < PA_31); // only PA0-PA30 is allowed
	assert_param(rx < PA_31); // only PA0-PA30 is allowed
...
void serial_baud(serial_t *obj, int baudrate)
{
	...
	if (puart_adapter->UARTx == UART0_DEV) {
		RCC_PeriphClockSourceSet(UART0, XTAL);
	} else if (puart_adapter->UARTx == UART1_DEV) {
		RCC_PeriphClockSourceSet(UART1, XTAL);
	} else if (puart_adapter->UARTx == UART2_DEV) {
		RCC_PeriphClockSourceSet(UART2, XTAL);
	} else {
		assert_param(0);
	}
	...
	if (baudrate <= 115200) {
		if (uart_config[obj->uart_idx].LOW_POWER_RX_ENABLE) {
			...
			if (puart_adapter->UARTx == UART0_DEV) {
				RCC_PeriphClockSourceSet(UART0, OSC2M);
			} else if (puart_adapter->UARTx == UART1_DEV) {
				RCC_PeriphClockSourceSet(UART1, OSC2M);
			} else if (puart_adapter->UARTx == UART2_DEV) {
				RCC_PeriphClockSourceSet(UART2, OSC2M);
			} else {
				assert_param(0);
			}

			/* Get actual OSC freq by calling OSC_Clk_Get() */
			UART_LPRxBaudSet(puart_adapter->UARTx, baudrate, OSC_Clk_Get());
			UART_RxCmd(puart_adapter->UARTx, ENABLE);
		}
	}
}
 ...
 * @param obj UART object defined in application software.
 * @param prxbuf Pointer to the buffer that saves data read from UART FIFO.
 * @param len Maximum received data length before timeout.
									   ...
									   void *force_cs)
{
	...
	PMBED_UART_ADAPTER puart_adapter = &(uart_adapter[obj->uart_idx]);
	HAL_Status ret = HAL_OK;
	u32 timeout_cnt = timeout_ms * current_baudrate / 1000;

	assert_param(timeout_cnt > 0);

	UART_RxToThreConfig(puart_adapter->UARTx, timeout_cnt);
	UART_RxByteCntClear(puart_adapter->UARTx);
...
void serial_set_flow_control(serial_t *obj, FlowControl type, PinName rxflow, PinName txflow)
{
	...
	/* only UART0 & UART1 support RTS & CTS pads */
	assert_param(uart_idx <= 1);

	/*autoflow control configuration*/
	if (type != FlowControlNone) {
		if ((rxflow < PA_31) && (txflow < PA_31)) {
			if (uart_idx == 0) {
				Pinmux_Config(rxflow, PINMUX_FUNCTION_UART0_RTS);
				Pinmux_Config(txflow, PINMUX_FUNCTION_UART0_CTS);
			} else {
				Pinmux_Config(rxflow, PINMUX_FUNCTION_UART1_RTS);
				Pinmux_Config(txflow, PINMUX_FUNCTION_UART1_CTS);
			}
		} else {
			assert_param(0);
		}
