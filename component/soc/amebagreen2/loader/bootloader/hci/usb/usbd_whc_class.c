...
USBD_ClassTypeDef  USBD_WHC_ClassDriver = {
...
USBD_WHC_ClassHandleTypeDef USBD_WHC_ClassHandle;
...
{
	u8 ret = 0U;
	USBD_WHC_ClassHandleTypeDef   *hcdc;
	...
	hcdc = (USBD_WHC_ClassHandleTypeDef *) pdev->pClassData;
...
static u8 USBD_Class_Setup(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
	USBD_WHC_ClassHandleTypeDef *hcdc = (USBD_WHC_ClassHandleTypeDef *) pdev->pClassData;
	...
	case USB_REQ_TYPE_VENDOR:
		if (req->wLength) {
			if (req->bmRequest & 0x80U) {
				ret = ((USBD_WHC_ClassCallbackTypeDef *)pdev->pUserData)->Setup(hcdc->Adapter, req, hcdc->Data);
				if (ret == HAL_OK) {
			...
			((USBD_WHC_ClassCallbackTypeDef *)pdev->pUserData)->Setup(hcdc->Adapter, req, NULL);
		}
		break;
...
static u8 USBD_Class_DataOut(USBD_HandleTypeDef *pdev, u8 epnum, u32 len)
{
	...
	if ((hcdc != NULL) && (pdev->pUserData != NULL)) {
		((USBD_WHC_ClassCallbackTypeDef *)pdev->pUserData)->Receive(hcdc->Adapter, len);
		ret = HAL_OK;
	}
...
static u8  USBD_Class_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
	...
	if ((hcdc != NULL) && (pdev->pUserData != NULL) && (hcdc->Request.bRequest != 0xFFU)) {
		((USBD_WHC_ClassCallbackTypeDef *)pdev->pUserData)->Setup(hcdc->Adapter, &hcdc->Request, hcdc->Data);
