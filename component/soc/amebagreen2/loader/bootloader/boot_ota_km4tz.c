...
/**
  * @brief check manifest pattern and version
  * @param none
  * @retval first valid OTA Idx
  */
u8 Boot_Fullmac_SlotSelect(void)
{
	u32 Vertemp;
	u8 i;
	...
	for (i = 0; i < 2; i++) {
		_memcpy((void *)&Manifest[i], (void *)OTA_Region[IMG_CERT][i], sizeof(Manifest_TypeDef));
		if (_memcmp(Manifest[i].Pattern, SPIC_CALIB_PATTERN, sizeof(SPIC_CALIB_PATTERN)) == 0) {
			Vertemp = (Manifest[i].MajorImgVer << 16) | Manifest[i].MinorImgVer; // get 32-bit full version number
			Ver[i] = (s64)Vertemp;
			break;
		} else {
...
u32 Boot_Fullmac_RSIP_Set(u8 img_type, u8 ImgIndex)
{
	...
	PhyAddr = OTA_Region[img_type][ImgIndex];
	LogAddr = (u32)__km4tz_flash_text_start__ - IMAGE_HEADER_LEN;
...
fih_ret Boot_Fullmac_LoadImage(u8 ImgIndex)
{
	...
	ImgAddr = Boot_Fullmac_RSIP_Set(IMG_IMG2, ImgIndex);
...
void Boot_Fullmac_OTA(void)
{
	...
	ImgIndex = Boot_Fullmac_SlotSelect();
	...
	for (i = 0; i < ValidIMGNum; i++) {
		FIH_CALL(Boot_Fullmac_LoadImage, fih_rc, ImgIndex);
	...
	RTK_LOGI(TAG, "FULLMAC BOOT from OTA %d, Version: %ld.%ld \n", ImgIndex + 1, ((version >> 16) & 0xFFFF), (version & 0xFFFF));
	return;

Fail:
	RTK_LOGE(TAG, "FULLMAC IMG invalid, ret = %x, BOOT FAIL!!\n", fih_rc);
