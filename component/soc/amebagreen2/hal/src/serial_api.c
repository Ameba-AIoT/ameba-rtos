...
#define UART_NUM			(MAX_UART_INDEX - 1)
...
void serial_baud(serial_t *obj, int baudrate)
{
	...
	} else if (puart_adapter->UARTx == UART3_DEV) {
		RCC_PeriphClockSourceSet(UART3, XTAL);
	} else {
		assert_param(0);
	}
	...
	if (baudrate <= 115200) {
		if (uart_config[obj->uart_idx].LOW_POWER_RX_ENABLE) {
			...
			if (puart_adapter->UARTx == UART0_DEV) {
				...
				RCC_PeriphClockSourceSet(UART3, OSC2M);
			} else {
				assert_param(0);
			}
									   ...
									   void *force_cs)
{
	...
	if (uart_config[obj->uart_idx].LOW_POWER_RX_ENABLE && current_baudrate <= 115200) {
		timeout_cnt = RX_CLK_OSC2M / 1000 * timeout_ms;
	} else {
...
void serial_set_flow_control(serial_t *obj, FlowControl type, PinName rxflow, PinName txflow)
{
	...
	assert_param(uart_idx == 0);

	/*autoflow control configuration*/
	if (type != FlowControlNone) {
		if (rxflow <= _PC_8 && txflow <= _PC_8) {
