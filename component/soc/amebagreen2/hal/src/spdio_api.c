...
static void SPDIO_Recycle_Rx_BD(IN PHAL_SPDIO_ADAPTER pgSPDIODev)
{
	...
	RTK_LOGS(TAG, RTK_LOG_DEBUG, "<==SDIO_Recycle_Rx_BD(%lu)\n", FreeCnt);
}
...
static void SPDIO_Notify_INT(u16 IntStatus)
{
	if (IntStatus & SDIO_WIFI_BIT_RPWM1_INT) {
		RTK_LOGI(NOTAG, "RPWM INT triggerd, value is: 0x%x\n", SDIO_RPWM1_Get(SDIO_WIFI));
	}

	if (IntStatus & SDIO_WIFI_BIT_RPWM2_INT) {
		RTK_LOGI(NOTAG, "RPWM2 INT triggerd, value is: 0x%x\n", SDIO_RPWM2_Get(SDIO_WIFI));
	}

	if (IntStatus & SDIO_WIFI_BIT_H2C_MSG_INT) {
		RTK_LOGI(NOTAG, "H2C MSG INT triggerd, MSG and EXT MSG value: 0x%x, 0x%x\n",
...
static void SPDIO_IRQ_Handler_BH(void *pData)
{
	...
	for (;;) {
		...
		if (IntStatus & SDIO_WIFI_BIT_H2C_DMA_OK || pgSPDIODev->WaitForTxbuf) {
...
bool SPDIO_Device_Init(struct spdio_t *obj)
{
	...
	InterruptRegister((IRQ_FUN)SPDIO_IRQ_Handler, SDIO_WIFI_IRQ, (u32)pgSPDIODev, SPDIO_IRQ_PRIORITY);
	InterruptEn(SDIO_WIFI_IRQ, SPDIO_IRQ_PRIORITY);
