...
#define NAND_CMD_PP              0x02            // Program load
#define NAND_CMD_PP_RANDOM       0x84            // Program load random(do not reset buffer)
#define NAND_CMD_QPP             0x32            // quad program load  1-1-4
#define NAND_CMD_QPP_RANDOM      0x34            // quad program load random  1-1-4(do not reset buffer)
...
/* Winbond/MXIC READ/PP CMD ADDR Sequence is CA15-8/CA7-0, so ADDR_LEN is x02
...
#define DATA_NAND_PAGE_SIZE_MAIN_BIT_EXP 		(data_flash_init_para.FLASH_pagesize_main_bit_exp)
#define DATA_NAND_PAGE_SIZE_MAIN 				(1U << DATA_NAND_PAGE_SIZE_MAIN_BIT_EXP)
#define DATA_NAND_ADDR_TO_PAGE_ADDR(addr) 		((addr) >> DATA_NAND_PAGE_SIZE_MAIN_BIT_EXP)
#define DATA_NAND_ADDR_TO_BYTE_ADDR(addr) 		((addr) & BIT_LEN_MASK_32(DATA_NAND_PAGE_SIZE_MAIN_BIT_EXP))
...
_LONG_CALL_ void NAND_RxCmd_Safe(u8 cmd, u8 addr_len, u8 *paddr, u32 read_len, u8 *read_data);
_LONG_CALL_ void NAND_TxCmd_Safe(u8 cmd, u8 AddrLen, u8 *pAddr, u32 DataLen, u8 *pData);
_LONG_CALL_ u8 NAND_TxCmd_Wait_Safe(u8 cmd, u8 AddrLen, u8 *pAddr, u32 DataLen, u8 *pData);
...
_LONG_CALL_ void SPIC_Config(SPIC_TypeDef *spi_flash, FLASH_InitTypeDef *FLASH_InitStruct);
_LONG_CALL_ void NAND_SetSpiMode(SPIC_TypeDef *spi_flash, FLASH_InitTypeDef *FLASH_InitStruct, u8 SpicBitMode);
...
_LONG_CALL_ int NAND_Reset(void);
/** @} */
