...
/* ECDSA */
/**
 * @brief  Configure the sharing attribute of an OTP hardware key (secure-only or shared with non-secure world).
 * @param  key_id PKE_ECDSA_PRIV_KEY_HW_0 or PKE_ECDSA_PRIV_KEY_HW_1.
 * @param  is_share 1 to share the key with the non-secure world, 0 for secure-only access.
 * @retval RTK_SUCCESS Key sharing configuration updated successfully.
 * @retval _ERRNO_CRYPTO_KEY_SECURE_ERR Called from non-secure context.
 */
