...
#define PLIC_INT_CLAIMED_REG_OFFSET     0x0C20          // the address offset of Interrupt Claimed Register
#define PLIC_INT_PEND_CLEAR_REG_OFFSET  0x0C40          // the address offset of Interrupt Pending Clear Register
#define PLIC_INT_PEND_SET_REG_OFFSET    0x0C60          // the address offset of Interrupt Pending Set Register
...
#define PLIC_IRQ_PEND_CLEAR_REG(irq) \
  REG32((PLIC_BASE) + (PLIC_INT_PEND_CLEAR_REG_OFFSET) + ((irq >> 5) << 2))
#define PLIC_IRQ_PEND_SET_REG(irq) \
  REG32((PLIC_BASE) + (PLIC_INT_PEND_SET_REG_OFFSET) + ((irq >> 5) << 2))
#define PLIC_IRQ_CLAIMED_REG(irq) \
  REG32((PLIC_BASE) + (PLIC_INT_CLAIMED_REG_OFFSET) + ((irq >> 5) << 2))
 ...
 */
extern uint32_t plic_get_pending(uint32_t reg_id);
...
/**
 ...
 \details Clear he pending status of the given IRQ ID.
 \param   irq_id the IRQ ID.
 */
extern void plic_clear_irq_pending(uint32_t irq_id);
...
/**
 ...
 \param   irq_id the IRQ ID.
 */
extern void plic_set_irq_pending(uint32_t irq_id);

/**
 \brief   Check interrupt claim status
 \details Check whether IRQ ID is claimed but not completed yet
 \para    irq_id the IRQ ID.
 \return  0: IRQ ID is not claimed
          1: IRQ ID is claimed
 */
extern uint32_t plic_get_irq_claimed(uint32_t irq_id);
