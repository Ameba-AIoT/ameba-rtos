...
void SOCPS_PowerManage(u8 regu_state)
{
	...
	if (chip_type != MEMORY_SINGLE_DIE) {
		/*case 1: The chip with internal flash is powered by external 3.3V, so the G5 pad cannot be shut down.*/
		if (chip_type != MEMORY_MCM_PSRAM) {
			reg_temp &= PMC_BIT_PST_G5_USE_MLDO;
		}
		...
		/*case 3:chip of sing die, The G5 pad's io is not powered, so its IO core power (control reg) and IO pad(driver)
