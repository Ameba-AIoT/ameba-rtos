...
void SPDIO_Notify_INT(u16 IntStatus)
{
	if (IntStatus & SDIO_WIFI_BIT_RPWM1_INT) {
		RTK_LOGS(NOTAG, RTK_LOG_INFO, "RPWM INT trigged, value is: 0x%x\n", SDIO_RPWM1_Get(SDIO_WIFI));
	}

	if (IntStatus & SDIO_WIFI_BIT_RPWM2_INT) {
		RTK_LOGS(NOTAG, RTK_LOG_INFO, "RPWM2 INT trigged, value is: 0x%x\n", SDIO_RPWM2_Get(SDIO_WIFI));
	}

	if (IntStatus & SDIO_WIFI_BIT_H2C_MSG_INT) {
		RTK_LOGS(NOTAG, RTK_LOG_INFO, "H2C MSG INT trigged, MSG and EXT MSG value: 0x%x, 0x%x\n", SDIO_H2C_MSG_Get(SDIO_WIFI),  SDIO_H2C_MSG_EXT_Get(SDIO_WIFI));
	}

	if (IntStatus & SDIO_WIFI_BIT_HOST_WAKE_CPU_INT) {
		RTK_LOGS(NOTAG, RTK_LOG_INFO, "SDIO_WIFI_BIT_HOST_WAKE_CPU_INT triggered\n");
	}

	if (IntStatus & SDIO_WIFI_BIT_H2C_SUS_REQ) {
		RTK_LOGS(NOTAG, RTK_LOG_INFO, "SDIO_WIFI_BIT_H2C_SUS_REQ triggered\n");
	}
}
...
void SPDIO_Recycle_Rx_BD(IN PSPDIO_ADAPTER pSPDIODev, spdio_device_tx_done_cb_ptr spdio_device_tx_done_cb)
{
	...
	while (SDIO_RXBD_RPTR_Get(SDIO_WIFI) != pSPDIODev->RXBDRPtr) {
		...
		SDIO_INCR_RING_IDX(pSPDIODev->RXBDRPtr, pSPDIODev->host_tx_bd_num);
...
s8 SPDIO_DeviceTx(PSPDIO_ADAPTER pSPDIODev, struct spdio_buf_t *pbuf)
{
	...
	u8 needed_rxbd_num = ((pkt_size - 1) / SPDIO_MAX_RX_BD_BUF_SIZE) + SPDIO_MIN_RX_BD_SEND_PKT;
#else
	u8 needed_rxbd_num = SPDIO_MIN_RX_BD_SEND_PKT;
#endif

	if (RxBdRdPtr != pSPDIODev->RXBDWPtr) {
		if (pSPDIODev->RXBDWPtr > RxBdRdPtr) {
			if ((pSPDIODev->RXBDWPtr - RxBdRdPtr) > (pSPDIODev->host_tx_bd_num - needed_rxbd_num)) {
	...
	pRxBdHdl->isPktEnd = 0;
	SDIO_INCR_RING_IDX(pSPDIODev->RXBDWPtr, pSPDIODev->host_tx_bd_num);
...
void SPDIO_TxBd_DataReady_DeviceRx(IN PSPDIO_ADAPTER pSPDIODev, spdio_device_rx_done_cb_ptr spdio_device_rx_done_cb)
{
	...
	do {
		...
		if ((pTxDesc->txpktsize + pTxDesc->offset) <= pSPDIODev->device_rx_bufsz) {
			...
			DCache_Invalidate((u32)(pTXBD->Address + pTxDesc->offset), pTxDesc->txpktsize);
			ret = spdio_device_rx_done_cb(pSPDIODev, pTxBdHdl->dev_rx_buf, (u8 *)(pTXBD->Address + pTxDesc->offset), pTxDesc->txpktsize, pTxDesc->type);
			if (ret == RTK_FAIL) {
			...
			// Noneed to change the device rx buffer address.
			DCache_CleanInvalidate(pTxBdHdl->dev_rx_buf->buf_addr, pTxBdHdl->dev_rx_buf->size_allocated);
		...
		if (RTK_SUCCESS != ret) {
			...
			SDIO_TXBD_RPTR_Set(SDIO_WIFI, pSPDIODev->TXBDRPtr);
		}
...
void SPDIO_Device_Init(PSPDIO_ADAPTER pSPDIODev)
{
	...
	pSPDIODev->RXBDWPtr = SDIO_RXBD_RPTR_Get(SDIO_WIFI);
