...
#define MASK_DDR_EN                ((u32)0x00000007 << 13)          /*!<R/W 0x0  Indicates the DDR mode in CMD_CH/DATA_CH/ADDR_CH. CTRLR0[15]: CMD_CH (always 2-Byte CMD type) CTRLR0[14]: DATA_CH CTRLR0[13]: ADDR_CH 0: Disable 1: Enable Note: Available only if configuration of DDR_EN is defined. */
 ...
 * @{
 **/
#define MASK_CUR_GP                ((u32)0x00000003 << 10)          /*!<R 0x0  Only used in frequency change function. Used to identify which group register data is internal used. When frqc_req and frqc_ack are both high, CUR_GP will switch to the group which in GP_NUM. */
...
#define BIT_SREX                   ((u32)0x00000001 << 3)          /*!<R/W 0x0  Set to block auto command before device can be normal accessed after exiting sleep mode. This bit will hold high until timing check counter reaches the set value in CTRLR0.CK_MTIMES. */
#define BIT_FRQC                   ((u32)0x00000001 << 2)          /*!<R/W 0x0  Set to enable frequency change function. SPIC will chop current transaction and then set frqc_req = 1 after setting this bit. Frquency change is finished when frqc_req and frqc_ack are both high and then the choped transaction will return and continue. Write: 0: Not to activate FRQC 1: Activate FRQC Read: 0: FRQC is still in progress 1: FRQC is done */
 ...
 * @{
 **/
#define BIT_NWEIM                  ((u32)0x00000001 << 16)          /*!<R/W 0x0  NAND Flash write error interrupt mask. 1: spi_nweir_r is not masked. 0: spi_nweir_r is masked. Note: Available only if configuration of NAND_EN is defined. */
#define BIT_STFIM                  ((u32)0x00000001 << 15)          /*!<R/W 0x0  Status FIFO full interrupt mask. 1: spi_stfir is not masked. 0: spi_stfir is masked. Note: Available only if configuration of NAND_EN is defined. */
#define BIT_STOIM                  ((u32)0x00000001 << 14)          /*!<R/W 0x0  Status FIFO overflow interrupt masked. 1: spi_stoir_r is not masked. 0: spi_stoir_r is masked. Note: Available only if configuration of NAND_EN is defined. */
#define BIT_STUIM                  ((u32)0x00000001 << 13)          /*!<R/W 0x0  Status FIFO underflow interrupt masked. 1: spi_stuir_r is not masked. 0: spi_stuir_r is masked. Note: Available only if configuration of NAND_EN is defined. */
 ...
 * @{
 **/
#define BIT_NWEIS                  ((u32)0x00000001 << 16)          /*!<R 0x0  NAND Flash write error interrupt status raw interrupt status after masking. 1: spi_nweir_r is active after masking. 0: spi_nweir_r is not active after masking. Note: Available only if configuration of NAND_EN is defined. */
#define BIT_STFIS                  ((u32)0x00000001 << 15)          /*!<R 0x0  Status FIFO full raw interrupt status after masking. 1: spi_stfir_r is active after masking. 0: spi_stfir_r is not active after masking. Note: Available only if configuration of NAND_EN is defined. */
#define BIT_STOIS                  ((u32)0x00000001 << 14)          /*!<R 0x0  Status FIFO overflow interrupt status after masking. 1: spi_stoir_r is active after masking. 0: spi_stoir_r is not active after masking. Note: Available only if configuration of NAND_EN is defined. */
#define BIT_STUIS                  ((u32)0x00000001 << 13)          /*!<R 0x0  Status FIFO underflows interrupt status after masking. 1: spi_stuir_r is active after masking. 0: spi_stuir_r is not active after masking. Note: Available only if configuration of NAND_EN is defined. */
 ...
 * @{
 **/
#define MASK_WR_EN_CMD_2ND_BYTE    ((u32)0x000000FF << 8)          /*!<R/W 0x06  Indicates SPI Flash command value of 2nd byte of write enable. Note: Available only if configuration of DDR_EN is defined. */
 ...
 * @{
 **/
#define BIT_INTERVAL_EN            ((u32)0x00000001 << 31)          /*!<R/W 0x0  Set to enable INTERVAL_ODD. Note: Available only if configuration of SPIC_LITE is not defined (SPIC Full ver). */
#define BIT_INTERVAL_ODD           ((u32)0x00000001 << 30)          /*!<R/W 0x0  Indicates SPI Flash read status with 2-Byte status but checking only odd byte or even byte. 0: even byte (2, 4, 6, ...) 1: odd byte (1, 3, 5, ...) Note: Available only if configuration of SPIC_LITE is not defined (SPIC Full ver). */
#define MASK_ST_CMD_LEN            ((u32)0x00000003 << 28)          /*!<R/W 0x1  Indicates the number of bytes in read status command. ST_CMD_LEN can be 1~3. Note: Available only if configuration of SPIC_LITE is not defined (SPIC Full ver). */
...
#define BIT_ST_CMD_DDR_EN          ((u32)0x00000001 << 25)          /*!<R/W 0x0  Indicates the DDR mode in ST_CMD_CH. Note: Available only if configuration of DDR_EN is defined. */
#define MASK_RD_ST_CMD_3RD_BYTE    ((u32)0x000000FF << 16)          /*!<R/W 0x00  Indicates SPI Flash command value of 3rd byte of read status. Note: Available only if configuration of SPIC_LITE is not defined (SPIC Full ver). */
#define RD_ST_CMD_3RD_BYTE(x)      (((u32)((x) & 0x000000FF) << 16))
#define GET_RD_ST_CMD_3RD_BYTE(x)  ((u32)(((x >> 16) & 0x000000FF)))
#define MASK_RD_ST_CMD_2ND_BYTE    ((u32)0x000000FF << 8)          /*!<R/W 0x05  Indicates SPI Flash command value of 2nd byte of read status. Note: Available only if configuration of SPIC_LITE is not defined (SPIC Full ver). */
...
#define BIT_DIS_WRAP_ALIGN         ((u32)0x00000001 << 15)          /*!<R/W 0x0  Set to disable wrap_align function (convert wrap to incremental through aligning start address to burst boundary) in auto mode. If DIS_WRAP_ALIGN = 1, SPIC will chop wrap transation into two incremental transactions. */
...
#define BIT_FULL_WR                ((u32)0x00000001 << 13)          /*!<R/W 0x1  Set to enable full write in auto mode. If FULL_WR = 0, SPIC will use spi_dm to do the partial write. Note: Available only if configuration of DM_EN is defined. */
#define BIT_DM_ACT                 ((u32)0x00000001 << 12)          /*!<R/W 0x1  DM (Data Mask) is active high or low, which is defined by specific PSRAM SPEC. If DM is active high (DM=1 means "don't write"), DM_ACT should be set to 1 and the corresponding byte of write data will not be written into the PSRAM. Note: Available only if configuration of DM_EN is defined. */
...
#define BIT_DR_FIXED               ((u32)0x00000001 << 3)          /*!<R/W 0x0  Indicate the AXI FIXED read mode on DR. If setting 1, user mode read DR will be FIXED. Otherwise, user mode read DR will be INCR. */
 ...
 * @{
 **/
#define MASK_FLASH_SIZE            ((u32)0x0000001F << 0)          /*!<R/W FLASH_MEM_SIZE - 12  Indicates the size of flash to select the target SPI Flash in auto mode. The number is an exponent of 2 (Flash size) -12 (the smallest flash size is 4kB). Ex: If flash size is 4MB (22-bit), set FLASH_SIZE = 10 (22-12). */
 ...
 * @{
 **/
#define BIT_DATA_UNIT_4B           ((u32)0x00000001 << 14)          /*!<R/W 0x0  Set (1) when the SPI device stores 4-byte data for each address. */
...
#define BIT_DATA_UNIT_2B           ((u32)0x00000001 << 12)          /*!<R/W 0x0  Set (1) when the SPI device stores 2-byte data for each address. Note: Available only if configuration of PSRAM_EN is defined. */
#define BIT_JEDEC_P2CMF            ((u32)0x00000001 << 11)          /*!<R/W 0x0  Set (1) when the SPI cmd is JEDEC Profile 2.0 Command Modifier Formats. (6-byte command and address) Note: Available only if configuration of PSRAM_EN is defined. */
#define BIT_PSRAM                  ((u32)0x00000001 << 10)          /*!<R/W 0x0  Set (1) when the SPI device is PSRAM. PSRAM will disable WEN and RDSR in auto write. Note: Available only if configuration of PSRAM_EN is defined. */
#define BIT_NAND_FLASH             ((u32)0x00000001 << 9)          /*!<R/W 0x0  Set (1) when the SPI device is NAND Flash. (not support yet) Note: Available only if configuration of NAND_EN is defined. */
#define BIT_NOR_FLASH              ((u32)0x00000001 << 8)          /*!<R/W 0x1  Set (1) when the SPI device is NOR Flash. */
#define MASK_ATOM_SIZE             ((u32)0x00000003 << 6)          /*!<R 0x0  Set (0) when the atomic size of SPI device is 1-byte. Set (1) when the atomic size of SPI device is 2-byte. (Usually using when OPI and DDR are enabled) Set (2) when the atomic size of SPI device is 4-byte. (Usually using when DQ16 and DDR are enabled) Set (3): reserved */
...
#define MASK_CS_ACTIVE_HOLD        ((u32)0x0000000F << 12)          /*!<R/W 0x00  For Flash chip select active hold time after SCLK rising edge (refer to tSLCH/CHSH, tCSS/tCSH) Note: Wait most 16 bus_clk cycles before CS inactive. (Implement CS_ACTIVE_Hold timing only, CS_ACTIVE_Setup timing using D-PHY to shift) */
 ...
 * @{
 **/
#define MASK_WR_DUMMY_LENGTH       ((u32)0x00000FFF << 0)          /*!<R/W 0x0  Indicates delay cycles for transmitting data in auto mode. It is referenced by bus_clk. Note: Available only if configuration of PSRAM_EN is defined. */
 ...
 * @{
 **/
#define MASK_CR_TPWR               ((u32)0x000000FF << 16)          /*!<R/W 0x40  It is used to configure the cycle counts before spi_csn becomes low when spi_pwr turns from low to high */
#define CR_TPWR(x)                 (((u32)((x) & 0x000000FF) << 16))
#define GET_CR_TPWR(x)             ((u32)(((x >> 16) & 0x000000FF)))
#define MASK_CR_IDLE_WINDOW        ((u32)0x000000FF << 8)          /*!<R/W 0x10  It is used to configure the cycle counts before turn off the spi_pwr when SPIC is IDLE. */
#define CR_IDLE_WINDOW(x)          (((u32)((x) & 0x000000FF) << 8))
#define GET_CR_IDLE_WINDOW(x)      ((u32)(((x >> 8) & 0x000000FF)))
#define MASK_CR_ACTIVE_SETUP       ((u32)0x0000000F << 0)          /*!<R/W 0x1  It is used to configure the setup time before the first edge (rising edge for mode0 and falling edge for mode3) of spi_sclk(refer to tCSS or tSLCH ). Note: It can't be configured 0x0 (if you configure 0x0 to this 4 bits field, this field will not be updated). */
#define CR_ACTIVE_SETUP(x)         (((u32)((x) & 0x0000000F) << 0))
#define GET_CR_ACTIVE_SETUP(x)     ((u32)(((x >> 0) & 0x0000000F)))
/** @} */
 ...
 * @{
 **/
#define MASK_PAGE_RD_ADDR_LEN      ((u32)0x00000003 << 18)          /*!<R/W 0x3  Indicates number of bytes in address phase in page read command. Note: Available only if configuration of NAND_EN is defined. */
#define PAGE_RD_ADDR_LEN(x)        (((u32)((x) & 0x00000003) << 18))
#define GET_PAGE_RD_ADDR_LEN(x)    ((u32)(((x >> 18) & 0x00000003)))
#define MASK_PAGE_RD_CH            ((u32)0x00000003 << 16)          /*!<R/W 0x0  Indicates channel number of command and address phase in transmitting page read command. 0: single channel 1: dual channels 2: quad channels 3: octal channel Note: Available only if configuration of NAND_EN is defined. */
#define PAGE_RD_CH(x)              (((u32)((x) & 0x00000003) << 16))
#define GET_PAGE_RD_CH(x)          ((u32)(((x >> 16) & 0x00000003)))
#define MASK_PAGE_RD_CMD           ((u32)0x000000FF << 0)          /*!<R/W 0x13  Indicates SPI Flash command value of page read command Note: Available only if configuration of NAND_EN is defined. */
...
typedef struct {
	...
	__IO uint32_t RSVD1[15];                              /*!<  Reserved,  Address offset:0x144-0x17C */
	union {
		__IO uint8_t  BYTE;
		__IO uint16_t HALF;
		__IO uint32_t WORD;
...
typedef struct {
	...
	u32 FLASH_cur_cmd; 			/*!< Specifies the current read cmd which is used to read data from flash in current bitmode. */
...
/* Exported functions --------------------------------------------------------*/
/* FLASH_Exported_Functions FLASH Exported Functions */
...
_LONG_CALL_ u32 FLASH_CalibrationPLL_GetSrc(void);
_LONG_CALL_ u32 FLASH_PLLInit_ClockDiv(FlashDivInt_E Div);
_LONG_CALL_ void FLASH_Calibration_PLL_Open(void);
_LONG_CALL_ void FLASH_Calibration_PLL_Close(void);
...
_LONG_CALL_ u32 FLASH_Read_DataIsRight(void);
_LONG_CALL_ u32 FLASH_CalibrationNew(FLASH_InitTypeDef *FLASH_InitStruct, u8 SpicBitMode, u8 new_calibration_end, u8 CalStep, u8 SpicClkDelay, u8 StartIdx);
_LONG_CALL_ u32 FLASH_CalibrationNewCmd(u32 NewStatus);
_LONG_CALL_ void FLASH_CalibrationPLLPSCmd(u32 NewStatus);
_LONG_CALL_ u32 FLASH_CalibrationPhaseIdx(u8 phase_idx);
_LONG_CALL_ void FLASH_CalibrationPLLPS_Shift(u32 phase_sel);
