...
/** @defgroup GPIO_DR
 * @brief PortA/B data register
 * @{
 **/
#define GPIO_MASK_PORT_DATA            ((u32)0xFFFFFFFF << 0)          /*!<R/W 0  Values written to this register are output on the I/O signals for Portx if the corresponding data direction bits for Portx are set to output mode and the corresponding control bit for Portx is set to software mode. The value read back equals to the last value written to this register. */
#define GPIO_PORT_DATA(x)              (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_PORT_DATA(x)          ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_DDR
 * @brief PortA/B data direction register
 * @{
 **/
#define GPIO_MASK_PORT_DATA_DIRECTION  ((u32)0xFFFFFFFF << 0)          /*!<R/W 0  Values written to this register independently control the direction of the corresponding data bit in Portx. The default direction can be configured as input or output after system reset through the GPIO_DFLT_DIR_x parameter. 0: Input (default) 1: Output */
 ...
 * @brief PortA/B interrupt enable register
 * @{
 **/
 ...
 * @brief PortA/B interrupt polarity register
 * @{
 **/
 ...
 * @brief PortA/B interrupt status register
 * @{
 **/
 ...
 * @brief PortA/B raw interrupt status register
 * @{
 **/
 ...
 * @brief PortA/B interrupt debounce register
 * @{
 **/
 ...
 * @brief PortA/B edge type interrupt clear register
 * @{
 **/
 ...
 * @brief External PortA/B data register
 * @{
 **/
 ...
 * @brief PortA/B level interrupt synchronization
 * @{
 **/
 ...
 * @brief PortA/B GPIO ID code register
 * @{
 **/
 ...
 * @brief PortA/B both edge interrupt register
 * @{
 **/
 ...
 * @brief PortA/B GPIO component version register
 * @{
 **/
#define GPIO_MASK_VER_ID_CODE          ((u32)0xFFFFFFFF << 0)          /*!<R 32'h3231302A  ASCII value for each number in the version, followed by *. For example, 32_30_31_2A represents the version 2.01*. */
...
typedef struct {
	...
	__IO uint32_t GPIO_DR;                                /*!< PORTA/B DATA REGISTER */
	__IO uint32_t GPIO_DDR;                               /*!< PORTA/B DATA DIRECTION REGISTER */
	__IO uint32_t GPIO_CTL;                               /*!< PORTA/B DATA SOURCE REGISTER */
} GPIO_Port_TypeDef;
...
typedef struct {
	...
	__IO uint32_t GPIO_INT_EN;                            /*!< PORTA/B INTERRUPT ENABLE REGISTER,  Address offset: 0x030 */
	__IO uint32_t GPIO_INT_MASK;                          /*!< PORTA/B INTERRUPT MASK REGISTER,  Address offset: 0x034 */
	__IO uint32_t GPIO_INT_TYPE;                          /*!< PORTA/B INTERRUPT TRIGGER TYPE REGISTER,  Address offset: 0x038 */
	__IO uint32_t GPIO_INT_POLARITY;                      /*!< PORTA/B INTERRUPT POLARITY REGISTER,  Address offset: 0x03C */
	__I  uint32_t GPIO_INT_STATUS;                        /*!< PORTA/B INTERRUPT STATUS REGISTER,  Address offset: 0x040 */
	__I  uint32_t GPIO_INT_STATUS_RAW;                    /*!< PORTA/B RAW INTERRUPT STATUS REGISTER,  Address offset: 0x044 */
	__IO uint32_t GPIO_DEBOUNCE;                          /*!< PORTA/B INTERRUPT DEBOUNCE REGISTER,  Address offset: 0x048 */
	__O  uint32_t GPIO_INT_EOI;                           /*!< PORTA/B EDGE TYPE INTERRUPT CLEAR REGISTER,  Address offset: 0x04C */
	__I  uint32_t GPIO_EXT_PORT[1];                       /*!< EXTERNAL PORTA/B DATA REGISTER,  Address offset: 0x050-0x050 */
	__IO uint32_t RSVD1[3];                               /*!<  Reserved,  Address offset:0x54-0x5C */
	__IO uint32_t GPIO_ITN_LS_SYNC;                       /*!< PORTA/B LEVEL INTERRUPT SYNCHRONIZATION Register,  Address offset: 0x060 */
	__I  uint32_t GPIO_ID_CODE;                           /*!< PORTA/B GPIO ID CODE REGISTER,  Address offset: 0x064 */
	__IO uint32_t GPIO_INT_BOTHEDGE;                      /*!< PORTA/B BOTH EDGE INTERRUPT REGISTER,  Address offset: 0x068 */
	__I  uint32_t GPIO_VER_ID_CODE;                       /*!< PORTA/B GPIO COMPONENT VERSION REGISTER,  Address offset: 0x06C */
