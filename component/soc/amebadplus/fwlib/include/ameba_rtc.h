...
#define RTC_BIT_PM          ((u32)0x00000001 << 22)          /*!<R/W 0  PM:AM/PM notation *0: AM or 24-hour format *1: PM */
...
#define RTC_MASK_OSEL       ((u32)0x00000003 << 5)          /*!<R/W 0  Output selection. These bits are used to select the flag to be routed to RTC_OUT output. *00: Output disabled *01: Alarm output enabled *10: Clock output is clk_spre (default: 1Hz) *11: Clock output is clk_apre (default: 256Hz) */
...
#define RTC_BIT_SUB1H       ((u32)0x00000001 << 1)          /*!<W 0  Subtract one hour (winter time changes) When this bit is set outside initialization mode, 1 hour is subtracted to the calendar time if the current hour is not 0, and this bit is always read as 0. *0: No effect *1: Subtract one hour to the current time */
#define RTC_SUB1H(x)        ((u32)(((x) & 0x00000001) << 1))
#define RTC_BIT_ADD1H       ((u32)0x00000001 << 0)          /*!<W 0  Add one hour (summer time changes) When this bit is set outside initialization mode, 1 hour is added to the calendar time, and this bit is always read as 0. *0: No effect *1: Add one hour to the current time */
#define RTC_ADD1H(x)        ((u32)(((x) & 0x00000001) << 0))
/** @} */

/** @defgroup RTC_ISR
 * @brief RTC Init Mode and Status Register
 * @{
 **/
...
#define RTC_BIT_DOVTHF      ((u32)0x00000001 << 15)          /*!<RW1C 0  Day over threshold flag This flag is set by hardware when the Day in RTC_TR over the DAY_THRES set in RTC_CR register. */
#define RTC_DOVTHF(x)       ((u32)(((x) & 0x00000001) << 15))
#define RTC_BIT_WUTF        ((u32)0x00000001 << 10)          /*!<RW1C 0  Wakeup timer flag This flag is set by hardware when the wakeup auto-reload counter reaches to. This flag must be cleared by software at least 2 RTCCLK periods before WUTF is set to '1' again. */
#define RTC_WUTF(x)         ((u32)(((x) & 0x00000001) << 10))
#define RTC_BIT_ALMF        ((u32)0x00000001 << 8)          /*!<RW1C 0  Alarm flag This flag is set by hardware when the time register (RTC_TR) matches the Alarm registers (RTC_ALMR1L and RTC_ALMR1H). */
#define RTC_ALMF(x)         ((u32)(((x) & 0x00000001) << 8))
#define RTC_BIT_INIT        ((u32)0x00000001 << 7)          /*!<R/W 0  Initialization mode *0: Free running mode. *1: Initialization mode used to program time and date register (RTC_TR) and prescaler register (RTC_PRER). Counters are stopped and start counting from the new value when INIT is set. */
...
#define RTC_BIT_WUTWF       ((u32)0x00000001 << 2)          /*!<R 0  Wakeup timer write flag This flag is set by hardware when WUT value can be changed, after the WUTE bit has been set to '0' in RTC_CR. *0: Wakeup timer configuration update not allowed except in initialization mode *1: Wakeup timer configuration update allowed */
#define RTC_WUTWF(x)        ((u32)(((x) & 0x00000001) << 2))
#define RTC_BIT_WUTRSF      ((u32)0x00000001 << 1)          /*!<RW1C 0  This bit is set by hardware each time the WUTE bit is copied into the shadow register. This bit is cleared by hardware in initialization mode. This bit can also be cleared by software. It is cleared either by software or by hardware in initialization mode. *0: WUTE is not yet synchronized. *1: WUTE is synchronized. */
...
/** @defgroup RTC_ALMR1L
 * @brief RTC Alarm 1 Register Low
 * @{
 **/
...
#define RTC_BIT_MSK1        ((u32)0x00000001 << 15)          /*!<R/W 0  Alarm minutes mask. *0: Alarm set if the minutes match *1: Minutes don't care in Alarm comparison */
...
#define RTC_BIT_MSK0        ((u32)0x00000001 << 7)          /*!<R/W 0  Alarm seconds mask *0: Alarm set if the seconds match *1: Seconds don't care in Alarm comparison */
...
/** @defgroup RTC_ALMR1H
 * @brief RTC Alarm 1 Register High
 * @{
 **/
#define RTC_BIT_MSK3        ((u32)0x00000001 << 9)          /*!<R/W 0  Alarm day mask *0: Alarm set if the days match *1: Days don't care in Alarm comparison */
 ...
 *****************************************************************************/
typedef struct {
	__IO uint32_t RTC_TR;                                 /*!< RTC TIME REGISTER,  Address offset: 0x000 */
	__IO uint32_t RTC_CR;                                 /*!< RTC CONTROL REGISTER,  Address offset: 0x004 */
	__IO uint32_t RTC_ISR;                                /*!< RTC INIT MODE AND STATUS REGISTER,  Address offset: 0x008 */
	__IO uint32_t RTC_PRER;                               /*!< RTC PRESCALER REGISTER,  Address offset: 0x00C */
	__IO uint32_t RTC_CALIBR;                             /*!< RTC CALIBRATION REGISTER,  Address offset: 0x010 */
	__IO uint32_t RTC_ALMR1L;                             /*!< RTC ALARM 1 REGISTER LOW Register,  Address offset: 0x014 */
	__IO uint32_t RTC_ALMR1H;                             /*!< RTC ALARM 1 REGISTER HIGH Register,  Address offset: 0x018 */
	__IO uint32_t RTC_WPR;                                /*!< RTC WRITE PROTECTION REGISTER,  Address offset: 0x01C */
	__IO uint32_t RTC_YEAR;                               /*!< RTC YEAR REGISTER,  Address offset: 0x020 */
	__IO uint32_t RTC_WUTR;                               /*!< RTC WAKEUP TIMER REGISTER,  Address offset: 0x024 */
