...
#define LEDC_BIT_EN                        ((u32)0x00000001 << 0)          /*!<R/W/ES 0x0  LEDC enable bit. 0: Disable 1: Enable This bit enable indicates that LEDC can be started when LEDC data has finished transmission, or this bit is cleared to 0 by hardware when software sets LEDC_SOFT_RESET. Software clearing LEDC_EN when LEDC FSM is not IDLE doesn't affect data transfer. */
/** @} */
 ...
 * @{
 **/
#define LEDC_BIT_WAIT_TIME0_EN             ((u32)0x00000001 << 9)          /*!<R/W 0  Wait time0 enable. When setting this bit, the controller automatically inserts waiting time before next LED package data. 0: Disable 1: Enable */
 ...
 * @{
 **/
#define LEDC_BIT_WAIT_TIME1_EN             ((u32)0x00000001 << 31)          /*!<R/W 0  Wait time1 enable. When setting this bit, the controller automatically inserts waiting time before next LED frame data. 0: Disable 1: Enable */
 ...
 * @{
 **/
#define LEDC_MASK_DATA                     ((u32)0x00FFFFFF << 0)          /*!<WPD 0  LEDC display data, the lower 24-bit is valid. Writ this register means push data to LEDC FIFO. Register LEDC_EN toggling from 0 to 1 will clear LEDC FIFO. Thus, only data pushed when LEDC_EN=1 will be send to LED. */
 ...
 * @brief LED FIFO Status Register
 * @{
 **/
...
#define LEDC_BIT_LED_TRANS_FINISH_INT      ((u32)0x00000001 << 0)          /*!<RW1C 0  When the data configured as TOTAL_DATA_LENGTH has been transferred complete, this bit is set. Clear 0 when software sets LEDC_SOFT_RESET. 0: Data transfer has not completed. 1: Data transfer has completed. */
/** @} */

/** @defgroup LEDC_DEBUG_SEL_REG
 * @brief LED Debug Select Register
 * @{
 **/
...
#define LEDC_MASK_DEBUG_SEL_FIFO           ((u32)0x0000000F << 3)          /*!<R/W 0  FIFO module debug out select. [6:5]: select debug_data of ledc_txfifo, valid range is 0 or 1. [4:3]: select output data byte when bit[6:5] is 0. 2'b00: byte0 2'b01: byte1 2'b1x: byte2 */
...
typedef struct {
	...
	__IO uint32_t LEDC_LED_INT_STS_REG;                   /*!< LED FIFO STATUS REGISTER,  Address offset: 0x024 */
	__IO uint32_t RSVD0[2];                               /*!<  Reserved,  Address offset:0x28-0x2C */
	__IO uint32_t LEDC_DEBUG_SEL_REG;                     /*!< LED DEBUG SELECT REGISTER,  Address offset: 0x030 */
