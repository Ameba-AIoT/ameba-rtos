...
typedef struct {
	...
	uint16_t flash_density    : 6;	/* NOR: 64Mb, 128Mb, 256Mb
                      NAND: 1Gb, 2Gb */
	uint16_t flash_voltage    : 2;	/* MCM_NOR_FLASH_1V8, MCM_NOR_FLASH_3V3 */
	uint16_t rsvd0            : 2;
 ...
 * @param voltage The voltage of the flash memory, such as MCM_NOR_FLASH_1V8
 *                or MCM_NOR_FLASH_3V3 or MCM_FLASH_VOLTAGE_INVALID.
 *
 * @return A MCM_MemTypeDef structure initialized with Flash settings.
 *
 * @example
 * MCM_MEM_FLASH_ONLY(MCM_TYPE_NAND_FLASH, MCM_NAND_FLASH_SIZE_1Gb, MCM_FLASH_VOLTAGE_INVALID)
 * MCM_MEM_FLASH_ONLY(MCM_TYPE_NOR_FLASH, MCM_NOR_FLASH_SIZE_32Mb, MCM_NOR_FLASH_3V3)
 */
#define MCM_MEM_FLASH_ONLY(type, size, voltage) \
   {                                  \
     .mem_type = type,              \
     .flash_density = size,         \
     .flash_voltage = voltage,      \
     .rsvd0 = 0,                    \
     .dram_info = {0}               \
   }
...
/** @defgroup CHIPINFO_FLASHVOLTAGE_Enumeration_Type Enumeration Type
  * @{
  */
enum MCM_FlashVoltage {
	MCM_FLASH_VOLTAGE_INVALID = 0,
	MCM_NOR_FLASH_1V8,
	MCM_NOR_FLASH_3V3,
};
