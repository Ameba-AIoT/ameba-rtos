 ...
 * @{
 **/
#define COMP_BIT_IT_DET_MOD              ((u32)0x00000001 << 31)          /*!<R/W 0h  This bit controls the interrupt bit_comp_wk* operation. comparison criterion matching is rising edge or not. 0: The detection mode is level detection. when the comparison criterion is match, bit_comp_wk* will send interrupt continuously. 1: The detection mode is edge detection. when the comparison criterion is match, bit_comp_wk* will send interrupt once at the time of matching. */
 ...
 * @{
 **/
#define COMP_MASK_LAST_CH                ((u32)0x00000003 << 0)          /*!<R 0h  Last channel used */
 ...
 * @{
 **/
#define COMP_MASK_DATA_VALID_CNT         ((u32)0x000000FF << 24)          /*!<R/W DCh  This field defines how long the comparator analog module data is valid for digital module after analog module works. It's about 55us. */
#define COMP_DATA_VALID_CNT(x)           (((u32)((x) & 0x000000FF) << 24))
#define COMP_GET_DATA_VALID_CNT(x)       ((u32)(((x >> 24) & 0x000000FF)))
#define COMP_MASK_CHSW_CNT               ((u32)0x000000FF << 16)          /*!<R/W DCh  This field defines how long the channel switch can jump to next channel at automatic channel switch mode. It's about 55us. */
#define COMP_CHSW_CNT(x)                 (((u32)((x) & 0x000000FF) << 16))
#define COMP_GET_CHSW_CNT(x)             ((u32)(((x >> 16) & 0x000000FF)))
#define COMP_MASK_EXT_WK_SHUT_CNT        ((u32)0x0000FFFF << 0)          /*!<R/W 12Ch  This field defines how long to disable comparator analog module when an external trigger event is sent to the comparator. The count must include the comparator response time to get a correct compare result.It's about 75us. */
#define COMP_EXT_WK_SHUT_CNT(x)          (((u32)((x) & 0x0000FFFF) << 0))
#define COMP_GET_EXT_WK_SHUT_CNT(x)      ((u32)(((x >> 0) & 0x0000FFFF)))
/** @} */

/** @defgroup COMP_CHSW_TIM_DIS
 * @brief Comparator Channel Switch Time Disable Register
 * @{
 **/
#define COMP_BIT_CH30SWTIME_DIS          ((u32)0x00000001 << 3)          /*!<R/W 0h  This field defines whether disable the channel switch time(bit_comp_chsw_cnt) from channel list 3 to 0, only valid in auto mode */
#define COMP_BIT_CH23SWTIME_DIS          ((u32)0x00000001 << 2)          /*!<R/W 0h  This field defines whether disable the channel switch time(bit_comp_chsw_cnt) from channel list 2 to 3, only valid in auto mode */
#define COMP_BIT_CH12SWTIME_DIS          ((u32)0x00000001 << 1)          /*!<R/W 0h  This field defines whether disable the channel switch time(bit_comp_chsw_cnt) from channel list 1 to 2, only valid in auto mode */
#define COMP_BIT_CH01SWTIME_DIS          ((u32)0x00000001 << 0)          /*!<R/W 0h  This field defines whether disable the channel switch time(bit_comp_chsw_cnt) from channel list 0 to 1, only valid in auto mode */
/** @} */
...
#define COMP_BIT_CONTINUOUS_EN           ((u32)0x00000001 << 30)          /*!<R/W 0h  enable comparator continuous sample function. continuous sample function can only take effect in autoswitch mode with autoshut is disabled, and 4 channels in list are must configured into same channel */
...
#define COMP_MASK_VS_DBC_CNT             ((u32)0x0000FFFF << 0)          /*!<R/W 0h  hysteresis debounce counter for ZCD */
#define COMP_VS_DBC_CNT(x)               (((u32)((x) & 0x0000FFFF) << 0))
#define COMP_GET_VS_DBC_CNT(x)           ((u32)(((x >> 0) & 0x0000FFFF)))
/** @} */
 ...
 * @{
 **/
#define COMP_MASK_VH_DBC_CNT             ((u32)0x0000FFFF << 16)          /*!<R/W 0h  Vref high level debounce counter for ZCD */
#define COMP_VH_DBC_CNT(x)               (((u32)((x) & 0x0000FFFF) << 16))
#define COMP_GET_VH_DBC_CNT(x)           ((u32)(((x >> 16) & 0x0000FFFF)))
#define COMP_MASK_VL_DBC_CNT             ((u32)0x0000FFFF << 0)          /*!<R/W 0h  Vref low level debounce counter for ZCD */
#define COMP_VL_DBC_CNT(x)               (((u32)((x) & 0x0000FFFF) << 0))
#define COMP_GET_VL_DBC_CNT(x)           ((u32)(((x >> 0) & 0x0000FFFF)))
/** @} */
/** @} */
...
typedef struct {
	...
	__I  uint32_t COMP_LAST_CH;                           /*!< COMPARATOR LAST CHANNEL REGISTER,  Address offset: 0x020 */
	...
	__IO uint32_t COMP_CHSW_TIM_DIS;                      /*!< COMPARATOR CHANNEL SWITCH TIME DISABLE REGISTER,  Address offset: 0x048 */
...
/**
  ...
  */
typedef struct {
	u8 CMP_ChIndex;			/*!< Specify the channel index */

	u8 CMP_Ref0;			/*!< Specify the internal reference voltage0, the value can be 0~31,
									Vref0 = CMP_Ref0*0.1v */
	u8 CMP_Ref1;			/*!< Specify the internal reference voltage1, the value can be 0~31,
									Vref1 = CMP_Ref1*0.1v */
	u8 CMP_WakeType;		/*!< Specify this channel wakeup system or ADC when criterion matches.
									This parameter can be a value or combination of
									@ref COMP_Compare_Wakeup_Type */
	u8 CMP_WakeSysCtrl;		/*!< Specify the criteria of when comparator channel should wakeup
									system, which can be a value of
									@ref COMP_Wakeup_Control */
	u8 CMP_WakeADCCtrl;		/*!< Specify the criteria of when comparator channel should wakeup
...
typedef struct {
	...
	CMP_CHTypeDef CMP_ChanCtrl[4];  // Specify the comparator channel control parameters
...
#define COMP_LEVEL_DETECT					((u8)0x00)
#define COMP_EDGE_DETECT					((u8)0x01)
...
#define IS_COMP_VS_BC_CNT(x)    ((x) < 0x10000)
#define IS_COMP_VH_BC_CNT(x)    ((x) < 0x10000)
#define IS_COMP_VL_BC_CNT(x)    ((x) < 0x10000)
/**
  * @}
  */
...
_LONG_CALL_ void CMP_SetCompMode(u8 det_mode);
