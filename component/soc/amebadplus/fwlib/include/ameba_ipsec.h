...
#define IPSEC_BIT_PK_UP                       ((u32)0x00000001 << 30)          /*!<R/W 1  Packet base update wptr to engine. If pk_up =1, the total number of source descriptor in one packet can't be over 16, and the total number of destination descriptor in one packet can't be over 8. */
#define IPSEC_MASK_SRC_FAIL_STATUS            ((u32)0x00000003 << 25)          /*!<R 0  Source descriptor failed status. 2'd1: Users write the first word twice consecutively 2'd2: Users write the second word directly without writing first word in the beginning 2'd3: Overflow (Detect users try to write source descriptor, but there isn't available FIFO node to use). */
...
#define IPSEC_MASK_SRPTR                      ((u32)0x000000FF << 16)          /*!<R 0  Source descriptor FIFO read pointer. When engine reads a descriptor and finishes it, SRPTR = SRPTR+2. */
...
#define IPSEC_MASK_SRC_DES_FIFO_EMPTY_CNT     ((u32)0x000000FF << 0)          /*!<R 0  Source descriptor FIFO empty counter. Use this field to check how many available FIFO nodes in source descriptor FIFO could use. */
#define IPSEC_SRC_DES_FIFO_EMPTY_CNT(x)       (((u32)((x) & 0x000000FF) << 0))
#define IPSEC_GET_SRC_DES_FIFO_EMPTY_CNT(x)   ((u32)(((x >> 0) & 0x000000FF)))
/** @} */
 ...
 * @{
 **/
#define IPSEC_BIT_RST                         ((u32)0x00000001 << 31)          /*!<R/W 0  IPsec engine reset. Write '1' to reset the crypto engine and DMA engine (used to clear fatal error). */
...
#define IPSEC_MASK_OK_INTR_CNT                ((u32)0x000000FF << 8)          /*!<R 0  Ok interrupt counter. Reading this field can know how many interrupts are coming to notify that crypto engine has calculated cryptographic feature results. Use this field in interrupt counter mode. */
#define IPSEC_OK_INTR_CNT(x)                  (((u32)((x) & 0x000000FF) << 8))
#define IPSEC_GET_OK_INTR_CNT(x)              ((u32)(((x >> 8) & 0x000000FF)))
#define IPSEC_BIT_INTR_MODE                   ((u32)0x00000001 << 7)          /*!<R/W 0  Select ok interrupt mode: 1'd0: Interrupt mode is general mode. 1'd1: Interrupt mode is counter mode. · General mode means no matter how many interrupts are coming to notify that crypto engine has calculated cryptographic feature results. There is always one interrupt signal shows in cmd_ok field, and nothing in ok_intr_cnt. So if the programmer want to clear this signal, just write '1' to cmd_ok. · Counter mode means the programmer can know how many interrupts are coming to notify that crypto engine has calculated cryptographic feature results from ok_intr_cnt. The programmer also can know there are at least one interrupt from reading cmd_ok. So if the programmer want to clear ok_intr_cnt signals, need to write ok_intr_cnt into clear_ok_int_num, then write '1' to cmd_ok. */
#define IPSEC_BIT_CMD_OK                      ((u32)0x00000001 << 4)          /*!<R/W 0  Command ok Interrupt. Reading this field to detect whether crypto engine has calculated a cryptographic feature result. Even if interrupt mode is counter mode, still can use this field to detect whether an interrupt is coming. Write '1' to clear this interrupt signal. */
#define IPSEC_BIT_DMA_BUSY                    ((u32)0x00000001 << 3)          /*!<R 0  Detect whether IPsec DMA is busy: 1'd0: DMA is not busy 1'd1: DMA is busy Used for debug to avoid data coherence issue (when user gets crypto finish calculated interrupt, user can make sure DMA engine has already move data to result buffer.) */
...
#define IPSEC_BIT_ARBITER_MODE                ((u32)0x00000001 << 16)          /*!<R/W 1  DMA arbiter mode: 1'd0: round-robin 1'd1: detect fifo wasted level There are 2 FIFOs in DMA, that are read-FIFO and write-FIFO. Detecting FIFO wasted level means that DMA will process which FIFO first depends on which wasted level is high. Because the wasted level is high means there are many FIFO nodes waiting to be processed in this FIFO, so DMA must process them in hurry. */
#define IPSEC_MASK_DMA_WAIT_CYCLE             ((u32)0x0000FFFF << 0)          /*!<R/W 0  Setting DMA wait cycles to assert next DMA request */
 ...
 * @{
 **/
#define IPSEC_BIT_LX_FAIL_3                   ((u32)0x00000001 << 18)          /*!<RW1C 0  When a burst done, burst length != burst cnt. Write “1” to clear this error signal. */
#define IPSEC_BIT_LX_FAIL_2                   ((u32)0x00000001 << 17)          /*!<RW1C 0  When IPsec is working, rdata from bus != rdata of ipsec or wdata of ipsec != wdata on bus Write “1” to clear this error signal. */
#define IPSEC_BIT_LX_FAIL_1                   ((u32)0x00000001 << 16)          /*!<RW1C 0  LX bus timing issue Write “1” to clear this error signal. */
...
#define IPSEC_BIT_DES_ERR3                    ((u32)0x00000001 << 13)          /*!<RW1C 0  Destination Descriptor Error 3 Interrupt. One of the error condition is that the sum of enl is greater than Encryption Total Length. The other error condition is about destination descriptor length error ((enl(Encryption length)==0) & (adl(authentication data length)==0)). Write '1' to clear this error signal. Sum of enl reference the register REG_IPSCSR_SENLR (offset 0x24). */
...
#define IPSEC_BIT_SRC_ERR9                    ((u32)0x00000001 << 9)          /*!<RW1C 0  Source Descriptor Error 9 Interrupt. The error condition is that sum of epl is not equal to EPTL (Encryption Padding Total Length). Write '1' to clear this error signal. Sum of epl reference the register REG_IPSCSR_SEPLR (offset 0x2C). */
#define IPSEC_BIT_SRC_ERR8                    ((u32)0x00000001 << 8)          /*!<RW1C 0  Source Descriptor Error 8 Interrupt. The error condition is that sum of apl is not equal to HPTL (Hash Padding Total Length). Write '1' to clear this error signal. Sum of apl reference the register REG_IPSCSR_SAPLR (offset 0x28). */
#define IPSEC_BIT_SRC_ERR7                    ((u32)0x00000001 << 7)          /*!<RW1C 0  Source Descriptor Error 7 Interrupt. The error condition is that sum of enl is not equal to ETL (Encryption Total Length). Write '1' to clear this error signal. Sum of enl reference the register REG_IPSCSR_SENLR (offset 0x24). */
#define IPSEC_BIT_SRC_ERR6                    ((u32)0x00000001 << 6)          /*!<RW1C 0  Source Descriptor Error 6 Interrupt. The error condition is that sum of a2eo is not equal to HTL (Header Total Length). Write '1' to clear this error signal. Sum of a2eo reference the register REG_IPSCSR_SAADLR (offset 0x20). */
#define IPSEC_BIT_SRC_ERR5                    ((u32)0x00000001 << 5)          /*!<RW1C 0  Source Descriptor Error 5 Interrupt. The error condition is that there is a source descriptor, but the data length ((a2eo/epl) +(enl) + (apl)) is 0. Write '1' to clear this error signal. */
...
#define IPSEC_BIT_SRC_ERR3                    ((u32)0x00000001 << 3)          /*!<RW1C 0  Source Descriptor Error 3 Interrupt. About source descriptor length error. One of the error condition is setting the first descriptor but (CL!=3). The other error condition is that there is a source descriptor but CL, KL, IL, PL, SHIVL are all 0. Write '1' to clear this error signal. */
...
#define IPSEC_BIT_SRC_ERR0                    ((u32)0x00000001 << 0)          /*!<RW1C 0  Source Descriptor Error 0 Interrupt. One of the error condition is that first segment descriptor is not set (FS=0), when descriptor is pointing to the first segment of the packet. The other error condition is that first segment descriptor is set (FS=1), when descriptor is not pointing to the first segment of the packet. Write '1' to clear this error signal. */
/** @} */
 ...
 * @{
 **/
#define IPSEC_MASK_A2EO_SUM                   ((u32)0x000007FF << 0)          /*!<R 0  Sum of additional authentication data length. */
 ...
 * @{
 **/
#define IPSEC_MASK_ENL_SUM                    ((u32)0x00FFFFFF << 0)          /*!<R 0  Sum of encryption data length. */
 ...
 * @{
 **/
#define IPSEC_MASK_APL_SUM                    ((u32)0x00000FFF << 0)          /*!<R 0  Sum of authentication padding data length.[Mix mode uses this field] */
 ...
 * @{
 **/
#define IPSEC_MASK_EPL_SUM                    ((u32)0x0000003F << 0)          /*!<R 0  Sum of encryption padding data length. [Mix mode uses this field] */
...
#define IPSEC_MASK_DMA_BURST_LENGTH           ((u32)0x0000003F << 16)          /*!<R/W 0x10  Set DMA burst length. The maximum setting length is 16 bursts. The minimum setting length is 1 burst. Note: If all bits are zero (6'd0), it means that the maximum burst size or undefined burst size. */
...
#define IPSEC_BIT_MAC_OUT_LITTLE_ENDIAN       ((u32)0x00000001 << 10)          /*!<R/W 0  Output mac is little endian: 1'd0: Big endian 1'd1: Little endian */
#define IPSEC_BIT_DATA_OUT_LITTLE_ENDIAN      ((u32)0x00000001 << 9)          /*!<R/W 0  Output data is little endian: 1'd0: Big endian 1'd1: Little endian */
 ...
 * @{
 **/
#define IPSEC_BIT_EFUSE_KEY_REQ               ((u32)0x00000001 << 31)          /*!<R/W 0  Request of loading key from OTP, cleared by key_rdy */
#define IPSEC_MASK_EFUSE_KEY_ID               ((u32)0x00000003 << 29)          /*!<R/W 0  2'b00: load key from OTP key1 2'b01: load key from OTP key2 2'b10: load key from OTP RSIP key_ecb 2'b11: load key from OTP RSIP key_ctr */
#define IPSEC_EFUSE_KEY_ID(x)                 (((u32)((x) & 0x00000003) << 29))
#define IPSEC_GET_EFUSE_KEY_ID(x)             ((u32)(((x >> 29) & 0x00000003)))
/** @} */

/** @defgroup ENG_DBG
 * @brief Crypto Engine Debug Register
 * @{
 **/
...
/** @defgroup DMA_LX_DBG
 * @brief Crypto Engine DMA LX Bus Debug Register
 * @{
 **/
...
/** @defgroup DMA_RX_DBG
 * @brief Crypto Engine DMA Rx Debug Register
 * @{
 **/
...
/** @defgroup DMA_TX_DBG
 * @brief Crypto Engine DMA Tx Debug Register
 * @{
 **/
...
#define IPSEC_MASK_DRPTR                      ((u32)0x000000FF << 16)          /*!<R 0  Destination descriptor FIFO read pointer. When engine reads a descriptor and finished it, DRPTR = DRPTR+2. */
#define IPSEC_DRPTR(x)                        (((u32)((x) & 0x000000FF) << 16))
#define IPSEC_GET_DRPTR(x)                    ((u32)(((x >> 16) & 0x000000FF)))
#define IPSEC_MASK_DWPTR                      ((u32)0x000000FF << 8)          /*!<R 0  Destination descriptor FIFO write pointer. When writting descriptor to DDSW successfully, DWPTR = DWPTR+2. */
 ...
 * @{
 **/
#define IPSEC_MASK_CFG_PK_ARBITER_MODE        ((u32)0x00000003 << 30)          /*!<R/W 0  Arbiter rule between secure and no-secure access 0: round-robin 1/2/3: decided by the load_count of source descpritor FIFO of secure and no-secure, the more full is the winner. */
#define IPSEC_CFG_PK_ARBITER_MODE(x)          (((u32)((x) & 0x00000003) << 30))
#define IPSEC_GET_CFG_PK_ARBITER_MODE(x)      ((u32)(((x >> 30) & 0x00000003)))
#define IPSEC_MASK_CFG_BUS3_PRIORITY_TH       ((u32)0x00000003 << 28)          /*!<R/W 0  The priority of bus3 (reserved now) */
...
#define IPSEC_MASK_CFG_BUS1_PRIORITY_TH       ((u32)0x00000003 << 24)          /*!<R/W   The priority of bus2-- non-secure access. If 0x100C[31:30]=1, the load_count of source descriptor FIFO is wr_ptr - rd_ptr + 0x100C[25:24]. Otherwise, the load_count of source descriptor FIFO is wr_ptr - rd_ptr. */
#define IPSEC_CFG_BUS1_PRIORITY_TH(x)         (((u32)((x) & 0x00000003) << 24))
#define IPSEC_GET_CFG_BUS1_PRIORITY_TH(x)     ((u32)(((x >> 24) & 0x00000003)))
#define IPSEC_MASK_DBG_DPTR                   ((u32)0x000000FF << 8)          /*!<R/W 0  Destination descriptor FIFO read pointer: Users read this pointer value can know what the ID number of FIFO node that is now pointed by read pointer is. If users want to know what the 32 bits value of a specified FIFO node, users can write an ID number to this read pointer, then get the 32 bits value from Debug Destination Descriptor Data Register. */
#define IPSEC_DBG_DPTR(x)                     (((u32)((x) & 0x000000FF) << 8))
#define IPSEC_GET_DBG_DPTR(x)                 ((u32)(((x >> 8) & 0x000000FF)))
#define IPSEC_MASK_DBG_SPTR                   ((u32)0x000000FF << 0)          /*!<R/W 0  Source descriptor FIFO read pointer: Users read this pointer value can know what the ID number of FIFO node that is now pointed by read pointer is. If users want to know what the 32 bits value of a specified FIFO node, users can write an ID number to this read pointer, then get the 32 bits value from Debug Source Descriptor Data Register. */
...
typedef struct {
	...
	__IO uint32_t SDFWR;                                  /*!< SOURCE DESCRIPTOR FIRST WORD REGISTER,  Address offset: 0x004 */
	...
	__IO uint32_t IPSCSR_DBG;                             /*!< DEBUG REGISTER,  Address offset: 0x018 */
	...
	__I  uint32_t ENG_DBG;                                /*!< CRYPTO ENGINE DEBUG REGISTER,  Address offset: 0x040 */
	__I  uint32_t DMA_LX_DBG;                             /*!< CRYPTO ENGINE DMA LX BUS DEBUG REGISTER,  Address offset: 0x044 */
	__I  uint32_t DMA_RX_DBG;                             /*!< CRYPTO ENGINE DMA RX DEBUG REGISTER,  Address offset: 0x048 */
	__I  uint32_t DMA_TX_DBG;                             /*!< CRYPTO ENGINE DMA TX DEBUG REGISTER,  Address offset: 0x04C */
	...
	__IO uint32_t DUMMY;                                  /*!< DUMMY REGISTER,  Address offset: 0x1018 */
} IPSEC_TypeDef;
/** @} */
/* AUTO_GEN_END */
...
typedef struct {
	...
	u32 state[16] __attribute__((aligned(32)));            /*!< intermediate digest state  */
	u8 isSHA2;						/*SHA2									*/
	u8 sha2type;					/*SHA2 type: SHA2_224/SHA2_256			*/
...
/** @defgroup CRYPTO_Length
  * @{
  */
#define CRYPTO_MAX_DIGEST_LENGTH	64  // SHA512 Digest length : 64
...
#define crypto_dbg_mem_dump(start, size, str_header) \
	if (GET_LOG_MASK(LEVEL_INFO) & BIT(MODULE_IPSEC)) { \
