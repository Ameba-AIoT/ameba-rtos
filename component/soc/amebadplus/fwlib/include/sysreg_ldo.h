...
#ifndef _DPLUS_LDO_H_
#define _DPLUS_LDO_H_
 ...
 * @{
 **/
#define LDO_BIT_REG_32KOSC_DUMMY           ((u32)0x00000001 << 2)          /*!<R/W 1'b0  // [RF][32KOSC] dummy */
#define LDO_BIT_GATED_STUP_OK_D            ((u32)0x00000001 << 1)          /*!<R/W 1'b0  // [RF][32KOSC] dummy */
...
/** @defgroup LDO_04
 * @brief 32_OSC_XTAL_POW
 * @{
 **/
#define LDO_MASK_DMY_1                     ((u32)0x00007FFF << 1)          /*!<R/W 15'd0   */
#define LDO_DMY_1(x)                       (((u32)((x) & 0x00007FFF) << 1))
#define LDO_GET_DMY_1(x)                   ((u32)(((x >> 1) & 0x00007FFF)))
#define LDO_BIT_POW_32KOSC                 ((u32)0x00000001 << 0)          /*!<R/W 1'b1  // [RF][32KOSC] 1'b1 : enable */
/** @} */
 ...
 * @{
 **/
#define LDO_BIT_SEL_LDO_VREF               ((u32)0x00000001 << 15)          /*!<R/W 1'b0  // [RF][32KOSC] REG_CTRL // 0: LDO low valtage setting 1:high voltage */
#define LDO_BIT_GATED_STUP_OK              ((u32)0x00000001 << 14)          /*!<R/W 1'b0  //1’b1 : Close Startup standby current ; //1’b0 : Startup standby current */
#define LDO_MASK_DMY_2                     ((u32)0x000000FF << 6)          /*!<R/W 8'b00000000  // [RF][32KOSC] REG_CTRL // dummy */
#define LDO_DMY_2(x)                       (((u32)((x) & 0x000000FF) << 6))
#define LDO_GET_DMY_2(x)                   ((u32)(((x >> 6) & 0x000000FF)))
#define LDO_MASK_RCAL                      ((u32)0x0000003F << 0)          /*!<R/W 6'b100000  // [RF][32KOSC] REG_CTRL // osc frequency is highier as RCAL's larger */
#define LDO_RCAL(x)                        (((u32)((x) & 0x0000003F) << 0))
#define LDO_GET_RCAL(x)                    ((u32)(((x >> 0) & 0x0000003F)))
/** @} */
 ...
 * @{
 **/
#define LDO_BIT_PSRR_FILTER                ((u32)0x00000001 << 15)          /*!<R/W 1  0: disable PSRR Filter 1: enable PSRR Filter */
#define LDO_BIT_RSV_DIODE                  ((u32)0x00000001 << 14)          /*!<R/W 0  0: disable rsv diode 1: enable rsv diode */
#define LDO_BIT_DIODE                      ((u32)0x00000001 << 13)          /*!<R/W 0  0: disable force diode 1: enable force diode */
#define LDO_BIT_OP_BUFFER                  ((u32)0x00000001 << 12)          /*!<R/W 1  0: PMOS buffer 1: NMOS buffer */
...
#define LDO_BIT_REG_ITAIL_HALF_EN_L_3318   ((u32)0x00000001 << 9)          /*!<R/W 0  // [REGU][LDO_UART] REG_CTRL // LDO iq half ctrl // 0(default): iq normal (for normal mode) // 1: iq half (for suspend mode) */
#define LDO_MASK_REG_STANDBY_L_3318        ((u32)0x00000003 << 7)          /*!<R/W 0  //11: iq=1.5uA; load=1mA~10mA (sleep) //10: iq=45uA; load=10mA(Start Up) (normal-0) //00: iq=45uA; load=5mA~160mA (normal-1) Note:this field controlled by syson pmc. When pmc require mode > 0 , and this field indicate pmc request. When pmc require mode is 0, this field has no use When mldo_tmode is assert , pmc controller is not active */
#define LDO_REG_STANDBY_L_3318(x)          (((u32)((x) & 0x00000003) << 7))
#define LDO_GET_REG_STANDBY_L_3318(x)      ((u32)(((x >> 7) & 0x00000003)))
#define LDO_BIT_EN_DIODE_L_3318            ((u32)0x00000001 << 6)          /*!<R/W 1  // [REGU][LDO_UART] REG_CTRL // PGATE diode clamp (this gating to disable by pd_regu_L) // 0: disable clamp // 1: enable clamp (default) */
#define LDO_MASK_VOADJ_L_3318              ((u32)0x0000000F << 2)          /*!<R/W 1000  // [REGU][LDO_UART] REG_CTRL // LDO VOSEL // 0000, 0001, 0010, 0011: 1.655, 1.672, 1.690, 1.708 // 0100, 0101, 0110, 0111: 1.727, 1.745, 1.764, 1.784 // 1000, 1001(default), 1010, 1011: 1.804, 1.825, 1.846, 1.868 // 1100, 1101, 1110, 1111: 1.890, 1.912, 1.935, 1.959 */
#define LDO_VOADJ_L_3318(x)                (((u32)((x) & 0x0000000F) << 2))
#define LDO_GET_VOADJ_L_3318(x)            ((u32)(((x >> 2) & 0x0000000F)))
#define LDO_BIT_PD_REGU_L_3318             ((u32)0x00000001 << 1)          /*!<R/W 1  // [REGU][LDO_UART] REG_CTRL // PowerDown CTRL // 0: normal-Mode(default) // 1: power-down Mode Note: This field is also controlled by syson pmc . When mldo_tmode is assert , pmc controller is not active */
#define LDO_BIT_EN_PC_BT_L_3318            ((u32)0x00000001 << 0)          /*!<R/W 0  // [REGU][LDO] REG_CTRL // PWRCUT enable // 0: LDO-Mode (default) // 1: PWRCUT-Mode */
/** @} */
 ...
 * @{
 **/
#define LDO_MASK_VOLT_CODE1_ALDO           ((u32)0x0000000F << 28)          /*!<R/W 4'b1000  Aon LDO code1 , which indicate 0.8v */
 ...
 * @{
 **/
#define LDO_MASK_APMC_SP2                  ((u32)0x0000007F << 16)          /*!<R/W 7'd57  Start Point for ST6 up */
#define LDO_APMC_SP2(x)                    (((u32)((x) & 0x0000007F) << 16))
#define LDO_GET_APMC_SP2(x)                ((u32)(((x >> 16) & 0x0000007F)))
#define LDO_MASK_APMC_SP1                  ((u32)0x0000007F << 8)          /*!<R/W 7'd55  Start Point for ST5 up */
 ...
 * @{
 **/
#define LDO_MASK_APMC_SP5                  ((u32)0x0000007F << 16)          /*!<R/W 7'd86  Start Point for ST2 dn */
...
#define LDO_MASK_APMC_SP3                  ((u32)0x0000007F << 0)          /*!<R/W 7'd75  Start Point for ST5 dn */
 ...
 * @{
 **/
#define LDO_MASK_ADBG_REGU                 ((u32)0xFFFFFFFF << 0)          /*!<R 0  REGU interface debug */
...
#define LDO_BIT_OPT_EN_DUMMY               ((u32)0x00000001 << 1)          /*!<R/W 1'b1  option for enable swr dummy load to speed up when pfm -> pwm. if opt_wait_mode is 0 , this bit has no use */
 ...
 *****************************************************************************/
typedef struct {
	__IO uint32_t LDO_TEST_KEY_REG;                    /*!<  Register,  Address offset: 0x0000 */
	__IO uint32_t LDO_RF_EN_DUMMY;                     /*!<  Register,  Address offset: 0x0004 */
	__IO uint32_t LDO_4M_OSC_CTRL;                     /*!<  Register,  Address offset: 0x0008 */
	__IO uint32_t LDO_4M_OSC_CTRL1;                     /*!<  Register,  Address offset: 0x000C */
	__IO uint32_t LDO_32_OSC_XTAL_POW;                 /*!<  Register,  Address offset: 0x0010 */
	__IO uint32_t LDO_32K_OSC_CTRL;                    /*!<  Register,  Address offset: 0x0014 */
	__IO uint32_t LDO_AONLDO;                          /*!<  Register,  Address offset: 0x0018 */
	__IO uint32_t LDO_BOD;                             /*!<  Register,  Address offset: 0x001C */
	__IO uint32_t LDO_3318;                               /*!<  Register,  Address offset: 0x0020 */
	__IO uint32_t LDO_RFAFE_1209;                         /*!<  Register,  Address offset: 0x0024 */
	...
	__IO uint32_t RSVD3[2];                               /*!<  Reserved,  Address offset:0x68-0x6C */
	__I  uint32_t LDO_DEBUG;                              /*!<  Register,  Address offset: 0x070 */
	__IO uint32_t RSVD4[2];                               /*!<  Reserved,  Address offset:0x74-0x78 */
	__IO uint32_t LDO_DUMMY;                              /*!<  Register,  Address offset: 0x07C */
} LDO_TypeDef;

#define LDO_BASE ((LDO_TypeDef *) (0x41008100))
...
#define LDO_MEM_DUMMY_500UA	0x2
#define LDO_MEM_DUMMY_700UA	0x3
