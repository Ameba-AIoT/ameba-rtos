...
void dma_memcpy_init(gdma_t *dma_obj, dma_irq_handler handler, uint32_t id)
{
	...
	dma_obj->index = 0;
	ch_num = GDMA_ChnlAlloc(dma_obj->index, (IRQ_FUN) dma_memcpy_int, (u32)dma_obj, 5);
	if (0xFF == ch_num) {
...
void dma_memcpy(gdma_t *dma_obj, void *dst, void *src, uint32_t len)
{
	...
	printf("%s: ==> Src=%p Dst=%p Len=%lu\r\n", __FUNCTION__, src, dst, (u32)len);
