 ...
 * @param  timer_id: An identifier that is assigned to the timer being created. Typically this would be used in
 *                   the timer callback function to identify which timer expired when the same callback function is
...
/**
 * @brief  Create a new software timer instance. This allocates the storage required by the new timer,
 *         initializes the new timers internal state, and returns a handle by which the new timer can be referenced.
 * @note   Usage example:
 * Create:
...
/**
 * @brief  changes the period of a timer that was previously created using a call to rtos_timer_create.
 * @param  p_handle: The handle of the created timer
