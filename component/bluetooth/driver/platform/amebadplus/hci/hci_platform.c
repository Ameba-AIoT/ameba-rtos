...
void hci_platform_bt_rf_calibration(void)
{
	...
	if (hci_phy_efuse[PEFUSE(0x741)] != 0xFF) {
		HCI_PRINT("hci_phy_efuse[PEFUSE(0x741)] = 0x%x\r\n", hci_phy_efuse[PEFUSE(0x741)]);
	}
...
#if 0
	printf("Read Logic Efuse(offset 0x%x): \n\r", HCI_LGC_EFUSE_OFFSET);
	for (i = 0; i < HCI_LGC_EFUSE_LEN; i++) {
		printf("%02x ", hci_lgc_efuse[i]);
		if ((i + 1) % 16 == 0) {
			printf("\r\n");
		}
	}
	printf("\n\r");
	printf("Read Phy Efuse(offset 0x%x): \n\r", HCI_PHY_EFUSE_OFFSET);
	for (i = 0; i < HCI_PHY_EFUSE_LEN; i++) {
		printf("%02x ", hci_phy_efuse[i]);
		if ((i + 1) % 16 == 0) {
			printf("\r\n");
		}
	}
...
static uint8_t hci_platform_parse_config(void)
{
	...
	uint32_t config_header, config_baudrate;
	...
	while (p < hci_init_config + hci_init_config_len) {
		...
		switch (entry_offset) {
		case 0x000c:
			memcpy((void *)&config_baudrate, (void *)p, sizeof(uint32_t));
			hci_platform_convert_baudrate((uint32_t *)&config_baudrate, &hci_cfg_work_uart_baudrate, 1);
			memcpy((void *)hci_cfg_work_bt_baudrate, (void *)&config_baudrate, sizeof(uint32_t));
			break;
		case 0x0018:
...
static uint8_t hci_platform_get_patch_info(void)
{
	...
	HCI_INFO("FW Version: %ld%ld", version_date, version_time);
...
uint8_t hci_platform_get_patch_cmd_buf(uint8_t *cmd_buf, uint8_t cmd_len)
{
		...
		if (patch_info->sent_fw_len < total_node_len) {
			if (patch_info->sent_fw_len + remain_len < total_node_len) {
				...
				if (node->payload_len != node->sent_payload_len) {
					HCI_ERR("Patch node has not been sent completely! payload_len = %ld, sent_payload_len = %ld", node->payload_len, node->sent_payload_len);
					return HCI_FAIL;
				}
	...
	if (remain_len > 0) {
		if (patch_info->fw_len != patch_info->sent_fw_len) {
			HCI_ERR("Firmware has not been sent completely! fw_len = %ld, sent_fw_len = %ld", patch_info->fw_len, patch_info->sent_fw_len);
			return HCI_FAIL;
		}
