...
{
	if (patch_info->config_buf) {
		osif_mem_free(patch_info->config_buf);
	}
	patch_info->config_buf = NULL;

	if (patch_info->ori_patch_buf) {
		osif_mem_free(patch_info->ori_patch_buf);
	}
	patch_info->ori_patch_buf = NULL;
...
static uint8_t _get_patch_info(uint8_t *p_patch, uint32_t patch_len)
{
	...
	info->ori_patch_buf = osif_mem_alloc(RAM_TYPE_DATA_ON, patch_len);
	memcpy(info->ori_patch_buf, p_patch, patch_len);
	info->patch_buf = info->ori_patch_buf;
	...
	*(info->patch_buf + fw_len - 4) = (uint8_t)(lmp_subversion >> 0);
	*(info->patch_buf + fw_len - 3) = (uint8_t)(lmp_subversion >> 8);
	*(info->patch_buf + fw_len - 2) = (uint8_t)(lmp_subversion >> 16);
	*(info->patch_buf + fw_len - 1) = (uint8_t)(lmp_subversion >> 24);
