...
void bt_zephyr_le_security_changed(struct bt_conn *conn, bt_security_t level, enum bt_security_err err)
{
	...
	/* The encryption changed event may occurred in 2 coditions:
	1. In pairing procedure, link is encrypted to distribute keys, auth_result should not reported in this situation
	    because pairing is still not finished, it will be reported later in pairint_complete/failed.
	2. Link encryption is established using saved LTK(without pairing). Only report auth_result in this situation. */
