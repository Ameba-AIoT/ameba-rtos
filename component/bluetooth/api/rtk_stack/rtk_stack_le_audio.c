...
static uint16_t rtk_stack_bap_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_BAP_DIS_ALL_DONE: {
		...
		BT_LOGD("LE_AUDIO_MSG_BAP_DIS_ALL_DONE: conn_handle 0x%x, pacs_is_found %d, sink_pac_num %d, source_pac_num %d\r\n",
				p_data->conn_handle, p_data->pacs_is_found, p_data->sink_pac_num, p_data->source_pac_num);
		BT_LOGD("ascs_is_found %d, sink_ase_num %d, source_ase_num %d\r\n",
				p_data->ascs_is_found, p_data->sink_ase_num, p_data->source_ase_num);
		BT_LOGD("bass_is_found %d, brs_char_num %d\r\n", p_data->bass_is_found, p_data->brs_char_num);
		...
		if (bap_pacs_get_info(p_data->conn_handle, &pacs_info)) {
			...
			BT_LOGD("PAC info: value_exist 0x%x, sink_pac_num %d, source_pac_num %d, snk_audio_loc 0x%x, src_audio_loc 0x%x\r\n",
					pacs_info.value_exist,
					pacs_info.sink_pac_num,
					pacs_info.source_pac_num,
			...
			BT_LOGD("PAC info: snk_sup_context 0x%x, src_sup_context 0x%x, snk_avail_context 0x%x, src_avail_context 0x%x\r\n",
					pacs_info.snk_sup_context,
					pacs_info.src_sup_context,
					pacs_info.snk_avail_context,
					pacs_info.src_avail_context);
...
static uint16_t rtk_stack_pacs_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_PACS_READ_AVAILABLE_CONTEXTS_IND: {
		...
		BT_LOGD(
			"LE_AUDIO_MSG_PACS_READ_AVAILABLE_CONTEXTS_IND: conn_handle 0x%x, cid=%d,default_sink_available_contexts=0x%x,default_source_available_contexts=0x%x\r\n",
	...
	case LE_AUDIO_MSG_PACS_WRITE_SINK_AUDIO_LOC_IND: {
		...
		BT_LOGD("LE_AUDIO_MSG_PACS_WRITE_SINK_AUDIO_LOC_IND: conn_handle 0x%x, sink_audio_locations 0x%x\r\n",
				p_data->conn_handle,
	...
	case LE_AUDIO_MSG_PACS_WRITE_SOURCE_AUDIO_LOC_IND: {
		...
		BT_LOGD("LE_AUDIO_MSG_PACS_WRITE_SOURCE_AUDIO_LOC_IND: conn_handle 0x%x, source_audio_locations 0x%x\r\n",
				p_data->conn_handle,
...
static uint16_t rtk_stack_pacs_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_PACS_CLIENT_DIS_DONE: { //LE_AUDIO_MSG_GROUP_PACS_CLIENT | 0x00,
		...
		BT_LOGD(
			"LE_AUDIO_MSG_PACS_CLIENT_DIS_DONE: conn_handle 0x%x is_found=%d load_from_ftl=%d,sink_pac_num=%d,source_pac_num=%d,sink_loc_writable=%d,sink_loc_exist=%d,source_loc_writable=%d,source_loc_exist=%d\r\n",
			p_data->conn_handle, p_data->is_found, p_data->load_from_ftl,
			p_data->sink_pac_num, p_data->source_pac_num,
	...
	case LE_AUDIO_MSG_PACS_CLIENT_WRITE_SINK_AUDIO_LOC_RESULT: { //LE_AUDIO_MSG_GROUP_PACS_CLIENT | 0x02,
		...
		APP_PRINT_INFO2("LE_AUDIO_MSG_PACS_CLIENT_WRITE_SINK_AUDIO_LOC_RESULT: conn_handle 0x%x, cause 0x%x",
						p_data->conn_handle,
						p_data->cause);
		BT_LOGD("LE_AUDIO_MSG_PACS_CLIENT_WRITE_SINK_AUDIO_LOC_RESULT: conn_handle 0x%x, cause 0x%x\r\n",
				p_data->conn_handle,
				p_data->cause);
	}
	break;
	...
	case LE_AUDIO_MSG_PACS_CLIENT_WRITE_SOURCE_AUDIO_LOC_RESULT: { //LE_AUDIO_MSG_GROUP_PACS_CLIENT | 0x03,
		...
		APP_PRINT_INFO2("LE_AUDIO_MSG_PACS_CLIENT_WRITE_SOURCE_AUDIO_LOC_RESULT: conn_handle 0x%x, cause 0x%x",
						p_data->conn_handle,
						p_data->cause);
		BT_LOGD("LE_AUDIO_MSG_PACS_CLIENT_WRITE_SOURCE_AUDIO_LOC_RESULT: conn_handle 0x%x, cause 0x%x\r\n",
				p_data->conn_handle,
				p_data->cause);
	}
	break;
...
static uint16_t rtk_stack_csis_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_CSIS_READ_SIRK_IND: {
		...
		APP_PRINT_INFO3("LE_AUDIO_MSG_CSIS_READ_SIRK_IND: conn_handle 0x%x, service_id %d, sirk_type %d",
						...
						p_data->sirk_type);
		BT_LOGD("LE_AUDIO_MSG_CSIS_READ_SIRK_IND: conn_handle 0x%x, service_id %d, sirk_type %d\r\n",
				p_data->conn_handle, p_data->service_id,
				p_data->sirk_type);
	}
	break;
	...
	case LE_AUDIO_MSG_CSIS_LOCK_STATE: {
		...
		BT_LOGD("LE_AUDIO_MSG_CSIS_LOCK_STATE: service_id %d, lock_state %d, lock_address_type %d\r\n",
				p_data->service_id, p_data->lock_state,
...
static uint16_t rtk_stack_csis_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
			...
			if (p_dis_done->srv_num) {
				...
				for (uint8_t i = 0; i < p_dis_done->srv_num; i++) {
					...
					if (gatt_client_find_primary_srv_by_include(p_dis_done->conn_handle, &srv_uuid, &include_srv)) {
						...
						BT_LOGD("LE_AUDIO_MSG_CSIS_CLIENT_DIS_DONE: instance_id %d, serv 0x%x\r\n",
								include_srv.instance_id, include_srv.p.uuid16);
					}
				}
			}
	...
	case LE_AUDIO_MSG_CSIS_CLIENT_READ_RESULT: {
		...
		BT_LOGD("LE_AUDIO_MSG_CSIS_CLIENT_READ_RESULT: cause 0x%x, conn_handle 0x%x, group_handle 0x%p, dev_handle 0x%p\r\n",
				p_read_result->cause, p_read_result->conn_handle, p_read_result->group_handle, p_read_result->dev_handle);
		...
		if (p_read_result->cause == GAP_SUCCESS) {
			if (p_read_result->group_handle == NULL) {
				if (set_coordinator_add_group(&p_read_result->group_handle, NULL,
					...
					BT_LOGE("%s: set_coordinator_add_group fail!\r\n", __func__);
					break;
				}
			} else {
			...
			if (set_coordinator_get_lock_state(p_read_result->group_handle, &lock_state)) {
				...
				BT_LOGD("LE_AUDIO_MSG_CSIS_CLIENT_READ_RESULT: lock_state %d\r\n", lock_state);
			}
			...
			if (set_coordinator_get_mem_info(p_read_result->group_handle, p_read_result->dev_handle, &mem_info)) {
				...
				BT_LOGD(
					"LE_AUDIO_MSG_CSIS_CLIENT_READ_RESULT: mem_info srv_instance_id %d, char_exit 0x%x, srv_uuid 0x%x, rank %d, lock %d, set_mem_size %d, sirk_type %d\r\n",
					mem_info.srv_instance_id, mem_info.char_exit, mem_info.srv_uuid, mem_info.rank,
	...
	case LE_AUDIO_MSG_CSIS_CLIENT_SEARCH_TIMEOUT: {
		...
		BT_LOGD("LE_AUDIO_MSG_CSIS_CLIENT_SEARCH_TIMEOUT: group_handle 0x%p, set_mem_size %d, search_done %d\r\n",
				p_result->group_handle, p_result->set_mem_size, p_result->search_done);
#if 0
	...
	case LE_AUDIO_MSG_CSIS_CLIENT_SEARCH_DONE: {
		...
		APP_PRINT_INFO3("LE_AUDIO_MSG_CSIS_CLIENT_SEARCH_DONE: group_handle %p, set_mem_size %d, search_done %d",
						p_result->group_handle, p_result->set_mem_size, p_result->search_done);
		BT_LOGD("LE_AUDIO_MSG_CSIS_CLIENT_SEARCH_DONE: group_handle 0x%p, set_mem_size %d, search_done %d\r\n",
				p_result->group_handle, p_result->set_mem_size, p_result->search_done);
#if 0
		set_coordinator_cfg_discover(NULL, false, 0);
#else
	...
	case LE_AUDIO_MSG_CSIS_CLIENT_SET_MEM_FOUND: {
		...
		BT_LOGD(
			"LE_AUDIO_MSG_CSIS_CLIENT_SET_MEM_FOUND: group_handle %p, dev_handle %p, bd_addr %s, addr_type %d, srv_uuid 0x%x, rank %d, set_mem_size %d",
			p_add->group_handle, p_add->dev_handle, TRACE_BDADDR(p_add->bd_addr), p_add->addr_type,
	...
	case LE_AUDIO_MSG_CSIS_CLIENT_LOCK_STATE: {
		...
		BT_LOGD("LE_AUDIO_MSG_CSIS_CLIENT_LOCK_STATE: group_handle 0x%p, event 0x%x, state %d\r\n",
				p_result->group_handle, p_result->event, p_result->lock_state);
	}
	break;
	...
	case LE_AUDIO_MSG_CSIS_CLIENT_SIRK_NOTIFY: {
		...
		BT_LOGD("LE_AUDIO_MSG_CSIS_CLIENT_SIRK_NOTIFY: group_handle 0x%p, sirk_type %d\r\n",
...
static void bt_stack_le_audio_sync_cb(T_BLE_AUDIO_SYNC_HANDLE sync_handle, uint8_t cb_type, void *p_cb_data)
{
	...
	//BT_LOGD("[%s] sync_handle = %p, cb_type = 0x%x\r\n", __func__,sync_handle,cb_type);
	...
	case MSG_BLE_AUDIO_PA_REPORT_INFO: {
		...
		if (p_info->cte_type == GAP_PERIODIC_ADV_REPORT_CTE_TYPE_NO_CTE) {
			p_ind->cte_type = RTK_BT_LE_GAP_CTE_TYPE_NONE;
		} else {
	...
	case MSG_BLE_AUDIO_BIG_SETUP_DATA_PATH: {
		//FIX the direction shall be input
		...
		if (p_cmd) {
			...
			BT_LOGE("[%s] MSG_BLE_AUDIO_BIG_SETUP_DATA_PATH: find no pending command \r\n", __func__);
		}
	}
	break;
	...
	case MSG_BLE_AUDIO_BIG_REMOVE_DATA_PATH: {
		...
		if (p_cmd) {
			...
			BT_LOGE("[%s] MSG_BLE_AUDIO_BIG_REMOVE_DATA_PATH: find no pending command \r\n", __func__);
		}
	}
	break;
	...
	case MSG_BLE_AUDIO_BASE_DATA_MODIFY_INFO: {
		...
		BT_LOGD("MSG_BLE_AUDIO_BASE_DATA_MODIFY_INFO: p_base_mapping %p\r\n",
				p_sync_cb->p_base_data_modify_info->p_base_mapping);
...
static rtk_bt_le_audio_bass_brs_data_t *rtk_stack_bass_brs_data_copy(T_BASS_BRS_DATA *p_src_brs_data)
{
	...
	if (!p_dst_brs_data) {
		BT_LOGE("p_dst_brs_data-> alloc fail, len = %d\r\n", size);
		return NULL;
	}
	...
	if (p_src_brs_data->bis_info_size) {
		...
		for (uint8_t i = 0 ; i < p_src_brs_data->num_subgroups; i++) {
			...
			if (len_offset > p_src_brs_data->bis_info_size)  {
				BT_LOGE("alloc size(%d) less than actually len(%d)\r\n", p_src_brs_data->bis_info_size, len_offset);
				osif_mem_free(p_dst_brs_data);
				return NULL;
			}
			BT_DUMPD(__func__, p_dst_brs_data->p_cp_bis_info[i].p_metadata, p_dst_brs_data->p_cp_bis_info[i].metadata_len);
		}
	}
	BT_DUMPD(__func__, (uint8_t *)p_dst_brs_data, sizeof(rtk_bt_le_audio_bass_brs_data_t) + p_src_brs_data->bis_info_size);
...
static rtk_bt_le_audio_bass_cp_bis_info_t *rtk_stack_bass_cp_bis_info_copy(T_BASS_CP_BIS_INFO *p_src_bis_info, uint16_t bis_info_size, uint8_t num_subgroups)
{
	...
	if (!p_dst_bis_info) {
		BT_LOGE("p_dst_bis_info alloc fail, len = %d\r\n", bis_info_size);
		return NULL;
	}
	...
	for (uint8_t i = 0 ; i < num_subgroups; i++) {
		...
		BT_DUMPD(__func__, p_dst_bis_info[i].p_metadata, p_dst_bis_info[i].metadata_len);
	}
...
static uint16_t rtk_stack_bass_cp_action(T_BASS_CP_IND *p_data)
{
	...
	BT_LOGD("LE_AUDIO_MSG_BASS_CP_IND: conn_handle 0x%x, cp_op 0x%x\r\n", p_data->conn_handle, p_data->p_cp_data->cp_op);
...
#if 0
		p_ind->op_param.add_source_param.p_cp_bis_info = (rtk_bt_le_audio_bass_cp_bis_info_t *)osif_mem_alloc(RAM_TYPE_DATA_ON,
																											  p_data->p_cp_data->param.add_source.bis_info_size);
		if (!p_ind->op_param.add_source_param.p_cp_bis_info) {
			BT_LOGE("p_ind->op_param.add_source_param.p_cp_bis_info fail, len = %d\r\n", p_data->p_cp_data->param.add_source.bis_info_size);
			rtk_bt_event_free(p_evt);
			break;
		}
		memset(p_ind->op_param.add_source_param.p_cp_bis_info, 0, p_data->p_cp_data->param.add_source.bis_info_size);
		memcpy((void *)p_ind->op_param.add_source_param.p_cp_bis_info, (void *)p_data->p_cp_data->param.add_source.p_cp_bis_info,
			   p_data->p_cp_data->param.add_source.bis_info_size);
#else
...
#if 0
		p_ind->op_param.modify_source_param.p_cp_bis_info = (rtk_bt_le_audio_bass_cp_bis_info_t *)osif_mem_alloc(RAM_TYPE_DATA_ON,
																												 p_data->p_cp_data->param.modify_source.bis_info_size);
		if (!p_ind->op_param.modify_source_param.p_cp_bis_info) {
			BT_LOGE("p_ind->op_param.modify_source_param.p_cp_bis_info fail, len = %d\r\n", p_data->p_cp_data->param.modify_source.bis_info_size);
			rtk_bt_event_free(p_evt);
			break;
		}
		memset(p_ind->op_param.modify_source_param.p_cp_bis_info, 0, p_data->p_cp_data->param.modify_source.bis_info_size);
		memcpy((void *)p_ind->op_param.modify_source_param.p_cp_bis_info, (void *)p_data->p_cp_data->param.modify_source.p_cp_bis_info,
			   p_data->p_cp_data->param.modify_source.bis_info_size);
#else
...
static uint16_t rtk_stack_bass_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_BASS_BRS_MODIFY: {
		...
		BT_LOGD("LE_AUDIO_MSG_BASS_BRS_MODIFY: sync handle %p, source_id %d\r\n", p_data->handle, p_data->source_id);
	...
	case LE_AUDIO_MSG_BASS_BA_ADD_SOURCE: {
		...
		APP_PRINT_INFO2("LE_AUDIO_MSG_BASS_BA_ADD_SOURCE: sync handle %p, source_id %d",
						p_data->handle, p_data->source_id);
		BT_LOGD("LE_AUDIO_MSG_BASS_BA_ADD_SOURCE: sync handle %p, source_id %d\r\n",
				p_data->handle, p_data->source_id);
	...
	case LE_AUDIO_MSG_BASS_LOCAL_ADD_SOURCE: {
		...
		APP_PRINT_INFO2("LE_AUDIO_MSG_BASS_LOCAL_ADD_SOURCE: sync handle %p, source_id %d",
						p_data->handle, p_data->source_id);
		BT_LOGD("LE_AUDIO_MSG_BASS_LOCAL_ADD_SOURCE: sync handle %p, source_id %d\r\n",
				p_data->handle, p_data->source_id);
		rtk_bt_le_audio_bass_ba_add_source_ind_t *p_ind = NULL;
		p_evt = rtk_bt_event_create(RTK_BT_LE_GP_AUDIO,
									RTK_BT_LE_AUDIO_EVT_BASS_BA_ADD_SOURCE_IND,
	...
	case LE_AUDIO_MSG_BASS_GET_PA_SYNC_PARAM: {
		...
		BT_LOGD("LE_AUDIO_MSG_BASS_GET_PA_SYNC_PARAM: sync handle %p, source_id %d, is_past %d, pa_interval 0x%x\r\n",
				p_data->handle, p_data->source_id, p_data->is_past, p_data->pa_interval);
		...
		if (cb_ret == RTK_BT_EVT_CB_OK) {
			BT_LOGD(
				"%s: RTK_BT_LE_AUDIO_EVT_BASS_GET_PA_SYNC_PARAM_IND, sync_handle = %p,source_id = %d,pa_sync_options = %d,pa_sync_skip = %d,pa_sync_timeout = %d,past_timeout = %d\r\n",
				__func__,
				p_data->handle, p_data->source_id,
	...
	case LE_AUDIO_MSG_BASS_GET_BIG_SYNC_PARAM: {
		...
		APP_PRINT_INFO2("LE_AUDIO_MSG_BASS_GET_BIG_SYNC_PARAM: sync handle %p, source_id %d",
						p_data->handle, p_data->source_id);
		BT_LOGD("LE_AUDIO_MSG_BASS_GET_BIG_SYNC_PARAM: sync handle %p, source_id %d\r\n",
				p_data->handle, p_data->source_id);
	...
	case LE_AUDIO_MSG_BASS_GET_BROADCAST_CODE: {
		...
		APP_PRINT_INFO2("LE_AUDIO_MSG_BASS_GET_BROADCAST_CODE: sync handle %p, source_id %d",
						p_data->handle, p_data->source_id);
		BT_LOGD("LE_AUDIO_MSG_BASS_GET_BROADCAST_CODE: sync handle %p, source_id %d\r\n",
				p_data->handle, p_data->source_id);
		...
		if (false == rtk_bt_check_evt_cb_direct_calling(RTK_BT_LE_GP_AUDIO, RTK_BT_LE_AUDIO_EVT_BASS_GET_BROADCAST_CODE_IND)) {
			BT_LOGE("%s: RTK_BT_LE_AUDIO_EVT_BASS_GET_BROADCAST_CODE_IND is not direct calling!\r\n", __func__);
			break;
		}
		...
		if (cb_ret == RTK_BT_EVT_CB_OK) {
			BT_DUMPD(__func__, broadcast_code, RTK_BT_LE_AUDIO_BROADCAST_CODE_LEN);
	...
	case LE_AUDIO_MSG_BASS_GET_PREFER_BIS_SYNC: {
		...
		BT_LOGD("LE_AUDIO_MSG_BASS_GET_PREFER_BIS_SYNC: sync handle %p, source_id %d, num_subgroups %d\r\n",
		...
		if (false == rtk_bt_check_evt_cb_direct_calling(RTK_BT_LE_GP_AUDIO, RTK_BT_LE_AUDIO_EVT_BASS_GET_PREFER_BIS_SYNC_IND)) {
			BT_LOGE("%s: RTK_BT_LE_AUDIO_EVT_BASS_GET_PREFER_BIS_SYNC_IND is not direct calling!\r\n", __func__);
			break;
		}
...
static uint16_t rtk_stack_bass_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_BASS_CLIENT_CP_RESULT: {
		...
		if (p_cmd) {
			...
			BT_LOGE("[%s] LE_AUDIO_MSG_BASS_CLIENT_CP_RESULT: find no pending command \r\n", __func__);
		}
	}
	break;
	...
	case LE_AUDIO_MSG_BASS_CLIENT_SYNC_INFO_REQ: {
		...
		BT_LOGD("LE_AUDIO_MSG_BASS_CLIENT_SYNC_INFO_REQ: conn_handle 0x%x, char_instance_id %d\r\n", p_data->conn_handle, p_data->char_instance_id);
...
static uint16_t rtk_stack_mcp_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	BT_LOGD("rtk_stack_mcp_client_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_mcp_server_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	//BT_LOGD("rtk_stack_mcp_server_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_vcs_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	BT_LOGD("rtk_stack_vcs_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_vcs_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	//BT_LOGD("rtk_stack_vcs_client_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_vocs_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	BT_LOGD("rtk_stack_vocs_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_vocs_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	BT_LOGD("rtk_stack_vocs_client_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_aics_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	BT_LOGD("rtk_stack_aics_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_aics_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	BT_LOGD("rtk_stack_aics_client_handle_msg: msg %x\r\n", msg);
...
static uint16_t rtk_stack_gmas_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	uint16_t app_result = BLE_AUDIO_CB_RESULT_SUCCESS;
	rtk_bt_evt_t *p_evt = NULL;
...
static uint16_t rtk_stack_gmas_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	uint16_t app_result = BLE_AUDIO_CB_RESULT_SUCCESS;
	rtk_bt_evt_t *p_evt = NULL;
...
static uint16_t rtk_stack_le_audio_msg_cback(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	//BT_LOGD("%s: msg=0x%04x,msg_group=0x%04x\r\n",__func__, msg, msg_group);
...
#endif
#if defined(CONFIG_BT_GMAP_SUPPORT) && CONFIG_BT_GMAP_SUPPORT
	case LE_AUDIO_MSG_GROUP_GMAS:
		app_result = rtk_stack_gmas_handle_msg(msg, buf);
		break;
	case LE_AUDIO_MSG_GROUP_GMAS_CLIENT:
		app_result = rtk_stack_gmas_client_handle_msg(msg, buf);
		break;
#endif
...
void bt_stack_le_audio_data_direct_callback(uint8_t cb_type, void *p_cb_data)
{
	...
	case BT_DIRECT_MSG_ISO_DATA_IND: {
		BT_LOGD(
			"BT_DIRECT_MSG_ISO_DATA_IND, conn_handle 0x%x, pkt_status_flag 0x%x, pkt_seq_num 0x%x, ts_flag 0x%x, time_stamp 0x%x,iso_sdu_len 0x%x, p_buf %p, offset %d\r\n",
...
static uint16_t bt_stack_le_audio_pacs_init(rtk_bt_le_audio_pacs_init_param_t *p_pacs)
{
	...
	BT_LOGD("%s sink_audio_location = 0x%x, source_audio_location = 0x%x\r\n", __func__,
	...
	BT_LOGD("%s sink_supported_contexts = 0x%x, source_supported_contexts = 0x%x\r\n", __func__,
	...
	if (!p_pacs->p_pac_sink_codec || p_pacs->pac_sink_codec_len == 0) {
		BT_LOGE("%s p_pac_sink_codec is not set, use default value\r\n", __func__);
	...
	*p_pacs->p_sink_pac_id = pacs_pac_add(SERVER_AUDIO_SINK, p_default_pac_sink_codec, pac_sink_codec_len, true);
	...
	if (!p_pacs->p_pac_source_codec || p_pacs->pac_source_codec_len == 0) {
		BT_LOGE("%s p_pac_source_codec is not set, use default value\r\n", __func__);
	...
	*p_pacs->p_source_pac_id = pacs_pac_add(SERVER_AUDIO_SOURCE, p_default_pac_source_codec, pac_source_codec_len, true);
	...
	if (pacs_init(&pacs_params) == false) {
		...
		BT_LOGD("%s pacs_init ok p_sink_pac_id=%d, p_source_pac_id=%d\r\n", __func__, *p_pacs->p_sink_pac_id, *p_pacs->p_source_pac_id);
	}
...
uint16_t bt_stack_le_audio_cap_init(rtk_bt_le_audio_app_conf_t *p_le_audio_app_conf)
{
	...
	if (p_le_audio_app_conf->cap_param.cap_role == RTK_BT_LE_AUDIO_CAP_ROLE_UNKNOWN) {
		BT_LOGE("%s set cap role according bap role\r\n", __func__);
	...
	BT_LOGD("%s p_le_audio_app_conf->cap_param.cap_role=%d\r\n", __func__, p_le_audio_app_conf->cap_param.cap_role);
	...
	if (p_le_audio_app_conf->cap_param.cap_role & RTK_BT_LE_AUDIO_CAP_ROLE_INITIATOR ||
		...
		BT_LOGD("%s mcp_media_control_client=%d mcp_media_control_server=%d\r\n", __func__, cap_init_param.mcp_media_control_client,
				cap_init_param.mcs.mcp_media_control_server);
...
static uint16_t bt_stack_le_audio_mcs_server_init(void *le_audio_app_conf)
{
	...
	reg_param.char_media_control_point.char_media_control_point_opcodes_supported_optional_property_notify =
		p_mcs_param->char_media_control_point_opcodes_supported_optional_property_notify;
...
#if defined(CONFIG_BT_GMAP_SUPPORT) && CONFIG_BT_GMAP_SUPPORT
static uint16_t bt_stack_le_audio_gmap_init(void *le_audio_app_conf)
{
	(void)le_audio_app_conf;
	uint8_t gmap_role = 0;
	rtk_bt_le_audio_app_conf_t *p_le_audio_app_conf = NULL;
...
uint16_t bt_stack_le_audio_init(rtk_bt_app_conf_t *papp_conf, void *io_msg_q, void *evt_msg_q)
{
	...
	if (ret != RTK_BT_OK) {
		BT_LOGE("%s:bt_stack_le_audio_bap_init fail\r\n", __func__);
		return ret;
	}
	...
	if (ret != RTK_BT_OK) {
		BT_LOGE("%s:bt_stack_le_audio_cap_init fail,ret= 0x%x\r\n", __func__, ret);
		return ret;
	}
	...
	if (p_le_audio_app_conf->cap_param.cap_role & RTK_BT_LE_AUDIO_CAP_ROLE_ACCEPTOR) {
		...
		if (ret != RTK_BT_OK) {
			BT_LOGE("%s:bt_stack_le_audio_pacs_init fail,ret= 0x%x\r\n", __func__, ret);
			return ret;
		}
	}
	...
	if (p_le_audio_app_conf->cap_param.cap_role & RTK_BT_LE_AUDIO_CAP_ROLE_INITIATOR) {
		...
		if (ret != RTK_BT_OK) {
			BT_LOGE("%s:bt_stack_le_audio_mcs_server_init fail,ret= 0x%x\r\n", __func__, ret);
			return ret;
		}
	}
#endif
	...
	if (ret != RTK_BT_OK) {
		BT_LOGE("%s:bt_stack_le_audio_tmap_init fail,ret= 0x%x\r\n", __func__, ret);
		return ret;
	}
#endif
#if defined(CONFIG_BT_GMAP_SUPPORT) && CONFIG_BT_GMAP_SUPPORT
	ret = bt_stack_le_audio_gmap_init(p_le_audio_app_conf);
	if (ret != RTK_BT_OK) {
		BT_LOGE("%s:bt_stack_le_audio_gmap_init fail,ret= 0x%x\r\n", __func__, ret);
		return ret;
	}
#endif
...
static uint16_t bt_stack_le_audio_get_prefer_codec_cfg(void *data)
{
	...
	BT_LOGD(
		"%s type_exist 0x%x, frame_duration %d, sample_frequency %d, codec_frame_blocks_per_sdu %d,octets_per_codec_frame=%d,audio_channel_allocation=0x%x)\r\n",
		__func__,
...
static uint16_t bt_stack_le_audio_get_prefer_qos_cfg(void *data)
{
	...
	BT_LOGD("%s sdu_interval 0x%d, framing %d, max_sdu %d, retransmission_number %d,max_transport_latency=%d,presentation_delay=%d)\r\n",
			__func__,
...
static uint16_t bt_stack_le_audio_pacs_get_info(void *data)
{
	...
	if (false == bap_pacs_get_info(param->conn_handle, &pacs_info)) {
		BT_LOGE("%s bap_pacs_get_info fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_pacs_get_pac_record(void *data)
{
	...
	if (!param->p_pac_tbl || !param->p_pac_num) {
		BT_LOGE("%s fail: p_pac_tbl or p_pac_num invalid \r\n", __func__);
		return RTK_BT_ERR_PARAM_INVALID;
	}
	...
	for (uint8_t i = 0; i < (*param->p_pac_num); i++) {
		...
		if (p_pac_tbl[i].metadata_length) {
			...
			if (p_pac_tbl[i].metadata_length <= RTK_BT_LE_AUDIO_METADATA_MAX_LEN) {
				...
				BT_LOGE("%s conn_handle 0x%x audio_role %d metadata_length(%d) is too long, skip copy metadata, \r\n", __func__, param->conn_handle,
						param->audio_role, p_pac_tbl[i].metadata_length);
			}
		}
	}
...
static uint16_t bt_stack_le_audio_pacs_get_lc3_table_mask(void *data)
{
	...
	if (RTK_BLE_AUDIO_SINK == param->audio_role) {
		...
		BT_LOGE("%s fail: direction(%d) error\r\n", __func__, param->audio_role);
		return RTK_BT_ERR_PARAM_INVALID;
	}

	*param->p_lc3_table_mask = lc3_table_mask;
	BT_LOGD("%s conn_handle %d, pref_audio_contexts 0x%x, channel_count %d, lc3_table_mask 0x%x\r\n", __func__, param->conn_handle,
			param->pref_audio_contexts, param->channel_count, (unsigned int)lc3_table_mask);
...
static uint16_t bt_stack_le_audio_pacs_get_bis_array_by_sync_info(void *data)
{
	...
	*param->p_bis_array = bis_array;
	BT_LOGD("%s conn_handle %d, sync_handle %p, filter %d, bis_array 0x%x\r\n", __func__, param->conn_handle, param->sync_handle, param->filter,
			(unsigned int)bis_array);
...
static uint16_t bt_stack_le_audio_sync_get_bis_codec_cfg(void *data)
{
	...
	if (param->p_codec_cfg == NULL) {
		BT_LOGE("%s fail: param error p_codec_cfg == NULL\r\n", __func__);
		return RTK_BT_ERR_PARAM_INVALID;
	}
...
static uint16_t bt_stack_le_audio_big_sync_establish(void *data)
{
	...
	BT_LOGE("%s sync_handle %p, encryption = %d, mse = %d, big_sync_timeout = %d, num_bis = %d, bis[0]= %d\r\n", __func__,
			p_param->sync_handle, sync_param.encryption, sync_param.mse, sync_param.big_sync_timeout, sync_param.num_bis, sync_param.bis[0]);
...
static uint16_t bt_stack_le_audio_sync_get_bis_info(void *data)
{
	...
	if (p_param->p_bis_info == NULL) {
		BT_LOGE("%s fail: param error p_bis_info is NULL\r\n", __func__);
		return RTK_BT_ERR_PARAM_INVALID;
	}

	if (false == ble_audio_get_bis_sync_info(p_param->sync_handle, &bis_info)) {
		BT_LOGE("%s ble_audio_get_bis_sync_info fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}

	p_param->p_bis_info->num_bis = bis_info.bis_num;
	if (bis_info.bis_num > RTK_BT_LE_AUDIO_RECEIVER_BIS_MAX_NUM) {
		BT_LOGE("%s bis_num(%d) > RTK_BT_LE_AUDIO_RECEIVER_BIS_MAX_NUM(%d) fail\r\n", __func__, bis_info.bis_num,
				RTK_BT_LE_AUDIO_RECEIVER_BIS_MAX_NUM);
...
static void bt_stack_le_audio_broadcast_source_cb(T_BROADCAST_SOURCE_HANDLE handle, uint8_t cb_type, void *p_cb_data)
{
	...
	case MSG_BROADCAST_SOURCE_RECONFIG: {
		...
		if (p_cmd) {
			...
			BT_LOGE("[%s] MSG_BROADCAST_SOURCE_RECONFIG: find no pending command \r\n", __func__);
		}
	}
	break;
	...
	case MSG_BROADCAST_SOURCE_SETUP_DATA_PATH: {
		...
		BT_LOGD("MSG_BROADCAST_SOURCE_SETUP_DATA_PATH: bis_idx %d,bis_conn_handle 0x%x,  cause 0x%x\r\n",
				p_sm_data->p_setup_data_path->bis_idx, p_sm_data->p_setup_data_path->bis_conn_handle, p_sm_data->p_setup_data_path->cause);
		...
		if (p_cmd) {
			...
			*param->p_bis_conn_handle = p_sm_data->p_setup_data_path->bis_conn_handle;
			...
			BT_LOGE("[%s] MSG_BROADCAST_SOURCE_SETUP_DATA_PATH: find no pending command \r\n", __func__);
		}
	}
	break;
	...
	case MSG_BROADCAST_SOURCE_REMOVE_DATA_PATH: {
		...
		if (p_cmd) {
			...
			BT_LOGE("[%s] MSG_BROADCAST_SOURCE_REMOVE_DATA_PATH: find no pending command \r\n", __func__);
		}
	}
	break;
...
static uint16_t bt_stack_le_audio_broadcast_source_create(void *data)
{
	...
	if (g_broadcast_source_handle == NULL) {
		...
		BT_LOGD("broadcast_source_add can add once only, because there is no broadcast_source_delete\r\n");
		broadcast_source_handle = g_broadcast_source_handle;
	}

	// // free base data
	// if (g_group1_idx != 0xFF) {
	//  if (false == base_data_del_group(g_group1_idx)) {
	//      BT_LOGE("%s: base delete group idx fail\r\n", __func__);
	//      return RTK_BT_ERR_LOWER_STACK_API;
	//  }
	//  g_group1_idx = 0xFF;
	// }

	// if add group in previous broadcast start, no need to add
	for (i = 0; i < num_groups; i++) {
		if (g_group1_idx != 0xFF) {
			*p_param->group[i].group_idx = g_group1_idx;
			num_subgroups = p_param->group[i].num_subgroups;
			for (j = 0; j < num_subgroups; j++) {
				if (g_subgroup_idx[j] != 0xFF) {
					*p_param->group[i].subgroup[j].subgroup_idx = g_subgroup_idx[j];
				}
				num_bis = p_param->group[i].subgroup[j].num_bis;
				for (k = 0; k < num_bis; k++) {
					if (g_bis_idx[k] != 0xFF) {
						*p_param->group[i].subgroup[j].bis[k].bis_idx = g_bis_idx[k];
					}
					BT_LOGD("group idx: %d, subgroup_idx[%d]: %d, bis_idx[%d]: %d \r\n", g_group1_idx, j, g_subgroup_idx[j], k, g_bis_idx[k]);
				}
			}
			goto out;
		}
		...
		*p_param->group[i].group_idx = group_idx;
		...
		for (j = 0; j < num_subgroups; j++) {
			...
			if (false == base_data_add_subgroup(&subgroup_idx, group_idx,
				...
				BT_LOGE("%s broadcast_source_add group %d subgroup %d fail\r\n", __func__, i, j);
				return RTK_BT_ERR_LOWER_STACK_API;
			}
			*p_param->group[i].subgroup[j].subgroup_idx = subgroup_idx;
			g_subgroup_idx[j] = subgroup_idx;
			for (k = 0;  k < num_bis; k++) {
				if (false == base_data_add_bis(&bis_idx, group_idx, subgroup_idx,
					...
					BT_LOGE("%s broadcast_source_add group %d subgroup %d bis %d fail\r\n", __func__, i, j, k);
					return RTK_BT_ERR_LOWER_STACK_API;
				}
...
out:
	BT_LOGD("%s broadcast_source_handle=%p\r\n", __func__, broadcast_source_handle);
...
static uint16_t bt_stack_le_audio_broadcast_source_config(void *data)
{
	...
	if (false == broadcast_source_set_eadv_param(p_param->broadcast_source_handle,
		...
		BT_LOGE("%s fail: broadcast_source_set_eadv_param fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_broadcast_source_remove_data_path(void *data)
{
	...
	if (false == broadcast_source_remove_data_path(p_param->broadcast_source_handle, p_param->bis_idx)) {
		BT_LOGE("%s fail: broadcast_source_remove_data_path fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_broadcast_source_reconfig(void *data)
{
	...
	if (false == base_data_update_metadata(p_param->group_idx,
										   ...
										   p_param->p_metadata)) {
		BT_LOGE("%s fail: base_data_update_metadata fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
	...
	if (false == broadcast_source_reconfig(p_param->broadcast_source_handle, pa_data_len, pa_data)) {
		BT_LOGE("%s fail: broadcast_source_reconfig fail\r\n", __func__);
...
static uint16_t bt_stack_le_audio_bass_client_cp_add_source_by_sync_info(void *data)
{
	...
	if (false == bass_cp_add_source_by_sync_info(p_param->sync_handle,
												 ...
												 p_param->bis_array,
												 p_param->is_req)) {
		BT_LOGE("%s fail: bass_cp_add_source_by_sync_info fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_bass_client_cp_modify_source_by_sync_info(void *data)
{
	...
	BT_LOGD("%s sync_handle %p conn_handle = %d, source_id = %d, pa_sync = %d\r\n", __func__, p_param->sync_handle, p_param->conn_handle,
			p_param->source_id, p_param->pa_sync);
...
static uint16_t bt_stack_le_audio_bass_client_write_cp_op(void *data)
{
	...
	case RTK_BT_LE_AUDIO_BASS_CP_REMOTE_SCAN_STOP: {
		if (false == bass_cp_remote_scan_stop(p_param->conn_handle, p_param->is_req)) {
			BT_LOGE("%s fail: bass_cp_remote_scan_stop fail\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	}
	break;
	...
	case RTK_BT_LE_AUDIO_BASS_CP_REMOTE_SCAN_START: {
		if (false == bass_cp_remote_scan_start(p_param->conn_handle, p_param->is_req)) {
			BT_LOGE("%s fail: bass_cp_remote_scan_start fail\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	}
	break;
	...
	case RTK_BT_LE_AUDIO_BASS_CP_ADD_SOURCE: {
		...
		if (false == bass_cp_add_source(p_param->conn_handle, &cp_data, p_param->is_req)) {
			BT_LOGE("%s fail: bass_cp_add_source fail\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	...
	case RTK_BT_LE_AUDIO_BASS_CP_MODIFY_SOURCE: {
		...
		if (false == bass_cp_modify_source(p_param->conn_handle, &cp_data, p_param->is_req)) {
			BT_LOGE("%s fail: bass_cp_modify_source fail\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
		if (cp_data.p_cp_bis_info) {
			osif_mem_free(cp_data.p_cp_bis_info);
	...
	case RTK_BT_LE_AUDIO_BASS_CP_REMOVE_SOURCE: {
		...
		if (false == bass_cp_remove_source(p_param->conn_handle, &cp_data, p_param->is_req)) {
			BT_LOGE("%s fail: bass_cp_remove_source fail\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	}
	break;
	...
	case RTK_BT_LE_AUDIO_BASS_CP_SET_BROADCAST_CODE: {
		...
		if (false == bass_cp_set_broadcast_code(p_param->conn_handle, &cp_data, p_param->is_req)) {
			BT_LOGE("%s fail: bass_cp_set_broadcast_code fail\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	}
	break;
...
static uint16_t bt_stack_le_audio_bass_client_past(void *data)
{
	...
	if (p_param->past_type == RTK_BT_LE_AUDIO_BASS_PAST_BY_REMOTE_SRC) {
		...
		if (false == bass_past_by_local_src((T_BROADCAST_SOURCE_HANDLE)p_param->handle, p_param->conn_handle, srv_data)) {
			BT_LOGE("%s fail: bass_past_by_local_src fail\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	}
...
static void bt_stack_le_audio_group_cb(T_AUDIO_GROUP_MSG msg, T_BLE_AUDIO_GROUP_HANDLE handle, void *buf)
{
	...
	case AUDIO_GROUP_MSG_DEV_CONN: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_DEV_CONN: group handle 0x%p, dev handle 0x%p",
				handle, p_data->dev_handle);
	...
	case AUDIO_GROUP_MSG_DEV_DISCONN: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_DEV_DISCONN: group handle 0x%p, dev handle 0x%p, cause 0x%x\r\n",
				handle, p_data->dev_handle, p_data->cause);
	...
	case AUDIO_GROUP_MSG_DEV_BOND_REMOVE: {
		...
		APP_PRINT_INFO2("AUDIO_GROUP_MSG_DEV_BOND_REMOVE: group handle 0x%x, dev handle 0x%x",
						handle, p_data->dev_handle);
		BT_LOGD("AUDIO_GROUP_MSG_DEV_BOND_REMOVE: group handle 0x%p, dev handle 0x%p\r\n",
				handle, p_data->dev_handle);
	...
	case AUDIO_GROUP_MSG_DEV_BOND_CLEAR: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_DEV_BOND_CLEAR: group handle 0x%p\r\n", handle);
	...
	case AUDIO_GROUP_MSG_DEV_EMPTY: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_DEV_EMPTY: group handle 0x%p\r\n", handle);
	...
	case AUDIO_GROUP_MSG_BAP_STATE: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_BAP_STATE: group handle %p, session handle 0x%p, curr_action %d, state %d, result %d, cause 0x%x\r\n",
				handle, p_data->handle, p_data->curr_action, p_data->state, p_data->result, p_data->cause);
		...
		if (msg_type) {
			...
			if (p_cmd) {
				...
				BT_LOGE("[%s] AUDIO_GROUP_MSG_BAP_STATE: find no pending command \r\n", __func__);
			}
		}
	...
	case AUDIO_GROUP_MSG_BAP_START_QOS_CFG: {
		...
		BT_LOGD(
			"AUDIO_GROUP_MSG_BAP_START_QOS_CFG: group handle %p, session handle %p, sink presentation delay(0x%x-0x%x),source presentation delay(0x%x-0x%x), sink latency 0x%x, source latency 0x%x\r\n",
			handle, p_data->handle,
		...
		T_AUDIO_ASE_QOS_CFG ase_qos = {0};
		T_AUDIO_ASE_QOS_CFG src_ase_qos = {0};
		if (false == rtk_bt_check_evt_cb_direct_calling(RTK_BT_LE_GP_AUDIO, RTK_BT_LE_AUDIO_EVT_BAP_START_QOS_CFG_IND)) {
			BT_LOGE("%s: RTK_BT_LE_AUDIO_EVT_BAP_START_QOS_CFG_IND is not direct calling!\r\n", __func__);
			break;
		}
		...
		p_ind->p_session_qos = (rtk_bt_le_audio_unicast_session_qos_t *)
							   &session_qos; //make sure the definition of T_AUDIO_SESSION_QOS_CFG and rtk_bt_le_audio_unicast_session_qos_t is same
		p_ind->p_snk_ase_qos = (rtk_bt_le_audio_unicast_ase_qos_t *)
							   &ase_qos; //make sure the definition of T_AUDIO_ASE_QOS_CFG and rtk_bt_le_audio_unicast_ase_qos_t is same
		p_ind->p_src_ase_qos = (rtk_bt_le_audio_unicast_ase_qos_t *)
							   &src_ase_qos; //make sure the definition of T_AUDIO_ASE_QOS_CFG and rtk_bt_le_audio_unicast_ase_qos_t is same
		/* Send event */
		rtk_bt_evt_indicate(p_evt, &cb_ret);
		if (cb_ret == RTK_BT_EVT_CB_OK) {
			BT_LOGD(
				"%s: sca = %d,packing = %d,framing = %d,sdu_interval_m_s = %d,sdu_interval_s_m = %d,latency_m_s = %d,latency_s_m = %d,sink_presentation_delay = %d,source_presentation_delay = %d\r\n",
				__func__,
					...
					for (int j = 0; j < session_info.dev_info[i].ase_num; j++) {
						if (session_info.dev_info[i].ase_info[j].direction == RTK_BLE_AUDIO_ISO_DATA_PATH_TX) {
							if (false == bap_unicast_audio_cfg_ase_qos(p_data->handle, session_info.dev_info[i].dev_handle,
																	   session_info.dev_info[i].ase_info[j].ase_id, &ase_qos)) {
								BT_LOGE("%s: bap_unicast_audio_cfg_ase_qos fail!\r\n", __func__);
								break;
							}
						} else if (session_info.dev_info[i].ase_info[j].direction == RTK_BLE_AUDIO_ISO_DATA_PATH_RX) {
							if (false == bap_unicast_audio_cfg_ase_qos(p_data->handle, session_info.dev_info[i].dev_handle,
																	   session_info.dev_info[i].ase_info[j].ase_id, &src_ase_qos)) {
								BT_LOGE("%s: bap_unicast_audio_cfg_ase_qos fail!\r\n", __func__);
								break;
							}
						}
	...
	case AUDIO_GROUP_MSG_BAP_START_METADATA_CFG: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_BAP_START_METADATA_CFG: group handle %p, session handle %p, dev_handle %p, ase_id %d, direction %d\r\n",
				handle, p_data->handle, p_data->dev_handle, p_data->ase_id, p_data->direction);
		...
		if (false == rtk_bt_check_evt_cb_direct_calling(RTK_BT_LE_GP_AUDIO, RTK_BT_LE_AUDIO_EVT_BAP_START_METADATA_CFG_IND)) {
			BT_LOGE("%s: RTK_BT_LE_AUDIO_EVT_BAP_START_METADATA_CFG_IND is not direct calling!\r\n", __func__);
			break;
		}
		...
		if (cb_ret == RTK_BT_EVT_CB_OK) {
			if (metadata_len > RTK_BT_LE_AUDIO_METADATA_MAX_LEN) {
				BT_LOGE("%s: RTK_BT_LE_AUDIO_EVT_BAP_START_METADATA_CFG_IND fail, use default metadata!\r\n", __func__);
	...
	case AUDIO_GROUP_MSG_BAP_CREATE_CIS: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_BAP_CREATE_CIS: group handle 0x%p, session handle 0x%p, dev_num 0x%x\r\n",
				handle, p_data->handle, p_data->dev_num);
	}
	break;
	case AUDIO_GROUP_MSG_BAP_CIS_DISCONN: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_BAP_CIS_DISCONN: group handle 0x%p, session handle 0x%p, conn_handle 0x%x, reason 0x%x\r\n",
				handle, p_data->handle, p_data->conn_handle, p_data->cause);
	}
	break;
	case AUDIO_GROUP_MSG_BAP_SESSION_REMOVE: {
		...
		APP_PRINT_INFO2("AUDIO_GROUP_MSG_BAP_SESSION_REMOVE: group handle 0x%x, session handle 0x%x",
						handle, p_data->handle);
		BT_LOGD("AUDIO_GROUP_MSG_BAP_SESSION_REMOVE: group handle %p, session handle %p\r\n",
				handle, p_data->handle);
		...
		if (p_cmd) {
			...
			BT_LOGE("AUDIO_GROUP_MSG_BAP_SESSION_REMOVE: find no pending command \r\n");
		}
	}
	break;
	...
	case AUDIO_GROUP_MSG_BAP_SETUP_DATA_PATH: {
		...
		APP_PRINT_INFO6("AUDIO_GROUP_MSG_BAP_SETUP_DATA_PATH: group handle 0x%x, session handle 0x%x, dev_handle 0x%x, ase_id 0x%x, direction %d, cis_conn_handle 0x%x",
						...
						p_data->cis_conn_handle);
		BT_LOGD(
			"AUDIO_GROUP_MSG_BAP_SETUP_DATA_PATH: group handle %p, session handle %p, dev_handle %p, ase_id 0x%x, direction %d, cis_conn_handle 0x%x\r\n",
			handle, p_data->handle,
			p_data->dev_handle,
			p_data->ase_id,
			p_data->path_direction,
	...
	case AUDIO_GROUP_MSG_BAP_REMOVE_DATA_PATH: {
		...
		APP_PRINT_INFO7("AUDIO_GROUP_MSG_BAP_REMOVE_DATA_PATH: group handle 0x%x, session handle 0x%x, dev_handle 0x%x, ase_id 0x%x, direction %d, cis_conn_handle 0x%x, cause 0x%x",
						...
						p_data->cause);
		BT_LOGD(
			"AUDIO_GROUP_MSG_BAP_REMOVE_DATA_PATH: group handle %p, session handle %p, dev_handle %p, ase_id 0x%x, direction %d, cis_conn_handle 0x%x, cause 0x%x\r\n",
			handle, p_data->handle,
			p_data->dev_handle,
			p_data->ase_id,
			p_data->path_direction,
	...
	case AUDIO_GROUP_MSG_BAP_METADATA_UPDATE: {
		...
		BT_LOGD("AUDIO_GROUP_MSG_BAP_METADATA_UPDATE: group handle %p, session handle %p, dev_handle %p, ase_id 0x%x,direction %d",
				handle, p_data->handle,
				p_data->dev_handle,
				p_data->ase_id,
				p_data->direction);
...
static uint16_t bt_stack_le_audio_group_allocate(void *data)
{
	...
	BT_LOGD("%s ble_audio_group_allocate ok,group_handle=%p\r\n", __func__, group_handle);
...
static uint16_t bt_stack_le_audio_group_delete_device(void *data)
{
	...
	if (false == ble_audio_group_del_dev(p_param->group_handle, &p_param->device_handle)) {
		BT_LOGE("%s fail: ble_audio_group_del_dev fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_group_find_device(void *data)
{
	...
	if (device_handle == NULL) {
		BT_LOGE("%s fail: ble_audio_group_find_dev fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_group_find_device_by_conn_handle(void *data)
{
	...
	if (device_handle == NULL) {
		BT_LOGE("%s fail: ble_audio_group_find_dev_by_conn_handle fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_group_get_device_info(void *data)
{
	...
	if (false == ble_audio_group_get_dev_info(p_param->group_handle, p_param->device_handle, &info)) {
		BT_LOGE("%s fail: ble_audio_group_get_dev_info fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}

	if (p_param->device_handle != info.dev_handle) {
		BT_LOGE("%s fail: set dev_handle(%p) != get dev_handle(%p)\r\n", __func__, p_param->device_handle, info.dev_handle);
		return RTK_BT_ERR_LOWER_STACK_API;
	}

	BT_LOGD("%s: info.is_used=%d,info.conn_state=%d,info.conn_handle=0x%x,info.dev_handle=%p\r\n", __func__, info.is_used, info.conn_state,
			info.conn_handle, info.dev_handle);
...
static uint16_t bt_stack_le_audio_stream_session_allocate(void *data)
{
	...
	if (stream_session_handle == NULL) {
		BT_LOGE("%s fail: audio_stream_session_allocate fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}

	BT_LOGD("%s audio_stream_session_allocate ok,stream_session_handle=%p\r\n", __func__, stream_session_handle);
...
static uint16_t bt_stack_le_audio_stream_session_release(void *data)
{
	...
	if (false == bap_unicast_audio_remove_session(stream_session_handle)) {
		BT_LOGE("%s fail: bap_unicast_audio_remove_session fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_stream_session_get_group_handle(void *data)
{
	...
	if (group_handle == NULL) {
		BT_LOGE("%s fail: audio_stream_session_get_group fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_unicast_get_session_info(void *data)
{
	...
	for (i = 0; i < session_info.dev_num; i++) {
		...
		for (j = 0; j < session_info.dev_info[i].ase_num; j++) {
			BT_LOGD("j=%d, ase_id=%d,cfg_idx=%d,ase_state=%d,direction=%d,channel_num=%d\r\n", j,
	...
	for (i = 0; i < session_info.dev_num; i++) {
		...
		if (session_info.dev_info[i].ase_num > RTK_BT_LE_AUDIO_STREAM_SRV_ASE_MAX) {
			BT_LOGE("%s fail:session_info.dev_info[%d].ase_num (%d) > max (%d)\r\n", __func__, i, session_info.dev_info[i].ase_num,
					RTK_BT_LE_AUDIO_STREAM_SRV_ASE_MAX);
			return RTK_BT_ERR_NO_RESOURCE;
		}
		for (j = 0; j < session_info.dev_info[i].ase_num; j++) {
	...
	for (i = 0; i < p_param->p_session_info->dev_num; i++) {
		...
		for (j = 0; j < p_param->p_session_info->dev_info[i].ase_num; j++) {
			BT_LOGD("j=%d, ase_id=%d,cfg_idx=%d,ase_state=%d,direction=%d,channel_num=%d\r\n", j,
					...
					p_param->p_session_info->dev_info[i].ase_info[j].ase_state, p_param->p_session_info->dev_info[i].ase_info[j].direction,
					p_param->p_session_info->dev_info[i].ase_info[j].channel_num);
		}
	}
#endif
	return RTK_BT_OK;
...
static uint16_t bt_stack_le_audio_unicast_config(void *data)
{
	...
	BT_LOGD("%s: stream_session_handle=%p,cfg_type=%d,device_num=%d\r\n", __func__, p_param->stream_session_handle, p_param->cfg_type,
			p_param->device_num);
	...
	if (false == bap_unicast_audio_cfg(p_param->stream_session_handle, (T_UNICAST_AUDIO_CFG_TYPE)p_param->cfg_type, p_param->device_num,
									   p_param->p_device_handle_tbl)) {
		BT_LOGE("%s fail: bap_unicast_audio_cfg fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_unicast_remove_config(void *data)
{
	...
	if (false == bap_unicast_audio_remove_cfg(stream_session_handle)) {
		BT_LOGE("%s fail: bap_unicast_audio_remove_cfg fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_unicast_get_session_qos(void *data)
{
	...
	if (false == bap_unicast_audio_get_session_qos(p_param->stream_session_handle, &cfg)) {
		...
		BT_LOGD(
			"bap_unicast_audio_get_session_qos ok, sca 0x%x, packing 0x%x, framing 0x%x, sdu_interval_m_s 0x%x, sdu_interval_s_m 0x%x, latency_m_s 0x%x, latency_s_m 0x%x \r\n",
...
static uint16_t bt_stack_le_audio_unicast_cfg_ase_qos(void *data)
{
	...
	if (false == bap_unicast_audio_cfg_ase_qos(p_param->stream_session_handle, p_param->device_handle, p_param->ase_id, &cfg)) {
		BT_LOGE("%s fail: bap_unicast_audio_cfg_ase_qos fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_unicast_start(void *data)
{
	...
	if (false == bap_unicast_audio_start(stream_session_handle)) {
		BT_LOGE("%s fail: bap_unicast_audio_start fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_unicast_release(void *data)
{
	...
	if (false == bap_unicast_audio_release(stream_session_handle)) {
		BT_LOGE("%s fail: bap_unicast_audio_release fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_unicast_update(void *data)
{
	...
	if (false == bap_unicast_audio_update(stream_session_handle)) {
		BT_LOGE("%s fail: bap_unicast_audio_update fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_mcp_server_set_param(void *data)
{
	...
	if (false == mcp_server_set_param(p_info->service_id, &set_param)) {
		BT_LOGE("%s mcp_server_set_param fail\r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
static uint16_t bt_stack_le_audio_vocs_write_cp_by_group(void *data)
{
	...
	BT_LOGD("%s group_handle %p, srv_instance_id %d,cp_op 0x%x,volume_offset %d\r\n", __func__,
...
static uint16_t bt_stack_le_audio_aics_write_cp_by_group(void *data)
{
	...
	BT_LOGD("%s group_handle %p, srv_instance_id %d,cp_op 0x%x,gaining_setting %d\r\n", __func__,
...
{
	uint16_t ret = 0;
	BT_LOGD("bt_stack_le_audio_act_handle: act = %d \r\n", p_cmd->act);
	switch (p_cmd->act) {
	...
	case RTK_BT_LE_AUDIO_ACT_SYNC_REMOVE_DATA_PATH: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_SYNC_REMOVE_DATA_PATH \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_RELEASE: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_RELEASE \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_SETUP_DATA_PATH: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_SETUP_DATA_PATH \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_REMOVE_DATA_PATH: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_REMOVE_DATA_PATH \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_RECONFIG: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BROADCAST_SOURCE_RECONFIG \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_GET_BRS_CHAR_DATA: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_GET_BRS_CHAR_DATA \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_CP_ADD_SOURCE_BY_SYNC_INFO: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_CP_ADD_SOURCE_BY_SYNC_INFO \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_CP_MODIFY_SOURCE_BY_SYNC_INFO: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_CP_MODIFY_SOURCE_BY_SYNC_INFO \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_WRITE_CP_OP: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_WRITE_CP_OP \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_PAST: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_BASS_CLIENT_PAST \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_UNICAST_STOP: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_UNICAST_STOP \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_UNICAST_RELEASE: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_UNICAST_RELEASE \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_UNICAST_UPDATE: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_UNICAST_UPDATE \r\n");
	...
	case RTK_BT_LE_AUDIO_ACT_MCP_SERVER_SET_PARAM: {
		BT_LOGD("RTK_BT_LE_AUDIO_ACT_MCP_SERVER_SET_PARAM \r\n");
...
#endif
	default:
		BT_LOGE("bt_stack_le_audio_act_handle: unknown act: %d \r\n", p_cmd->act);
		ret = 0;
		break;
	}
