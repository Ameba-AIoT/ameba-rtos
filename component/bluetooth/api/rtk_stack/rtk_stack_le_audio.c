...
static uint16_t rtk_stack_gmas_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_GMAS_READ_IND: {
		...
		APP_PRINT_INFO3("LE_AUDIO_MSG_GMAS_READ_IND: conn_handle %x,cid %d,char_uuid %d\r\n", read_ind->conn_handle, read_ind->cid,
						read_ind->char_uuid);
		BT_API_PRINT(BT_API_DEBUG, "LE_AUDIO_MSG_GMAS_READ_IND: conn_handle %x,cid %d,char_uuid %d\r\n", read_ind->conn_handle,
		...
		default:
			BT_API_PRINT(BT_API_ERROR, "%s: unknown char_uuid 0x%x, not process\r\n", __func__, read_ind->char_uuid);
		...
		if (!gmas_features_read_confirm(read_ind->conn_handle, read_ind->cid,
			...
			BT_API_PRINT(BT_API_ERROR, "%s: gmas_features_read_confirm error\r\n", __func__);
			return BLE_AUDIO_CB_RESULT_APP_ERR;
		}
...
static uint16_t rtk_stack_gmas_client_handle_msg(T_LE_AUDIO_MSG msg, void *buf)
{
	...
	case LE_AUDIO_MSG_GMAS_CLIENT_DIS_DONE: {
		...
		APP_PRINT_INFO3("LE_AUDIO_MSG_GMAS_CLIENT_DIS_DONE: conn_handle %x,is_found %d,load_from_ftl %d\r\n", discovery_ind->conn_handle, discovery_ind->is_found,
						discovery_ind->load_from_ftl);
		BT_API_PRINT(BT_API_DEBUG, "LE_AUDIO_MSG_GMAS_CLIENT_DIS_DONE: conn_handle %x,is_found %d,load_from_ftl %d\r\n", discovery_ind->conn_handle,
					 discovery_ind->is_found, discovery_ind->load_from_ftl);
	...
	case LE_AUDIO_MSG_GMAS_CLIENT_READ_ROLE_RESULT: {
		...
		APP_PRINT_INFO3("LE_AUDIO_MSG_GMAS_CLIENT_READ_ROLE_RESULT: conn_handle 0x%x,role 0x%x,cause %d \r\n", read_res->conn_handle, read_res->gmap_role,
						read_res->cause);
		BT_API_PRINT(BT_API_DEBUG, "LE_AUDIO_MSG_GMAS_CLIENT_READ_ROLE_RESULT: conn_handle 0x%x,role 0x%x, cause %d\r\n", read_res->conn_handle, read_res->gmap_role,
					 read_res->cause);
	...
	case LE_AUDIO_MSG_GMAS_CLIENT_READ_FEATURE_RESULT: {
		...
		BT_API_PRINT(BT_API_DEBUG, "LE_AUDIO_MSG_GMAS_CLIENT_READ_FEATURE_RESULT: conn_handle 0x%x,char_uuid 0x%x,features 0x%x,cause %d\r\n",
					 read_feat->conn_handle, read_feat->char_uuid, read_feat->features, read_feat->cause);
...
static uint16_t bt_stack_le_audio_gmap_init(void *le_audio_app_conf)
{
	...
	if ((p_le_audio_app_conf->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_UGT) || (p_le_audio_app_conf->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_UGG) ||
		...
		if (gmas_init(gmap_role) == false) {
			BT_API_PRINT(BT_API_ERROR, "%s:gmas_init fail, return value is false\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	}
	if ((p_le_audio_app_conf->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_UGG) || (p_le_audio_app_conf->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_BGS)) {
		if (gmas_client_init() == false) {
			BT_API_PRINT(BT_API_ERROR, "%s:gmas_client_init fail, return value is false\r\n", __func__);
			return RTK_BT_ERR_LOWER_STACK_API;
		}
	}
	return RTK_BT_OK;
...
uint16_t bt_stack_le_audio_init(rtk_bt_app_conf_t *papp_conf, void *io_msg_q, void *evt_msg_q)
{
	...
	if (ret != RTK_BT_OK) {
		BT_API_PRINT(BT_API_ERROR, "%s:bt_stack_le_audio_gmap_init fail,ret= 0x%x\r\n", __func__, ret);
		return ret;
	}
#endif
...
static void bt_stack_le_audio_broadcast_source_cb(T_BROADCAST_SOURCE_HANDLE handle, uint8_t cb_type, void *p_cb_data)
{
	...
	case MSG_BROADCAST_SOURCE_STATE_CHANGE: {
		...
		if (p_sm_data->p_state_change->state == BROADCAST_SOURCE_STATE_CONFIGURED_STARTING) {
			BT_API_PRINT(BT_API_DEBUG, "%s: BROADCAST_SOURCE_STATE_CONFIGURED_STARTING\r\n", __func__);
		} else if (p_sm_data->p_state_change->state == BROADCAST_SOURCE_STATE_CONFIGURED) {
			BT_API_PRINT(BT_API_DEBUG, "%s: BROADCAST_SOURCE_STATE_CONFIGURED\r\n", __func__);
		}
...
static uint16_t bt_stack_le_audio_broadcast_source_create(void *data)
{
	...
	if (g_broadcast_source_handle == NULL) {
		...
		BT_API_PRINT(BT_API_DEBUG, "broadcast_source_add can add once only, because there is no broadcast_source_delete\r\n");
	...
	//      BT_API_PRINT(BT_API_ERROR, "%s: base delete group idx fail\r\n", __func__);
	//      return RTK_BT_ERR_LOWER_STACK_API;
	//  }
	//  g_group1_idx = 0xFF;
	// }
			...
			*p_param->group[i].group_idx = g_group1_idx;
			...
			for (j = 0; j < num_subgroups; j++) {
				...
				for (k = 0; k < num_bis; k++) {
					...
					BT_API_PRINT(BT_API_DEBUG, "group idx: %d, subgroup_idx[%d]: %d, bis_idx[%d]: %d \r\n", g_group1_idx, j, g_subgroup_idx[j], k, g_bis_idx[k]);
				}
			}
			goto out;
		}
...
static uint16_t bt_stack_le_audio_broadcast_source_config(void *data)
{
	...
	BT_API_PRINT(BT_API_DEBUG, "%s: T_BROADCAST_SOURCE_STATE state = 0x%x\r\n", __func__, src_info.state);
	...
	case AUDIO_GROUP_MSG_BAP_START_QOS_CFG: {
		...
		if (cb_ret == RTK_BT_EVT_CB_OK) {
			...
			if (false == bap_unicast_audio_get_session_info(p_data->handle, &session_info)) {
				BT_API_PRINT(BT_API_ERROR, "%s: bap_unicast_audio_get_session_info fail!\r\n", __func__);
				break;
			}
					...
					for (int j = 0; j < session_info.dev_info[i].ase_num; j++) {
						if (session_info.dev_info[i].ase_info[j].direction == RTK_BLE_AUDIO_ISO_DATA_PATH_TX) {
							if (false == bap_unicast_audio_cfg_ase_qos(p_data->handle, session_info.dev_info[i].dev_handle,
								...
								BT_API_PRINT(BT_API_ERROR, "%s: bap_unicast_audio_cfg_ase_qos fail!\r\n", __func__);
								break;
							}
...
static uint16_t bt_stack_le_audio_gmas_client_read_role(void *data)
{
	...
	if (false == gmas_read_gmap_role(p_conn_handle)) {
		BT_API_PRINT(BT_API_ERROR, "%s fail: gmas_read_role failed \r\n", __func__);
		return RTK_BT_ERR_LOWER_STACK_API;
	}
...
uint16_t bt_stack_le_audio_act_handle(rtk_bt_cmd_t *p_cmd)
{
	...
	case RTK_BT_LE_AUDIO_ACT_GMAS_READ_ROLE: {
		BT_API_PRINT(BT_API_DUMP, "RTK_BT_LE_AUDIO_ACT_GMAS_READ_ROLE \r\n");
		ret = bt_stack_le_audio_gmas_client_read_role(p_cmd->param);
		break;
	}
	case RTK_BT_LE_AUDIO_ACT_GMAS_READ_FEATURES: {
		BT_API_PRINT(BT_API_DUMP, "RTK_BT_LE_AUDIO_ACT_GMAS_READ_FEATURES \r\n");
