...
typedef enum {
	...
	RTK_BT_MESH_STACK_ACT_PB_GATT_CON,
	RTK_BT_MESH_STACK_ACT_PB_GATT_DISCON,
...
typedef enum {
	...
	RTK_BT_MESH_STACK_EVT_CAPABILITY_NOT_SUPPORT_OF_DEVICE,
...
/**
 * @typedef   rtk_bt_mesh_stack_act_pb_gatt_con_t
 * @brief     BLE MESH establish pb gatt connection param structure.
 */
typedef union {
	rtk_bt_le_create_conn_param_t conn_param;  // For RTK mesh stack, establish a BLE link
	struct {
		uint8_t is_prov_link;  // Whether is provisioning link
		union {
			uint8_t uuid[16];  // For zephyr mesh stack, establish a PB-GATT link and start provisioning
			uint8_t net_idx;  // For zephyr mesh stack, establish a BLE GATT link with any provisioned device support Mesh Proxy service, can send or receive mesh message optionally
		} u;
	} param;
} rtk_bt_mesh_stack_act_pb_gatt_con_t;

/**
 * @typedef   rtk_bt_mesh_stack_act_pb_gatt_discon_t
 * @brief     BLE MESH pb gatt disconnection param structure.
 */
typedef union {
	uint16_t conn_handle;  // For RTK mesh stack, disconnect a BLE GATT link use connect handle
	struct {
		uint8_t is_prov_link;  // Whether is provisioning link
		uint8_t net_idx;  // For zephyr mesh stack, disconnect a BLE GATT link with the provisioned device support Mesh Proxy service
	} param;
...
typedef enum {
	...
	RTK_BT_MESH_LPN_REQ_REASON_UNKNOWN,
...
typedef enum {
	...
	// RTK_BT_MESH_PB_GENERIC_CB_MSG_TRANSMITED maybe usefull calc ecdh secret after send public key to save time
...
/**
 * @typedef   rtk_bt_mesh_stack_set_retrans_param_t
 * @brief     BLE MESH set retransmit parameter.
 */
typedef struct {
	uint8_t ttl;
