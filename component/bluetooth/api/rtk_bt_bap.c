...
uint16_t rtk_bt_bap_ext_scan(bool enable)
{
	if (enable) {
		...
		if (rtk_bt_le_gap_ext_scan_start()) {
			printf("%s rtk_bt_le_gap_ext_scan_set_param fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
	} else {
...
uint16_t rtk_bt_bap_bass_scan(bool enable, bool remote_scan_enable, uint8_t group_idx)
{
	...
	if (rtk_bt_le_audio_group_get_used_device_num(group_handle, true, &device_num)) {
		printf("%s rtk_bt_le_audio_group_get_used_device_num fail \r\n", __func__);
	} else {
	...
	if (!p_dev_tbl) {
		printf("%s p_dev_tbl osif_mem_alloc fail \r\n", __func__);
		return RTK_BT_FAIL;
	} else {
	...
	for (i = 0; i < device_num; i++) {
		...
		if (ret != RTK_BT_OK) {
			printf("%s get_all_device_info fail for group_handle = %p \r\n", __func__, group_handle);
			continue;
		}
...
uint16_t rtk_bt_bap_broadcast_source_start(void)
{
	...
	if (rtk_bt_le_audio_get_prefer_codec_cfg(bap_broadcast_source_cfg.cfg_codec_index, &codec_cfg)) {
		printf("%s rtk_bt_le_audio_get_prefer_codec_cfg fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	printf("%s prefer_codec_cfg: type_exist: 0x%x frame_duration: 0x%x sample_frequency: 0x%x codec_frame_blocks_per_sdu: %d  octets_per_codec_frame: %d audio_channel_allocation: 0x%ld\r\n",
		   __func__,
	...
	if (bap_broadcast_source_cfg.qos_manual_cfg == true) {
		...
		if (rtk_bt_le_audio_qos_preferred_cfg_get(bap_broadcast_source_cfg.cfg_codec_index, bap_broadcast_source_cfg.cfg_qos_type, &qos_cfg_preferred)) {
			printf("%s rtk_bt_le_audio_qos_preferred_cfg_get fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
	}
	...
	printf("%s: presentation_delay:%ld sdu_interval: %ld rtn: %d max_sdu: %d max_transport_latency: %d framing: %d\r\n", __func__,
	...
	if (rtk_bt_le_audio_broadcast_source_create(1, &def_le_audio_broadcast_source_group_param, &p_bap_bro_sour_info->broadcast_source_handle)) {
		printf("%s rtk_bt_le_audio_broadcast_source_create fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	if (rtk_bt_le_gap_get_bd_addr(&bd_addr)) {
		printf("%s rtk_bt_le_gap_get_bd_addr fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	...
	if (rtk_bt_le_audio_broadcast_source_config(p_bap_bro_sour_info->broadcast_source_handle,
		...
		printf("%s rtk_bt_le_audio_broadcast_source_config fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
...
uint16_t rtk_bt_bap_broadcast_source_stop(void)
{
	...
	for (uint8_t i = 0; i < app_bap_bro_sour_info.bis_info.num_bis; i++) {
		...
		printf("broadcast source remove data path %s for broadcast_source_handle 0x%p bis_index %d,bis_conn_handle 0x%x, ret: 0x%x\r\n",
		...
		if (ret == RTK_BT_OK) {
			...
			printf("%s: rtk_bt_le_audio_broadcast_source_remove_data_path fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
	}
														  ...
														  rtk_bt_le_audio_sync_handle_t sync_handle)
{
	...
	if (!p_sync_dev_info) {
		printf("%s not find sync dev info for sync_handle %p\r\n", __func__, sync_handle);
		return RTK_BT_FAIL;
	}
	...
	if (num_bis) {
		...
		printf("%s num_bis=%d, p_sync_dev_info->bis_idx[0]=%d\r\n", __func__, num_bis, p_sync_dev_info->bis_idx[0]);
		return RTK_BT_OK;
	}
...
static uint16_t rtk_bt_le_audio_broadcast_sink_big_sync(app_bt_le_audio_bap_broadcast_sink_info_t *p_bap_bro_sink_info, rtk_bt_le_addr_t adv_addr)
{
	...
	if (p_sync_dev_info->pa_sync_state == RTK_BT_LE_AUDIO_PA_SYNC_STATE_SYNCHRONIZED &&
		...
		printf("broadcast sink big establish %s for sync_handle 0x%p ! ret: 0x%x\r\n",
...
uint16_t rtk_bt_bap_broadcast_sync_start(rtk_bt_bap_bass_scan_dev_info_t *p_info)
{
	...
	if (!p_bscan_dev_info) {
		printf("%s: (%s) is not contained within bass scan device list, please do rtk_bt_bap_bass_scan firstly \r\n", __func__, addr_str);
		return RTK_BT_FAIL;
	}
	/* do pa sync */
	{
		...
		if (ret != RTK_BT_OK) {
			printf("%s rtk_bt_le_audio_sync_find fail (ret=0x%x)\r\n", __func__, ret);
			return RTK_BT_FAIL;
		}
		...
		if (ret != RTK_BT_OK) {
			printf("%s rtk_bt_le_gap_ext_scan_set_param fail for addr_str (%s)\r\n", __func__, addr_str);
			return ret;
		}
		/* if ext scan is stop previous, rtk_bt_le_gap_ext_scan_start() will be scuess */
		/* else if ext scan is started previous, rtk_bt_le_gap_ext_scan_start() will be fail cause currently is started */
		/* so there is no need to judge return value of rtk_bt_le_gap_ext_scan_start() */
		/* if rtk_bt_le_gap_ext_scan_start() finally fail due to other reason, the flow will also be terminated by rtk_bt_le_audio_broadcast_sink_big_sync() following */
		rtk_bt_le_gap_ext_scan_start();
	}
...
static uint16_t rtk_bt_bap_broadcast_big_sync_term(app_bt_le_audio_sync_dev_info_t *p_sync_dev_info)
{
	...
	if (p_sync_dev_info->big_sync_state == RTK_BT_LE_AUDIO_BIG_SYNC_STATE_SYNCHRONIZED) {
		...
		printf("broadcast sink big terminate %s for addr_str %s ! ret: 0x%x\r\n", ((RTK_BT_OK != ret) ? "fail" : "ok"),
			   addr_str, ret);
	}
...
uint16_t rtk_bt_bap_broadcast_sync_term(rtk_bt_le_addr_t *padv_addr)
{
	...
	if (rtk_bt_bap_broadcast_big_sync_term(p_sync_dev_info)) {
		printf("%s: big sync terminate fail for addr_str (%s)\r\n", __func__, addr_str);
		return RTK_BT_FAIL;
	}
	if (p_sync_dev_info->pa_sync_state != RTK_BT_LE_AUDIO_PA_SYNC_STATE_TERMINATED) {
		...
		printf("%s rtk_bt_le_audio_pa_sync_terminate addr_str (%s) %s, ret: 0x%x\r\n", __func__, addr_str,
			   ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
	}
...
uint16_t rtk_bt_bap_broadcast_sink_cfg(uint8_t channel)
{
	...
	case RTK_BT_LE_AUDIO_STEREO:
		...
		break;
	default:
		printf("%s: unknown channel allocation \r\n", __func__);
		break;
	}
...
uint16_t rtk_bt_bap_broadcast_assistant_sync_term(rtk_bt_le_addr_t *padv_addr)
{
	...
	printf("rtk_bt_le_audio_sync_release %s, ret: 0x%x\r\n", ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
	ret = app_bt_le_audio_sync_dev_list_remove(p_sync_dev_info->sync_handle);
	printf("%s app_bt_le_audio_sync_dev_list_remove addr_str (%s) %s, ret: 0x%x\r\n", __func__, addr_str,
		   ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
...
uint16_t rtk_bt_bap_broadcast_assistant_reception_start(rtk_bt_le_addr_t *padv_addr, uint8_t group_idx)
{
	...
	if (group_idx >= group_num) {
		printf("%s error: group_idx(%d) >= group_num (%d)\r\n", __func__, group_idx, group_num);
		return RTK_BT_ERR_PARAM_INVALID;
	}
	group_handle = app_bt_le_audio_group_list_find_by_group_idx(group_idx);
	if (group_handle == NULL) {
	...
	p_sync_dev_info = app_bt_le_audio_sync_dev_list_find_by_addr(*padv_addr);
	if (p_sync_dev_info == NULL) {
		printf("%s: src_adv_addr (%s) not find\r\n", __func__, padv_addr->addr_val);
		return RTK_BT_ERR_PARAM_INVALID;
	} else {
		if (p_sync_dev_info->pa_sync_state == RTK_BT_LE_AUDIO_PA_SYNC_STATE_SYNCHRONIZED) {
	...
	if (ret != RTK_BT_OK) {
		printf("%s get_device_num fail for group_handle = %p\r\n", __func__, group_handle);
		return RTK_BT_FAIL;
	} else {
		printf("%s device_num = %d\r\n", __func__, device_num);
	}
	...
	p_dev_tbl = (rtk_bt_le_audio_group_device_info_t *)osif_mem_alloc(RAM_TYPE_DATA_ON, dev_tbl_size);
	if (p_dev_tbl == NULL) {
		printf("%s p_dev_tbl osif_mem_alloc fail, size=%d\r\n", __func__, (int)dev_tbl_size);
	...
	for (i = 0; i < device_num; i++) {
		...
		if (p_dev_tbl[i].is_used && p_dev_tbl[i].conn_state == RTK_BT_LE_AUDIO_GAP_CONN_STATE_CONNECTED) {
			...
			if (p_device_info->bass_is_found == 0) {
				printf("%s bass_is_found not support for conn_handle=%d\r\n", __func__, p_dev_tbl[i].conn_handle);
				continue;
			}
			ret = rtk_bt_le_audio_pacs_get_bis_array_by_sync_info(p_dev_tbl[i].conn_handle, p_sync_dev_info->sync_handle, RTK_BT_LE_AUDIO_PACS_BA_CHECK_FILTER_CODEC,
																  &bis_array);
			if (ret != RTK_BT_OK) {
				printf("%s get_bis_array_by_sync_info fail for conn_handle = %d\r\n", __func__, p_dev_tbl[i].conn_handle);
				continue;
			}
			...
			printf("%s bass add source for sync_handle %p %s, ret: 0x%x\r\n", __func__, p_sync_dev_info->sync_handle,
				   ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
		}
	}
	if (p_dev_tbl) {
		...
		if (p_dev_tbl[i].is_used && p_dev_tbl[i].conn_state == RTK_BT_LE_AUDIO_GAP_CONN_STATE_CONNECTED) {
			...
			if (p_device_info->bass_is_found) {
				...
				if (j != p_device_info->brs_char_num) {
					...
					printf("%s bass remove source for sync_handle %p %s, ret: 0x%x\r\n", __func__, p_sync_dev_info->sync_handle,
						   ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
				} else {
					printf("%s not find sync_handle %p in brs_char_tbl for conn_handle=%d\r\n", __func__, p_sync_dev_info->sync_handle, p_dev_tbl[i].conn_handle);
				}
			}
		}
	}
...
static uint16_t rtk_bt_bap_unicast_client_audio_config(rtk_bt_le_audio_group_handle_t group_handle, rtk_bt_le_audio_device_handle_t *device_handle_tbl)
{
	...
	if (rtk_bt_le_audio_unicast_get_cfg_mask(p_group_info->stream_session_handle,
											 ...
											 device_handle_tbl,
											 &audio_cfg_mask)) {
		printf("%s: rtk_bt_le_audio_unicast_get_cfg_mask fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	if (audio_cfg_mask) {
		if (p_group_info->play_mode == RTK_BT_LE_AUDIO_PLAY_MODE_MEDIA) {
	...
	if (audio_cfg_type >= RTK_BT_LE_UNICAST_AUDIO_CFG_MAX) {
		printf("%s: audio_cfg_type (0x%x) error\r\n", __func__, audio_cfg_type);
		return RTK_BT_ERR_UNSUPPORTED;
	} else {
		printf("%s: get audio_cfg_type (0x%x) ok\r\n", __func__, audio_cfg_type);
	}
...
static void le_audio_unicast_client_fix_codec_mask(uint32_t *sup_snk_mask)
{
	...
	printf("%s before fix_codec_mask mask = 0x%x\r\n", __func__, (unsigned int)mask);
...
static uint16_t rtk_bt_bap_unicast_client_codec_cfg(rtk_bt_le_audio_group_handle_t group_handle)
{
	...
	for (i = 0; i < p_group_info->dev_num; i++) {
		...
		if (p_device_info == NULL) {
			printf("%s p_device_info is NULL\r\n", __func__);
			continue;
		}
		...
		while (sup_snk_mask) {
			...
			if (sup_snk_mask & 0x01) {
				if (rtk_bt_le_audio_get_prefer_codec_cfg(idx, &codec_cfg)) {
					printf("%s rtk_bt_le_audio_get_prefer_codec_cfg fail idx 0x%x \r\n", __func__, idx);
					return RTK_BT_FAIL;
				}
				printf("%s prefer_codec_cfg idx is %d\r\n", __func__, idx);
				break;
			}
			sup_snk_mask >>= 1;
			idx++;
	...
	if (idx >= RTK_BT_LE_CODEC_CFG_ITEM_LC3_MAX) {
		printf("%s prefer_codec_cfg idx (%d) is unsupport\r\n", __func__, idx);
		return RTK_BT_ERR_UNSUPPORTED;
	} else {
		printf("%s codec cfg idx %d, type_exist 0x%x, frame_duration %d, sample_frequency %d, codec_frame_blocks_per_sdu %d, octets_per_codec_frame=%d, audio_channel_allocation=0x%x)\r\n",
			   __func__,
	...
	if ((p_codec_cfg->type_exist & RTK_BT_LE_CODEC_CFG_SAMPLING_FREQUENCY_EXIST) == 0) {
		printf("%s sample_frequency is exist, use default 16k\r\n", __func__);
	...
	if ((p_codec_cfg->type_exist & RTK_BT_LE_CODEC_CFG_FRAME_DURATION_EXIST) == 0) {
		printf("%s frame_duration is exist, use default 10ms\r\n", __func__);
	...
	if ((p_codec_cfg->type_exist & RTK_BT_LE_CODEC_CFG_OCTET_PER_CODEC_FRAME_EXIST) == 0) {
		printf("%s codec_frame_blocks_per_sdu is exist, use default 40\r\n", __func__);
	...
	if ((p_codec_cfg->type_exist & RTK_BT_LE_CODEC_CFG_TYPE_BLOCKS_PER_SDU_EXIST) == 0) {
		printf("%s codec_frame_blocks_per_sdu is exist, use default 1\r\n", __func__);
...
static uint16_t rtk_bt_bap_unicast_client_qos_cfg(rtk_bt_le_audio_group_handle_t group_handle)
{
	...
	printf("%s: sdu_interval:%ld, framing:0x%x, max_sdu:%d, rtn:%d, max_transport_latency:%d, presentation_delay:%ld\r\n",
...
static uint16_t rtk_bt_le_audio_unicast_client_cfg_ase_codec(rtk_bt_le_audio_group_handle_t group_handle)
{
	...
	if (rtk_bt_le_audio_unicast_get_session_info(p_group_info->stream_session_handle, &session_info)) {
		printf("%s rtk_bt_le_audio_unicast_get_session_info fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	printf("%s: stream_session_handle %p state=%d cfg_type=%d conn_dev_num=%d dev_num=%d\r\n", __func__,
	...
	for (i = 0; i < session_info.dev_num; i++) {
		printf("%s i=%d,dev_handle=%p,conn_state=%d,ase_num=%d\r\n", __func__, i,
		...
		if (rtk_bt_le_audio_group_get_device_info(group_handle,
												  ...
												  &dev_info)) {
			printf("%s rtk_bt_le_audio_group_get_device_info fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
		if (dev_info.conn_state != RTK_BT_LE_AUDIO_GAP_CONN_STATE_CONNECTED) {
			printf("%s skip cfg_ase_codec when not connect \r\n", __func__);
			continue;
		}
		if (rtk_bt_le_audio_pacs_get_info(dev_info.conn_handle, &pacs_info)) {
			printf("%s rtk_bt_le_audio_pacs_get_info fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
		for (j = 0; j < session_info.dev_info[i].ase_num; j++) {
			printf("%s j=%d, ase_id=%d,cfg_idx=%d,ase_state=%d,direction=%d,channel_num=%d\r\n", __func__, j,
			...
			if (session_info.dev_info[i].ase_info[j].direction == RTK_BLE_AUDIO_SINK) {
				...
				printf("%s: source ase prefer codec: audio_channel_allocation=0x%x\r\n", __func__,
					   (unsigned int)app_lea_source_ase_prefer_codec.codec_cfg.audio_channel_allocation);
				printf("%s: source ase prefer codec: type_exist 0x%x, frame_duration %d, sample_frequency %d, codec_frame_blocks_per_sdu %d,octets_per_codec_frame=%d)\r\n",
					   __func__,
				...
				printf("%s skip cfg_ase_codec direction is wrong \r\n", __func__);
				continue;
			}
			printf("%s unicast_cfg_ase_codec audio_channel_allocation=0x%x\r\n", __func__, (unsigned int)p_prefer_codec->codec_cfg.audio_channel_allocation);
			printf("%s type_exist 0x%x, frame_duration %d, sample_frequency %d, codec_frame_blocks_per_sdu %d,octets_per_codec_frame=%d)\r\n", __func__,
			...
			if (session_info.dev_info[i].ase_info[j].direction == RTK_BLE_AUDIO_SINK) {
				if (rtk_bt_le_audio_unicast_cfg_ase_codec(p_group_info->stream_session_handle, session_info.dev_info[i].dev_handle,
					...
					printf("%s rtk_bt_le_audio_unicast_cfg_ase_codec fail \r\n", __func__);
					return RTK_BT_FAIL;
				}
			} else if (session_info.dev_info[i].ase_info[j].direction == RTK_BLE_AUDIO_SOURCE) {
#if defined(LEA_SOURCE_ASE_PREFER_CODEC_CONFIG) && LEA_SOURCE_ASE_PREFER_CODEC_CONFIG
...
uint16_t rtk_bt_bap_unicast_client_start(uint8_t group_idx, rtk_bt_le_audio_play_mode_t play_mode)
{
	...
	if (group_idx >= group_num) {
		printf("%s: group_idx %d must less than group num %d\r\n", __func__, group_idx, group_num);
		return RTK_BT_ERR_PARAM_INVALID;
	}
	group_handle = app_bt_le_audio_group_list_find_by_group_idx(group_idx);
	if (!group_handle) {
	...
	printf("%s: start, group_idx=%d,group_num=%d,play_mode=%d\r\n", __func__, group_idx, group_num, play_mode);
	p_group_info = app_bt_le_audio_group_list_find(group_handle);
	if (p_group_info == NULL) {
		printf("%s: p_group_info is NULL\r\n", __func__);
		return RTK_BT_FAIL;
	...
	if (!p_group_info->stream_session_handle) {
		if (rtk_bt_le_audio_stream_session_allocate(p_group_info->group_handle, &p_group_info->stream_session_handle)) {
			printf("%s: rtk_bt_le_audio_stream_session_allocate fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
		if (!p_group_info->stream_session_handle) {
			printf("%s: allocate stream_session_handle fail\r\n", __func__);
			return RTK_BT_FAIL;
		}
	}
	...
	if (!p_device_handle_tbl) {
		printf("%s osif_mem_alloc (len=%d) fail\r\n", __func__, device_handle_tbl_len);
		return RTK_BT_FAIL;
	}
	...
	if (rtk_bt_bap_unicast_client_audio_config(p_group_info->group_handle, p_device_handle_tbl)) {
		printf("%s rtk_bt_bap_unicast_client_audio_config fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	...
	if (rtk_bt_bap_unicast_client_codec_cfg(p_group_info->group_handle)) {
		printf("%s rtk_bt_bap_unicast_client_codec_cfg fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	/* config unicast session prefer qos*/
	if (rtk_bt_bap_unicast_client_qos_cfg(p_group_info->group_handle)) {
		printf("%s: rtk_bt_bap_unicast_client_qos_cfg fail \r\n", __func__);
		return RTK_BT_FAIL;
	}
	if (p_group_info->bap_state == RTK_BT_LE_AUDIO_STREAM_STATE_IDLE) {
		...
		printf("%s: no need to start unicast stream\r\n", __func__);
		if (p_device_handle_tbl) {
...
uint16_t rtk_bt_bap_unicast_client_release(uint8_t group_idx)
{
	...
	if (p_group_info->bap_state == RTK_BT_LE_AUDIO_STREAM_STATE_CONFIGURED ||
		...
		if (rtk_bt_le_audio_unicast_release(p_group_info->stream_session_handle)) {
			printf("%s: rtk_bt_le_audio_unicast_release fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
		printf("%s: release successfully\r\n", __func__);
		...
		if (rtk_bt_le_audio_stream_session_release(p_group_info->stream_session_handle)) {
			printf("%s: rtk_bt_le_audio_stream_session_release fail \r\n", __func__);
			return RTK_BT_FAIL;
		}
		printf("%s: remove session ok\r\n", __func__);
	} else {
		return RTK_BT_FAIL;
	}
