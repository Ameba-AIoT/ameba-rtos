...
} _PACKED4_ trans_unseg_access_t, *trans_unseg_access_p;
...
} _PACKED4_ trans_seg_msc_t, *trans_seg_msc_p;
...
} _PACKED4_ trans_seg_access_t, *trans_seg_access_p;
...
typedef struct
{
    ...
    uint8_t seg: 1;
    uint8_t params[11];
} _PACKED4_ trans_unseg_ctl_t, *trans_unseg_ctl_p;
...
typedef struct
{
    ...
    uint32_t seq_zero: 13;
    uint32_t rfu: 1;
} _PACKED4_ trans_seg_ctl_msc_t, *trans_seg_ctl_msc_p;
...
typedef struct
{
    ...
    uint8_t params[8];
} _PACKED4_ trans_seg_ctl_t, *trans_seg_ctl_p;
...
typedef struct
{
    ...
    uint8_t block_ack[4];
} _PACKED4_ trans_seg_ack_t, *trans_seg_ack_p;

typedef struct
{
    uint8_t fsn: 1; //!< Friend Sequence Number
    uint8_t rfu: 7;
} _PACKED4_ frnd_poll_t, *frnd_poll_p;
...
} _PACKED4_ frnd_update_t, *frnd_update_p;
...
} _PACKED4_ frnd_criteria_t, *frnd_criteria_p;
...
} _PACKED4_ frnd_req_t, *frnd_req_p;
...
} _PACKED4_ frnd_offer_t, *frnd_offer_p;

typedef struct
{
    uint16_t lpn_addr; //!< Low Power Node Unicast Address
    uint16_t lpn_counter;
} _PACKED4_ frnd_clear_t, *frnd_clear_p;

typedef struct
{
    uint16_t lpn_addr; //!< Low Power Node Unicast Address
    uint16_t lpn_counter;
} _PACKED4_ frnd_clear_conf_t, *frnd_clear_conf_p;

typedef struct
{
    uint8_t tran_num;
    uint16_t group_addr[5]; //!< List of Group Addresses and Virtual Addresses.
} _PACKED4_ frnd_sub_list_add_t, *frnd_sub_list_add_p;

typedef struct
{
    uint8_t tran_num;
    uint16_t group_addr[5]; //!< List of Group Addresses and Virtual Addresses.
} _PACKED4_ frnd_sub_list_remove_t, *frnd_sub_list_remove_p;

typedef struct
{
    uint8_t tran_num;
} _PACKED4_ frnd_sub_list_conf_t, *frnd_sub_list_conf_p;
/** @} */
...
} _PACKED4_ trans_hb_t, *trans_hb_p;
...
} _PACKED4_ trans_ping_t, *trans_ping_p;
...
} _PACKED4_ trans_pong_t, *trans_pong_p;
#endif
...
#if MESH_LPN
    plt_timer_t timer_lpa; //!< lpn poll ack
#endif
...
#if MESH_LPN
void trans_lpn_poll_ack_handle_timeout(void);
#endif
///@endcond
