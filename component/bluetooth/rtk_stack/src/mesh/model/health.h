...
} _PACKED4_ health_curt_stat_t, *health_curt_stat_p;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_HEALTH_FAULT_GET)];
    uint16_t company_id;
} _PACKED4_ health_fault_get_t, *health_fault_get_p;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_HEALTH_FAULT_CLEAR_UNACK)];
    uint16_t company_id;
} _PACKED4_ health_fault_clear_unack_t, *health_fault_clear_unack_p;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_HEALTH_FAULT_CLEAR)];
    uint16_t company_id;
} _PACKED4_ health_fault_clear_t, *health_fault_clear_p;
...
typedef struct
{
    ...
    uint8_t test_id;
    uint16_t company_id;
} _PACKED4_ health_fault_test_t, *health_fault_test_p;
...
typedef struct
{
    ...
    uint8_t test_id;
    uint16_t company_id;
} _PACKED4_ health_fault_test_unack_t, *health_fault_test_unack_p;
...
#define HEALTH_SERVER_GET_PERIOD          				3 //!< @ref health_server_period_t
#define HEALTH_SERVER_SET_PERIOD    					4 //!< @ref health_server_period_t
#define HEALTH_SERVER_GET_ATTENTION                     5 //!< @ref health_server_attn_t
#define HEALTH_SERVER_SET_ATTENTION                		6 //!< @ref health_server_attn_t
#define HEALTH_SERVER_GET_CURRENT          				7 //!< @ref health_server_current_get_t
...
typedef struct {
    ...
    uint8_t fault_count;
	uint8_t *fault_array;
} health_server_current_get_t;

typedef struct {
    uint16_t company_id;
	uint8_t *fault_array;
} health_server_fault_clear_t;

typedef struct {
    uint8_t test_id;
    uint16_t company_id;
	uint8_t *fault_array;
} health_server_fault_test_t;

typedef struct {
	uint8_t fast_period_divisor;
} health_server_period_t;

typedef struct {
	uint8_t attn;
} health_server_attn_t;
