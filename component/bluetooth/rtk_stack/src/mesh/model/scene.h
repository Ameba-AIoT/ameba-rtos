...
typedef struct
{
    ...
    uint16_t current_scene;
    uint16_t scenes[0];
} _PACKED4_ scene_register_status_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_SCENE_DELETE)];
    uint16_t scene_number;
} _PACKED4_ scene_delete_t;
...
typedef struct
{
    uint16_t scene_number;
    scene_status_code_t status;
} scene_setup_server_get_register_status_t;
...
} scene_server_get_status_recall_t;

typedef struct {
	uint16_t num_scenes;
	uint16_t *scenes; //variable length
} scene_server_get_scenes_t;

typedef struct {
	uint16_t num_scenes;
} scene_server_get_scenes_num_t;
