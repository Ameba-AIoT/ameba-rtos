...
} _PACKED4_ light_lc_om_get_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_LIGHT_LC_MODE_SET)];
    uint8_t mode;
} _PACKED4_ light_lc_om_set_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_LIGHT_LC_MODE_STATUS)];
    uint8_t mode;
} _PACKED4_ light_lc_om_status_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_LIGHT_LC_LIGHT_ON_OFF_GET)];
} _PACKED4_ light_lc_light_on_off_get_t;
...
typedef struct
{
    ...
    uint8_t delay;
} _PACKED4_ light_lc_light_on_off_set_t;
...
typedef struct
{
    ...
    generic_transition_time_t trans_time;
} _PACKED4_ light_lc_light_on_off_status_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_LIGHT_LC_PROPERTY_GET)];
    uint16_t property_id;
} _PACKED4_ light_lc_property_get_t;
...
} _PACKED4_ light_lc_property_set_t;
...
typedef struct
{
    ...
    uint16_t property_id;
    uint8_t property_value[0];
} _PACKED4_ light_lc_property_status_t;
/** @} */
