...
} _PACKED4_ blob_transfer_start_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_BLOB_TRANSFER_CANCEL)];
    uint8_t blob_id[8];
} _PACKED4_ blob_transfer_cancel_t;
...
} _PACKED4_ blob_transfer_status_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_BLOB_BLOCK_GET)];
} _PACKED4_ blob_block_get_t;
...
} _PACKED4_ blob_chunk_transfer_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_BLOB_INFO_GET)];
} _PACKED4_ blob_info_get_t;
