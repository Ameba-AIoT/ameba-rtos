...
} _PACKED4_ light_lightness_get_t;
...
} _PACKED4_ light_lightness_set_t;
...
} _PACKED4_ light_lightness_stat_t;

typedef struct
{
    uint8_t opcode[ACCESS_OPCODE_SIZE(MESH_MSG_LIGHT_LIGHTNESS_LINEAR_GET)];
} _PACKED4_ light_lightness_linear_get_t;
...
typedef struct
{
    ...
    /* If the Transition Time field is present, the Delay field shall also be present;
       otherwise these fields shall not be present. */
    uint8_t delay; //!< Message execution delay in 5 millisecond steps
} _PACKED4_ light_lightness_linear_set_t;
...
typedef struct
{
    ...
    uint16_t range_min;
    uint16_t range_max;
} _PACKED4_ light_lightness_range_set_t, *light_lightness_range_set_p;
...
typedef struct
{
    ...
    uint16_t range_min;
    uint16_t range_max;
} _PACKED4_ light_lightness_range_stat_t;
/** @} */
