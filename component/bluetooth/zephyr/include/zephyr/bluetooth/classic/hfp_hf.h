...
struct bt_hfp_hf_cb {
	...
	void (*xapl_complete)(struct bt_conn *conn, uint32_t xapl_feat);

	void (*speaker_volume_set)(struct bt_conn *conn, uint32_t volume);

	void (*microphone_volume_set)(struct bt_conn *conn, uint32_t volume);
#endif
};
