...
struct bt_hfp_ag_cb {
	...
	void (*microphone_volume_ind)(struct bt_hfp_ag *ag, uint32_t volume);

	void (*dial)(struct bt_hfp_ag *ag, const char *number);

	void (*dial_last)(struct bt_hfp_ag *ag);

	void (*dial_cancel)(struct bt_hfp_ag *ag);
#endif
};
