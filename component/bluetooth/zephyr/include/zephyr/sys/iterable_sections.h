...
#define STRUCT_ARRAY_DECLARE(struct_type) \
  struct struct_type *STRUCT_ARRAY(struct_type)[]


#define STRUCT_ARRAY_SIZE(struct_type) \
  _CONCAT(STRUCT_ARRAY(struct_type), _size)

#define STRUCT_ARRAY_SIZE_DECLARE(struct_type) \
  int STRUCT_ARRAY_SIZE(struct_type)

/**
 * @brief Iterate over a specified iterable section (alternate) for __in_section is not supported.
 */
#define STRUCT_SECTION_FOREACH_ALTERNATE_NO_SECTION(struct_type, iterator)                          \
	extern STRUCT_ARRAY_DECLARE(struct_type);                             \
	extern STRUCT_ARRAY_SIZE_DECLARE(struct_type);                             \
  int _##struct_type##_idx = 0;                                                           \
	for (struct struct_type *iterator = STRUCT_ARRAY(struct_type)[_##struct_type##_idx];                  \
		     _##struct_type##_idx < STRUCT_ARRAY_SIZE(struct_type);                       \
	     iterator = STRUCT_ARRAY(struct_type)[++_##struct_type##_idx])
...
#define STRUCT_SECTION_FOREACH_ALTERNATE(secname, struct_type, iterator) \
	STRUCT_SECTION_FOREACH_ALTERNATE_NO_SECTION(struct_type, iterator)
/**
