...
enum bt_rslv_pend_reason {
	...
	BT_RSLV_PEND_BY_CONNECT = 0x4,
};

struct bt_id_cb {
	void (*rslv_op_complete) (struct bt_keys *keys, bool is_add, uint8_t privacy_mode);
	void (*rslv_op_pend) (enum bt_rslv_pend_reason pend_reason);
};

int bt_id_register_cb(struct bt_id_cb *id_cb);
#endif
