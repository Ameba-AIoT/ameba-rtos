...
static bool bt_stack_sbc_decoder_init(sbc_decoder_state_t *pstate, sbc_mode_t mode, void *pcontext)
{
	...
	case SBC_MODE_STANDARD:
		...
		status = OI_CODEC_SBC_DecoderReset(&(bluedroid_decode_state.decoder_context), bluedroid_decode_state.decoder_data, sizeof(bluedroid_decode_state.decoder_data),
										   2, 2, FALSE);
		break;
	case SBC_MODE_mSBC:
		status = OI_CODEC_mSBC_DecoderReset(&(bluedroid_decode_state.decoder_context), bluedroid_decode_state.decoder_data,
											sizeof(bluedroid_decode_state.decoder_data));
		break;
	default:
		break;
	}
									  ...
									  int sample_rate, int bitpool, sbc_channel_mode_t channel_mode)
{
	...
	case SBC_MODE_STANDARD:
		...
		case 16000:
			...
			break;
		case 32000:
			bluedroid_encoder_state.context.s16SamplingFreq = SBC_sf32000;
			break;
		case 44100:
			bluedroid_encoder_state.context.s16SamplingFreq = SBC_sf44100;
			break;
		case 48000:
			bluedroid_encoder_state.context.s16SamplingFreq = SBC_sf48000;
			break;
		default:
			bluedroid_encoder_state.context.s16SamplingFreq = 0;
			break;
		}
		break;
	case SBC_MODE_mSBC:
...
static void sbc_decoder_process_msbc_data(sbc_decoder_state_t *state, int packet_status_flag, uint8_t *buffer, int size, int16_t *ppcm_data)
{
	...
	while (input_bytes_to_process > 0) {
		...
		case OI_STATUS_INVALID_PARAMETERS:
			...
			if (OI_CODEC_mSBC_DecoderReset(&(bd_decoder_state.decoder_context), bd_decoder_state.decoder_data,
										   sizeof(bd_decoder_state.decoder_data)) != OI_STATUS_SUCCESS) {
...
#endif
static uint16_t sbc_decoder_process_sbc_data(sbc_decoder_state_t *state, uint8_t *buffer, int size, int16_t *ppcm_data)
{
	bludroid_decoder_state_t *decoder_state = (bludroid_decoder_state_t *)state->decoder_state;
	int keep_decoding = 1;
...
static uint16_t decoder_process_data(sbc_decoder_state_t *pstate, int packet_status_flag, uint8_t *buffer, int size, int16_t *ppcm_data, uint32_t *ppcm_size)
{
	...
	if (pstate->mode == SBC_MODE_mSBC) {
		...
		status = sbc_decoder_process_sbc_data(pstate, buffer, size, ppcm_data);
...
static uint16_t sbc_encode_process_mono_data(int16_t *data, uint32_t size, struct enc_codec_buffer *pencoder_buffer, uint8_t *p_frame_num,
...
static uint16_t sbc_encode_process_dual_data(int16_t *data, uint32_t size, struct enc_codec_buffer *pencoder_buffer, uint8_t *p_frame_num,
											 uint32_t *p_actual_len)
{
