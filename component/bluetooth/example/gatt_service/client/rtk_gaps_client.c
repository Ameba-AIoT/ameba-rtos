...
#if defined(GAPS_CLIENT_SHOW_DETAIL) && GAPS_CLIENT_SHOW_DETAIL
...
static void gaps_client_read_res_hdl(void *data)
{
	...
	if (att_handle == conn_gaps_db->char_db[GAPS_CHAR_DEVICE_NAME].char_val_handle) {
		char_data = conn_gaps_db->char_db[GAPS_CHAR_DEVICE_NAME].char_data;
		if (char_data) {
		...
		char_data = conn_gaps_db->char_db[GAPS_CHAR_CEN_ADDR_RES].char_data;
		if (char_data) {
			osif_mem_free(char_data);
		}
		char_data = (void *)osif_mem_alloc(RAM_TYPE_DATA_ON, len);
