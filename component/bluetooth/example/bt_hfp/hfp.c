...
static rtk_bt_evt_cb_ret_t rtk_bt_hfp_app_callback(uint8_t evt_code, void *param, uint32_t len)
{
	...
	case RTK_BT_HFP_EVT_SCO_DATA_IND: {
		...
		if (pdata_in->status == RTK_BT_SCO_PKT_STATUS_OK) {
			...
			if (audio_codec_conf.codec_index == RTK_BT_AUDIO_CODEC_CVSD) {
				...
				/* do mSBC plc */
				if (rtk_bt_audio_recvd_data_in(audio_codec_conf.codec_index, hfp_demo_audio_track_hdl, hfp_demo_codec_entity, NULL, 0, 0)) {
					BT_LOGE("[HFP] SCO Data Receiving FAIL %d \r\n", audio_codec_conf.codec_index);
				}
			}
		}
