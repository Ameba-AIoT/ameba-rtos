	...
	case RTK_BT_LE_ISO_EVT_DATA_RECEIVE_IND: { //BT api shall not be called here
		...
		if (p_bt_direct_iso->iso_sdu_len) {
...
#if 0
			{
				...
				static uint16_t last_pkt_num[APP_LE_ISO_DATA_SEND_PATH_NUM] = {0};
				static uint16_t conn_hdl[APP_LE_ISO_DATA_SEND_PATH_NUM] = {0};
				for (uint16_t i = 0; i < APP_LE_ISO_DATA_SEND_PATH_NUM; i++) {
					if (conn_hdl[i] == 0) {
				...
				if (cur_idx >= APP_LE_ISO_DATA_SEND_PATH_NUM) {
