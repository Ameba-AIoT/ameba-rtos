...
static rtk_bt_evt_cb_ret_t ble_scatternet_gap_app_callback(uint8_t evt_code, void *param, uint32_t len)
{
	...
	case RTK_BT_LE_GAP_EVT_RESOLV_LIST_PENDING_IND: {
		...
		printf("[APP] WARN: Resolving list modification is pending because of adv(%d) scan(%d) connect(%d)!!!\r\n",
	...
	case RTK_BT_LE_GAP_EVT_CONNLESS_IQ_REPORT_IND: {
		...
		printf("[APP] Connectionless IQ report: sync_id %u, sync_handle 0x%x, channel_index %u, "       \
		...
		printf("[APP] Connectionless IQ sample: ");
		for (uint8_t i = 0; i < report_ind->sample_count * 2; ++i) {
			printf("%d ", report_ind->iq_sample[i]);
		}
		printf("\r\n");
		break;
	}
	...
	case RTK_BT_LE_GAP_EVT_CONN_IQ_REPORT_IND: {
		...
		printf("[APP] Connection IQ report: conn_handle %d, rx_phy %d, data_chan_index %d, "        \
		...
		printf("[APP] Connection IQ sample: ");
		for (uint8_t i = 0; i < report_ind->sample_count * 2; ++i) {
			printf("%d ", report_ind->iq_sample[i]);
		}
		printf("\r\n");
	...
	case RTK_BT_LE_GAP_EVT_CONN_CTE_REQ_FAIL_IND: {
		...
		printf("[APP] CTE request fail conn_handle %u, cause %u\r\n", fail_ind->conn_handle, fail_ind->cause);
		break;
	}
