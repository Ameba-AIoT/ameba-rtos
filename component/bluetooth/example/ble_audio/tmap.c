...
int bt_tmap_main(uint8_t role, uint8_t enable, uint32_t sound_channel)
{
	if (1 == enable) {
		...
		case RTK_BT_LE_AUDIO_TMAP_ROLE_BMR: {
			...
			DiagSnPrintf(&dev_name[strlen(APP_LE_AUDIO_TMAP_BMR_DEVICE_NAME)], 10, " %s (%02X%02X)", channel, bd_addr.addr_val[1], bd_addr.addr_val[0]);
		...
		case RTK_BT_LE_AUDIO_TMAP_ROLE_UMR: {
			...
			DiagSnPrintf(&dev_name[strlen(APP_LE_AUDIO_TMAP_UMR_DEVICE_NAME)], 10, " %s (%02X%02X)", channel, bd_addr.addr_val[1], bd_addr.addr_val[0]);
