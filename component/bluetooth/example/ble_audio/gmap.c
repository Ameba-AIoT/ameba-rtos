...
#define APP_LE_AUDIO_GMAP_UNICAST_GAME_GATEWAY_DEVICE_NAME "Ameba LE Audio UGG"
#define APP_LE_AUDIO_GMAP_UNICAST_GAME_TERMINAL_DEVICE_NAME "Ameba LE Audio UGT"
#define APP_LE_AUDIO_GMAP_BROADCAST_GAME_SENDER_DEVICE_NAME "Ameba LE Audio BGS"
#define APP_LE_AUDIO_GMAP_BROADCAST_GAME_RECEIVER_DEVICE_NAME "Ameba LE Audio BGR"
...
static uint8_t gmap_role = RTK_BT_LE_AUDIO_CAP_ROLE_UNKNOWN;
...
static char *bt_le_audio_media_player_name = "RTK player";
...
static void app_bt_le_audio_gmap_encode_data_control(bool enable)
{
	BT_LOGA("[APP] %s %d\r\n", __func__, enable);
	if (enable) {
...
static rtk_bt_evt_cb_ret_t app_le_audio_gattc_callback(uint8_t event, void *data, uint32_t len)
{
	(void)len;
...
static rtk_bt_evt_cb_ret_t app_le_audio_gatts_callback(uint8_t event, void *data, uint32_t len)
{
	(void)len;
...
/* LE GAP APP Callback */
static rtk_bt_evt_cb_ret_t app_bt_le_gap_callback(uint8_t evt_code, void *param, uint32_t len)
{
	(void)len;
	char le_addr[30] = {0};
	char *role = NULL;
...
static rtk_bt_evt_cb_ret_t app_bt_bap_callback(uint8_t evt_code, void *data, uint32_t len)
{
	...
	case RTK_BT_LE_AUDIO_EVT_ASCS_SETUP_DATA_PATH_IND: {
		...
		if (param->iso_chann_t.path_direction == RTK_BLE_AUDIO_ISO_DATA_PATH_TX) {
			if (app_bt_le_audio_find_total_path_num(RTK_BLE_AUDIO_ISO_DATA_PATH_TX) == param->source_ase_num) {
	...
	case RTK_BT_LE_AUDIO_EVT_BIG_REMOVE_DATA_PATH: {
		...
		BT_LOGD("[APP] broadcast sink big remove data path ind: bis_conn_handle: %08x, idx 0x%x, cause: 0x%x\r\n",
				param->bis_conn_handle, param->bis_idx, param->cause);
...
/* Game Audio Profile APP Callback */
static rtk_bt_evt_cb_ret_t app_bt_gmap_callback(uint8_t evt_code, void *data, uint32_t len)
{
	(void)len;
	int16_t ret = 0;

	switch (evt_code) {
	case RTK_BT_LE_AUDIO_GMAS_EVT_READ_IND: {
		rtk_bt_le_audio_gmas_read_ind_t *param = (rtk_bt_le_audio_gmas_read_ind_t *)data;
		BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_READ_IND: conn_handle %d, cid %d, char_uuid %d\r\n",
				param->conn_handle, param->cid, param->char_uuid);
		break;
	}
	case RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_DISCOVERY_DONE_IND: {
		rtk_bt_le_audio_gmas_client_dis_t *param = (rtk_bt_le_audio_gmas_client_dis_t *)data;
		BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_DISCOVERY_DONE_IND: conn_handle %d,is_found %d, load_from_ftl %d\r\n",
	...
	case RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_ROLE_RESULT: {
		...
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_UGG) {
			BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_ROLE_RESULT: GMAP Unicast Game Gateway\r\n");
			char_uuid = RTK_BT_GMAS_CHAR_UUID_UGG_FEATURES;
		}
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_UGT) {
			BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_ROLE_RESULT: GMAP Unicast Game Terminal\r\n");
			char_uuid = RTK_BT_GMAS_CHAR_UUID_UGT_FEATURES;
		}
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_BGS) {
			BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_ROLE_RESULT: GMAP Broadcast Game Sender\r\n");
			char_uuid = RTK_BT_GMAS_CHAR_UUID_BGS_FEATURES;
		}
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_BGR) {
			BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_ROLE_RESULT: GMAP Broadcast Game Receiver\r\n");
			char_uuid = RTK_BT_GMAS_CHAR_UUID_BGR_FEATURES;
		}
		BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_ROLE_RESULT: conn_handle %d,cause %d \r\n", param->conn_handle, param->cause);

		ret = rtk_bt_le_audio_gmas_client_read_features(param->conn_handle, char_uuid);
		if (ret) {
	...
	case RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: {
		rtk_bt_le_audio_gmas_client_read_features_result_t *param = (rtk_bt_le_audio_gmas_client_read_features_result_t *) data;
		switch (param->char_uuid) {
		case RTK_BT_GMAS_CHAR_UUID_UGG_FEATURES: {
			if (param->features & 0x01) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGG MULTIPLEX FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x02) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGG 96 KBPS FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x04) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGG MULTISINK FEATURE SUPPORT\r\n");
			}
			break;
		}
		case RTK_BT_GMAS_CHAR_UUID_UGT_FEATURES: {
			if (param->features & 0x01) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGT SOURCE FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x02) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGT 80 KBPS FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x04) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGT SINK FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x08) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGT 64 KBPS FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x10) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGT MULTIPLEX FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x20) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGT MULTISINK FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x40) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: UGT MULTISOURCE FEATURE SUPPORT\r\n");
			}
			break;
		}
		case RTK_BT_GMAS_CHAR_UUID_BGS_FEATURES: {
			if (param->features & 0x01) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: BGS 96 KBPS FEATURE SUPPORT\r\n");
			}
			break;
		}
		case RTK_BT_GMAS_CHAR_UUID_BGR_FEATURES: {
			if (param->features & 0x01) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: BGR MULTISINK FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x02) {
				BT_LOGA("[APP] RTK_BT_LE_AUDIO_GMAS_EVT_CLIENT_READ_FEATURES_RESULT: BGR MULTIPLEX FEATURE SUPPORT\r\n");
			}
			break;
		}
		default:
	...
	if (1 == enable) {
		...
		case RTK_BT_LE_AUDIO_GMAP_ROLE_BGS: {
			...
			/* config le audio app configuration */
			{
				...
				p_lea_app_conf->gmap_role = RTK_BT_LE_AUDIO_GMAP_ROLE_BGS;
				memset((void *)p_lea_app_conf->device_name, 0, RTK_BT_GAP_DEVICE_NAME_LEN);
				memcpy((void *)p_lea_app_conf->device_name, (uint8_t *)APP_LE_AUDIO_GMAP_BROADCAST_GAME_SENDER_DEVICE_NAME,
					   strlen((const char *)APP_LE_AUDIO_GMAP_BROADCAST_GAME_SENDER_DEVICE_NAME));
				p_lea_app_conf->device_name_len = strlen((const char *)APP_LE_AUDIO_GMAP_BROADCAST_GAME_SENDER_DEVICE_NAME);
			}
			/* set GAP config */
			{
			...
			/* Initilize GAP part */
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_GAP, app_bt_le_gap_callback));
			...
			gmap_role = p_lea_app_conf->gmap_role;
		...
		case RTK_BT_LE_AUDIO_GMAP_ROLE_BGR: {
			...
			/* config le audio app configuration */
			{
				...
				memset((void *)p_lea_app_conf->device_name, 0, RTK_BT_GAP_DEVICE_NAME_LEN);
				memcpy((void *)p_lea_app_conf->device_name, (uint8_t *)APP_LE_AUDIO_GMAP_BROADCAST_GAME_RECEIVER_DEVICE_NAME,
					   strlen((const char *)APP_LE_AUDIO_GMAP_BROADCAST_GAME_RECEIVER_DEVICE_NAME));
				p_lea_app_conf->device_name_len = strlen((const char *)APP_LE_AUDIO_GMAP_BROADCAST_GAME_RECEIVER_DEVICE_NAME);
			}
			/* set GAP config */
			{
			...
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_GATTS, app_le_audio_gatts_callback));
			...
			strcpy(dev_name, APP_LE_AUDIO_GMAP_BROADCAST_GAME_RECEIVER_DEVICE_NAME);
			DiagSnPrintf(&dev_name[strlen(APP_LE_AUDIO_GMAP_BROADCAST_GAME_RECEIVER_DEVICE_NAME)], 10, " %s (%02X%02X)", channel, bd_addr.addr_val[1], bd_addr.addr_val[0]);
			app_bt_le_audio_ext_adv_init((uint8_t *)dev_name, strlen((const char *)APP_LE_AUDIO_GMAP_BROADCAST_GAME_RECEIVER_DEVICE_NAME) + 9);
			/* Register le audio event callback */
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_BAP, app_bt_bap_callback));
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_CAP, app_bt_cap_callback));
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_GMAP, app_bt_gmap_callback));
		...
		case RTK_BT_LE_AUDIO_GMAP_ROLE_UGG: {
			...
			/* config le audio app configuration */
			{
				...
				p_lea_app_conf->cap_role = RTK_BT_LE_AUDIO_CAP_ROLE_INITIATOR;
				p_lea_app_conf->gmap_role = RTK_BT_LE_AUDIO_GMAP_ROLE_UGG;
				memset((void *)p_lea_app_conf->device_name, 0, RTK_BT_GAP_DEVICE_NAME_LEN);
				memcpy((void *)p_lea_app_conf->device_name, (uint8_t *)APP_LE_AUDIO_GMAP_UNICAST_GAME_GATEWAY_DEVICE_NAME,
					   strlen((const char *)APP_LE_AUDIO_GMAP_UNICAST_GAME_GATEWAY_DEVICE_NAME));
				p_lea_app_conf->device_name_len = strlen((const char *)APP_LE_AUDIO_GMAP_UNICAST_GAME_GATEWAY_DEVICE_NAME);
			}
			/* set GAP config */
			{
			...
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_GATTC, app_le_audio_gattc_callback));
			/* register le audio event callback */
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_BAP, app_bt_bap_callback));
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_CAP, app_bt_cap_callback));
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_GMAP, app_bt_gmap_callback));
		...
		case RTK_BT_LE_AUDIO_GMAP_ROLE_UGT: {
			...
			/* config le audio app configuration */
			{
				...
				memset((void *)p_lea_app_conf->device_name, 0, RTK_BT_GAP_DEVICE_NAME_LEN);
				memcpy((void *)p_lea_app_conf->device_name, (uint8_t *)APP_LE_AUDIO_GMAP_UNICAST_GAME_TERMINAL_DEVICE_NAME,
					   strlen((const char *)APP_LE_AUDIO_GMAP_UNICAST_GAME_TERMINAL_DEVICE_NAME));
				p_lea_app_conf->device_name_len = strlen((const char *)APP_LE_AUDIO_GMAP_UNICAST_GAME_TERMINAL_DEVICE_NAME);
			}
			/* set GAP config */
			{
				bt_app_conf.app_profile_support = RTK_BT_PROFILE_GATTS | RTK_BT_PROFILE_LEAUDIO | RTK_BT_PROFILE_BAP | RTK_BT_PROFILE_CAP | RTK_BT_PROFILE_GMAP;
			...
			/* Mix RTK_BT_DEV_NAME channel bt mac address */
			strcpy(dev_name, APP_LE_AUDIO_GMAP_UNICAST_GAME_TERMINAL_DEVICE_NAME);
			DiagSnPrintf(&dev_name[strlen(APP_LE_AUDIO_GMAP_UNICAST_GAME_TERMINAL_DEVICE_NAME)], 10, " %s (%02X%02X)", channel, bd_addr.addr_val[1], bd_addr.addr_val[0]);
			app_bt_le_audio_ext_adv_init((uint8_t *)dev_name, strlen((const char *)APP_LE_AUDIO_GMAP_UNICAST_GAME_TERMINAL_DEVICE_NAME) + 9);
			/* Register LE audio event callback */
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_BAP, app_bt_bap_callback));
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_CAP, app_bt_cap_callback));
			BT_APP_PROCESS(rtk_bt_evt_register_callback(RTK_BT_LE_GP_GMAP, app_bt_gmap_callback));
...
#endif
			app_bt_le_audio_gmap_encode_data_control(false);
			/* Deinitialize app data path struct*/
			app_bt_le_audio_remove_data_path_all();
			/* Disable BT */
			BT_APP_PROCESS(rtk_bt_disable());
			/* Deinitialize Audio */
