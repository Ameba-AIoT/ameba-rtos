...
static void gmap_ext_scan_timer_handle(void *arg)
{
	...
	if (gmap_ext_scan_time_remaining > 0) {
		...
		BT_APP_PRINT(BT_APP_INFO, "%s: gmap_ext_scan_time_remaining = %d\r\n", __func__, gmap_ext_scan_time_remaining);
	} else {
...
static rtk_bt_evt_cb_ret_t app_bt_le_audio_gap_app_callback(uint8_t evt_code, void *param, uint32_t len)
{
	...
	case RTK_BT_LE_GAP_EVT_BOND_MODIFY_IND: {
		...
		printf("[APP] Bond info modified, op: %d", bond_mdf_ind->op);
		break;
	}
...
static uint16_t app_bt_le_audio_encode_record_data(app_lea_iso_data_path_t *p_iso_path)
{
	...
	if (!p_iso_path->record_hdl) {
		BT_APP_PRINT(BT_APP_WARNING, "%s audio record hdl is null\r\n", __func__);
		goto exit;
	}
	read_size = rtk_bt_audio_record_data_get(RTK_BT_AUDIO_CODEC_LC3,
											 ...
											 encode_byte, true);
	if (!read_size) {
		BT_APP_PRINT(BT_APP_WARNING, "%s audio record data read size is 0, get fail \r\n", __func__);
		goto exit;
	}
	p_iso_path->encode_byte = encode_byte;
	/* encode */
...
{
	BT_APP_PRINT(BT_APP_INFO, "%s %d\r\n", __func__, enable);
	if (enable) {
		if (g_app_lea_encode_enable == true) {
			BT_APP_PRINT(BT_APP_WARNING, "%s: encode is alreay enabled\r\n", __func__);
			return;
		}
		...
		if (g_app_lea_encode_enable == false) {
			BT_APP_PRINT(BT_APP_WARNING, "%s: encode is alreay disabled\r\n", __func__);
			return;
		}
	...
	if (tx_path_num) {
		for (uint8_t i = 0; i < tx_path_num; i ++) {
			...
			if (p_iso_path) {
				if (p_iso_path->record_hdl) {
					found_record = true;
					BT_APP_PRINT(BT_APP_INFO, "Found record handle: index 0x%x, record handle 0x%p \r\n",
...
static rtk_bt_evt_cb_ret_t app_bt_le_audio_callback(uint8_t evt_code, void *data, uint32_t len)
{
	...
	case RTK_BT_LE_AUDIO_EVT_GMAS_READ_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_READ_IND: conn_handle %d, cid %d, char_uuid %d\r\n",
	...
	case RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_DISCOVERY_DONE_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_DISCOVERY_DONE_IND: conn_handle %d,is_found %d, load_from_ftl %d\r\n",
					 param->conn_handle, param->is_found, param->load_from_ftl);
		ret = rtk_bt_le_audio_gmas_client_read_role(param->conn_handle);
		if (ret) {
			BT_APP_PRINT(BT_APP_ERROR, "%s gmas client read role failed\r\n", __func__);
			break;
		}
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_ROLE_RESULT: {
		...
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_UGG) {
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_ROLE_RESULT: GMAP Unicast Game Gateway\r\n");
		}
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_UGT) {
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_ROLE_RESULT: GMAP Unicast Game Terminal\r\n");
		}
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_BGS) {
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_ROLE_RESULT: GMAP Broadcast Game Sender\r\n");
		}
		if (param->gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_BGR) {
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_ROLE_RESULT: GMAP Broadcast Game Receiver\r\n");
		}
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_ROLE_RESULT: conn_handle %d,cause %d \r\n", param->conn_handle, param->cause);

		ret = rtk_bt_le_audio_gmas_client_read_features(param->conn_handle, param->gmap_role);
		if (ret) {
			BT_APP_PRINT(BT_APP_ERROR, "%s gmas client read features failed\r\n", __func__);
			break;
		}
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: {
		...
		case GMAS_CHAR_UUID_UGG: {
			if (param->features & 0x01) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGG MULTIPLEX FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x02) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGG 96 KBPS FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x04) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGG MULTISINK FEATURE SUPPORT\r\n");
			}
			break;
		}
		case GMAS_CHAR_UUID_UGT: {
			if (param->features & 0x01) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGT SOURCE FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x02) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGT 80 KBPS FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x04) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGT SINK FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x08) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGT 64 KBPS FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x10) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGT MULTIPLEX FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x20) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGT MULTISINK FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x40) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: UGT MULTISOURCE FEATURE SUPPORT\r\n");
			}
			break;
		}
		case GMAS_CHAR_UUID_BGS: {
			if (param->features & 0x01) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: BGS 96 KBPS FEATURE SUPPORT\r\n");
			}
			break;
		}
		case GMAS_CHAR_UUID_BGR: {
			if (param->features & 0x01) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: BGR MULTISINK FEATURE SUPPORT\r\n");
			}
			if (param->features & 0x02) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_GMAS_CLIENT_READ_FEATURES_RESULT: BGR MULTIPLEX FEATURE SUPPORT\r\n");
			}
			break;
		}
		default:
			BT_APP_PRINT(BT_APP_ERROR, "Unknown char_uuid: 0x%x\r\n", param->char_uuid);
			break;
		}
		break;
	}
	...
	case RTK_BT_LE_AUDIO_EVT_BROADCAST_SOURCE_STATE_IND: {
		...
		if (param->broadcast_source_state == RTK_BT_LE_AUDIO_BROADCAST_SOURCE_STATE_IDLE) {
			...
			if (param->cause == 0) {
				...
				if (ret != RTK_BT_OK) {
					BT_APP_PRINT(BT_APP_ERROR, "app_bt_le_audio_broadcast_source_setup_data_path fail,ret = 0x%x\r\n", ret);
					break;
				}
	...
	case RTK_BT_LE_AUDIO_EVT_PA_BIG_INFO_IND: {
		...
		BT_APP_PRINT(BT_APP_DUMP, "broadcast sink pa biginfo indication: sync_handle: %p, num_bis 0x%x, sync_id 0x%x, pa_sync_handle 0x%x, encryption 0x%x\r\n",
	...
	case RTK_BT_LE_AUDIO_EVT_PA_ADV_REPORT_IND: {
		...
		BT_APP_PRINT(BT_APP_DUMP,
					 "broadcast sink pa adv report: sync_handle: %p,sync_id %d, pa_sync_handle 0x%x,tx_power %d, rssi %d, cte_type %d, data_status 0x%x, data_len %d\r\n",
	...
	case RTK_BT_LE_AUDIO_EVT_MICS_CLIENT_DISCOVERY_DONE_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_MICS_CLIENT_DISCOVERY_DONE_IND: conn_handle %d,is_found %d,load_from_ftl %d,mic_mute 0x%x\r\n",
	...
	case RTK_BT_LE_AUDIO_EVT_MICS_CLIENT_NOTIFY_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_MICS_CLIENT_NOTIFY_IND: conn_handle %d,mic_mute %d\r\n", param->conn_handle, param->mic_mute);
		app_bt_le_audio_device_info_t *p_device_info = NULL;
		p_device_info = app_bt_le_audio_group_list_find_dev_by_conn_handle(param->conn_handle);
		if (p_device_info) {
			p_device_info->mic_mute = param->mic_mute;
	...
	case RTK_BT_LE_AUDIO_EVT_CSIS_CLIENT_DISCOVERY_DONE_IND: {
		...
		if (param->is_found == false) {
			if (gmap_role & RTK_BT_LE_AUDIO_GMAP_ROLE_BGS) {
				...
				BT_APP_PRINT(BT_APP_ERROR, "%s unknown role 0x%x \r\n", __func__, gmap_role);
				break;
			}
	...
	case RTK_BT_LE_AUDIO_EVT_MICS_WRITE_MUTE_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_MICS_WRITE_MUTE_IND: mic_mute %d\r\n", param->mic_mute);
		//call record api
		if (!record_handle_exist_check_func()) {
			BT_APP_PRINT(BT_APP_ERROR, "%s: Audio record is not exist\r\n", __func__);
			break;
		}
		if (param->mic_mute == 0x01) {
			...
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_MICS_WRITE_MUTE_IND: muted\r\n");
		} else if (param->mic_mute == 0x00) {
			// unmuted
			rtk_bt_audio_record_set_mute(0, false);
			rtk_bt_audio_record_set_mute(1, false);
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_MICS_WRITE_MUTE_IND: unmuted\r\n");
		} else if (param->mic_mute == 0x02) {
			//muted disabled
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_MICS_WRITE_MUTE_IND: set mute disabled\r\n");
		} else {
			BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_MICS_WRITE_MUTE_IND: mic device set mute wrong cmd !!!\r\n");
		}
		break;
	}
	...
	case RTK_BT_LE_AUDIO_EVT_AICS_CP_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_AICS_CP_IND: conn_handle %d, srv_instance_id %d, cp_op 0x%x, gain_setting %d\r\n",
			...
			if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_BLUETOOTH_IDX) {
				...
				if (ret == RTK_BT_OK) {
					...
					if (cur_gain_mode == RTK_BT_LE_AUDIO_AICS_GAIN_MODE_AUTOMATIC_ONLY || cur_gain_mode == RTK_BT_LE_AUDIO_AICS_GAIN_MODE_AUTOMATIC) {
						BT_APP_PRINT(BT_APP_ERROR, "%s: cannot set gain setting ! current gain mode: %d is automatic!\r\n", __func__, cur_gain_mode);
						break;
					}
				}
				...
				if (lea_cur_mic_bst_gain > RTK_BT_AUDIO_MICBST_GAIN_40DB) {
					BT_APP_PRINT(BT_APP_WARNING, "%s: mic bst gain exceed the max 40 db !\r\n", __func__);
					break;
				}
				// set mic bst gain
				rtk_bt_audio_record_set_mic_bst_gain(RTK_BT_AUDIO_AMIC1, lea_cur_mic_bst_gain);
				BT_APP_PRINT(BT_APP_INFO, "Ameba MIC set bst gain success! current bst gain is %d db\r\n", 5 * lea_cur_mic_bst_gain);
			}
			break;
		}
		case RTK_BT_LE_AUDIO_AICS_CP_UNMUTE: {
			if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_BLUETOOTH_IDX) {
				rtk_bt_audio_track_set_mute(false);
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_MUTE: aics Ameba bluetooth set unmuted\r\n");
			} else if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_MIC_IDX) {
				if (!record_handle_exist_check_func()) {
					BT_APP_PRINT(BT_APP_ERROR, "%s: Audio record is not exist\r\n", __func__);
					break;
				...
				rtk_bt_audio_record_set_mute(0, false);
				rtk_bt_audio_record_set_mute(1, false);
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_MUTE: aics Ameba MIC set unmuted\r\n");
			}
			break;
		}
		case RTK_BT_LE_AUDIO_AICS_CP_MUTE: {
			if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_BLUETOOTH_IDX) {
				rtk_bt_audio_track_set_mute(true);
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_MUTE: aics Ameba bluetooth set muted\r\n");
			} else if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_MIC_IDX) {
				if (!record_handle_exist_check_func()) {
					BT_APP_PRINT(BT_APP_ERROR, "%s: Audio record is not exist\r\n", __func__);
					break;
				...
				rtk_bt_audio_record_set_mute(0, true);
				rtk_bt_audio_record_set_mute(1, true);
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_MUTE: aics Ameba MIC set muted\r\n");
			}
			break;
		}
		case RTK_BT_LE_AUDIO_AICS_CP_SET_MANUAL_GAIN_MODE: {
			...
			if (ret == RTK_BT_OK) {
				...
				if (cur_gain_mode == RTK_BT_LE_AUDIO_AICS_GAIN_MODE_AUTOMATIC_ONLY || cur_gain_mode == RTK_BT_LE_AUDIO_AICS_GAIN_MODE_MANUAL_ONLY) {
					BT_APP_PRINT(BT_APP_ERROR, "%s: set gain mode failed ! Setting gain mode is not supported for current gain mode: %d\r\n", __func__, cur_gain_mode);
					break;
				}
			}
			if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_BLUETOOTH_IDX) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_SET_MANUAL_GAIN_MODE: srv_instance_id %d set Manual Gain Mode success\r\n", param->srv_instance_id);
			} else if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_MIC_IDX) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_SET_MANUAL_GAIN_MODE: srv_instance_id %d set Manual Gain Mode success\r\n", param->srv_instance_id);
			}
			break;
		...
		case RTK_BT_LE_AUDIO_AICS_CP_SET_AUTOMATIC_GAIN_MODE: {
			...
			if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_BLUETOOTH_IDX) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_SET_AUTOMATIC_GAIN_MODE: srv_instance_id %d set Automatic Gain Mode success\r\n", param->srv_instance_id);
			} else if (param->srv_instance_id == RTK_BT_LE_AUDIO_DEFAULT_AICS_MIC_IDX) {
				BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_AICS_CP_SET_AUTOMATIC_GAIN_MODE: srv_instance_id %d set Automatic Gain Mode success\r\n", param->srv_instance_id);
			}
			break;
	...
	case RTK_BT_LE_AUDIO_EVT_AICS_WRITE_INPUT_DES_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_AICS_WRITE_INPUT_DES_IND: conn_handle %d, srv_instance_id %d,input_des_len %d,p_input_des %s\r\n",
	...
	default:
		BT_APP_PRINT(BT_APP_DEBUG, "default evt_code 0x%04x\r\n", evt_code);
		break;
	}
...
int bt_gmap_main(uint8_t role, uint8_t enable)
{
	if (1 == enable) {
		...
		default:
			BT_APP_PRINT(BT_APP_DEBUG, "unknown role 0x%x\r\n", role);
			break;
		}
	} else if (0 == enable) {
		if (!gmap_demo_init_flag) {
		...
		case RTK_BT_LE_AUDIO_GMAP_ROLE_BGS: {
			...
			if (g_gmap_bgs_info.status == RTK_BLE_AUDIO_INITIATOR_BROADCAST_START) {
				BT_APP_PRINT(BT_APP_ERROR, "%s: please stop first before disable \r\n", __func__);
				return -1;
			}
