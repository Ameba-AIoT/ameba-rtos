...
static void cap_ext_scan_timer_handle(void *arg)
{
	...
	if (cap_ini_ext_scan_time_remaining > 0) {
		...
		BT_APP_PRINT(BT_APP_INFO, "%s: cap_ini_ext_scan_time_remaining = %d\r\n", __func__, cap_ini_ext_scan_time_remaining);
	} else {
...
static void cap_comm_ext_scan_timer_handle(void *arg)
{
	...
	if (cap_comm_ext_scan_time_remaining > 0) {
		...
		BT_APP_PRINT(BT_APP_INFO, "%s: cap_comm_ext_scan_time_remaining = %d\r\n", __func__, cap_comm_ext_scan_time_remaining);
	} else {
		/* ext scan time out and close ext scan */
		rtk_bt_le_gap_ext_scan_stop();
		BT_APP_PRINT(BT_APP_INFO, "%s: Time out and stop ext scan\r\n", __func__);
	...
	case RTK_BT_LE_GAP_EVT_CONNECT_IND: {
		...
		if (!conn_ind->err) {
			...
			if (cap_role == RTK_BT_LE_AUDIO_CAP_ROLE_INITIATOR) {
				...
				printf("[APP] no cap demo role 0x%x", cap_role);
			}
		} else {
...
static rtk_bt_evt_cb_ret_t app_bt_le_audio_callback(uint8_t evt_code, void *data, uint32_t len)
{
	...
	case RTK_BT_LE_AUDIO_EVT_BASS_CLIENT_BRS_DATA_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_BASS_CLIENT_BRS_DATA_IND: conn_handle 0x%x, notify %d, read_cause 0x%x, char_instance_id %d\r\n",
					 param->conn_handle, param->notify, param->read_cause, param->char_instance_id);
		if (param->p_brs_data && param->p_brs_data->brs_is_used) {
			BT_APP_PRINT(BT_APP_DEBUG, "RTK_BT_LE_AUDIO_EVT_BASS_CLIENT_BRS_DATA_IND: source_id %d, source_adv_sid %d, pa_sync_state %d, big_encryption %d\r\n",
			...
			if (param->p_brs_data->num_subgroups) {
				BT_APP_PRINT(BT_APP_DEBUG, "RTK_BT_LE_AUDIO_EVT_BASS_CLIENT_BRS_DATA_IND: num_subgroups %d, bis_info_size %d, bis_sync_state 0x%x, metadata_len %d\r\n",
		...
		if (p_device_info && p_device_info->bass_is_found) {
			if (param->p_brs_data) {
				...
				if (param->p_brs_data->brs_is_used && param->p_brs_data->big_encryption == RTK_BT_LE_AUDIO_BASS_BRS_BIG_BROADCAST_CODE_REQUIRED) {
					...
					BT_APP_PRINT(BT_APP_INFO, "bass write cp set broadcast code %s, ret: 0x%x\r\n", ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
				}
			}
		}
		break;
	...
	case RTK_BT_LE_AUDIO_EVT_BASS_CLIENT_SYNC_INFO_REQ_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "RTK_BT_LE_AUDIO_EVT_BASS_CLIENT_SYNC_INFO_REQ_IND: conn_handle 0x%x, char_instance_id %d\r\n", param->conn_handle,
				...
				for (uint8_t i = 0; i < p_device_info->brs_char_num; i++) {
					if (p_device_info->brs_char_tbl[i].instance_id == param->char_instance_id) {
						if (p_device_info->brs_char_tbl[i].sync_handle) {
							...
							BT_APP_PRINT(BT_APP_INFO, "rtk_bt_le_audio_bass_client_past %s, ret: 0x%x\r\n", ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
						}
					}
				}
			}
	...
	case RTK_BT_LE_AUDIO_EVT_CAP_DISCOVERY_DONE_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO,
					 "RTK_BT_LE_AUDIO_EVT_CAP_DISCOVERY_DONE_IND: conn_handle 0x%x, load_from_ftl %d, cas_is_found %d, cas_inc_csis %d, vcs_is_found %d, mics_is_found %d\r\n",
		...
		if (param->cas_is_found == 0 && param->vcs_is_found == 0 && param->mics_is_found == 0) {
			BT_APP_PRINT(BT_APP_WARNING, "conn_handle(%d) not support csis,vcs and mics, skip it\r\n", param->conn_handle);
			break;
		}
		break;
	}
	...
	case RTK_BT_LE_AUDIO_EVT_PA_SYNC_STATE_IND: {
		...
		BT_APP_PRINT(BT_APP_INFO, "broadcast sink pa sync state change: sync_handle: %p, sync_state 0x%x, action 0x%x, cause: 0x%x\r\n",
		...
		if (param->sync_state == RTK_BT_LE_AUDIO_PA_SYNC_STATE_SYNCHRONIZED) {
			...
			if (bap_role & RTK_BT_LE_AUDIO_BAP_ROLE_BRO_ASSI) {
				...
				//stop ext scan
				rtk_bt_le_gap_ext_scan_stop();
				BT_APP_PRINT(BT_APP_DEBUG, "rtk_bt_le_gap_ext_scan_stop %s after pa sync synchronized! ret: 0x%x\r\n", ((RTK_BT_OK != ret) ? "fail" : "ok"), ret);
	...
	if (1 == enable) {
		...
		case RTK_BT_LE_AUDIO_CAP_ROLE_INITIATOR: {
			...
			if (cap_ini_ext_scan_flag) {
				printf("[LE Audio] Create initiator ext scan timer \r\n");
				if (false == osif_timer_create(&cap_ini_ext_scan_timer, "lea_ini_ext_scan_timer", NULL,
					...
					printf("[LE Audio] Create initiator ext scan timer fail \r\n");
		...
		case RTK_BT_LE_AUDIO_CAP_ROLE_COMMANDER: {
			...
			if (cap_comm_ext_scan_flag) {
				printf("[LE Audio] Create commander ext scan timer \r\n");
				if (false == osif_timer_create(&cap_comm_ext_scan_timer, "lea_comm_ext_scan_timer", NULL,
					...
					printf("[LE Audio] Create commander ext scan timer fail \r\n");
		...
		case RTK_BT_LE_AUDIO_CAP_ROLE_ACCEPTOR: {
			...
			if (g_acceptor_info.status == RTK_BLE_AUDIO_ACCEPTOR_START || g_acceptor_info.status == RTK_BLE_AUDIO_ACCEPTOR_BIG_SYNC) {
				BT_APP_PRINT(BT_APP_ERROR, "%s: please stop stream first before disable \r\n", __func__);
				return -1;
			}
			/* stop and remove ea */
