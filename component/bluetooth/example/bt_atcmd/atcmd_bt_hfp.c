...
static int atcmd_bt_hfp_connect(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_connect(bd_addr)) {
		BTHFP_AT_PRINTK("HFP connect fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP connecting to device %s ...", addr_str);
...
static int atcmd_bt_hfp_disconnect(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_disconnect(bd_addr)) {
		BTHFP_AT_PRINTK("HFP disconnect fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP disconnecting to device %s ...", addr_str);
...
static int atcmd_bt_hfp_sco_connect(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_sco_connect(bd_addr)) {
		BTHFP_AT_PRINTK("HFP sco connect fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP sco connecting to device %s ...", addr_str);
...
static int atcmd_bt_hfp_sco_disconnect(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_sco_disconnect(bd_addr)) {
		BTHFP_AT_PRINTK("HFP sco disconnect fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP sco disconnecting to device %s ...", addr_str);
...
static int atcmd_bt_hfp_call_income(int argc, char **argv)
{
	...
	hexdata_str_to_bd_addr(argv[0], bd_addr, RTK_BD_ADDR_LEN);
	if (strlen(argv[1]) > 20) {
		BTHFP_AT_PRINTK("HFP call income phone illegal \r\n");
		return -1;
	}
	if (rtk_bt_hfp_call_incoming(bd_addr, argv[1], strlen(argv[1]) + 1, (uint8_t)str_to_int(argv[2]))) {
		BTHFP_AT_PRINTK("HFP call income fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP call income to device %s ... number is %s length is %d type is %s ", addr_str, argv[1], strlen(argv[1]), argv[2]);
...
static int atcmd_bt_hfp_call_answer_req(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_call_answer(bd_addr)) {
		BTHFP_AT_PRINTK("HFP call answer req fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP call answer req to device %s ...", addr_str);
...
static int atcmd_bt_hfp_call_terminate_req(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_call_terminate(bd_addr)) {
		BTHFP_AT_PRINTK("HFP call terminate req fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP call terminate req to device %s ...", addr_str);
...
static int atcmd_bt_hfp_batt_level_report(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_batt_level_report(bd_addr, power_level)) {
		BTHFP_AT_PRINTK("HFP batt level report fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP batt level report to device %s ...", addr_str);
...
static int atcmd_bt_hfp_speaker_gain_report(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_speaker_gain_report(bd_addr, gain)) {
		BTHFP_AT_PRINTK("HFP speaker gain report fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP speaker gain report to device %s ...", addr_str);
...
static int atcmd_bt_hfp_microphone_gain_report(int argc, char **argv)
{
	...
	if (rtk_bt_hfp_microphone_gain_report(bd_addr, gain)) {
		BTHFP_AT_PRINTK("HFP microphone gain report fail \r\n");
		return -1;
	}
	rtk_bt_br_addr_to_str(bd_addr, addr_str, sizeof(addr_str));
	BTHFP_AT_PRINTK("HFP microphone gain report to device %s ...", addr_str);
...
#else
	return atcmd_bt_excute(argc, argv, hfp_cmd_table, "[ATBC][hfp]");
#endif
}