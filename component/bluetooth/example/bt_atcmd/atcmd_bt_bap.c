...
static int atcmd_bt_bap_broadcast_source_start(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_source_start()) {
		BLEBAP_AT_PRINTK("BAP broadcast source start fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP start successfully \r\n");
...
static int atcmd_bt_bap_broadcast_source_stop(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_source_stop()) {
		BLEBAP_AT_PRINTK("BAP broadcast source stop fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP stop successfully \r\n");
...
static int atcmd_bt_bap_broadcast_source_qos_cfg(int argc, char **argv)
{
	...
	if (qos_type != 2 && qos_type != 3) {
		BLEBAP_AT_PRINTK("BIS qos type %d , should be 2~3 \r\n", qos_type);
		return -1;
	}
	...
	if (qos_type == RTK_BT_LE_QOS_CFG_BIS_LOW_LATENCY) {
		BLEBAP_AT_PRINTK("BAP broadcast source prefer QoS: BIS LOW LATENCY \r\n");
	} else if (qos_type == RTK_BT_LE_QOS_CFG_BIS_HIG_RELIABILITY) {
		BLEBAP_AT_PRINTK("BAP broadcast source prefer QoS: BIS HIG RELIABILITY \r\n");
	}
...
static int atcmd_bt_bap_ext_scan(int argc, char **argv)
{
	...
	if (rtk_bt_bap_ext_scan(enable)) {
		BLEBAP_AT_PRINTK("BAP ext scan fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP ext scan successfully \r\n");
...
static int atcmd_bt_bap_broadcast_sink_sync_start(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_sync_start(&info)) {
		BLEBAP_AT_PRINTK("BAP broadcast sink sync fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast sink sync successfully \r\n");
...
static int atcmd_bt_bap_broadcast_sink_sync_term(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_sync_term(&addr)) {
		BLEBAP_AT_PRINTK("BAP broadcast sink sync term fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast sink sync term successfully \r\n");
...
static int atcmd_bt_bap_broadcast_sink_cfg(int argc, char **argv)
{
	...
	if (strcmp(argv[0], "left") == 0) {
		...
		BLEBAP_AT_PRINTK("Unknown channel allocation");
		return -1;
	}
	if (rtk_bt_bap_broadcast_sink_cfg(channel)) {
		BLEBAP_AT_PRINTK("BAP broadcast sink config channel fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast sink config channel successfully \r\n");
...
static int atcmd_bt_bap_bass_scan(int argc, char **argv)
{
	...
	if (rtk_bt_bap_bass_scan(enable, remote_scan_enable, group_idx)) {
		BLEBAP_AT_PRINTK("BAP bass scan fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP bass scan successfully \r\n");
...
static int atcmd_bt_broadcast_assistant_sync_start(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_assistant_sync_start(&info)) {
		BLEBAP_AT_PRINTK("BAP broadcast assistant sync fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast assistant sync successfully \r\n");
...
static int atcmd_bt_broadcast_assistant_sync_term(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_assistant_sync_term(&addr)) {
		BLEBAP_AT_PRINTK("BAP broadcast assistant sync term fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast assistant sync term successfully \r\n");
...
static int atcmd_bt_broadcast_assistant_reception_start(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_assistant_reception_start(&addr, group_idx)) {
		BLEBAP_AT_PRINTK("BAP broadcast assistant reception start fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast assistant reception start successfully \r\n");
...
static int atcmd_bt_broadcast_assistant_reception_stop(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_assistant_reception_stop(&addr, group_idx)) {
		BLEBAP_AT_PRINTK("BAP broadcast assistant reception stop fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast assistant reception stop successfully \r\n");
...
static int atcmd_bt_broadcast_assistant_reception_remove(int argc, char **argv)
{
	...
	if (rtk_bt_bap_broadcast_assistant_reception_remove(&addr, group_idx)) {
		BLEBAP_AT_PRINTK("BAP broadcast assistant reception remove fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP broadcast assistant reception remove successfully \r\n");
...
static int atcmd_bt_bap_unicast_client_start(int argc, char **argv)
{
	...
	if (rtk_bt_bap_unicast_client_start(group_idx, (rtk_bt_le_audio_play_mode_t)play_mode)) {
		BLEBAP_AT_PRINTK("BAP unicast client start fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP unicast client start successfully \r\n");
...
static int atcmd_bt_bap_unicast_client_stop(int argc, char **argv)
{
	...
	if (rtk_bt_bap_unicast_client_stop(group_idx)) {
		BLEBAP_AT_PRINTK("BAP unicast client stop fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP unicast client stop successfully \r\n");
...
static int atcmd_bt_bap_unicast_client_release(int argc, char **argv)
{
	...
	if (rtk_bt_bap_unicast_client_release(group_idx)) {
		BLEBAP_AT_PRINTK("BAP unicast client release fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP unicast client release successfully \r\n");
...
static int atcmd_bt_bap_unicast_client_prefer_qos_cfg(int argc, char **argv)
{
	...
	if (prefer_qos_type != 0 && prefer_qos_type != 1) {
		BLEBAP_AT_PRINTK("wrong CIS Qos type: %d , should be 0~1 \r\n", prefer_qos_type);
		return -1;
	}
	...
	if (prefer_qos_type == RTK_BT_LE_QOS_CFG_CIS_LOW_LATENCY) {
		BLEBAP_AT_PRINTK("BAP unicast client prefer session Qos: CIS LOW LATENCY \r\n");
	} else if (prefer_qos_type == RTK_BT_LE_QOS_CFG_CIS_HIG_RELIABILITY) {
		BLEBAP_AT_PRINTK("BAP unicast client prefer session QoS: CIS HIG RELIABILITY \r\n");
	}
...
static int atcmd_bt_bap_unicast_client_ase_target_qos_cfg(int argc, char **argv)
{
	...
	if (target_qos < 1 || target_qos > 3) {
		BLEBAP_AT_PRINTK("wrong ase target QoS: %d , should be 1~3 \r\n", target_qos);
		return -1;
	}

	app_lea_default_prefer_codec.target_latency = target_qos;
	if (target_qos == RTK_BLE_AUDIO_ASCS_ASE_TARGET_LOWER_LATENCY) {
		BLEBAP_AT_PRINTK("BAP unicast client ase target QoS: 0x%x Low Latency \r\n", target_qos);
	} else if (target_qos == RTK_BLE_AUDIO_ASCS_ASE_TARGET_BALANCED) {
		BLEBAP_AT_PRINTK("BAP unicast client ase target QoS: 0x%x Balanced \r\n", target_qos);
	} else if (target_qos == RTK_BLE_AUDIO_ASCS_ASE_TARGET_HIGHER_RELIABILITY) {
		BLEBAP_AT_PRINTK("BAP unicast client ase target QoS: 0x%x High Reliability \r\n", target_qos);
	}
...
static int atcmd_bt_bap_unicast_server_cfg(int argc, char **argv)
{
	...
	if (rtk_bt_bap_unicast_server_cfg(channel)) {
		BLEBAP_AT_PRINTK("BAP unicast server config channel fail \r\n");
		return -1;
	}
	BLEBAP_AT_PRINTK("BAP unicast server config channel successfully \r\n");
...
int atcmd_bt_bap_cmd(int argc, char *argv[])
{
	...
	if (strcmp(argv[0], "broadcast") == 0) {
		if (strcmp(argv[1], "source") == 0) {
			...
			BLEBAP_AT_PRINTK("Set bap broadcast sink");
			...
			BLEBAP_AT_PRINTK("Set bap broadcast assistant");
			...
			BLEBAP_AT_PRINTK("Set bap scan delegate");
