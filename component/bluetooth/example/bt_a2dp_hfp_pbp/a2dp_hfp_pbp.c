...
#define APP_BT_A2DP_PBP_DEMO_OUPUT_SAMPLE_RATE          48000
#endif
#define APP_BT_LE_AUDIO_DEFAULT_SDU_INTERVAL_M_S_US     10000 //units: us
#define APP_BT_LE_AUDIO_DEFAULT_SDU_INTERVAL_S_M_US     10000 //units: us
...
#define APP_BT_PBP_BROADCAST_SOURCE_ENCODE_TASK_STACK_SIZE     (1024 * 6)
...
typedef struct {
	...
	union {
		struct {
			...
			uint8_t broadcast_name[APP_BT_LE_EXT_ADV_MAX_BROADCAST_NAME_LENGTH];
		} bass_scan_info_t;
	};
...
static struct app_demo_task_t nc_task = {
	.hdl = NULL,
	.sem = NULL,
	.run  = 0,
};
...
/* class of device */
static uint32_t cod = (0x000014 | (0x04 << 8) | (0x24 << 16));
/* supervision_timeout */
	...
	while (pos < report_data_len) {
		...
		if ((length > 0x01) && ((pos + length) <= report_data_len)) {
			...
			case RTK_BT_LE_GAP_ADTYPE_BROADCAST_NAME: {
				if (length <= (APP_BT_LE_EXT_ADV_MAX_BROADCAST_NAME_LENGTH + 1)) {
...
/* ---------------------------- APP drop packet handle  ---------------------------- */
static void app_bt_handle_packet_drop_reset(void)
{
	if (pkt_drop_flag) {
...
static void app_bt_handle_packet_drop_number_count(void)
{
	if (!pkt_drop_mtx) {
		BT_LOGE("[APP] %s: pkt_drop_mtx is null \r\n", __func__);
		return ;
	}
	osif_mutex_take(pkt_drop_mtx, BT_TIMEOUT_FOREVER);
	pkt_drop_flag = true;
...
static void app_bt_handle_packet_discarding(uint32_t dequeue_bytes)
{
	if (!pkt_drop_flag) {
		return;
	}
		...
		for (uint16_t i = 0; i < APP_LE_AUDIO_DEMO_DATA_PATH_NUM; i ++) {
			if (app_le_audio_data_path[i].used && (app_le_audio_data_path[i].path_direction == RTK_BLE_AUDIO_ISO_DATA_PATH_TX)) {
				...
				if (a2dp_play_flag == false) {
					app_le_audio_data_path[i].sdu_sync_flag = false;
				}
			}
		}
		if (pbp_tx_flag) {
...
static uint32_t app_bt_le_audio_calculate_dequeue_bytes(rtk_bt_le_audio_cfg_codec_t *p_codec)
{
	uint8_t bytes_per_num = (APP_LE_AUDIO_DEFAULT_PCM_BIT_WIDTH / 8);
	uint32_t dequeue_byte = 0, pcm_frame_size = 0;
	...
	dequeue_byte = pcm_frame_size * APP_BT_A2DP_PBP_DEMO_OUPUT_CHANNEL_NUM * bytes_per_num;

	return dequeue_byte;
}

static uint16_t app_bt_le_audio_pcm_data_dequeue(int8_t *output_data, uint32_t dequeue_len)
{
	uint8_t ret = RTK_BT_OK;
...
static uint16_t app_bt_le_audio_prepare_pcm_data_from_queue(void *p_data, uint32_t *p_len, rtk_bt_le_audio_cfg_codec_t *p_codec)
{
	uint8_t ret = RTK_BT_OK;
	uint32_t dequeue_len = 0;

	dequeue_len = app_bt_le_audio_calculate_dequeue_bytes(p_codec);
	if (dequeue_len > APP_DEQUEUE_BUF_MAX_LEN) {
		BT_LOGE("[APP] %s: dequeue_len %d exceed the max length!\r\n", __func__, (int)dequeue_len);
		return RTK_BT_FAIL;
	}
	if (!app_bt_le_audio_queue_water_level_is_enough()) {
		memset((void *)p_data, 0, dequeue_len);
	} else {
		ret = app_bt_le_audio_pcm_data_dequeue((int8_t *)p_data, dequeue_len);
		if (ret) {
			BT_LOGE("[APP] %s: dequeue bytes %d is not enough, set buf default 0!\r\n", __func__, (int)dequeue_len);
			memset((void *)p_data, 0, dequeue_len);
			app_bt_handle_packet_drop_number_count();
		}
	}
	*p_len = dequeue_len;
...
static uint16_t app_bt_le_audio_encode_data(app_bt_le_audio_data_path_t *p_iso_path, uint8_t out_channels, int8_t *input_data, uint32_t input_len)
{
	uint8_t ret = RTK_BT_OK;
	uint8_t bytes_per_num = (APP_LE_AUDIO_DEFAULT_PCM_BIT_WIDTH / 8);
	...
	/* parsing pcm data */
	if (out_channels == APP_BT_A2DP_PBP_DEMO_OUPUT_CHANNEL_NUM) {
		/* no need to parse */
		memcpy(p_iso_path->p_encode_data, input_data, encode_byte);
	} else if (APP_BT_A2DP_PBP_DEMO_OUPUT_CHANNEL_NUM == 2 && out_channels == 1) {
		/* parsing from pcm data*/
		audio_channel_allocation = app_bt_le_audio_translate_le_chnl_to_audio_chnl(p_codec->audio_channel_allocation);
		app_bt_le_audio_parse_two_chnl_pcm_data_to_one_chnl((short *)input_data, input_len / 2,
															p_iso_path->p_encode_data, encode_byte / 2,
															audio_channel_allocation);
	} else {
		BT_LOGE("[APP] %s: wrong encode channels number:%d !\r\n", __func__, out_channels);
		return RTK_BT_FAIL;
	}
	/* encode */
...
static void app_bt_pbp_bsrc_encode_task_entry(void *ctx)
{
	...
	while (app_bt_pbp_bsrc_encode_task.run) {
		...
		/* dequeue from pcm data queue */
		if (app_bt_le_audio_prepare_pcm_data_from_queue((void *)dequeue_buf, &output_len, &lea_codec_t)) {
			BT_LOGE("[APP] %s app_bt_le_audio_prepare_pcm_data_from_queue failed %d\r\n", __func__);
		}
		...
		for (uint16_t i = 0; i < APP_LE_AUDIO_DEMO_DATA_PATH_NUM; i ++) {
			if (app_le_audio_data_path[i].used && (app_le_audio_data_path[i].path_direction == RTK_BLE_AUDIO_ISO_DATA_PATH_TX)) {
				/* encode data */
				{
					...
					if (RTK_BT_OK != app_bt_le_audio_encode_data(&app_le_audio_data_path[i], output_channels, dequeue_buf, output_len)) {
		...
		{
			app_bt_handle_packet_discarding(output_len);
		}
...
static rtk_bt_evt_cb_ret_t app_bt_bap_callback(uint8_t evt_code, void *data, uint32_t len)
{
	...
	case RTK_BT_LE_AUDIO_EVT_ISO_DATA_RECEIVE_IND: {
		...
		if (p_bt_direct_iso->pkt_status_flag == RTK_BT_LE_ISO_ISOCH_DATA_PKT_STATUS_LOST_DATA) {
			...
			BT_LOGD("[APP] pkt status lost data: iso_conn_handle 0x%x, pkt_seq_num:0x%d \r\n", p_bt_direct_iso->iso_conn_handle, p_bt_direct_iso->pkt_seq_num);
		} else {
	...
	case RTK_BT_LE_AUDIO_EVT_BIG_INFO_ADV_REPORT: {
		...
		g_pbp_bsrc_receiver_interval_us = param->sdu_interval;
	...
	case RTK_BT_LE_AUDIO_EVT_BIG_SYNC_STATE_IND: {
		...
		BT_LOGA("[APP] big sync state change: adv_type:%d, advertiser_address = [%02x:%02x:%02x:%02x:%02x:%02x] \r\n",
				param->adv_type,
				param->adv_addr_val[5], param->adv_addr_val[4],
				param->adv_addr_val[3], param->adv_addr_val[2],
				param->adv_addr_val[1], param->adv_addr_val[0]);
		BT_LOGA("[APP] big sync state change: adv_sid:0x%x, broadcast_id=[%02x:%02x:%02x] \r\n",
				param->adv_sid,
				param->broadcast_id[0],
				param->broadcast_id[1],
				param->broadcast_id[2]);
		...
		/* search device in scan list*/
		{
			app_bt_le_audio_scan_dev_info_t *p_scan_dev_info = NULL;
			rtk_bt_le_addr_t adv_addr = {0};
			adv_addr.type = param->adv_type;
			memcpy(adv_addr.addr_val, param->adv_addr_val, RTK_BD_ADDR_LEN);
			p_scan_dev_info = app_bt_le_audio_scan_dev_list_find(adv_addr, APP_BT_LE_AUDIO_BASS_SCAN_TYPE);
			if (!p_scan_dev_info) {
				BT_LOGE("[APP] big sync state change: find no match device in scan list, adv_type:%d, adv_address=[%02x:%02x:%02x:%02x:%02x:%02x] \r\n",
						param->adv_type,
						param->adv_addr_val[5],
						param->adv_addr_val[4],
						param->adv_addr_val[3],
						param->adv_addr_val[2],
						param->adv_addr_val[1],
						param->adv_addr_val[0]);
			} else {
				BT_LOGA("[APP] big sync state change: find device in scan list, adv_sid %x, broadcast_id: %02x,%02x,%02x, broadcast_name: %s \r\n",
						p_scan_dev_info->bass_scan_info_t.adv_sid,
						p_scan_dev_info->bass_scan_info_t.broadcast_id[0],
						p_scan_dev_info->bass_scan_info_t.broadcast_id[1],
						p_scan_dev_info->bass_scan_info_t.broadcast_id[2],
						p_scan_dev_info->bass_scan_info_t.broadcast_name);
			}
		}
		break;
	}
...
static uint16_t rtk_bt_hfp_cvsd_parse_decoder_struct(rtk_bt_hfp_codec_t *phfp_codec, rtk_bt_cvsd_decode_t *pcvsd_decoder_t)
{
	...
	hfp_audio_track_hdl = rtk_bt_audio_track_add(RTK_BT_AUDIO_CODEC_CVSD, (float)APP_BT_DEFAULT_A2DP_PBP_AUDIO_LEFT_VOLUME,
												 (float)APP_BT_DEFAULT_A2DP_PBP_AUDIO_RIGHT_VOLUME,
...
static uint16_t rtk_bt_hfp_sbc_parse_decoder_struct(rtk_bt_hfp_codec_t *phfp_codec, rtk_bt_sbc_decode_t *psbc_decoder_t)
{
	...
	hfp_audio_track_hdl = rtk_bt_audio_track_add(RTK_BT_AUDIO_CODEC_mSBC, (float)APP_BT_DEFAULT_A2DP_PBP_AUDIO_LEFT_VOLUME,
												 (float)APP_BT_DEFAULT_A2DP_PBP_AUDIO_RIGHT_VOLUME, 2,
	...
	psbc_decoder_t->min_bitpool = phfp_codec->msbc.bitpool;
	psbc_decoder_t->max_bitpool = phfp_codec->msbc.bitpool;
	hfp_audio_track_hdl = rtk_bt_audio_track_add(RTK_BT_AUDIO_CODEC_mSBC, (float)APP_BT_DEFAULT_A2DP_PBP_AUDIO_LEFT_VOLUME, (float)0, 1,
												 psbc_decoder_t->sampling_frequency, BT_AUDIO_FORMAT_PCM_16_BIT, 0, NULL, true);
	...
	if (rtk_bt_audio_record_config(&hfp_demo_audio_record_table)) {
		BT_LOGE("[HFP] rtk_bt_audio_record_config fail \r\n");
		rtk_bt_audio_track_del(RTK_BT_AUDIO_CODEC_mSBC, hfp_audio_track_hdl);
		hfp_audio_track_hdl = NULL;
		return 1;
	}
	/* AIVoice use 16k record */
...
static rtk_bt_evt_cb_ret_t app_bt_hfp_callback(uint8_t evt_code, void *param, uint32_t len)
{
	...
	case RTK_BT_HFP_EVT_SCO_DISCONNCTED_IND: {
		...
		rtk_bt_audio_track_del(audio_hfp_codec_conf.codec_index, hfp_audio_track_hdl);
		rtk_bt_audio_record_del(audio_hfp_codec_conf.codec_index, hfp_audio_record_hdl);
...
int bt_a2dp_hfp_pbp_main(uint8_t enable)
{
	if (1 == enable) {
		...
		/* A2DP part */
		{
			...
			DiagSnPrintf(&dev_name[strlen(APP_A2DP_SINK_HFP_HF_PBP_DEVICE_NAME)], 7, "(%02X%02X)", bd_addr.addr_val[1], bd_addr.addr_val[0]);
