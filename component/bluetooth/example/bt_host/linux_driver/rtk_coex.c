...
static void bt_coex_send_vendor_cmd(uint16_t cmd_id, uint8_t *pbuf, uint8_t len)
{
	...
	struct hci_dev *hdev = p_rtk_bt_coex_priv->hdev;
	uint16_t opcode = cmd_id;
	uint8_t param_len = len - 4;
	void *param = (void *)(pbuf + 4);

	struct sk_buff *skb;
	skb = hci_prepare_cmd(hdev, opcode, param_len, param);
	...
	skb->dev = (void *)p_rtk_bt_coex_priv->hdev;
#endif
	...
	DBG_BT_COEX("%s: opcode 0x%x", __func__, opcode);

	DBG_BT_COEX_DUMP("bt_coex_send_vendor_cmd: pbuf = ", pbuf, len);

	DBG_BT_COEX_DUMP("tx pdata = ", skb->data, skb->len);
...
static void bt_coex_set_profile_info_to_fw(void)
{
	...
	pbuf = (uint8_t *)kmalloc((1 + handle_number * 6) + 4, GFP_ATOMIC);
	if (!pbuf) {
		return;
	}
...
static void bt_coex_setup_check_timer(struct rtk_bt_coex_conn_t *p_conn, uint16_t profile_idx)
{
	...
	p_monitor_node = (struct rtk_bt_coex_monitor_node_t *)kmalloc(sizeof(struct rtk_bt_coex_monitor_node_t), GFP_ATOMIC);
	if (!p_monitor_node) {
		return;
	}
	...
	spin_lock_irqsave(&p_rtk_bt_coex_priv->monitor_lock, flags);
...
static void bt_coex_handle_connection_complet_evt(uint8_t *p_evt_data)
{
	...
	if (!p_conn) {
		...
		p_conn = (struct rtk_bt_coex_conn_t *) kmalloc(sizeof(struct rtk_bt_coex_conn_t), GFP_ATOMIC);
		if (!p_conn) {
			return;
		}
...
static void bt_coex_handle_l2cap_conn_req(struct rtk_bt_coex_conn_t *p_conn, uint16_t psm, uint16_t scid, uint8_t dir)
{
	...
	p_profile = kmalloc(sizeof(struct rtk_bt_coex_profile_info_t), GFP_ATOMIC);
	if (!p_profile) {
		return;
	}
...
#if defined(HCI_BT_COEX_BR_EDR_SUPPORT) && HCI_BT_COEX_BR_EDR_SUPPORT
static void bt_coex_monitor_timer_handler(struct timer_list *timer)
{
	...
	mod_timer(&p_rtk_bt_coex_priv->monitor_timer, jiffies + msecs_to_jiffies(BT_COEX_MONITOR_INTERVAL));
}
#endif
...
void bt_coex_send_mailbox_cmd(uint8_t *user_data, uint16_t length)
{
	...
	pbuf = (uint8_t *)kmalloc(1 + length + 4, GFP_ATOMIC);
	if (!pbuf) {
		return;
	}
...
static void bt_coex_parse_mailbox_event(uint8_t status, uint8_t mailbox_id, uint8_t payload_len, uint8_t *payload)
{
	...
	case RTK_COEX_MAILBOX_BT_INFO_REPORT_BY_ITSELF:
		if (status == 0) {
			pdata = (uint8_t *)kmalloc(1 + payload_len, GFP_ATOMIC);
...
void bt_coex_process_rx_frame(uint8_t type, uint8_t *pdata, uint16_t len)
{
	...
	// DBG_BT_COEX_DUMP("rx pdata = ", pdata, len);
	if (type == HCI_EVT) {
...
void bt_coex_process_tx_frame(uint8_t type, uint8_t *pdata, uint16_t len)
{
	...
	// DBG_BT_COEX_DUMP("tx pdata = ", pdata, len);
	if (type == HCI_ACL) {
...
{
	DBG_BT_COEX("BT COEX Init \r\n");
	p_rtk_bt_coex_priv = (struct rtk_bt_coex_priv_t *)kmalloc(sizeof(struct rtk_bt_coex_priv_t), GFP_ATOMIC);
	...
	spin_lock_init(&p_rtk_bt_coex_priv->monitor_lock);
}

void bt_coex_probe(struct hci_dev *hdev)
{
	p_rtk_bt_coex_priv->hdev = hdev;
}

void bt_coex_open(struct hci_dev *hdev)
{
#if defined(HCI_BT_COEX_BR_EDR_SUPPORT) && HCI_BT_COEX_BR_EDR_SUPPORT
	p_rtk_bt_coex_priv->hdev = hdev;
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 14, 0)
	timer_setup(&p_rtk_bt_coex_priv->monitor_timer, bt_coex_monitor_timer_handler, 0);
#else
	setup_timer(&p_rtk_bt_coex_priv->monitor_timer, bt_coex_monitor_timer_handler, 0);
#endif
	p_rtk_bt_coex_priv->monitor_timer.expires = jiffies + msecs_to_jiffies(BT_COEX_MONITOR_INTERVAL);
...
void bt_coex_close(void)
{
#if defined(HCI_BT_COEX_BR_EDR_SUPPORT) && HCI_BT_COEX_BR_EDR_SUPPORT
	del_timer_sync(&p_rtk_bt_coex_priv->monitor_timer);
#endif
}
