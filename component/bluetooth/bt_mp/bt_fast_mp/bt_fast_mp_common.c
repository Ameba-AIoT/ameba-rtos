...
static void bt_fast_mp_test_start_dut_func(uint8_t bt_channel)
{
	...
	if (false == rtk_bt_mp_dtm_set_disable_tx_power_tracking()) {
	...
	if (bt_efuse.tx_power_le_1m != 0xFF || bt_efuse.tx_power_le_2m != 0xFF) {
		if (false == rtk_bt_mp_dtm_set_tx_power_index(bt_efuse.tx_power_le_1m, bt_efuse.tx_power_le_2m)) {
	...
	if (bt_efuse.bit_map & bt_efuse.bit_map_flatnessk_valid) {
		if (false == rtk_bt_mp_dtm_set_tx_flatnessk(bt_efuse.tx_flatness_k)) {
	...
	if (false == rtk_bt_mp_dtm_set_tx_count(BT_TX_PKT_COUNT)) {
	...
	if (false == rtk_bt_mp_dtm_tx_test_v3(bt_channel, BT_TX_PKT_LENGTH, BT_TX_PKT_PL, BT_TX_PHY,
	...
	if (false == rtk_bt_mp_dtm_test_end(&num_pkts)) {
	...
	if (false == rtk_bt_mp_dtm_rx_test_v3(bt_channel, BT_RX_PHY, BT_RX_MODU_IDX, BT_RX_EXP_CTE_LEN,
	...
	if (false == rtk_bt_mp_dtm_get_rx_report(&rssi)) {
