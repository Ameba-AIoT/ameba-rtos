...
def create_littlefs_image(source_dir, output_image, block_size, block_count):
    ...
    # file_max = total image size (block_size*block_count). This matches the
    # cfg->file_max the SDK sets per partition (LFS_FLASH_SIZE /
    # LFS_SECOND_FLASH_SIZE), so the on-device mount check in lfs.c
    # (superblock.file_max <= cfg->file_max) passes instead of failing with
    # "Unsupported file_max". Default (0) would store LFS_FILE_MAX (~2GB) and
    # be rejected on a device configured with a small file_max.
    lfs = LittleFS(block_size=block_size, block_count=block_count, disk_version=0x00020000,
                   file_max=block_size * block_count)
...
def main():
    ...
    parser.add_argument("-f", "--fixed_block_count",
                        action="store_true",
                        help="Do not auto-grow the block count to fit the source directory; "
                             "try to build the image with -c as given (fails if it does not fit)")
    ...
    rolfs_fit = args.rolfs_header and args.block_count is None and not args.fixed_block_count

    # --fixed_block_count: honour -c verbatim, no estimate and no auto-grow.
    if not rolfs_fit and not args.fixed_block_count:
    ...
    try:
        ...
        if not args.fixed_block_count:
            raise
        print(f"Error: source directory does not fit in the fixed geometry "
              f"block_size={block_size} block_count={block_count} "
              f"({block_size * block_count} bytes): {e}")
        exit(1)
