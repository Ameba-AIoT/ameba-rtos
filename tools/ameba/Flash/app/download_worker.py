    ...
    def run(self):
        ...
        try:
            ...
            if os.path.isfile(self.image_path):
                ...
                # that spans several partitions of the device profile layout. Mark it
                # so the download handler can split it by layout and skip the all-FF
                # padding between/after partitions instead of programming it (mirrors
                # the CLI single-image path). When the image does not line up with any
                # layout partition, expand_merged_image falls back to trimming the
                # trailing padding, so this is safe for plain single-partition writes.
                img.split_by_layout = True
                images_info = [img]
