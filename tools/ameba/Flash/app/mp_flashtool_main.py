...
class MPFlashToolMain(QMainWindow):
    ...
    def __init__(self):
        ...
        self._profile_json = None           # raw JSON dict of the loaded profile, for write-back
        self._layout_mandatory_checkboxes = []   # per-row "mandatory" checkboxes of the layout table
        self._layout_full_erase_checkboxes = []  # per-row "full erase" checkboxes of the layout table
    ...
    def _build_profile_selection_tab(self, tab_widget):
        ...
        self.profile_line_edit.setPlaceholderText("Select a profile (.rdev)...")
        ...
        # The Layout table is editable: users may tweak Image Name / Address /
        # Memory Type and toggle the Mandatory / Full Erase checkboxes. The edits
        # are synced back into self.profile_info before flashing (so the run uses
        # them) and written back to the profile file on close. Factory Mode locks
        # editing (see _apply_factory_mode).
        self.profile_image_table.setEditTriggers(
    ...
    def _build_options_tab(self, tab_widget):
        ...
        # Whether a single (merged) image is split by the profile layout so the
        # all-FF padding between/after partitions is skipped. Persisted in the
        # Settings General section and shared with the CLI.
        self.split_single_image_by_layout_checkbox = QCheckBox("Split single image by layout")
        self.split_single_image_by_layout_checkbox.setChecked(False)
        left_layout.addWidget(self.split_single_image_by_layout_checkbox)
    ...
    def start_download(self):
        ...
        # Sync the (editable) Layout table back into self.profile_info so the
        # run uses the user's edits — including Image Name / Address / Memory
        # Type and the Mandatory / Full Erase selection.
        sync_errors = self._sync_table_to_profile()
        if sync_errors:
            QMessageBox.warning(
                self, "Invalid Layout",
                "Please fix the following before downloading:\n\n"
                + "\n".join(sync_errors),
            )
            return

        # Whether to split a single (merged) image by the profile layout is
        # controlled by the config checkbox. When on, expand_merged_image emits
        # only the partitions marked mandatory in the Layout table; when off, the
        # whole image is programmed into flash as-is.
        split_by_layout = self.split_single_image_by_layout_checkbox.isChecked()
        ...
        for row, port in checked_rows:
            ...
            worker = DownloadWorker(
                ...
                split_by_layout=split_by_layout,
    ...
    def _app_root_dir(self):
        """Tool's root directory: the executable folder when frozen, else the
        project root (the parent of this ``app`` package)."""
        if getattr(sys, 'frozen', False):
            return os.path.dirname(sys.executable)
        return os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    def _start_dir(self, current):
        """Directory a file dialog should open in, based on an existing path.

        Returns the path itself if it is a directory, its parent directory if
        it points at a file (or a not-yet-existing path under a real folder),
        or the tool's root directory when the input is empty/invalid.
        """
    ...
    def _start_location(self, current):
        """Start location for an open-file dialog.

        Returns the file path itself when it exists so the dialog preselects
        it; otherwise falls back to :meth:`_start_dir`.
        """
        current = (current or "").strip()
        if os.path.isfile(current):
            return current
        return self._start_dir(current)
    ...
    def browse_image_file(self):
        ...
        if action == file_action:
            file_path, _ = QFileDialog.getOpenFileName(
                ...
                self._start_location(self.image_line_edit.text()),
            ...
            dir_path = QFileDialog.getExistingDirectory(
                ...
                self._start_dir(self.image_line_edit.text())
            )
            if dir_path:
    ...
    def open_profile_file(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Device Profile",
            self._start_location(self.profile_line_edit.text()),
            "Device Profile (*.rdev);;All Files (*)"
        )
        if not file_path:
            return
    ...
    @staticmethod
    def _memory_type_from_str(text):
        """Parse a Memory Type cell back to its int code.

        Accepts a name ("NOR", case-insensitive) or a numeric string; returns
        None when the value cannot be understood.
        """
        names = {"RAM": 0, "NOR": 1, "NAND": 2, "VENDOR": 3, "HYBRID": 4}
    ...
    def _parse_address(text):
        """Parse a hex/decimal address cell; returns None when invalid."""
        try:
            return int(str(text).strip(), 0)
        except (TypeError, ValueError):
            return None

    def _sync_table_to_profile(self):
        """Copy the Layout table edits back into self.profile_info.images.

        Returns a list of human-readable error strings for any cell whose value
        could not be parsed; those cells keep the profile's previous value.
        """
        errors = []
        if self.profile_info is None:
            return errors
        for i, image in enumerate(self.profile_info.images):
            name_item = self.profile_image_table.item(i, 1)
            start_item = self.profile_image_table.item(i, 2)
            end_item = self.profile_image_table.item(i, 3)
            mtype_item = self.profile_image_table.item(i, 4)

            if name_item is not None:
                image.image_name = name_item.text().strip()

            if start_item is not None:
                start = self._parse_address(start_item.text())
                if start is None:
                    errors.append(f"Row {i + 1}: invalid Start Address '{start_item.text()}'")
                else:
                    image.start_address = start

            if end_item is not None:
                end = self._parse_address(end_item.text())
                if end is None:
                    errors.append(f"Row {i + 1}: invalid End Address '{end_item.text()}'")
                else:
                    image.end_address = end

            if mtype_item is not None:
                mtype = self._memory_type_from_str(mtype_item.text())
                if mtype is None:
                    errors.append(f"Row {i + 1}: invalid Memory Type '{mtype_item.text()}'")
                else:
                    image.memory_type = mtype

            if i < len(self._layout_mandatory_checkboxes):
                image.mandatory = self._layout_mandatory_checkboxes[i].isChecked()
            if i < len(self._layout_full_erase_checkboxes):
                image.full_erase = self._layout_full_erase_checkboxes[i].isChecked()
        return errors

    def _write_profile_back(self):
        """Persist the (possibly edited) layout back into the profile file.

        Updates only the per-image fields in the raw JSON so any other keys are
        preserved, then re-saves with the same encryption used on load. Best
        effort: failures are swallowed so closing the tool never blocks.
        """
        if (self.profile_info is None or not self.current_profile_path
                or self._profile_json is None):
            return
        try:
            images = self._profile_json.get("Images", [])
            for i, image in enumerate(self.profile_info.images):
                if i >= len(images):
                    break
                entry = images[i]
                entry["ImageName"] = image.image_name
                entry["StartAddress"] = image.start_address
                entry["EndAddress"] = image.end_address
                entry["MemoryType"] = image.memory_type
                entry["FullErase"] = image.full_erase
                # The Layout table now reflects the profile's mandatory flag
                # as-is (no per-memory-type default), so persisting it round-trips
                # faithfully: untouched rows keep their value, edited rows are saved.
                entry["Mandatory"] = image.mandatory
            JsonUtils.save_to_file(
                self.current_profile_path, self._profile_json, need_encrypt=True
            )
        except Exception:
            pass
    ...
    def _toggle_language(self, cn_mode):
        ...
        # ── Layout tab ────────────────────────────────────────────────────
        self._lbl_profile.setText("Profile：" if cn_mode else "Profile:")
        ...
        self.profile_line_edit.setPlaceholderText(
            "选择 profile 文件 (.rdev)..." if cn_mode else "Select a profile (.rdev)..."
        )
        ...
        self.split_single_image_by_layout_checkbox.setText(
            "按 Profile 布局拆分单固件" if cn_mode else "Split single image by profile layout"
        )
    ...
    def _apply_profile_from_path(self, file_path):
        try:
            ...
            return str(e)
        # Keep the raw JSON so edits can be written back on close without
        # dropping any keys the object model does not track.
        ...
        # Keep references to the per-row "mandatory" checkboxes so the download
        # flow can tell which layout regions the user selected.
        self._layout_mandatory_checkboxes = []
        self._layout_full_erase_checkboxes = []
        self.profile_image_table.setRowCount(len(self.profile_info.images))
        for i, image in enumerate(self.profile_info.images):
            # Mandatory checkbox (centered) — user-editable. Reflect the
            # profile's actual mandatory state as-is (no per-memory-type
            # special casing).
            cb = QCheckBox()
            ...
            self._layout_mandatory_checkboxes.append(cb)
            ...
            # Full Erase checkbox (centered) — user-editable
            fe_cb = QCheckBox()
            fe_cb.setChecked(image.full_erase)
            fe_cb.setEnabled(True)
            fe_wrapper = QWidget()
            ...
            self._layout_full_erase_checkboxes.append(fe_cb)
        ...
        self._update_window_title()
        # The freshly created Layout checkboxes default to enabled; re-apply the
        # current factory mode state so they stay locked when factory mode is on
        # (e.g. after switching profiles while factory mode is enabled).
        self._apply_factory_mode(self.factory_mode_checkbox.isChecked())
    ...
    def _settings_path(self):
        return os.path.join(self._app_root_dir(), "Settings.json")
    ...
    def load_settings(self):
        ...
        self.split_single_image_by_layout_checkbox.setChecked(
            bool(settings.split_single_image_by_layout)
        )
    ...
    def _apply_factory_mode(self, enabled):
        ...
        self.split_single_image_by_layout_checkbox.setEnabled(configurable)
        ...
        for cb in self._layout_mandatory_checkboxes:
            cb.setEnabled(configurable)
        for cb in self._layout_full_erase_checkboxes:
            cb.setEnabled(configurable)
        if enabled:
