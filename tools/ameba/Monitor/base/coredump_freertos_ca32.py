...
DATAABORT = '.*?(Address of Instruction causing Data abort.*)'
...
STACK_DATA_PATTERN = re.compile(r'.*?\s+\[([\da-f]+)]\s+(((?:0x)?[\da-f]{8}\s*)+)', flags=re.I)
...
class CoreDump:
    ...
    def _check_coredump_trigger_before_print(self, line):  # type: (str) -> None
        ...
        dataabort_mo = re.match(DATAABORT, line.strip(), flags=re.I)

        if dataabort_mo:
            print_yellow('dataabort')
            print(line)
            self.dataabort_line = line
            return
...
class StackDump():
    def __init__(self, dump_file, dataabort, stack_depth=None):
        self.dump_file = dump_file
        self.crash_info = ""
        self.stack_depth = stack_depth
        self.stack_end = None
        self.dataabort_line = dataabort
    ...
    def parse(self):
        ...
        with open(self.dump_file, "r", encoding="utf-8") as f:
            for line in f:
                if line.find("LR =") >= 0:
                    index = line.find('0x')
                    print('LR: ', line[index:])
                    self.lr = StackData(0, line[index:])
                ...
                if stack_start == True and stack_data_mo:
                    ...
                    datas = [StackData(start_addr + i * 4, f"0x{x}" if not x.startswith("0x") else x) for i, x in
        ...
        if self.dataabort_line:
            index = self.dataabort_line.find('0x')
            if index >= 0:
                print(self.dataabort_line[index:])
                # self.pc = int(self.dataabort_line[index:], 16)
                self.pc = StackData(0, self.dataabort_line[index:])
                # print(hex(self.pc))

        all_stack_datas = dump_datas
        ...
        stack_datas = [self.pc, self.lr]

        self.stack_datas = stack_datas + all_stack_datas


class RTOS_COREDUMP_Parser():
    def __init__(self, dump_file, elf_file, rom_file, toolchain_prefix, data_abort_line):
        ...
        self.data_abort = data_abort_line

    def get_stack_dump(self):
        stack_dump = StackDump(self.dump_file, self.data_abort)
        stack_dump.parse()
        return stack_dump
    ...
    def print_call_info(self):
        ...
        for data in stack_dump.stack_datas:
            output = self.address_decoder.decode_address(f"0x{data.actual_val:08x}")
            if output:
