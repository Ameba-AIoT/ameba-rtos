...
│   └── kconfig.py               kconfig_get / kconfig_set / kconfig_search / kconfig_apply_file / kconfig_save_file
├── resources/
...
| `kconfig_save_file` | `(file: str, soc?: str)` | `tools/kconfig.py` | 把当前 `.config` 存成 `prj.conf` 风格文件（仅 **diff vs `default.conf`**），可回喂给 `kconfig_apply_file` / `build_firmware(config_files=...)` |
