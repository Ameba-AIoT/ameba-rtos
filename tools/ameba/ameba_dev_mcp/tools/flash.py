...
def full_erase_flash(alias: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Step 2: load project entry to get memory_type
    try:
        pinfo = load_project_info(PROJECT_ROOT)
    except ConfigLoadError as ex:
        return _err_envelope(ex.errors, alias=alias)
    ...
    entry = pinfo.projects.get(board.soc)
    memory_type = (
        (entry.memory_type if entry else None)
        or pinfo.defaults.memory_type
        or "nor"
    )
...
def register_flash_tools(mcp: FastMCP) -> None:
    ...
    async def full_erase_flash_tool(alias: Optional[str] = None) -> Dict[str, Any]:
        ...
        All connection parameters come from board_info.json5.
        Memory type (NOR/NAND) is read from project_info.json5.
