...
SOC_CONFIG = load_soc_config(os.path.join(GNU_SCRIPT, 'ameba_gdb_config.json'))
...
if FPGA == True:
    if SOC_TYPE == 'RTL8720F':
        ...
        gdb_port_ns = CONFIG.get('GDB_PORTS', {}).get('km4ns', 2331)
        pre_commands = f" -ex 'set $GDB_PORT={gdb_port_ns}'"

        rom_config_ns = CONFIG.get('ROM', {}).get('km4ns', [])
        ex_commands_ns = ""
        for item in rom_config_ns:
            ex_commands_ns += f" -ex 'restore {item[0]} binary {item[1]}'"
        ex_commands_ns += " -ex 'quit'"

        cmd = 'cd ' + ASDK_NS +  ' && ' + CMAKE_GDB + pre_commands + ' -x ' + ROM_SCRIPT + ex_commands_ns
        print(' GDB CMD : ', cmd)
        os.system(cmd)
    ...
    gdb_port = CONFIG.get('GDB_PORTS', {}).get(target_prj, 2335)
    pre_commands = f" -ex 'set $GDB_PORT={gdb_port}'"

    rom_config = CONFIG.get('ROM', {}).get(target_prj, [])
    ex_commands = ""
    for item in rom_config:
        ex_commands += f" -ex 'restore {item[0]} binary {item[1]}'"

    if target_prj == 'kr4': # workaround for KR4 default cache policies
        ex_commands += " -ex 'dump binary memory ram_2_dump.bin 0x20022000 0x20030000'"

    ex_commands += " -ex 'quit'"

    cmd = 'cd ' + ASDK +  ' && ' + CMAKE_GDB + pre_commands + ' -x ' + ROM_SCRIPT + ex_commands
    print(' GDB CMD : ', cmd)
    os.system(cmd)
