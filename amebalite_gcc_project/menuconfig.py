...
config_file_kr4 = os.path.join(CONFIG_SCRIPTS_DIR, '.config_kr4')
config_file_km4 = os.path.join(CONFIG_SCRIPTS_DIR, '.config_km4')
header_file_kr4 = os.path.join('project_kr4/inc', 'platform_autoconf.h')
header_file_km4 = os.path.join('project_km4/inc', 'platform_autoconf.h')
marker = ['General config', 'KR4 config', 'KM4 config', 'Config end']
...
def main(argc, argv):
    ...
    config_kr4 = extract_core_role(general_lines, '_FOR_KR4')
    print(config_kr4)
    run_command('setconfig.py', f' --config-in {config_in}' + f' --config-out {config_file_kr4} ' + config_kr4)
    run_command('genconfig.py', f'--config-in {config_file_kr4} --header-path {header_file_kr4}')

    kr4_lines = extract_lines(config_file_kr4, marker[0], marker[1]) + extract_lines(config_file_kr4, marker[1], marker[2])
    with open(config_file_kr4, 'w') as file:
        file.writelines(kr4_lines)

    process_and_write_lines(config_file_kr4, config_file_kr4, '_FOR_KM4', '_FOR_KR4')
    process_and_write_lines(header_file_kr4, header_file_kr4, '_FOR_KM4', '_FOR_KR4')
    ...
    process_and_write_lines(config_file_km4, config_file_km4, '_FOR_KR4', '_FOR_KM4')
    process_and_write_lines(header_file_km4, header_file_km4, '_FOR_KR4', '_FOR_KM4')
